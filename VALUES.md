...
| # | Value | Requirement | Mechanism | Where |
|---|---|---|---|---|
| X1 | No secret in a file | A connection is a reference (`env:NAME` or `file:path`); a literal connection string in `estate/posture.json`, a profile or an argument is exit 6. | references only; a profile containing `Password=` is exit 6 naming the file | `Io.Tests: "inline credential refused"` |
