...
document corpus, its audits, its decision log, its Twin (v3's synthetic copy) and its
`ssdt-agent` skills.*

---

## 0. How to read this, and the answer

This is a design document: it proposes, and it amends nothing on its own authority. It is
written to be argued with, and it says where it is uncertain.

The question is large and the evidence is specific, so the document is long, and a reader can
stop early. §0 is the answer. §1–§4 are the diagnosis. §5–§10 are the design. §11–§12 say what
is cut from v2 and what is carried from v1. §13–§16 are the laws, the migration path, the process
...
disposable copy by letting DacFx publish it, and writes the pull request description a reviewer
can approve by reading. The knowledge that already exists (the operation catalog, the shared
reasoning, the description forms) becomes what a developer's session reads first. The synthetic
copy becomes the thing every proof runs on. Everything is sized to what has a consumer today:
about a quarter of v2's source, written in C# with v2's F# as the specification (§6.6), a fifth
of its tests, a twelfth of the repository's hand-written prose, and one CLI.

**Why this and not more of v2.** This repository found the right idea three times and each time
covered it with apparatus. The right idea, in the `ssdt-agent` tree's words, is that *proving is
classifying*: what a schema change does is settled by publishing it against data and reading
what DacFx did, never by reading its SQL. v1 first found it as *evidence-gated tightening* (its
default policy mode: `NOT NULL` only when the measured data shows no NULLs). v2 typed it and then
proved it after the fact with a canary. The tree found it again, sharpest, as the classification
of a change by a Strict publish, after the cutover had largely happened. Meanwhile v2's source
grew to 118,686 lines of F# with 118,513 lines of tests, a 30,911-line decision log, 179 root
documents totalling 119,384 lines, forty-eight axioms, eighteen theorems, and a survival list of
fifteen traps that cost agents real time. The repository's own most recent review says it
plainly: *over-documented and under-mechanized*. v3 takes that review at its word and completes
the change of direction v2 had begun.
...
tightening decision tables. The synthetic-data generator and its laws. The operation catalog and
the ten-section pull request description. The OSSYS rowset SQL. The two-release finding. The
proving loop.
...
with 566 exemptions, the chapter/hand-over/decision ritual, and every document that restates
...
of two plus a script pack. Personas become phases. Docs become generated status plus a few short
hand-written documents. Agent sessions get a write budget.

The rest of this document supports those sentences. Its companion, `V3_INSTRUCTION_ARCHITECTURE.md`,
...
None of this judges the people or the agents who built it. It is what happens when a strong
idea is developed by many high-context sessions, each rewarded for leaving something named
behind. v3's process rules (§15) exist to change that reward.

---

## 1. The lineage

Three generations of one job, and the job changed while the tools were being built.
...
cir-v1.json`, 422 lines). It measures the live database for the facts the metamodel cannot
...
green. Its shape is a sequence: ingest, analyze, emit, orchestrate, operate. Its architecture
guardrails fit on one page and were mostly honoured. Its two failure modes were the ordinary
ones of a growing C# codebase: a 12-step `BindAsync` chain that returns a 30-field result
record, and a handful of files that each hold a whole concern (`CommandConsole.cs` at 2,511
lines, `EntityDependencySorter.cs` at 2,188).

v1 shipped. It was the tool that got the first environments cut over.

### 1.2 v2 — `projection` (F#, `sidecar/projection/`)
...
- a transfer subsystem for moving rows between environments with surrogate-key remap, two-phase
...
- a synthetic-data generator (`SyntheticData.generate`, which v2 wrote as σ : Profile → Data),
  with a privacy contract and a stability law;
...
a theorem v2 proves about itself. `THE_USE_CASE_ONTOLOGY.md` said v2 is a *publication-and-provenance
engine that terminates at an eject*. Each is true, and each is larger than the one before.

### 1.3 The change of direction — the Twin and the `ssdt-agent` tree (July–August 2026)

Then the cutover began, in stages, and the job changed. By late August 2026 QA
...
- **The Twin**, v2's synthetic copy (`THE_TWIN.md`, charter complete 2026-07-18): one command
  that holds a local SQL Server current with the repository's definitions and fills it with
  deterministic, masked, distribution-faithful synthetic data. It reuses the kernel's
  `SyntheticData.generate`. It is designed to be split out into its own tool. It is ~5,000 lines
  and 128 tests, and its own review calls it lean.
- **The `ssdt-agent` tree** (July–August 2026): a set of skills for an agent that helps a
  developer state a change in plain words, prove it on a disposable copy, and open a pull
  request a reviewer can approve by reading. Forty-five operation skills. Six shared-reasoning
  skills. A nine-state authoring machine. A ten-section pull request description. Twenty
  numbered findings about what DacFx does, of which eighteen stand and two are recorded
  refutations of the tree's own earlier prose (F5 overturned by F9; F8's mechanism corrected by
  F9). A nightly lane that re-proves 41 sample changes against live SQL Server.
...
mechanisms (a downloaded synthetic copy, a one-command verdict, molecular proofs, a PR-side
reproduction), and by the end of that same session had built three of them as scripts.

The *projection* half of v2's F# (metamodel → catalog → passes → SSDT bundle, the 40,564-line
pipeline and the 14,097-line CLI in front of it) is, in the tree's own words, "an optional
accelerant, not wired": the tree's proving loop calls sqlpackage and DacFx directly through
1,875 lines of JavaScript. The *synthesis* half (`SyntheticData.generate`, the Twin, the
transfer realization it reuses) *is* wired: the nightly proof lane runs the 41 sample changes
inside `Twin.Tests.Integration` on the Twin's own generated estate. So the thing the team will
use is a set of skills, a script pack, and the synthetic copy. The part of v2 that was its point
sits beside that and outside the loop.
...
|---|---|---|
| v1 | the tightening policy | *evidence-gated*: `NOT NULL` only when the measured data shows zero NULLs (or within budget) |
| v2 | the canary and the estate | *fidelity as a theorem*: deploy, read back, diff; `Ingest ∘ Project = id` modulo named erasures |
| v2's change of direction | the `ssdt-agent` tree | *proving is classifying*: publish the change to a real-shaped copy; DacFx's behaviour is the classification |
...
pull request description. v3 makes that the whole design.
...
→ change → published on a copy → verdict → pull request description. The model is one of
several places a state can come from. The proof is the centre.

---

## 2. The job

The work, assembled from `KICKOFF.md`, `V2_PRODUCTION_CUTOVER.md`, `THE_USE_CASE_ONTOLOGY.md`,
...
- **Two sink archetypes** drove v2's transfer design: *full-rights* on-prem SQL Server (DDL,
  ...
  (no DDL, the sink assigns keys). v2 measured 3.5 million rows in flight at the time of writing
...
### 2.2 The pipeline and its one fixed rule
...
- and the two-release pattern has its own failure mode, which presents as a green deploy: after
  ...
  database* (F17, proven 2026-08-28). The window between R1 and R2 must therefore be a lock
...
This paragraph sizes the design. Every layer of prose an agent must read on a Windows laptop
with a modest context budget is a failure point. The consumer of this system is a Copilot
session with a small context, never a monorepo agent with an hour to read.

### 2.4 The workflows, post-eject

v2's ontology catalogued nine "proteins" for the cutover era (loads, re-keys, publications,
idempotent redeploy, in-place evolution, eject, drift, canary). After the eject, the daily workflows are:
...
rows back up, eject) remain, and they are *finite*: they end, and the post-eject workflows do
...
- The verdict is one tool call with a structured result, in place of eight shell commands and
  word of mouth.
- The synthetic copy is downloaded rather than built, and never contains real data.
...
- The repository's description of itself is generated where it can be and short where it
  cannot, so that it cannot stay wrong for long.
...
v2's documents treat v1 as an "editorial donor," which is generous in tone and dismissive in
substance. Read on its own terms, v1 is a competent, conventionally layered C# system that solved
the first problem well enough to ship, and that discovered most of the domain truths v2 later formalised.
...
violating column: `UPDATE` to a sentinel, `DELETE`, or `SELECT` for review. This is the first
form of "proving is classifying": the schema one may emit depends on the data found. v2 kept the
...
970-test suite with 936 green and every failure triaged by name is a sound state.
...
the "same logical user, different id per environment" problem end to end. And the DMM
comparator (`IDmmLens<T>`: project SMO, T-SQL, or an SSDT project into one canonical
`DmmTable[]` and diff) is the only place either generation compares *arbitrary* schema sources
rather than "model versus deployed." v2 sunset the second and carried the first only as
`UserFkReflowPass`. v3 wants both, smaller.
...
transform does not need. The whole of `build-ssdt` is: read model, read evidence, decide, emit,
...
and compared against a DMM baseline. It never *published* the emitted schema to a database and read
back what happened. That is the step v2 added as the canary and the step the `ssdt-agent` made the
centre of everything. v1's tightening asked the data before emitting; nothing asked DacFx after.
...
v2 is harder to assess than v1 because it audited itself without pause and most of what can be
...
This section says which conclusions v3 accepts, which it sharpens, and what they could not see from inside.
...
*in a quotient*: `PhysicalColumn.Type` is the coarse `PrimitiveType` rather than the storage
type, and `CRYSTALLINE_FORM.md` §3.2 shows exactly why the coarseness matters (a `BIGINT → INT`
...
correct factoring of measurement, and it is why a live measurement and a synthetic one can share
one derivation. ~6,000 SQL round-trips became ~900 at 300-table scale. v3 keeps it.

**`SyntheticData.generate`, v2's σ : Profile → Data.** The generator writes deterministic,
FK-aware, masked rows from evidence; measuring a generated set recovers the evidence (v2's law
`π ∘ σ ≈ id`); values are content-addressed to `(master, kind, column, row)` so a schema edit
regenerates only the columns it touched ("S-stable"). The Twin wraps it in one command, a closed
config, evidence tiers (a committed literal-free shape tier; a rich tier kept out of the repo),
scenarios that rewrite and never generate, and a post-generation trust gate. ~5,000 lines, 128
...
the ten-section pull request description are the product. They stay.
...
**(b) v2 kept generalising after the job had specialised.** The estate board (2,633
...
author, review, gate, synthetic copy, watch, remember, and none of it needs a torsor.
...
ritual. Hand-over letters are prepend-only (3,887 lines). Decisions are appended (30,911 lines, 480
dated entries, more than half marked as operator decisions). The README's "Status at chapter X
close" sections are a changelog inside the entry page. `CLAUDE.md` was rebuilt from scratch because
"the predecessor's table drifted against the code within weeks," and its rebuilt form declares any
restated count a first-class defect, then restates fifteen survival rules because each cost an agent
real time. This is a system that knows its documents are wrong and has responded by writing more
documents about being wrong. The August review measured the agent-facing prose at three to four
times what its consumer can use; `CRYSTALLINE_FORM.md` estimated 35–55% of the corpus as
compressible sediment. Both are conservative.
...
**(f) It built the tool the agent would use, and the agent used scripts.** The `ssdt-agent`
tree says v2's F# is "an optional accelerant, not wired." That is exactly true of the
projection half (metamodel → catalog → 21 passes → SSDT bundle, plus the 40,564-line pipeline
and 14,097-line CLI around it) and exactly false of the synthesis half: the Twin calls
`Transfer.runSynthetic`, `Deploy.executeBatch`, and the transfer's wipe, and the nightly proof
lane runs the 41 sample changes inside `Twin.Tests.Integration` on the Twin's generated estate.
The tree's proving loop is `prove.mjs`: build the `.sqlproj`, script the change, publish under
the Strict profile, parse the outcome, return one JSON verdict. Its synthetic copy comes from
`bake.mjs`: a fingerprint-versioned `.bacpac`. Its gate is `inflight-check.mjs` plus an ADO
pipeline template. So of the 118,686 lines, the part the team's workflow rests on is the Twin
(~5,000 lines) and the generator and publish code it reuses; the part that was v2's reason to
exist is outside the loop. Meanwhile, the CLI that is meant to be the operator surface requires
`dotnet run --project src/Projection.Cli --` from inside `sidecar/projection`, a pinned SDK
with `rollForward: disable`, and Docker for its most useful verbs. There is no published
binary. The most important conclusion in this document is the simplest: **the working product
is the skills plus a proving loop plus a synthetic copy, and it is 1,875 lines of JavaScript
away from the F# that was supposed to be its core.**
...
strength: an empirical check found real bugs, and the culture closed them in a day. The cost is
...
and the expansion ran at the same speed. v3's §14 keeps the check (a deployed-database diff
...
Every v2 audit optimised inside the frame "v2 is the tool; make it smaller, purer, more
coherent." `THE_PROJECTION_PRINCIPLE.md` is the high point of that frame: one move (measure
once, project many ways, let agreement be the law) applied at every level. It is a well-made
document, and it proposes five more keystone abstractions.

What the frame cannot see is that the job changed beneath it. The cutover is finite and nearly
done. After it, v2's F# is a library that three post-eject tools call into: a prover, a
synthetic copy, and a watcher. The question v3 asks is "what is the smallest thing that proves
a change, and which parts of v2 are that thing", in place of "how do we make the projection
crystalline". §11 answers module by module.
...
Neither a model nor a projection: a *change*, the displacement between two states of a schema,
...
Consequence: the kernel's central types are `Schema` (a state), `Change` (a displacement between
two schemas, with a rename map), and `Verdict` (what DacFx did when the change was published
...
### 5.3 The pull request description is the product

The thing a lead approves is the ten-section pull request description: verdict, intent, what
changes, before promoting, the data, how it ships, what proving showed, after-deploy checks,
rollback, not checked. It is agentless, finding-on-top, proof-beneath, in a DBA's words. v2's
`THE_VOICE.md` (twelve rules) and the tree's `THE_RECORD.md` (nine rules) already say how to
write it. The sample pull request for make-mandatory is the exemplar. The pipeline reproduces
its proof; the human makes the business call.

Consequence: every kernel output that reaches a human is shaped for the description.
Diagnostics are findings with codes; verdicts carry the verbatim `Msg`; rollback is computed as
the inverse change where one exists and named as "not auto-undone" where it does not. There is
no separate presentation layer with its own AST, because the description is the one rendering.
...
(author, review, gate, synthetic copy, watch, remember) or by being required to finish the
finite cutover (transfer, reverse leg, eject), in which case it is kept as the clearly labelled
cutover tools and retired when the cutover ends. Nothing is kept because a law would read
better with it. v2's own rule ("primitives at the second consumer; zero-consumer
symmetry-builds get deleted") is applied to v2 itself, module by module, in §11.
...
one README per package, one architecture page, an ADR log of one-line entries. The v2 corpus is
archived whole, with an index, out of the working tree: provenance is findable rather than read.

Consequence: hand-written docs in the working tree under 7,000 lines outside `knowledge/`, and
`knowledge/` itself under 11,000; agent sessions have a write budget (§15) that forbids new
doctrine documents.

### 5.6 Three corollaries

**One language, one solution, one CLI.** C#, with v2's F# kept as the specification the port is
checked against (§6.6): closed hierarchies encode the unions, ScriptDom and DacFx are C#
libraries, and the tree's proof scripts become verbs of the tool they used to call. v1's C# is
frozen as a reference implementation until v3 passes the parity checks, then archived. The
Twin's verbs fold into the one CLI as `synthetic-copy` subcommands; the JavaScript scripts
become verbs.

**Personas are phases.** Intake, author, and review are stages of one conversation with one
agent rather than three agents with a hand-over protocol. The reviewer keeps a separate skill
because the reviewer is a separate person with a separate trust model; but the *gate* is the
pipeline.
...
zero orphans in a generated set, and so on) each have exactly one test whose name is the law in
...
/
  ...
  kernel/                   C# library. Pure. No I/O, no clock, no Task. ≤ 11,000 lines (§6.6).
    ...
    Evidence.cs             what the data says: per-column facts, per-reference orphans, per-index duplicates, distributions
    Decide.cs               the tightening tables: nullability · foreign key · unique (pure functions)
    Change.cs               the change: ChannelDiff per channel + rename map + facet alterations
    Order.cs                topological order with an explicit cycle policy
    Statement.cs            the typed SQL statement DU (from v2, verbatim)
    SyntheticData.cs        generate : Evidence → rows (from v2's SyntheticData, corrections folded in)
    PullRequestDescription.cs the ten-section pull request description as a type, with its render

  io/                       C# library. One adapter per substrate; everything that touches a disk, a socket, or a clock. ≤ 17,000 lines.
    Ossys.cs                the OSSYS rowsets: the SQL contract + 22 handlers → Schema (from v2's Adapters.OssysSql/Osm)
    SqlServer.cs            read-side (INFORMATION_SCHEMA/sys.*) → Schema; aggregate queries → Evidence; fingerprints
...
Three packages of code, one of knowledge, one of CI. The dependency direction is one line:
`kernel ← io ← cli`; `knowledge` and `ci` depend on `cli`'s verbs and nothing depends on them.
`kernel` references no package that can perform I/O, which is how purity is enforced: by the
absence of a capability rather than by an analyzer.

Two dependency laws are kept from v2 as tests rather than doctrine. The first is v2's "pure core"
(`Projection.Core` has no I/O, no clock, no `Task`; enforced by a 142-line typed-tree analyzer
plugin). v3 enforces it as a test that asserts the kernel project's package references are the BCL
and `System.Collections.Immutable` only, plus the banned-API analyzer's list (`BannedSymbols.txt`)
forbidding `System.IO`, `System.Net`, `DateTime.Now`, `Random`, and `Task` in `kernel/`. The second
is the Twin's kernel manifest (`Twin.Core → Projection.Core` only, enforced by `BoundaryTests.fs`).
v3's form: `io/` does not reference `cli/`, and nothing references `knowledge/` or `ci/` except by
reading files. Both tests live beside the twelve laws (§13).
                      ...
                      v                            v
                            cli/  (estate)
                             ...
                             io/   Ossys · SqlServer · Ssdt · Render · Emit · Publish · SyntheticCopy · [Move]
                               |
                               v
                           kernel/  Schema · Identity · SqlType · Evidence · Decide · Change · Order · Statement · SyntheticData · PullRequestDescription

   external:  ScriptDom (Render, Ssdt)   DacFx (Ssdt, Publish, SyntheticCopy)   Microsoft.Data.SqlClient (SqlServer, SyntheticCopy, Move)
              Testcontainers (tests only)   Bogus (SyntheticData realization, io side)   CsCheck (tests only)
```

Compare v2: fifteen projects, with `Projection.Pipeline` (40,564 lines) sitting between the kernel
and the CLI as an orchestration layer that owned config binding, forty-six run kinds, seven declared
spines, three seams, and the estate. v3 has no orchestration layer. A verb is a function in `cli/`
that calls `io/` functions that call `kernel/` functions and returns a typed result. If a verb needs
staging, it is staged in the verb. "Declared ⇔ executed" is true because the verb *is* the declaration.

### 6.3 What each package owns, and what it refuses to own

**kernel/** is the domain: every shape, as a value object built by a smart constructor or as a
closed hierarchy, and every decision. It refuses to know where anything came from (`Schema`
carries no connection, no path), when anything happened (no clock; the boundary stamps), or how
anything is displayed (no renderers per DU; `PullRequestDescription.render` is the one
exception, because the description is a kernel value). It has one error type (`Error`), one
finding type, and returns plain records and lists. There is no `Lineage<Diagnostics<'a>>`; a
decision function returns its decisions and its findings as two fields of one record, and the
caller appends them to the run's findings. There is no transform registry; the order of passes
is a list of functions in `Decide.fs`, and the test that the list is complete is the compiler.

**io/** owns every effect: one adapter per substrate (DacFx, SQL Server through SqlClient,
ScriptDom, git, the file system and the OS, Docker and LocalDB), named for the thing it touches
and owning that substrate's whole surface, so that every state and failure the substrate can
present is mapped at the adapter's boundary to a domain value or to a named `Error` with a
remedy, and no call site catches a substrate's exception. It makes no decisions:
`SqlServer.Measure` returns evidence, never a verdict; `Publish.strict` returns what DacFx did,
never what it means; `Ossys.read` returns a `Schema`, and the OutSystems-type to SQL-type
mapping it applies is a kernel function it calls (`SqlType.ofOutSystems`). This is v2's
"discover, then derive" boundary made structural.

**cli/** owns the verbs, the exit codes, the config file, and the human-facing rendering of
findings and verdicts. Each verb is a use case: a function that returns a typed result, which
`cli/` renders after parsing the arguments, so a test reaches every verb's logic through a
library call. There is no `View` AST, no `Voice` register, no themed TTY renderer; there is
Markdown to stdout, JSON with `--json`, and the pull request description.

**knowledge/** owns what only a human or a DacFx run can know. It refuses to restate what the
CLI can print: no verb tables, no config references, no status. Skills cite verbs by name and
findings by number.
...
The budget is derived from measurements. Each line is the v2 module the v3 module descends from, its
...
| `kernel/Evidence.fs` | `Profile.fs`, `ProfileDerivation.fs`, `EvidenceCache.fs`, `SamplingPolicy.fs`, `Statistics.fs`, `Meter.fs` | 2,952 | the user axes (cutover), joint distributions (unbuilt; the generator needs marginals), FK selectivity, the god-file derivation | 1,300 |
...
| `kernel/Change.fs` | `CatalogDiff.fs`, `ChangeManifest.fs` | 1,195 | episode coordinates, the norm, tolerance residuals; keep the nine facets, rename detection, and the changelog shape | 700 |
...
| `kernel/SyntheticData.fs` | `SyntheticData.fs`, `SyntheticCorrection.fs`, `SyntheticVolume.fs`, `Centrality.fs` + its pass, `Twin.Core/Evidence.fs`, `DerivedEvidence.fs` | 2,112 | the ranking's manifest reporting; nothing else material (the generator and the shape tier are what the synthetic copy is built from) | 1,500 |
| `kernel/PullRequestDescription.fs` | new; `THE_RECORD_FORMS.md` as a type | — | — | 300 |
...
| `io/SyntheticCopy.fs` | `Twin.Core/*` (config, scenarios, fingerprint, estate definition), `Twin.Runtime/*`, `Pipeline/Bulk.fs`, `FakerRealization.fs`, `DockerImageEmitter.fs`, `Twin.Cli/Render.fs`; `scripts/bake.mjs` (251 lines of JavaScript) | 4,464 | two of the three ways to stand a copy up; Voice rendering; the config surface trimmed to one database | 3,300 |
...
| `io/Move.fs` **[cutover tools, frozen]** | `TransferRun.fs`, the nine `Transfer*.fs`, `PeerTransfer.fs`, `SurrogateCapture`/`PackedSurrogateRemap`/`KeymapSpill`/`CaptureJournal`, `MovementSurface.fs`, `MovementSpec.fs`, `CapabilitySurvey.fs`, `MigrationRun.fs`, `FidelityCompareRun.fs`, `Cli/Faces/Transfer.fs` | ~15,400 | not ported. The reverse leg is built, canary-gated, and has not run; v2's code stays buildable in `archive/v2` until the leg runs or is cancelled (§16). If step 8 (§14.2) happens, the port is ≤ 3,000 lines | 0 (≤ 3,000) |
| **io total** | | **~22,200 (+4,400 optional, +15,400 frozen)** | | **~14,100 (≤ 15,000; +3,600 optional)** |
| `cli/` | `Projection.Cli/*` (14,097), `Twin.Cli/Program.fs` (225), `Pipeline/Config.fs` (2,189) | 16,511 | `Voice.fs`, `View.fs`, `Watch.fs`, `Navigator`, `TtyRenderer`, the boards, 30+ plan actions, the 60-key config; keep `CliExit.fs` (73), the change rendering (~200), the dispatcher (~200), the synthetic-copy verbs (~350) | 1,500 |
...
cutover tools) are the deletions with no replacement: `Pipeline.fs` (3,414) and its `Compose.run*`
...
Second, the budget is three numbers, because one number hides the decisions that set it.
**Kernel ≤ 8,000. io ≤ 15,000, plus ≤ 4,000 if the OSSYS reader survives. cli ≤ 1,500.** The
planning pass summed its own floors to ~28,400 with the reader in and the synthetic copy
counted separately; this table lands at ~24,000 without the reader and ~27,500 with it. The gap
between the two passes is real deletion: the readback type folded into a function, the manifest
gone, the migration emitter reduced to a preview, the estate board reduced to a table. These
floors are in F# notation, the language of the specification; §6.6 restates them for the C# the
team will build. The three decisions that move the total are the OSSYS reader (±3,600),
`io/Move` (0 or ≤ 3,000), and nothing else; `Render` keeps all 26 statement kinds on purpose,
because v3 still emits (the bundle for the synthetic copy, the seeds, the refactorlog, and the
drift report in file terms), and a statement stream that can express the whole repository is
what makes law 3 possible.

Third, the two largest line items are `Emit` and `Render`; with `SyntheticCopy` they are half
of io. Together with `Ossys` they are the domain: what the platform says, what SQL Server
accepts, how to write it, and how to fill it. That is where the lines should be. The budget is
enforced as a test: `wc -l` per package against this table, failing above the ceiling, so that
the next "just one more pass" is a red build rather than a chapter.

### 6.5 Tests, docs, and the two budget lines v2 never drew

| Region | v2 today | v3 budget | How |
|---|---:|---:|---|
| kernel tests | ~60,000 (pure pool share) | 7,500 | property tests on `Decide`, `Change`, `Order`, `SyntheticData`, `Statement → Render` round trips; the kernel halves of the twelve laws; no per-DU renderer tests because there are no per-DU renderers |
| io tests | 21,742 (Integration) + 6,702 (Twin integration) + ~26,000 (pure pool, adapters and targets) | 8,500 | one SQL Server fixture (Docker or LocalDB, chosen by environment; the CDC class isolated per instance), the read-side round trip, the publish verdicts, the synthetic copy's laws, `Move`'s phase tests while the cutover tools exist. This is the half that proves anything about DacFx, and the pure pool must not crowd it out |
| the proven changes | 41 `SamplePr*` facts in 13 classes | 2,500 | the 45 (and the compound) sample changes as rows of data under `tests/Golden/changes/`, run by one parameterised test in the proof lane (§10.3) |
...
| hand-written docs outside `knowledge/` | 119,384 (v2 root) + ~60,000 (v1: `readme.md`, `docs/`, `handbook/`, `ssdt-playbook/`, `notes/`) | ≤ 7,000 | `README.md` 150; `ARCHITECTURE.md` (this document's successor) ≤ 3,000; `DECISIONS.md` one line per entry; one README per package (≤ 80 × 8); `PORTABILITY.md` (94); `THE_TWIN.md` and `THE_SYNTHETIC_DATA_DESIGN.md` (619, kept as they are, beside `io/SyntheticCopy`) |
| `knowledge/` | 21,724 (`ssdt-agent`, markdown) | ≤ 11,000 | 45 ops ≈ 3,400 (about 75 lines each); shared ≈ 750; `authoring.md` + `reviewing.md` + `description.md` ≈ 1,500; `findings.md` ≈ 500; a dozen samples ≈ 950; ledgers ≈ 300; the eight handbook chapters ≈ 2,500 |
...
| the fixture corpus | v1: ~26,000 lines of fixture JSON, SQL and `.sqlproj`; v2: `GoldenCatalog.fs` (468) + `proving-ground/` + `twin.scale.json` | ≤ 3,000 | one golden schema (≤ 500 lines of F#), its emitted bundle, the proven changes as data, `twin.scale.json`; v1's three edge-case fixture sets stay in `archive/v1` and serve as the OSSYS parity check (§14.1) only while `io/Ossys` exists |

The archive keeps everything. Nothing is destroyed. The working tree stops carrying it.

### 6.6 The C# encoding

v3 is built in C#. The team that will maintain it maintains C#; the review §2.3 asks for is a
review of code as well as of descriptions; and a tool nobody on the team can read is what v2
became. v2's F# stays as the specification v3 is ported from: its tests are the first thing
ported, and the kernel types in §7 are shown in F# because it is the most compact notation for a
type and the one the specification is written in. The encoding into C# is mechanical; this section
gives its rules, so a reader of §7 can write the C# without asking, and the budget in C# lines.

**What F# was buying, and the C# form of each.**

| F# benefit the design relies on | Where it matters | C# form |
|---|---|---|
| closed unions with exhaustive matching | `Statement` (26 cases), `NullabilityDecision`, `Reader`, `Shipping`, `CyclePolicy`, the `Error` codes, `SqlType` | an abstract record with a private constructor and nested sealed derived records, so nothing outside the file can add a case, plus one `Match` method whose signature takes a delegate per case. Exhaustiveness moves from the compiler's pattern check to the method signature. Hand-written for unions under eight cases; generated (`dunet`, or a forty-line in-repo source generator) for `Statement` |
| immutable records with structural equality and copy-with | every kernel type; laws 1, 6, 7; `Change.between` | C# records. One trap: record equality over a collection compares the reference, which would silently break law 1 and every change comparison. The kernel gets one `SortedArray<T>` (a readonly struct over `ImmutableArray<T>` with element-wise equality and ordinal-sorted construction, about eighty lines), and every kernel list is a `SortedArray<T>` |
| smart constructors, no nulls | `Key`, `Name`, `Error` | `readonly record struct` with a private constructor and static factories returning `Result<T>`, an in-repo type of about sixty lines whose `Failed` case carries an `Error`, rather than a library. Nullable reference types on, warnings as errors; `Option` is not missed. No LanguageExt or similar: making C# imitate F# defeats the reason for the switch |
...
**Where C# is better.** DacFx, ScriptDom, and System.Text.Json are C#
...
the SSDT-project reader (`SsdtProjectDmmLens`), the remediation builder, the order validator,
the ScriptDom fixtures.
...
**No F# island.** It is tempting to keep the proven pieces (the statement builders, the
generator, the synthetic copy's check) as F# libraries under a C# kernel. That produces the
shape that ended v2: a corner of the codebase one person understands, which then becomes the
corner nobody touches and everybody works around. Everything is ported. v2's F# is the
specification: step 1 of §14.2 ports its kernel tests first, then the types, then the
functions, with the byte-level parity checks (§14.1) unchanged because they never depended on
the language that produced the bytes. The generator gets one extra fixture, v2's generated rows
at a fixed seed, so a port that drifts fails law 11 on the first run.

**Determinism has to be designed in, because nothing in C# supplies it.** F#'s structural
equality and ordered maps gave v2 stable output for free. In C#: every kernel collection is a
`SortedArray<T>` sorted at construction with ordinal comparers (v2's canonicalize pass, as a
constructor); no emitter iterates a `Dictionary`; no `GetHashCode` of a string reaches an
output; `Random` is banned from the kernel and `SyntheticData.generate` takes its random source
as a parameter. Laws 1 and 11 land in step 1, before any emitter exists, so that the first
emitter is written under them.
...
verbatim, it says so and does not repeat it. Where v3 departs from v2, the departure is explained
in the line after the type. Field counts are deliberate: `Attribute` in v2 has twenty fields;
`Column` here has eleven, and each of the nine removed is accounted for. The types are written in
F# because it is the most compact notation for a type and the one v2's specification is written
in; v3 itself is C# (§6.6), and every type below follows §6.6's rules, which shows two of them worked.
...
/// A stable identity that survives renames. Made once (a GUID) and carried as the
...
module Name =
    let create : string -> Result<Name>
    let value (Name s) = s

/// The one error type of the kernel. Code is dot-separated and stable (it is what a description
/// cites and what a test asserts); its first word is its category, which chooses the exit (§8).
/// Where is a Key when the error is about one object.
type Error = { Code: string; Message: string; Remedy: string; Where: Key option }

/// The one finding type: the run continues past a finding. Codes share the errors' namespace
/// so a description can cite either.
...
`ValidationError` (338 construction sites in v2) becomes `Error`; `DiagnosticEntry` becomes
...
- **`Column` loses nine of `Attribute`'s twenty fields.** `Type: PrimitiveType` and
  ...
  `Precision`, `Scale` are facets of the storage type rather than separate fields. `IsPrimaryKey`
...
- **No smart constructor per record.** `Schema.validate : Schema -> Error list` runs once at
...
and ColumnEvidence =
    { Nulls: int64; Distinct: int64 option; MaxLength: int option
      Distribution: Distribution option; Measurement: Measurement }
and OrphanEvidence = { Count: int64; Sample: SqlLiteral list; Measurement: Measurement }
and DuplicateEvidence = { Groups: int64; Sample: SqlLiteral list list; Measurement: Measurement }
and Distribution =
...
and Measurement = Observed | Sampled of rows: int64 | TimedOut | Skipped of reason: string
```

Kept from v2: the shape of `ColumnProfile`, `ForeignKeyReality`, and the two distributions;
the `Measurement` case that distinguishes "we looked" from "we did not" (v1 conflated these,
and v2's `ProbeStatus` was its correction); the discover-once/derive-pure factoring (the io
side materialises row streams into aggregates; every field above is a pure function of those
...
`JointDistributions`. The user populations belong to `Move` (the cutover tools) and are read
there. CDC awareness belongs to `Publish` (a CDC-enabled table changes what an `ALTER` may do;
that is a verdict input rather than evidence about data). Cardinality, selectivity, and joint
distributions had one consumer each (bridge retarget readiness; the synthetic copy's
`perParent`; a synthesis mode explicitly bounded as "v1 fidelity"); `perParent` becomes a
scenario knob. What remains is what the decision tables and the generator read.
...
### 7.6 `Change.fs` — a change

```fsharp
type ChannelDiff<'alteration> =
    { Created: Key list; Dropped: Key list; Renamed: (Key * Name * Name) list; Altered: 'alteration list }

type ColumnFacet = Type | Nullability | Identity | Default | Computed | Collation | Order
type ColumnAlteration = { Key: Key; Facets: Set<ColumnFacet> }        // non-empty by construction
type ReferenceAlteration = { Key: Key; Facets: Set<ReferenceFacet> }  // Columns | Target | Actions | State
type IndexAlteration = { Key: Key; Facets: Set<IndexFacet> }

type TableChange =
    { Key: Key
      Columns: ChannelDiff<ColumnAlteration>; References: ChannelDiff<ReferenceAlteration>
      Indexes: ChannelDiff<IndexAlteration>; Checks: ChannelDiff<Key>; Triggers: ChannelDiff<Key>
      PrimaryKey: (Name list * Name list) option; Seed: SeedChange option }
type Change =
    { Tables: ChannelDiff<TableChange>; Sequences: ChannelDiff<Key>
      Renames: (Key * (Name * Name) * (Name * Name)) list }       // the refactorlog, derived

let between : from: Schema -> to': Schema -> Change
let inverse : Change -> Change option        // Some when every change has a lossless inverse; the rollback section
let isEmpty : Change -> bool
let dataLoss : Change -> DataLossStep list  // narrow · drop column · drop table · NOT NULL on populated · lossy retype
```

This is v2's `ChannelDiff<'change>` (the collapse `CRYSTALLINE_FORM.md` recommended and v2
shipped), its cases renamed to DacFx's words (created, dropped, altered), with the episode
coordinates, the schema norm, the tolerance residual, and the manifest series removed. Two
...
```fsharp
type CyclePolicy =
    | Refuse                                   // default for schema: a cycle is a finding rather than a fallback
...
let order : CyclePolicy -> Schema -> Result<Order>
```
...
already-generated parents so orphans are impossible, single-column uniques, then
distributions), splitmix64 seeding, `decimal` arithmetic, content-addressing to `(master,
table, column, row)` for S-stability, the hybrid-by-cardinality privacy contract (≤ 50 distinct
values preserve, above that synthesize), and the corrections overlay (PII classification →
Faker realization at the boundary). The Twin's `DerivedEvidence` (a schema-derived floor:
email-named columns get email addresses, amount-named numerics are non-negative, status-named
text gets a small vocabulary) moves in as `SyntheticData.floor : Schema -> Evidence`.

Kept, too, after a correction: volume weighting by centrality. The PageRank pass was filed with
the advisory analytics, but `SyntheticVolume.byCentrality` is its consumer, and
`Twin.Runtime/Mint.fs` and `Pipeline/SyntheticLoadRun.fs` call it to decide how many rows each
table is generated with. The pass's manifest reporting goes; the ranking (~90 lines) stays as
`SyntheticData.volumes : Schema -> RowTiers -> Volume`, with the row tiers from the ledger
overriding the rank where they exist.

### 7.10 `PullRequestDescription.fs` — the product as a type
...
type PullRequestDescription =
    { Verdict: string                               // one line
      ...
      HowItShips: Shipping                          // OneRelease | OneReleaseRelaxed | TwoRelease of pre: string * note: string | Refused of Error
      WhatProvingShowed: Attempt list               // tried / did / realized, each with the verbatim Msg
      AfterDeploy: Query list
      Rollback: RollbackNote                        // inverse change rendered, plus "not auto-undone"
      NotChecked: string list }
let render : PullRequestDescription -> string                       // Markdown, agentless register, one section per field
```
...
| `Episode`, `Lifecycle`, `ChangeManifest`, `Ledger` | ~850 | git is the timeline; the refactorlog is derived from `Change`; the operations ledger is a file in `knowledge/ledgers/` |
| `ApprovalWorkflow`, `ActConsent`, `ApprovedDataCorrections`, `DataCorrectionReceipt`, `WriteSignoff` | ~1,300 | approval is the pull request; data corrections are a pre-deploy script the description names |
...
| `SliceSpec`, `Closure`, `DataLoadPlan`, `BridgeRowDelta`, `BridgeRetarget`, `SurrogateRemap`, `UserRemap`, `UserIdentity`, `RowFidelity`, `RawValueCodec`, `CanaryResidual`, `SyntheticVolume` | ~3,300 | `Move` (the cutover tools) keeps a reduced `SurrogateRemap`/`UserRemap`; the rest had one consumer that is deleted or is the synthetic copy's scenario knob |
...
| 7 | build failed |
| 9 | refused by name: a named environment, an unregistered copy, a git branch, an aggregate query outside the allowlist. Every other error exits on the code whose meaning it has, chosen by its category, the word before the first dot (`cli/Contract.cs`, `ExitByCategory`; the operator's ruling, 2026-09-25) |
...
2. Dispatch on the source:
   - **ossys**: run `outsystems_metadata_rowsets.sql` (v1's script, byte-identical), read the 22
     ...
     metamodel. Keys derived from the metamodel's attribute/entity identities via `Key.ofName`
...
3. `Schema.validate`; each error exits 2 with its code.
...
### 8.2 `estate measure` — what the data says
...
1. For every table in the schema, one batched aggregate query: `COUNT_BIG(*)`, per-column
   `SUM(CASE WHEN col IS NULL …)`, `MAX(LEN(col))` for text, distinct counts where cheap; sampled
   above the threshold (v1's `TableSamplingPolicy`, v2's `SamplingPolicy`), with `Measurement`
   recording which.
...
4. Distributions (categorical top-N with a truncation flag; numeric and temporal percentiles)
   only when `--distributions` is given, because they are what the generator needs and what a
   privacy review must see.
5. A fingerprint per table (`COUNT_BIG`, `MAX(pk)`, `CHECKSUM_AGG(BINARY_CHECKSUM(…))`), v2's
   one-batch staleness query, so `check` can tell whether evidence is stale.
...
prediction gates nothing. `--apply` writes the tightened schema (`Decide.apply`). This is v1's
...
tree's `classify-mechanism` skill does by hand: given the change (`Change.between`) and the
...
(the change shows a `Renamed` pair, or a `Dropped`+`Created` pair of the same type at the same
...
6. `<Project>.refactorlog` from `Change.Renames` when `--renames` or a prior schema is given,
   and the description's "after deploy" queries as `Verify/*.sql`. A `.sqlproj` is written only
   by `emit --init` for a project that does not exist yet, in the classic format (the one the
...
facets named; `dataLoss` steps listed under a heading the description reuses verbatim;
`inverse` rendered as the rollback where it exists. `--json` gives the `Change`. Exit 5 with
`--fail-on-change` when the change is non-empty (the drift check in CI).
...
This is `prove.mjs` as a verb, with the synthetic copy as the default target.
...
6. Re-read the schema (`read --from sql`) and re-validate constraint trust (`is_not_trusted`)
   so a foreign key that landed untrusted is a finding rather than a silent success (the F5/F9
   lesson and the DacFx-version risk, made mechanical).
...
evidence store are gone. A divergence is a row; what to do about it is the description's job.
...
1. `check inflight`: the tables this pull request reshapes (from `diff --from <base> --to HEAD`)
   ...
   row printed.
2. `synthetic-copy restore` the named artifact (or `synthetic-copy up` when none is named).
3. `prove --project … --target synthetic-copy` on the pull request's *combined* change, Strict.
4. `describe` regenerated from the verdict, the change, and the evidence; if `--pr-body` is
   given, its "how it ships" and "what proving showed" sections are compared to the regenerated
   ones, and a mismatch is a finding (the machine checks the shape; the human reads the rest).
5. `changelog.json`: the change between base and head as data (tables and columns created,
   dropped, renamed with their refactorlog keys, and retyped), the release tag, and the open
...
remap where the sink assigns keys; user re-key from a CSV map), phase-1 bulk insert in
`Order.order DeferNullable` order with deferred legs NULL, phase-2 `UPDATE`, verify counts and
row hashes, exit 9 with the count on dropped rows unless `--allow-drops`. Resume is a
checkpoint table on the sink; revert is a delete of captured keys. What is *not* kept from v2's
3,402-line `TransferRun`: streaming-versus-materialized selection, capability descent ladders,
keymap spill to temp tables, bridge-row staging, consent fingerprints, per-act signoff,
journals as a second resume mechanism, slices, and the go board. If the reverse leg at 200
...
sweeps disposable databases and containers older than a day (the crash-cleanup guarantee) and
prints what it swept. The companion document's §9.2 gives the line's exact shape.

### 8.13 What is gone from the verb surface

v2's `projection` had, per its own `THE_CLI.md`: `<flow> --go`, `check` (eight subcommands),
`diff`, `compare`, `explain` (five), `seal` (two), `report`, `synth-correct`, `inspect`, `init`,
`revert`, `setup`, four `slice-*` verbs, and a deprecated `--watch`; plus the Twin's ten; plus five
scripts. v3's thirteen verbs cover the six post-eject workflows and the cutover tools. Named flows
in config (`projection <flow>`) are gone: a flow was a saved argument list, which is a shell script or a CI step.

---

## 9. The knowledge layer

The `ssdt-agent` tree is the part of v2 that was right about the job. v3 keeps its content and changes
its shape, following the August review's Layer 2 ("skills as reasoning, personas as phases") one step further.

### 9.1 What is kept, and where it lives

```
knowledge/
  README.md              the entry page: the thesis in one page; how a session proceeds; where things are
  authoring.md           the ONE authoring skill: confirm intent → measure → prove → classify → ship → fork → describe → verify
  reviewing.md           the ONE review skill: reproduce → scope → attack → dispose
  description.md         the register (nine rules) and the ten-section template, on one page, with the make-mandatory exemplar
  ops/<operation>.md     the 45 operation skills, trimmed of restated mechanics (each ≈ 60–90 lines)
  ...
  findings.md            F1–F20 and successors: the proven DacFx facts, dated, struck-through when overturned
  samples/               a dozen sample descriptions, one per shape (add-optional, make-mandatory, narrow,
...
skills fold into it. `THE_RECORD.md` (308 lines), `THE_RECORD_FORMS.md` (143), and the tree's
register checks fold into `description.md` (one page beside the template, as the review
recommended).
...
| S2 measure | `estate measure --tables …` | counts and violating rows captured |
...
(§10.3), and the skill serves the lead's own questioning rather than the queue.
...
template for GitHub and Azure DevOps. The degradation steps (Visual Studio 2026 18.5+ with
discovered skills; 18.4 with agents plus the index; 2022 with prompt files; ask-only) are kept
...
The pilot the August review put first ("run it on one champion laptop; nothing in the packaging is
verified on the team's actual build") is still the first thing to do; §16 lists it as an operator decision.

### 9.5 What it costs to keep the tree true

The August review's §3.3 named the cost: each operation carried four surfaces (a skill, a
sample pull request, a self-test prompt, a synthetic-copy fact), and 978 lines of gate and
packager code (1,052 today: `ssdt-agent-gates.mjs` 370, `ssdt-agent-package.mjs` 682) existed
to hold them in agreement. v3 keeps two of the four: the skill (knowledge) and the fact (proof,
as a row of data in `tests/Golden/changes/`). The sample descriptions shrink to a dozen shapes
and are checked by the proof lane like any other change. The self-test prompts go. With two
surfaces, the gate code is three tests (§9.2) and the packager is one verb. The count that must
agree is one: `ops/` directories = golden sample changes, and the proof lane fails when it does
not.
...
with their windows and the tables they hold.

---

## 10. The synthetic copy and the three lanes
...
the most serious unresolved one.

v2 ended with two proving mechanisms in two languages on two DacFx versions
(`Pipeline/Deploy.fs`, 1,303 lines of F# on in-process DacFx 162.5.57; `scripts/prove.mjs`, 427
lines of JavaScript on an external sqlpackage 170.x) and three ways to stand up a copy
(`DockerImageEmitter`, `TwinContainer`, `bake.mjs`). Two proof corpora on divergent DacFx
versions was already the tree's own second finding. v3 has one of each: `io/Publish` proves
in-process on the pinned package, and `io/SyntheticCopy` stands the copy up. The JavaScript is
retired at step 4 (§14.2).

### 10.3 Layer 3 — the reviewer's mechanism is the pipeline
...
Anything else fails as a tooling failure, with its output. The template
`pipelines/ssdt-agent-pr-validation.yml` is the model; v3's is the same steps with the scripts
replaced by verbs.

The PR gate also publishes `changelog.json` (§8.10) as a pipeline artifact on every green run, so
the SSIS team's per-sprint mapping has a machine-readable source that names renames by refactorlog
key. During a two-release window the changelog carries both the shape the model declares and the
shape the database holds, marked, because for one release they disagree and the SSIS team must know which it maps.
...
business call: what value fills the blank rows; whether the orphan is deleted or reassigned) stays
a human act, posed by the description's "not checked / still open" section and answered in review.
...
five dispositions, and its v3 replacement. This section is what the ledger says when it is read
whole. The dispositions are: **KEEP** (survives close to verbatim), **KEEP-SIMPLIFIED** (the
...
| Disposition | Lines named | What it is |
|---|---:|---|
| KEEP / KEEP-SIMPLIFIED | ~46,000 → **~24,000 kept** | the IR, the evidence, the four decision tables, the diff, the statement stream and its ScriptDom builders, the readers, the generator and the synthetic copy, the seed emitter, the refactorlog, the exit codes, the purity analyzer |
...
### 11.2 The five largest cuts

1. **The largest deletable cluster is plumbing that exists because the chain is long, and no
   subsystem.** `TransformRegistry` (565) + `RegisteredTransforms` (227) + `PassChainAdapter`
   ...
   it. With a chain of four function calls, `registered ⇔ executed` is the compiler's guarantee
   rather than a property to prove.
...
4. **A 1,600-line subtree with one circular consumer.** Four advisory passes and their tuning
   ...
   which sets the synthetic copy's row allocation; `BoundedContext`, behind a default-off flag)
   are the correction §7.9 records.
5. **~15,400 lines of transfer and movement that are finite and unfinished.** `TransferRun`
   ...
   exist for is built and canary-gated and has not run. Frozen and buildable until it runs or is
   cancelled (§16).

### 11.3 What the dispositions do not touch

Nine things are carried close to verbatim, because each one was proven against a live SQL
Server and the proof would have to be repeated if a line moved: `Statement.fs`; `ScriptDomBuild.fs`
...
| root (`README`, `CLAUDE`, `KICKOFF`, `EXECUTION_PLAN`, `STORYBOARD`, the four `USE_CASE_ONTOLOGY`s, …) | 12 / 8,154 | `CLAUDE.md` §4's fifteen survival rules move to `knowledge/` where each is still true of v3 (most are about the Docker pool and the read side); the rest archives |
...
| the synthetic copy (`THE_TWIN.md`, `THE_SYNTHETIC_DATA_DESIGN.md`) | 2 / 619 | kept as they are, beside `io/SyntheticCopy` |
...
The family counts are the planning pass's grouping and overlap a little at the edges; the measured
total is 179 files and 119,384 lines. `docs/`, `handbook/`, `ssdt-playbook/` and `notes/` at the v1
root (72,554 lines) archive with v1, except the eight handbook chapters the ops cite (`knowledge/handbook/`).
...
`THE_DECISION_TREE.md` (215, as the core of `authoring.md`), the description forms (451 → one
...
The question this section was planned to answer was "what did v1 have that v2 never carried?" The
answer, after reading v1's trunk and the parity matrix's sixty-odd NOT-MAPPED rows against the six
post-eject workflows (§2.4), is short: **v2 dropped almost nothing the post-eject job needs, and the
one thing it dropped that matters most, reading an SSDT project or a dacpac as a first-class
operand, is the thing v3 is built around.** Appendix E is the ledger; this is its conclusion.
...
1. **The SSDT project and the dacpac as readers.** v1's `Osm.Dmm` read a project through
   ...
   lines) reads both through DacFx's `TSqlModel` rather than by re-parsing the files, so what
   v3 reads is what DacFx will publish.
2. **The supplemental Users model.** `config/supplemental/ossys-user.json` (341 lines) and
   ...
   the synthetic copy has to generate those foreign keys against something.
3. **The three-option remediation generator.** `RemediationQueryBuilder.cs` is 73 lines that
   ...
   the description's "the data" section prints them. This is `ask-the-developer` with its SQL
   included.
...
- **Transient retry is v2's.** The outline listed "Polly retry" as something v1 had
...
The DMM comparator with its three readers (2,201 lines: the canary is its successor and `diff`
is its verb); the load-harness DMV queries (572: the scale lane measures wall-clock on the
synthetic copy, and
...
v2 had 48 axioms and 18 theorems, with 81 live witnesses and 38 skip-stubs. v3 has twelve laws.
Each is one test whose name is the law in English, each is listed in `LAWS.md` by a script that
reads the test tree, and a law without a green test is not in the list. There are no buckets, no
ladder, no matrix, no numbering. The v2 identifiers are given here once, as provenance, and never again.
...
as a property on `Order` rather than a law), or a statement about v2's own construction that v3
does not make (the torsor, the two rulers, the writer-fidelity law, the commuting square), or a
property of a subsystem v3 does not carry (the estate verdict's totality, the reverse-leg
realization selector, the certificate on three surfaces).

---

## 14. The path from here to v3

A step-by-step replacement rather than a rewrite, in an order chosen so that every step leaves
something the team can use and nothing the team relies on gets worse. The parity checks are
fixed before any code moves.

### 14.1 The parity checks
...
| 3 | `io/Ssdt` (DacFx `TSqlModel` read), `io/SqlServer` (read-side + measurement), `io/Ossys` (if it survives §16's first decision); `estate read`, `estate measure`, `estate decide`. | parity checks 3 and 4; law 2 | `check drift` becomes possible |
...
with the parity checks green, never a branch that lives for a month.
...
pinned behaviour v3 drops (the estate lanes, the episode replay, the capability descent, the Voice
bijection, the transform registry), it is left behind and its absence is noted once in `archive/INDEX.md`.

### 14.4 Freeze points and rollback

Two standing v2 commitments are honoured. *"V1 stays warm through cutover+30 regardless"*:
`archive/v1` keeps its solution buildable and its fixture tests running in CI until the parity
checks (§14.1) are green at step 4, and then for thirty days after the last environment's
cutover date, whichever is later; retiring that lane is a one-line decision. *"V2 is
self-contained; every commit cherry-pickable"*: the archive is a move rather than a rewrite, so
v2's history and its cherry-pickability are intact; what ends is the promise that new work
lands there.

v2 keeps running its own CI from the archive until step 6, so at any point before that the team's
workflow (the tree, `prove.mjs`, `Twin.Cli`) is untouched. Steps 4 and 5 are the two that change
what the team runs; each ships with the old script kept for one release as a fallback that prints a
deprecation line. If the pilot (§16) fails on Copilot's instruction-following against the new
bodies, the fallback is the old bundle, regenerated from the archive.
...
donor (the modules in §6.4's left column, ~63,000 lines, about a third kept close to verbatim).
Most of the work is deletion and re-wiring. The one new module is `io/Ssdt` (read an SSDT project
through DacFx), and v2's Twin already does most of it in `EstateModel.fs`. The port from F# is
mechanical for most of the kernel, which is already mostly data and pure functions; the two places
it is not (the generator's determinism and the statement builders' completeness) are covered by
law 11's golden fixture and by law 2.

---

## 15. How sessions work in v3

Nobody who wrote v2's mass wanted mass. Each session was asked to leave the repository more
legible than it found it, and each concluded the best way was a new named surface: a chapter
close, a hand-over letter, a decision entry, an axiom, a pillar, a survival rule, a codified
failure mode. Each was locally reasonable; the sum is 221,000 lines of prose. v3 changes the reward.
...
- **code and tests**, without limit, subject to the parity checks and the laws;
...
### 15.2 The pull request carries the hand-over

There is no `HANDOFF.md`. A session ends by opening or updating a pull request whose
description says what it did, what it verified, and what is left, in the description's
register (agentless, finding on top, proof beneath). The next session reads the open pull
requests and `git log`. v2's letters were "forward-looking, second-person" by rule; the rule
existed because the letters were the only place state lived. In v3 state lives in the tree,
the tests, the ledgers, and the open PRs.

### 15.3 Knowledge is curated rather than accreted

`knowledge/` grows only by: a new op skill (when the team meets an operation the catalog lacks,
proven first on the synthetic copy), a new finding (with its captured output), a struck-through
finding (with the date and what overturned it), a new sample description when a new *shape*
appears (a new instance of an existing shape adds none), and edits that make an existing skill
shorter or truer. The three tests that replaced the gates (citations resolve; op count =
sample-change count = sample-description count; register rules hold) run on every change to
`knowledge/`. A skill that grows past 120 lines is split rather than kept.
...
Refute in the open. When a finding is overturned, strike it through with a date and the run that
overturned it; never edit it away. When a law's test goes red for a reason other than a bug, the
law changes in the same pull request as the test, and the decision line says why. When a session
disagrees with this document, it says so in the pull request and changes `ARCHITECTURE.md` in the
same change. This is the culture v2 built and the only ceremony v3 keeps.

---

## 16. Risks, open questions, and the decisions only the operator can make

Ranked by how much of the design would change if the answer went the other way. Each item names
what it moves and the default assumed, so no section waits on an answer.
...
1. **Pin the pipeline's DacFx and run the one `is_not_trusted` check.** A declarative foreign-key
   ...
   *trusted* on sqlpackage 170.x (the DacFx the hand proofs ran). `estate/toolchain.md` lists
   ...
   pins; law 10's meaning. *Default assumed:* the pipeline is on the 162 family until shown
   otherwise, so the package pin starts at 162.5.57, every description re-stamps its DacFx
   version, and the first act after the pin is one real publish with `is_not_trusted` read
   back. An afternoon on the right machine; open for three weeks.
2. **Run the Visual Studio Copilot pilot on one champion laptop.** Nothing in the packaging has
   run on the team's build; the review called it the riskiest assumption in the system.
   *Moves:* whether `knowledge/` is what a developer's session reads first or an artifact only
   a monorepo agent reads; which of the degradation steps the team lands on; whether
   `authoring.md` at one skill is followed or ignored by Copilot's skill discovery. *Default
   assumed:* the 18.4 step (agents plus the index), with the path-scoped instruction files as
...
5. **Who are the dev leads?** `estate/reviewers.md` has two rows that read "fill in at the Dev
   ...
   the latter until the roster is data. *Default assumed:* the gate is relied on and the
...
8. **Run the synthetic copy's evidence import against real Dev.** `synthetic-copy evidence
   import/derive/verify` exist as verbs; the proving loop still runs against eight tables and
   about thirty-five rows, and the review called the wiring "the single most valuable
   integration not yet done." *Moves:* whether the synthetic copy is distribution-faithful or
   schema-derived. *Default assumed:* the derived floor until the import runs, and the bake
   lane refuses to label an artifact "faithful" until it has.
...
- **The refresh nobody owns.** Every change has a second half: a human refreshing the external
  ...
  it; no description section, template, or decision-tree state owned it until §9.3. The failure
  is a runtime error in the application, invisible to every publish and every gate. `check
...
- **A green deploy that destroys data.** F17: with a contract-phase R1 landed, one more publish
  ...
  *Successfully published database*. This is why the lag window is a lock (§8.10) and why the
  gate is mandatory. It is also why a data correction with no receipt is unrecoverable and
  looks successful: F15 showed the seed silently undoing a pre-deploy repoint. The description's
...
- **A rename by typing.** Post-eject the refactorlog is maintained by Visual Studio's rename
  refactoring and never by editing the `.sql` text. Without the entry DacFx drops and re-adds
  the column and the publish is green. `classify` refuses the change (§8.3) and `emit
...
- **Two mechanisms for one job.** v2 ended with proving in F# on in-process DacFx 162 and
  proving in JavaScript on external sqlpackage 170, and three ways to stand a copy up. Two proof
  corpora on divergent DacFx versions was already the tree's second finding. §10.2 picks one of
  each; the one choice that remains, in-process DacFx at the pinned version or an external
  sqlpackage at a path, exists only for a DacFx release the package feed does not offer, and
  the verdict names which ran.
...
- **v3 repeating v2's curve.** v2 grew from ~77,000 to 118,686 source lines in nine weeks, with
  ...
  the same growth. The budget is a CI test (§6.4), the write budget is a rule with a list of
  ...
  reason they are mechanisms rather than advice.
- **Scale.** F20 found the nonclustered index build to be the first SQL Server cost visible
  over tool overhead, at about a million rows. Whether any Prod table is above that is unknown
  here (item 7). LocalDB's 10 GB ceiling sits near the scale lane's largest scenario (1.18
  million rows); the scale lane runs on a full instance.
...
- Does the SSIS team map from the model or from the database during a two-release window? The
  changelog (§8.10) carries both shapes so that either answer works; the question is asked once.

---

## Appendix A — Glossary and renames

One vocabulary; where v1 or v2 used a different word for the same thing, the old word is listed beside it.
...
| **aggregate query** (`AggregateQuery`) | one `SELECT` of integer answers (counts, `SUM(CASE …)`, `MAX(LEN …)`) that the allowlist admits and `SqlServer.Measure` runs; `estate measure` is built from them | a profiling query | a probe |
...
provenance record; the bundle's `manifest.json` stays), torsor, delta (a change; δ as a norm),
CDC-as-norm, adjunction, pillar, axiom, theorem, bucket, ladder, matrix, chapter, handoff,
spine, seam, binding, face, voice, view, board, lane (except in the synthetic copy's data
lanes), verdict (in the estate sense), posture, archetype (a sample change; kept only inside
...
held, patience → `FileLock.Take`, `Timeout`, and a process run that exits, is not found or times out.
...
| op skills that mention the Integration Studio refresh | 9 of 45 (+ `os-vocabulary`, `_index/multi-phase`); 0 mentions in the pull request template, `THE_RECORD.md`, or the decision tree | `grep -ril` |
...
The pattern in B.2 is the whole corpus's: directionally honest, numerically loose, and never
reconciled because reconciliation had no owner. §15.4's generated surfaces are the owner.

---

## Appendix C — The operation catalog (kept, 45)

The op skills are the knowledge layer's vocabulary: they dispatch on the developer's own phrasing
("tick the Mandatory checkbox") and carry the flip conditions that decide how a change ships. All
45 are kept and each becomes shorter; grouped by family, with the proven shape on a populated table.
...
guidance and proof rather than in fidelity. Two of the seven deserve ops in v3's first quarter: CDC
...
`4e844fc`. "Consumers" means files outside the defining module that reference the concept, where
that number decided the disposition: **KEEP** · **KEEP-SIMPLIFIED** · **FOLD-INTO** · **RETIRE-AFTER-EJECT**
(frozen with the cutover tools, §16 item 4) · **DELETE**. The v3 column names where the capability lives (§6–§8).
...
| `Message.fs`; `Result.fs` (`ValidationError`, 338 `create` sites) | 58; 269 | four vocabularies for one thing, with `Diagnostics.fs` and `EstateFinding.fs` | FOLD-INTO | `Error` and `Finding` (§7); the library `Result` |
...
| `SamplingPolicy.fs`; `SamplingDiagnostics.fs` | 57; 29 | `Twin.Runtime/EvidenceImport.fs` | KEEP; FOLD-INTO | `Evidence.Measurement` |
| `Statistics.fs`; `Meter.fs` | 30; 30 | percentile math | KEEP | same |
| `Centrality.fs` + `Passes/CentralityPass.fs` | 18 + 173 | `SyntheticVolume.byCentrality` → `Twin.Runtime/Mint.fs`, `Pipeline/SyntheticLoadRun.fs` (the correction §7.9 records) | KEEP-SIMPLIFIED | `SyntheticData.volumes`; manifest reporting goes |
| `SyntheticVolume.fs` | 73 | `Mint.fs`, `SyntheticLoadRun.fs` | KEEP | `SyntheticData` |
...
| Module | Lines | Consumers / evidence | Disposition | v3 |
|---|---:|---|---|---|
| `Policy.fs` (Selection / Emission / Insertion / Tightening / UserMatching / BridgeRetarget) | 1,318 | v2's config surface; v3's unit is a change | KEEP-SIMPLIFIED | `Policy` with nine fields (§7.5) |
...
| Module | Lines | Consumers / evidence | Disposition | v3 |
|---|---:|---|---|---|
| `RegisteredTransforms.fs` (the 21-step chain); `TransformRegistry.fs`; `PassChainAdapter.fs`; `ComposeState.fs` | 227; 565; 162; 124 | a single-definition-site registry is elegant *because* the chain is long | DELETE | read → measure → decide → emit, four calls |
...
| `Bench.fs`; `PinnedWriting.fs` | 360; 157 | 279 `Bench.scope` sites and one gate script whose result is void under concurrent load | DELETE | the scale lane measures wall-clock |
...
| `DataCorrectionReceipt.fs`; `ApprovedDataCorrections.fs`; `ApprovalWorkflow.fs`; `ActConsent.fs` | 237; 526; 208; 329 | in-flight correction approvals | RETIRE-AFTER-EJECT | the receipt's content survives as three fields of the description's data section; the workflow does not |

### D.6 `Projection.Core` — synthesis and movement

| Module | Lines | Consumers / evidence | Disposition | v3 |
|---|---:|---|---|---|
| `SyntheticData.fs` (σ) | 824 | the Twin's generated set; `π ∘ σ ≈ id`; zero orphans by construction | KEEP | `SyntheticData` |
| `SyntheticCorrection.fs` (`PiiKind`/`MaskRule`/`FakerGenerator`) | 345 | "no real data on laptops" is how | KEEP | `SyntheticData` |
...
| `ApplyRunbookEmitter.fs` | 86 | an operator checklist for a manual apply; Octopus applies | RETIRE-AFTER-EJECT | the description's after-deploy section |
...
| `DockerImageEmitter.fs` | 290 | one of three ways to stand a copy up | FOLD-INTO `io/SyntheticCopy` | one path |
...
| `DistributionsEmitter.fs` | 344 | operator diagnostics; distributions reach the generator directly | DELETE | nothing |
...
| `SummaryFormatter.fs`; `DecisionLogEmitter.fs`; `SuggestConfigEmitter.fs`; `ActionableDiagnostics.fs`; `EstateOverlayEmitter.fs`; `Routing.fs` | 343; 321; 157; 152; 98; 104 | board rendering; a per-column rationale dump; ranked config edits; the RELAX lane | DELETE | the description; `Finding` |
...
| `Retry.fs` (Polly 8.5.0) | 113 | `MetadataSnapshotRunner.fs:837`; a v2 extension rather than a v1 carry | KEEP | `io/SqlServer` |
...
| `LiveProfiler.fs` | 872 | evidence capture; the Twin's evidence import | KEEP | `io/SqlServer` |
| `EvidenceFingerprint.fs`; `ServerDigest.fs`; `DataIntegrityChecker.fs`; `AsyncStream.fs`; `ConnectionResolver.fs`; `SqlPolicy.fs` | 128; 124; 150; 84; 73; 56 | the one-round-trip staleness query; `CHECKSUM_AGG`; the aggregate queries that predict what the publish proves; secret-free connections; the 300 s default timeout | KEEP | `io/SqlServer` |
...
| `SyntheticLoadRun.fs` | 234 | the pre-Twin generation path | FOLD-INTO | `io/SyntheticCopy` |
| `DriftRun.fs`; `ProfileCaptureRun.fs` | 34; 53 | permanent workflows | KEEP | `check drift`; `measure` |
...
| `Comparison.fs` | 754 | rendering a change for a human | KEEP-SIMPLIFIED | `diff`'s rendering, ~200 |
...
| `Twin.Core/Evidence.fs`; `DerivedEvidence.fs` | 507; 172 | the literal-free shape tier; the zero-config floor | KEEP | `SyntheticData` |
...
| Item | Lines | Evidence | Disposition | v3 |
|---|---:|---|---|---|
...
| `GoldenCatalog.fs` | 468 | the parity checks | KEEP | `tests/Golden/` |
| `CatalogDiffTests.fs` (54 facts) | 1,156 | v3's centre deserves its combinatorics | KEEP | `Change`'s tests |
...
| `scripts/perf-gate.sh` + `bench/`; `lint-discipline.sh` (609 lines, 566 exemptions); `verifiability-gate.sh` | — | a result void under load; a lint with an exemption every 210 lines; bucket honesty for buckets that go | DELETE | the scale lane; the dependency laws |
...
| Part | Files / lines | Disposition | v3 |
|---|---:|---|---|
...
| `scripts/prove.mjs`; `bake.mjs`; `inflight-check.mjs`; `ssdt-agent-gates.mjs`; `ssdt-agent-package.mjs` | 427; 251; 145; 370; 682 | the shipped halves of Layers 1, 0 and 3; the four-surface tax | KEEP-SIMPLIFIED, as verbs | `prove`; `synthetic-copy bake`; `check inflight`; three tests; `knowledge package` |
| `proving-ground/` | 8 tables, ~35 rows | KEEP-SIMPLIFIED | the golden schema; the synthetic copy fills it |
| `copilot-package/`; `.claude/` | 80 / 1,210; 73 / 1,405 | KEEP (generated) | regenerated, never hand-edited |
| `pipelines/`; `pr-template/` | —; 1 / 87 | KEEP | `ci/`; the description template |

---

## Appendix E — The v1 carry ledger

Source: the parity matrix's NOT-MAPPED and V1-SUNSET rows and the v1 trunk itself, tested against
one question: does a post-eject workflow (§2.4) need it? Sizes are `wc -l` on `src/` at `4e844fc`.
...
| table sampling policy | `Osm.Pipeline/Profiling/TableSamplingPolicy.cs` · ~61 | carried: `SamplingPolicy.fs` (57), consumed by the Twin's evidence import | **already carried** | `Evidence.Measurement` |
...
Five carries, two as tests, four already carried, the rest left. §12's sentence holds: v2 dropped
almost nothing the post-eject job needs, and what it dropped that matters most is what v3 is built around.
...
In order of return on the hour; each carries a finding this document depends on and does not restate.
...
2. `ssdt-agent/FINDINGS_AND_CHANGES.md` (502) — F1–F20, the DacFx facts proven against a live
...
The repository was read in three passes on 2026-09-17 against `4e844fc`: twenty-eight ingestion
digests covered every directory and document family; four roll-ups consolidated them by region;
a planning pass critiqued the outline and the drafts, produced the deletion and carry ledgers,
checked the size budgets bottom-up, and named the risks the outline had not. The author read the
load-bearing sources directly and re-measured every number a section depends on; Appendix B
records where the measurements disagreed with the documents or the passes.
