...
`CLAUDE.md`, its chapter and handoff and decision ritual, the `ssdt-agent` tree's skills,
...
It is built the same way as its companion. §0 is the answer. §1–§3 are the diagnosis and the
principles. §4 is the values register, the part the code document did not have. §5 is the
inventory, every document in both repositories with its reader, its moment, its budget and
its owner. §6–§9 are the drafts: the engine repository's files, the knowledge layer's files,
the estate repository's generated bundle, and the agentic interface (the CLI as the tool
surface, the hooks, the session protocols). §10 is how it stays true. §11 is the migration
of the documents. §12 is the risks. The appendices hold the retired vocabulary, the reading
paths, and the numbers.

**Contents.** §0 the answer · §1 who reads what, when · §2 what v2 taught · §3 principles ·
...
**The answer.** A document is a product with a named reader and a named moment, and it is
...
package. The knowledge tree is the product's front door and carries domain truth only:
one authoring skill, one reviewing skill, one record page, the operation catalog, the shared
reasoning, the findings with receipts, a dozen samples, the ledgers. Everything an agent
...
are wrappers over a verb. The engine and the estate are two repositories, which is the
companion's default and its ninth operator decision, not a settled fact; §8 says what changes
if they merge. The values the codebase upholds are written once, each with the
...
**The one mechanism that carries the rest.** The inventory of documents is data
...
of v2's write budget (§15 of the companion) a test can hold; the rest of the budget (what a
session may write, and whether a name or a rationale is good) is judged by a reviewer, and §4
says so rather than pretending a lint can judge it.
...
`.sql` file is open. The record register (agentless, finding first, true verb, evidence
beneath, admit the unverified). The findings file with database names as receipts. The
ledgers. A read order stated as a list. "Verify before you diagnose." The one-door prompt. The
citation gate.
...
Stop hook that ran a performance gate after every message, the PreToolUse probe that ran
...
|---|---|---|---|---|---|---|---|
| **A developer** (OutSystems-native, Copilot in Visual Studio, the estate repository) | the one-door prompt (`#prompt:schema-change`) | the router (`copilot-instructions.md`, attaches itself) | `knowledge/authoring.md` → one `ops/<op>.md` → the shared skill it names | — | the verdict's `remedy` line; `ops/<op>.md` "the named trap" | the record they wrote | the one business question, asked by the tool |
| **A reviewer / dev lead** | `knowledge/record.md` (one page) | — | — | the pull request body; `knowledge/reviewing.md` if reproducing | the gate's `gate.json` | the ledgers (`operations.md`, `in-flight.md`) | `refusals.md` (escalations land here) |
...
| **A Copilot session** (authoring, the estate repository) | the router | the path-scoped instruction that attached | `authoring.md` phases S0–S8 with their verbs | `reviewing.md` | the verdict JSON | the record | the fork form (`ask-the-developer`, folded into `authoring.md`) |
| **A CI lane** (proof, bake, gate) | `ci/README.md` | — | — | `estate gate` | the lane's own log; `gate.json` | `changelog.json` | — |
| **A review bot** (Claude Code Review, Copilot review, on either repository) | the PR template (its sections are the bot's checklist) | — | — | `knowledge/record.md` for a schema PR; `VALUES.md` for an engine PR | — | — | — |
| **A new joiner, either repository** | `README.md` (150 lines, five minutes) | — | — | — | — | — | — |
| **A developer on the bottom rung** (agent mode off; the router and the instructions are all that attach) | the router, read by hand | the router | `knowledge/INDEX.md` → the operation → `authoring.md`, read as documents; the verbs typed by hand | — | the verdict | the record | — |
| **Whoever deploys and refreshes** (the Integration Studio half, per environment) | — | — | — | — | — | the record's after-deploy section for that environment | — |
...
- **Five files do most of the work.** `AGENTS.md`, the router, `authoring.md`, `record.md`,
...
| the chapter ritual | 56 `CHAPTER_*.md` · 11,692 lines · open/close/pre-scope skeleton with a binding presentation contract per chapter | the session that wrote it |
| the handoff ritual | `HANDOFF.md` 3,887 lines, prepend-only, second-person letters; plus backlogs | the next session, once |
...
- **The record register.** Nine rules and a banned list that turn an agent's output into
...
- **Findings with receipts.** `FINDINGS_AND_CHANGES.md` names twenty engine behaviours, each
...
- **A read order stated as a list.** `ssdt-agent/README.md` says: read `THE_RECORD.md`, then
  `agents/`, then `skills/`, then the proving ground. Four lines. Every entry file in v3 ends
  the same way.
- **"Verify before you diagnose."** The root `AGENTS.md` rule that an agent runs the probe
  before claiming a tool is missing. In v3 the probe is a verb (`estate doctor`), and the rule
  is one line pointing at it.

### 2.2a Built and not yet exercised, and kept on that understanding

These are good designs with no consumer on record. They are kept because their shape is right,
and the pilot (companion §16, item 2) is where they earn the other column.
...
- **The one-door prompt.** `#prompt:ssdt-schema-change` runs intake and authoring in one
...
- **The ledgers.** Eight append-only tables of 272 lines designed to carry the estate's actual
  ...
  ships through the pipeline. A fact looked up is a fact; a fact remembered is a guess; a ledger
  with no rows is a format.
- **The estate pipeline.** `ssdt-agent-check.yml` exists and must be registered by a person in
  Azure DevOps; nothing records that it has been.

### 2.3 What did not work, and why

- **Rules as prose that a mechanism could own.** Fifteen survival rules, each restated in full
  because "each has cost an agent real time." Read them again (Appendix C of the companion
  lists which are still true of v3). Rule 1 (never run the two test pools together) is a CI
  ...
  F# compiler facts that die with F#. Rule 10 is about a file v3 does not have. Rules 11 and 13
  ...
  which is a sentence in `record.md`'s "what proving showed" guidance). The list was honest and
  it was the wrong medium.
- **The ritual.** Chapters, handoffs, and four-field decision entries: 49,336 lines of process
...
- **Four hooks, 814 lines.** A 431-line SessionStart script that installs a SDK, a tool, a
  daemon, an image and a container; a probe on every shell command; a performance gate after
...
- **The three-agent hand-off protocol.** Intake, change-author, reviewer, each a 160–320-line
  ...
  The one-door prompt had already merged the first two. v3 has two: authoring (one session, the
  phases inside it) and reviewing (a separate person with a separate trust model), and the
  reviewer's engine is the pipeline.
- **Four surfaces per operation.** A skill, a sample pull request, a self-test prompt, and a
  Twin fact, kept in agreement by five gates in 370 lines and a 682-line packager (1,875 lines
...
### 2.4 The one sentence

The corpus was directionally honest and numerically loose because prose was the mechanism of
record. v3 makes the mechanism the record and lets prose say only what a mechanism cannot.

---

## 3. The principles

These are not a second doctrine. They are the companion's thesis (§5.1–§5.5) and its three
corollaries, restated for documents, each with the v2 fact that taught it; nothing here is
minted that the code document does not already assert. The mapping is given in brackets.
...
6. **The record is the product, the pull request is the handoff, git is the timeline.** [§5.3] A
...
7. **Sized for the smallest context that must hold it.** [§5.4; §2.3's sizing paragraph] The developer's context is Copilot in
   Visual Studio with the SQL file open. The whole one-door path (router → `authoring.md` → one
   operation → one shared skill → `record.md`) is under 600 lines, and the budgets test computes
   that sum. An operation is under 90 lines; the router under 60; the front door under 150.
...
8. **Values are stated once, each with its mechanism and the mechanism's name.** [§5.6's third corollary: laws as tests] `VALUES.md`
   ...
   a known weakness, not a hidden one. v2's counter-example: 187 stated guarantees across
   twenty-four themes, of which the ones that held were the ones with a test, and no file said
   which those were.

One rule sits above the eight and applies to every surface, not only records: **the register.**
Agentless, finding first, the true verb, evidence beneath, exact names, the unverified admitted,
ending on the move, no private nicknames. `AGENTS.md` is written in it; so is this document's
...
The register is written in the same voice as everything else: the requirement is a sentence
with a verb, the mechanism is a name a reviewer can grep for.

Three things are judged by people and no lint judges them: whether a name is the domain's,
whether a rationale is substantive, and whether a record reads well to the lead. v2 stated all
three as rules and then measured compliance by counting markers (566 `LINT-ALLOW` sites, each
supposedly carrying an analysis; the tree's own guide named the failure mode
"performance-of-compliance"). v3's answer is to shrink the surface being judged (an operation
under ninety lines, a one-page register, a ten-section record, a six-section engine template)
and to say "prose only" in the rows below where that is the truth. Of v2's twenty-four
families of stated guarantees, the string-composition, mutation, type-closedness and layering
families collapse into L7 and L8 and the analyzers; lineage and the transform registry die with
the chain; the cutover governance archives with the cutover; the rest are here.

### 4.1 Safety

| # | Value | Requirement | Mechanism | Where | v2 |
|---|---|---|---|---|---|
| S1 | The guard is never relaxed | No verb, flag, profile or script in v3 relaxes `BlockOnPossibleDataLoss` against a named environment; the Permissive profile is accepted only for a disposable target. | `io/Publish` refuses a `Target.Environment` under Permissive (exit 9); the only profiles the gate accepts are the two under `ci/profiles/` | `Io.Tests: "permissive never reaches an environment"` | `HANDOFF_SESSION_2026_08_26.md` §2 (the owner's axiom); prose only |
...
| S3 | The explicit negative is a finding | Every record section renders; a section with nothing to report says so in one sentence and is never omitted or padded. | `Record.render` refuses an empty section | `Kernel.Tests: "every section renders"` | `pr-template/schema-change.md` ground rule; gate `register` |
| S4 | A rename keeps its data | A delta containing a rename with no refactorlog entry is refused before any publish. | `classify` raises `RenameWithoutRefactorlog` as `Block` | law 6; `Io.Tests` | `_index/identity-and-refactorlog`; prose only |
| S5 | Data corrections leave a receipt | Any pre-deploy script that modifies rows is recorded with rows-before, rows-after and the approving human in the record's data section. | `Record.TheData` carries the three fields; the gate refuses a data-modifying pre-deploy whose record lacks them | `Io.Tests: "reconcile without receipt is refused"` | `ApprovedDataCorrections` (1,669 lines, cutover-era); the receipt survives, the workflow does not |
...
| D5 | Seeds converge, never churn | A guarded `MERGE` over unchanged rows touches zero rows; the content hash is unchanged. | law 11's seed half; `Emit` writes only guarded `MERGE` | `Io.Tests` | F12 (`pg_seed`); `_index/idempotent-seed`; mechanized in the proof lane |
| D6 | The mint repeats itself | The same seed mints byte-identical rows; σ takes its generator as a parameter. | law 11; `Random` banned in `kernel/` | `Kernel.Tests`; the golden fixture of v2's rows at one seed | Twin law 1; mechanized (`Check.fs`) |
| D7 | Fidelity in the quotient | Reading back what was emitted reproduces it after `SqlType.coarsen`, modulo a named list that starts empty. | law 2 | `Io.Tests` | the canary; A18/T16; mechanized |

### 4.3 Privacy

| # | Value | Requirement | Mechanism | Where | v2 |
|---|---|---|---|---|---|
| P1 | No real data on a laptop | The substrate is minted from a literal-free shape tier; the rich tier never enters a repository; no runbook restores a real backup. | law 12; `twin evidence verify` refuses a literal; `.gitignore` and a CI grep refuse `evidence.rich.*` | `Io.Tests`; CI | Twin law 3; mechanized; `PROVING_PATH_WINDOWS.md`'s `.bak` route is deleted |
| P2 | Profiling reads shape, not rows | `profile` captures counts, null rates, lengths, distinct counts and distributions; it never writes a row value into evidence, a log, or a verdict. | `Evidence` has no field for a value; the profiler's SQL is reviewed against that type; the sampling policy caps reads | `Kernel.Tests` (the type); `Io.Tests` (the probe SQL is aggregate-only) | `SamplingPolicy`; partly mechanized |
| P3 | Masking is the default | Any value that must be realistic (names, emails, phones) is minted by a masking rule, never copied. | `Synth.Correction` (PII kind → Faker realization at the io boundary) | `Kernel.Tests` | `SyntheticCorrection`; mechanized |
...
| X5 | No telemetry, no phone-home | `estate` opens no network connection except to the SQL Server it was given and, for `twin restore`, the artifact path it was given; the build sends nothing either. | `HttpClient` and `System.Net` banned in `kernel/` and `cli/`; `io/` allows only `Microsoft.Data.SqlClient` and Docker's local socket; `DOTNET_CLI_TELEMETRY_OPTOUT=1` in `Directory.Build.props` and every lane; one sentence in the README | `BannedSymbols.txt`; `Io.Tests`; CI | not stated; the opt-out variable appears nowhere in either generation; new |
...
| # | Value | Requirement | Mechanism | Where | v2 |
|---|---|---|---|---|---|
| R1 | The engine is pinned and stamped | Every verdict names the DacFx version, the SQL Server image digest, and the tool version it ran on; a version that differs from the pin is a refusal. | law 10; `Verdict.Engine` fields; `ledgers/toolchain.md` read by `prove` | `Io.Tests` | `estate/toolchain.md`; the pin is UNPINNED today; partly mechanized (the hook stamps `-unpinned`) |
| R2 | Every claim has a receipt | A verdict carries the disposable database's name, the generated script's hash, and the timestamp; a record cites them; a finding in `findings.md` names its database. | `Verdict` fields are required; the register lint refuses a "what proving showed" section with no receipt | `Kernel.Tests`; `Budgets.Tests: register` | `FINDINGS_AND_CHANGES.md` (database names as receipts); prose only |
| R3 | Git is the timeline | There is no run store, no episode store, no lifecycle file; artifacts are named by fingerprint; the pull request is the record of a change. | absence (nothing to mechanize); the manifest has no such file | — | `Episode`/`Lifecycle`/`RunLedger` (deleted) |
...
| R5 | The supported window is stated | .NET: the current LTS only (10 today; 9 is a standard-term release and leaves support in November 2026). SQL Server: 2019 and 2022, and LocalDB of the same versions, with the image pinned by digest, never `latest`. DacFx: the pinned version and the one before it. Visual Studio: the rung the pilot lands on. | `global.json` pins the LTS feature band with `rollForward: latestPatch` (v1 and v2 pin `9.0.314` with roll-forward disabled; the loosening is a decision line); `twin up` and `prove` probe `@@VERSION` and refuse an engine outside the window by name; `Io.Tests` runs on both SQL versions | CI matrix; `ledgers/toolchain.md` | `estate/toolchain.md` names the floating `2022-latest` tag as a risk; the rest is new |

### 4.6 Portability and operability

| # | Value | Requirement | Mechanism | Where | v2 |
|---|---|---|---|---|---|
| O1 | Docker is a substrate, never a requirement | Every proof that does not need CDC or the scale tier runs on LocalDB; the verb chooses the substrate and says which it chose. | `io/Twin` has one substrate abstraction with two artifacts; `prove` refuses CDC proofs on LocalDB with the reason | `Io.Tests` on both substrates; `PORTABILITY.md` | `PORTABILITY.md`; partly mechanized (`prove.mjs` config) |
...
| O3 | One command per intent | Every step of every workflow is an `estate` verb with `--json`; no document shows a tool invocation that is not an `estate` verb, and every hook is under ten lines over a verb. | the docs lint refuses `sqlpackage `, `sqlcmd `, `docker ` in prose outside `knowledge/handbook/`; the hooks budget | `Budgets.Tests: "no folklore"`; `Budgets.Tests: hooks ≤ 30 lines` | the review's Layer 1; `prove.mjs`; partly mechanized |
| O4 | Refuse and route | An unsupported object or operation is refused with who does it and what to check; the tool never guesses. | `Refusal` codes catalogued in `--help --json`; the "not covered" list in the catalog | `Cli.Tests: "every refusal code has a remedy"` | `rebuild-index` (refuse-and-route); prose only |
| O5 | Cleanup is guaranteed | A disposable database or container is named `estate_<fingerprint>_<pid>`, dropped on exit or cancellation, and swept by `estate twin gc` on the next start if a crash left it. | `try/finally` around every `Disposable`; the sweep on `doctor`; cancellation tokens through `io/` | `Io.Tests: "kill mid-prove, next run sweeps"` | v2 leaked 209 databases on one warm container (rule 2); partly mechanized later |
| O6 | Two sessions do not collide | Two `estate` processes on one machine never share a database name, a scratch directory, a bundle output or the Twin's state. | one prefix per session (`estate_<host>_<pid>_<rand>`); `twin up` takes an advisory lock on `[twin].[__state]`; `emit --out` refuses a non-empty directory it did not create; the invariant is restated in `tests/README.md` because the only written statement of it today is the self-test protocol, which is deleted | `Io.Tests` (two concurrent proves) | `self-test/PROTOCOL.md` (`PG_<id>_<rand>`); prose only |
| O7 | Long operations can be cancelled | Every verb honours Ctrl-C and `--timeout` within a second, leaves the state a normal exit leaves, and exits 130. | one `CancellationToken` threaded from `cli/` through `io/`; `Console.CancelKeyPress` → cooperative cancel → the cleanup path of O5 | `Io.Tests: "cancel mid-prove"` | eight occurrences of `CancellationToken` in 118,686 lines; a mint of 1.18 million rows that cannot be interrupted is the concrete failure; new |
| O8 | Time is budgeted | On a 300-table estate: `classify`, `diff`, `decide` under ten seconds; `read` and `emit` under sixty; `prove` bounded by the engine, not by the tool, and under five minutes at the ≤1M tier; `twin restore` under five minutes; a profiling probe capped by the sampling policy and a 300-second timeout. The numbers accrete from real proofs (`prove` records its wall-clock and tier) rather than from a synthetic bench. | the scale lane measures and records in `ledgers/scale-datapoints.md`; one scale test in `Io.Tests` over the golden schema replicated to 300 tables; never a Stop hook | `Scale.Tests` | `Bench` + the perf gate (deleted: void under load, self-feeding); the 62-line datapoints ledger is the part that had value |
...
| O11 | Large output is truncated, never dumped | A verdict's verbatim message and a delta over 300 tables do not overflow an agent's context; a payload that can be large carries `"truncated": true, "full": "<path>"` and a `--summary` form, and a 300-table delta's default rendering is under a stated line count. | the truncation fields in the JSON contract; a test over the golden schema replicated to 300 tables | `Cli.Tests: "large delta renders short"` | not stated; the consumer is a modest-context session; new |
| O12 | Time is UTC, stamped at the boundary | Every date a record, a ledger row or a fingerprint carries is UTC (`yyyy-MM-dd`, or the round-trip format), produced by one clock in `io/`, never in the kernel; the in-flight window comparison uses the same clock. | `DateTime.Now` banned everywhere; `UtcNow` only behind one `IClock` in `io/`; a format lint on ledger dates | `BannedSymbols.txt`; `Budgets.Tests: ledgers` | nothing stated the zone; the window gate compared against "today" on whatever machine ran it; new |
| O10 | Offline works | With no network, every verb except `twin restore` from a remote path succeeds against a local SQL Server, and the build restores from the local cache. | X5's bans; `RestoreLockedMode` with a warmed cache in CI | CI (one lane runs with networking disabled) | not stated; new |

### 4.7 Legibility and maintainability

| # | Value | Requirement | Mechanism | Where | v2 |
|---|---|---|---|---|---|
| L1 | The register applies to every surface | Every record, refusal message, README, and this file are agentless, finding-first, true-verbed, evidence-beneath, exact-named, unverified-admitted, and free of the retired vocabulary. | the register lint runs over `knowledge/samples/`, every `Refusal.Message`, and every hand-written file in the manifest | `Budgets.Tests: register` (three tests); `Cli.Tests: refusals in register` | `THE_RECORD.md` (nine rules) + gate `register`; mechanized for records only |
...
| L3 | Budgets are tests | Code per package, tests, docs outside `knowledge/`, `knowledge/`, each hook, the front door, the router, an operation, and the one-door path each have a ceiling, and exceeding one is a red build. | `Budgets.Tests` reads `ci/budgets.json` | `Budgets.Tests` | `CRYSTALLINE_FORM.md` estimated; nothing enforced; new |
...
| L11 | Reading time is budgeted | The README reads in five minutes; the front door in five; an operation in three; the prose a Copilot session must hold to author one change (the router, the one instruction that matched, one operation, one shared file, the record page) fits beside the open file, and the sum is a test. Today that path is 665 lines (router 60 + instruction 23 + the largest operation 131 + the two record pages 451); after the trim it is about 320. | the budgets test computes the path sum with a ceiling the pilot sets | `Budgets.Tests: "one-door path"` | the review's §3.6 ("every hop is a failure point"); the companion budgeted a Claude Code session's reading and not a Copilot session's; new |
...
| # | Value | Requirement | Mechanism | Where | v2 |
|---|---|---|---|---|---|
| A1 | Prove before you claim | An agent never states how a change ships without a verdict; a pre-proof classification is marked provisional in its first word. | `classify` prints `provisional:`; the gate regenerates the record from the verdict and diffs it | `Cli.Tests`; the PR gate | the tree's thesis; the rubric; prose |
| A2 | Verify before you diagnose | Before claiming a tool, a daemon or a database is missing, an agent runs `estate doctor` and quotes its line. | the verb; the SessionStart hook prints it | `AGENTS.md` (one line) | root `AGENTS.md` "verify-before-diagnose"; the probe hook |
| A3 | Ask one question | Authoring poses exactly one business question a human must answer, in the developer's words, and never answers it by reading data. | `authoring.md` S0's exit guard; the record's `Not checked` names the open question | `Budgets.Tests: register` (a record with two open questions fails) | `intake` rules; prose |
...
| A9 | The record is what the gate reads | The PR body's "how it ships" and "what proving showed" sections match the regenerated record or the gate says where they differ. | `estate gate --pr-body` | the PR gate | `author-pr`'s hard rules; prose |
...
| # | Value | Requirement | Mechanism | Where | v2 |
|---|---|---|---|---|---|
| E1 | Emit then read is the identity | Reading back what was emitted reproduces it in the quotient, modulo a named list that starts empty. | law 2 | `Io.Tests` | the canary; mechanized |
...
| E5 | An unparseable object is refused, not degraded | A trigger body ScriptDom cannot parse, or any object the reader cannot represent exactly, is a refusal with a code, never a comment marker or a tolerated divergence. | a `Refusal` code (`read.unparseable`) and one test per object kind | `Io.Tests: "unparseable trigger is refused"` | `ToleratedDivergence.TriggerBodyUnparsedDropped`; prose plus a tolerance; the tolerance goes |
| E6 | Data-loss steps are named before the publish | Every statement the guard will refuse on a populated table is listed by `diff` before any publish runs. | `Delta.dataLoss` | `Kernel.Tests` | the tree's classification step; mechanized in the delta |
| E7 | Rollback is computed or admitted | Every record says how to reverse the change or names what is not auto-undone, with the recorded originals a manual restore would use. | law 7; `Record`'s rollback section is required | `Kernel.Tests` | `THE_RECORD_FORMS.md`; prose |
...
| # | Value | Requirement | Mechanism | Where | v2 |
|---|---|---|---|---|---|
| G1 | The pipeline reproduces the proof | The gate rebuilds the dacpac, restores the substrate, publishes the pull request's combined delta under the production posture, and posts the verdict and the record diff. | `estate gate`; the PR lane | `ci/` | the review's Layer 3; half-shipped (`inflight-check.mjs`, the ADO template) |
| G2 | The human makes the business call | The one question only a human can answer is posed, recorded with its owner, and never answered by reading data. | `authoring.md` S0 and S6; the record's `Not checked` names it if open | `Budgets.Tests: register` | `THE_DECISION_TREE.md` S6; prose |
...
| G5 | Prod is not Dev | Before any Prod release, Prod's row counts land in `row-tiers.md` with their date, and the gate reads the tier for the target environment. | `estate profile --target prod --counts`; the gate's ledger read is per environment | `Io.Tests` | the tier ledger holds four sample tables; new |
...
- **The string-composition lints** (rules 3–5, 18b–e: `+`, `String.Format`, `String.Concat`,
  ...
  analyzer settings and one forty-line writer defend better; that sentence is this section in
  miniature.
- **The mutation lints** (`let mutable`, mutable collections, `<-`, mutable record fields).
  Replaced by records, `Seq<T>`, and `readonly` structs; the kernel's purity is a project
...
| 2 connection failures mean the container died | `prove`/`twin` refuse with `substrate down: run estate twin up` (O5) |
| 3 never `pgrep`-guard or `tail` a run | dies with the shell folklore; the lanes run tests directly and every job has a timeout |
...
| 5, 6 F# compiler shapes | die with F# |
| 7 `{ x with … }` silently inherits constructor defaults | dies with F#, and its C# sibling is one line in `tests/README.md`: a `with` expression on a record copies fields and bypasses any validating factory, so invariants that span fields (a primary key names existing columns) live in `Schema.Create` and in the laws, never in a setter |
| 8 `ReadSide` marks every table `Static` | fixed by construction; kept as a case in law 2 |
| 9 `ISNULL(col,col)` strips IDENTITY through `SELECT INTO`; a `CASE` wrapper does not | a proven engine fact: an entry in `knowledge/findings.md` with its receipt, cited by the identity-swap operation |
| 10 `HANDOFF.md` is prepend-only | no such file |
| 11, 13 the perf gate's baseline and solo verdicts | no such gate |
| 12 soft-skipped Docker tests look like passes | still true of xUnit in any language: L6 (no skips) and the matrix's per-substrate report, and the sentence stays in `tests/README.md` |
| 14 the content hash and the `''`/`NULL` comparator | two sentences in `record.md`'s guidance on "what proving showed"; the comparator is `RowFidelity`, kept |
...
| 1 (first half) never run the pools together | weakened, not retired: two test projects and a fifth of the lines make the OOM unlikely, and `Io.Tests` is declared serial, which is the same rule in different clothes; one line in `tests/README.md` |

Four survive as sentences in the test README (1's second half, 4, 12, 15), three become a law or
a finding (8, 9, 14), one gains a C# sibling (7), and the rest die with the code they describe.
That ratio is the argument for §3's second principle.
...
| `io/Twin/README.md` | hand | a maintainer · a change to the substrate | 620 (kept as-is) | `THE_TWIN.md` + `THE_SYNTHETIC_DATA_DESIGN.md` |
...
| `.github/PULL_REQUEST_TEMPLATE/schema-change.md` | generated (from `knowledge/record.md`) | a developer · a schema PR (this repository's own golden project) | — | `pr-template/schema-change.md` |
...
| `knowledge/record.md` | hand | everyone who writes a record · a change, a review | 120 | `THE_RECORD.md` (308), `THE_RECORD_FORMS.md` (143), `author-pr` (223) |
...
| `.github/prompts/schema-change.prompt.md` | generated | vendored | Copilot on Visual Studio 2022 · the one door |
...
| `estate/posture.json` | hand | **estate** | every verb · always (the publish posture, the writable targets, the substrate) |
| `estate/evidence.shape.json`, `estate/twin.json` | hand (produced by verbs, committed) | **estate** | `twin` · the bake lane |
...
through a verb that appends a row (`check cdc`, `profile --counts`).
...
One paragraph: what the tool does (reads a schema from anywhere, profiles the data beneath it,
computes the change between two states, proves the change against a real-shaped disposable
copy by letting DacFx publish it, writes the record a reviewer approves by reading).

## The verbs                       — one line each, pointing to cli/VERBS.md; no count in prose
## Try it in five minutes          — estate doctor · estate twin restore <artifact> ·
...
the tree a developer's Copilot session reads in the estate repository. Read this file, then `NEXT.md`,
...
Run `estate doctor`. Its one line names the SDK, the DacFx pin, the substrate (Docker or
LocalDB) and the Twin artifact, and every refusal in it carries a remedy. Quote that line
before claiming a tool, a daemon or a database is missing.
...
evidence beneath, names exact objects, admits what was not checked, and ends on the move. No
numbered tiers or mechanisms, no pillars, no axioms, no private nicknames; the retired words
and their replacements are listed in `knowledge/record.md`.
...
- The SessionStart hook makes `dotnet` exist and runs `estate doctor`; the SessionEnd hook
  runs `estate twin down --if-idle`. Both are under ten lines, in `.claude/hooks/`.
...
```json
{
  "permissions": {
    "allow": [
      ...
      "Bash(estate twin up*)", "Bash(estate twin down*)",
...
the substrate refuses with the remedy, which is what the probe used to say.

```bash
#!/usr/bin/env bash
# .claude/hooks/session-start.sh — make dotnet exist, then let the verb speak.
set -u
...
```bash
#!/usr/bin/env bash
# .claude/hooks/session-end.sh — release the substrate if nothing else holds it.
dotnet estate twin down --if-idle >/dev/null 2>&1 || true
```
...
first four and `estate twin up` for the fifth, both verbs a human can run from a terminal and
...
the honest negative is the section a reviewer relies on.
...
- **`cli/VERBS.md`** — from `estate --help --json`: every verb, its flags, its JSON output
  schema, its exit codes and refusal codes with remedies. The bundle's router and
...
`knowledge/` is the product's front door and the only prose a developer's session reads. It
carries what is true of SQL Server, DacFx, the estate and the platform, with receipts, in the
register. It carries nothing about the engine's construction and nothing about itself. The
companion's §9 decided its shape; this section drafts its files.

### 7.1 `knowledge/README.md` — the front door (150 lines)
...
database, populated with real-shaped data, and read what the publish engine does. What it
does is the classification. `estate prove` does this in one command.
...
reviewing.md — how a review reproduces, scopes, attacks and disposes
record.md — the register and the ten sections every record has
...
findings.md — what has been proven on a live engine, with the database that proves it
samples/ — a dozen records, one per shape
...
## Read order
record.md · authoring.md · the operation that matches · the shared file it points to. That is
...
The decision tree's nine states, each naming its verb and its exit guard, with intake and
authoring as phases of one session. The draft's skeleton, with the guards verbatim from the
tree where they were right:

```markdown
# Authoring a change

One session, nine steps, one door. A compound request (several tables, several operations) is
split first: name each atom, order them (create before reference, add before drop), and run
the steps once per atom; the record is one pull request per release, not per atom.
...
## S2 · Probe            `estate profile --tables <t> --target twin`
...
## S4 · Prove            `estate prove --project … --target twin` (Strict; then `--permissive` on a block)
...
fact; each option with its consequence; one question) and record the answer with its owner.
Exit: posed and recorded, or answered.

## S7 · Record           `estate record …`
...
## S8 · Verify
Read the record against `record.md`. Every sentence denotes; every claim has its receipt;
...
row counts), attack (inject a violating row; play a blocked change forward under
...
§10.3) is the reviewer's engine for every pull request; this file is for the lead's sparring
and for the review of a change the gate could not fully reproduce. The invariant it carries:
no disposition exceeds the scope established; an unscoped cascade cannot be approved.

### 7.4 `knowledge/record.md` — the register and the template on one page (120 lines)
...
application behaviour, other environments, production scale, in "Not checked". 8 End on the
move: the next action as a bare imperative aimed at the object. 9 No numbered axes, engine
jargon, or private nicknames (the list is at the end).
...
## What changes      — <object>: <from> → <to>, one line per change; then the honest negative
...
document's source). A record that uses a retired word fails the register test.
```

The pull-request templates for GitHub and Azure DevOps are generated from the block between
the `template` markers, so the three cannot disagree. `estate record` renders seven of the ten
sections from the delta, the evidence and the verdict, and writes an explicit placeholder into
...
still contains a placeholder, which is the move the tree's ledger gate already makes for refusal
blocks. A generated skeleton good enough to ship unedited is how "approved by reading" degrades
into "approved as a form", and the placeholder is the guard against it.
...
## The named trap                     — the guard is table-has-rows, not column-has-blanks (F7)
## How it flips                       — empty table: in place · populated: two releases, even after the blanks are filled
## Prove it                           — `estate profile --tables …` · `estate prove …` · on a block, `--permissive`
## The verdict, to the developer      — the paragraph, in the record register
## On the record                      — which sections this op fills specially; the sample: samples/make-mandatory.md
...
idempotent, when it retires). Each: the members of the class; the mechanism, verbatim from the
engine with its finding number; the ladder (empty · populated · violating); how the per-op
...
```markdown
## F7 · 2026-08-21 · The guard fires on row presence, not on rule violation
...
kept. Every finding names its engine version, because the engine version decided F5.
...
defaulted column as one delta). Each is a record, in the register, proven on the golden
...
`classify` reads it; a table listed here turns a column-list change into a `Block` finding
...
integrity · the refactorlog · deployment safety and the guard · multi-phase changes · Change
...
Studio; its rungs are kept because they are honest about what is verified.
...
built into a dacpac, deployed through Azure DevOps into Octopus with the data-loss guard on.
The guard cannot be relaxed for a single deploy.
...
`#prompt:schema-change` is the door. A request to review a schema pull request is a review
...
## Writing
Every record (pull request body, review, disposition) follows `knowledge/record.md`. Teach the
developer in conversation; never in the record.
...
| `pre-deploy.instructions.md` | `Script.PreDeployment.sql` and `Scripts/PreDeploy/**/*.sql` | a pre-deploy prepares data for a change the model does not yet declare; never put the model change and the pre-deploy ALTER in one release; a data-modifying pre-deploy carries its receipt in the record |
...
description: Author a schema change on this estate. Use for any request to change the data
  model in the developer's words. Follows knowledge/authoring.md: intake, edit, probe,
  classify, prove, ship, fork, record, verify, with `estate` verbs.
---
Read `knowledge/authoring.md` now and follow it. The operation files are under `knowledge/ops/`;
the record register is `knowledge/record.md`.
```

`reviewing.agent.md` is the same shape over `knowledge/reviewing.md`. Two honest notes travel
...
`schema-change.prompt.md`, is the door for editors without agents: its first instruction is the
...
S0." On the 2022 rung the prompt is the only door, which is the rung where a compound request most
easily becomes one over-stuffed pull request; the gate backs the prompt by refusing a delta with
...
are generated from `record.md`'s template block. `AGENTS.md` at the estate root is the router's
text under the tool-agnostic name, for any agent that is not Copilot (a Claude Code session
opened on the estate repository reads it and finds the same one rule and the same door).
...
by the next vendoring, so the seed-once rule is a bug fix, not a refinement. The estate's own pipeline runs `--check` on every
...
hand-edits a vendored file; a drifted copy is a failed check, not a mystery. Two Azure DevOps
...
publishes the pull request's combined delta on a hosted Windows agent with LocalDB and no Docker)
has never run: its five placeholders are unfilled and its trigger is `none`.

### 8.6 The rungs

Kept from `ADOPTION.md`, because it is honest: Visual Studio 2026 18.5 and later discovers
...
`estate/posture.json` (the publish posture; the writable targets; the substrate), the project
...
`estate/evidence.shape.json` and `estate/twin.json` (produced by verbs the team runs, committed
by the team), `.editorconfig` and `.gitattributes` (D3), and the SSDT project itself. The
engine reads them; only a verb that appends a row (`check cdc`, `profile --counts`) writes to
...
{ "schema":   "estate.prove/1",
  "engine":   { "dacfx": "162.5.57", "sqlserver": "2022-CU14 sha256:…", "estate": "3.0.12" },
  "receipt":  { "database": "estate_9f3a1c_41872", "script": "sha256:…", "at": "2026-09-17T14:02:11Z" },
  "verdict":  { "outcome": "blocked", "shape": "two-releases", "message": "Msg 50000 … rows exist …" },
  "findings": [ { "code": "guard.row-presence", "severity": "block",
                  "subject": "dbo.Customer.Email", "message": "…", "remedy": "…" } ],
  "exit":     3 }
```

Three rules make it an API rather than an output. **Versioned:** `schema` names the verb and
a major; within a major, fields are only added; a removal or a rename is a new major and a
decision line. **Frozen exit codes:** 0 done · 1 bad arguments · 2 an input could not be parsed · 3 blocked by
the guard · 4 target unreachable · 5 divergence found · 6 configuration refused (an unknown key,
an inline credential, a toolchain pin mismatch) · 7 build failed · 9 refused by name (companion §8); `cli/VERBS.md` lists them and a test asserts the
list never shrinks. **Every refusal carries a remedy:** the `findings[].remedy` field is
required for `severity: block` and for exit codes 2, 4, 6 and 9, and the remedy is a verb or a
file path, never a paragraph. A checked-in JSON schema per verb under `cli/schemas/` is the
test's oracle, and `--help --json` emits the same schemas, so the verb reference is generated
from the thing the tests check.
...
```
estate doctor READY | sdk=10.0.4 | dacfx=162.5.57 (pinned) | substrate=localdb (2022) |
  twin=9f3a1c (restored 2026-09-16, current) | estate=../estate (posture ok, 2 open windows)
```

`DEGRADED` names what is missing and the remedy (`--install` for the SDK and the local tool;
`estate twin up` for the substrate; `estate twin restore <artifact>` for a stale twin). `--json`
carries the same fields. The SessionStart hook prints this line and nothing else; a session
quotes it before diagnosing anything (A2). It replaces 431 lines of hook with a verb a human
can run.
...
them. SessionStart makes `dotnet` exist and runs
`doctor`. SessionEnd releases the substrate if idle. There is no PreToolUse hook (a verb that
needs the substrate refuses with the remedy), no Stop hook (nothing runs after every message),
and no hook installs a daemon or pulls an image (`estate twin up` does, and refuses with a
reason when it cannot). A hook that grows past ten lines is a verb that has not been written.
...
pre-approved. `move`, and any `prove` whose
target names an environment rather than the twin, always ask. Generated files and the archive
are denied to edits and, for the archive, to reads. The permission file is the mechanism for
A6 and for the generated-file rule; a session that needs the archive says so and is granted it
for that task.
...
the operator can make, write it under "Waiting on a person" in `NEXT.md` and stop; do not
guess and do not ask in a chapter.
...
verb. The record is `estate record`'s output; the agent supplies the intent, the fork answer
and nothing else in its own words. The pull request uses the schema template. The gate
reproduces the proof and diffs the record; the developer refreshes Integration Studio per
environment after each deploy, and the record's after-deploy section says so.

**(c) A review session.** `reviewing.md`. The gate's `gate.json` is the first evidence; the
reviewer reproduces only when trust is in question, attacks with the two moves, and renders
one of four dispositions in the register. A returned change goes to the author; an escalated
one carries one question and the dependency scope to the lead.

**(d) A CI lane.** Three verbs at most. Proof (nightly): `twin restore` → `prove` over the
proven changes → publish the results `LAWS.md` reads. Bake (merge to `main`): `profile` (when
the source is real Dev, owner-side) → `twin bake` → `knowledge package --vendor` → a pull
...
is honest rather than blind: the session edits the CREATE, writes the intent, and opens the pull
request marked provisional; the gate proves the change and posts the record; the reviewer reads
the gate's record. Nothing is ever classified from the text.
...
A Claude Code session and a Copilot session produce the same record because neither writes
it: `estate record` renders it from the verdict, the delta and the evidence, and the agent
fills two fields (the intent in the developer's words; the fork answer with its owner). The
register is therefore the verb's property, tested once, and the agents' styles cannot leak
into the artifact a lead approves. The records that ship are written in the estate repository
and would otherwise never meet the lint, so the gate runs the register lint over the pull
request body there, and refuses a body that still carries a placeholder from `estate record`. In conversation, the agent is allowed the second person and
warmth (v2's conversation register, kept as one paragraph in `record.md`); the record is not.
...
| 2 | `Budgets` | `ci/budgets.json`'s ceilings hold: lines per code package (`wc -l`), tests, docs outside `knowledge/`, `knowledge/`, each hook (≤ 10), the router (≤ 60), the front door (≤ 150), every operation (≤ 90), and the one-door path sum (≤ 600) | L3, L11, O3 |
...
| 4 | `Register.Refusals` | every `Refusal` and `Finding` message in the CLI passes the register and carries a remedy where required | L1, O4 |
...
| 15 | `FindingsAppendOnly` | no `F<n>` heading present on `main` is absent on the branch; every finding names its engine version and its receipt | R4, A7 |
| 18 | `RecordInPullRequest` (runs in the estate's pipeline, in the gate) | the pull request body passes the register lint and contains no placeholder `estate record` wrote for a human section | L1, S3, A9 |
...
pinned engine (a sample whose verdict changed is a red lane); the bake lane regenerates the
bundle and opens the estate pull request; the gate reproduces a schema change's proof and diffs
its record. And one rule with no test, stated as such: `ARCHITECTURE.md` is edited only to
make it true again, one paragraph at a time. Tests 9 and 10 catch the two ways it most often
went false in v2 (a verb list and a law list); the rest is review.
...
| 2–5 | a package README as each package lands; `cli/VERBS.md` and `cli/CONFIG.md` generation when the CLI has verbs; `io/Twin/README.md` moved unchanged | `THE_TWIN.md`, `THE_SYNTHETIC_DATA_DESIGN.md`, `CONFIG_REFERENCE.md` |
| 6 | `knowledge/`: `README.md`, `authoring.md`, `reviewing.md`, `record.md`, the 45 operations trimmed, the eight shared files, `findings.md`, twelve samples, the ledgers (with `cdc-tracked.md`), the eight handbook chapters, `PORTABILITY.md`; `INDEX.md` and the pointers generated; the bundle regenerated with two agents, one prompt and five instructions; the first vendoring pull request to the estate | the `ssdt-agent` tree; v1's `handbook/` |
| 7 | the two schema pull-request templates generated from `record.md`; the engine template; the gate's `gate.json` and `changelog.json` posted on pull requests | `pr-template/`, `author-pr` |
...
Three setup surfaces have named successors: `KICKOFF.md`'s first-read role goes to `AGENTS.md`;
`GETTING_STARTED.md`'s stand-up role goes to `tests/README.md` and `estate doctor --install`;
`PROVING_PATH_WINDOWS.md`'s Windows runbook goes to `knowledge/PORTABILITY.md` and the same
verb, with the `.bak` route it described gone (it put real data on laptops).

Two things the migration does not do. It does not rewrite v2's documents into the register;
they are provenance and are archived as written. And it does not merge `DECISIONS.md`'s 480
entries into the one-line log; the log opens at step 0 with v3's decisions, and the archive
index points at the old file for anything earlier.
...
- **The tool has to reach the laptop.** Everything in `authoring.md` assumes `estate` is on
  ...
  the tool is on one champion laptop, the bundle's best line stands: stop and say so, rather
...
- **MCP on the team's build.** §9.3 is an open question, not a design: whether Visual Studio's
...
- **The manifest test as friction.** A session that wants to write prose will find it cannot
  add a file without a manifest row. That is the point, and the escape hatches are the
  ...
  around. One unintended hatch is `findings.md`, which is append-only and
...
- **The register reads cold to a person.** Agentless, finding-first prose is what a reviewer
  needs and not what a developer wants to hear. v2 solved this with two registers (the record;
  the conversation) and v3 keeps both: the record is the verb's output; the conversation is the
  agent's, second person allowed, warmth allowed, teaching allowed. The risk is an agent that
  puts the conversation in the record; test 3 catches it in samples and the gate's record diff
  catches it in pull requests.
- **The archive is reachable.** An agent asked how tightening works will search, find v2's
  ...
  either; Copilot honours none of it. Three mechanisms hold: the archive's exclusion from every
  count and lint is itself a test; `AGENTS.md` says in its first screen that nothing under
  `archive/` is current; and v2's documents are archived as one compressed bundle beside the
  index, so search tools find the index and not the corpus, while v2's code stays buildable
  until step 9. After step 9 the archive is a branch and the residual disappears.
...
- **A compound request through the one door.** The door is one conversation; a compound
...
first is the record's banned list, kept verbatim from `THE_RECORD.md` §7 and living in
`record.md`; it governs what a developer or a lead reads. The second is the engine's retired
vocabulary; it governs every hand-written file in the engine repository and the vendored
knowledge. Both are banned outside `archive/` and this appendix.

### A.1 The record's banned list (from `THE_RECORD.md` §7, unchanged)

| Banned on the record or in conversation | Say instead |
|---|---|
| numbered axes as output: `Mechanism 3`, `Tier 2`, `+1 tier` | how it ships; what the lead weighs |
| the tree's private nicknames: `magic line`, `the spine`, `the graduation`, `graduate`, `level up`, `the oracle`, `the flip`, `the corpse`, `proving ground` (in a record), `blast radius`, `naked rename` | the finding, named literally; `a rename with no refactorlog entry` |
...
| second person in the record | allowed only in conversation |
...
| Persona 1, Persona 2, intake, change-author, reviewer (as roles); the change-spec; the review packet | authoring (a phase of one session), reviewing; the record |
...
| project, projection, Π, σ, π (as verbs or symbols in prose) | emit, mint, profile |
| the canary; the quotient (as a noun in prose) | law 2 |
| torsor, commuting square, norm, δ, Kleisli, writer, lineage, episode, ledger (the algebra), seam, binding, capability descent, lane descent, plane, estate board, go board, Voice, View, Watch, the register lint (as a script) | nothing; the things are gone; a `Finding` list, a `Delta`, a `Refusal`, `check environments` |
...
| the proving ground (in the engine) | the golden project |
...
**A developer's first change (one session).** The door opens `authoring.md`. S0 names the
operation and asks the one question. S1 edits the CREATE. S2–S4 are three verbs. S5 picks the
shape from the verdict. S6 if proving forked. S7 is `estate record`. S8 reads it against
`record.md`. The pull request is the record; the gate reproduces it. Total prose on the path:
the router (60) + `authoring.md` (150) + one operation (≤ 90) + one shared file (≤ 120) +
`record.md` (120) = under 600 lines, and test 2 keeps it there.

**A reviewer's first review (fifteen minutes).** `record.md` (five). The pull request body,
findings first. `gate.json` (the reproduced verdict; the record diff). If trust is in question,
...
ingestion pass said otherwise.

| Quantity | Value |
|---|---:|
| v2 root documents | 179 files · 119,384 lines |
| `sidecar/projection/CLAUDE.md` | 302 lines; §4's fifteen survival rules at lines 105–179 |
| root `AGENTS.md` · `architecture-guardrails.md` | 253 · 48 lines; eight pillars listed |
| `DECISIONS.md` | 30,911 lines · 480 dated entries over 69 days · Status/Context/Decision/Reasoning per entry |
| `HANDOFF.md` | 3,887 lines (an ingestion pass reported 30,911; corrected by `wc -l`) |
| chapters | 56 files · 11,692 lines |
| `.claude/` | 72 files: `settings.json` 47 · four hooks 431 + 171 + 118 + 94 = 814 · 65 skill pointers + 3 agent pointers, 8 lines each, generated |
| `.github/workflows` | 6 lanes · 440 lines |
| `ssdt-agent` entry surfaces | `README.md` 162 · `CLAUDE.md` 24 · `THE_RECORD.md` 308 · `THE_RECORD_FORMS.md` 143 · `THE_DECISION_TREE.md` 215 · agents 162 + 318 + 263 = 743 |
...
| self-test | 9 files · 2,493 lines; `prompts.md` 1,046 |
| the Copilot bundle | 80 files: router 60 · 4 instructions (9–23) · 3 agents · 4 prompts (14–27) · 65 pointers · PR template ×2 (87) · `ADOPTION.md` 173 · fingerprint `047e6c5dbd9c` |
| the gates and packager | 370 + 682 lines of JavaScript; five gates (citations, register, mirror, packaging, estate); 1,875 lines of JavaScript under `scripts/` in all |
...
| the Copilot one-door path today | router 60 + largest instruction 23 + largest operation 131 + `THE_RECORD.md` 308 + `THE_RECORD_FORMS.md` 143 = 665 lines; after the trim about 320 |
...
| v1 documents | root: `readme.md` 1,162 and six others, 5,405 together · `docs/` 23,945 · `handbook/` 32 chapters, 8,980 · `ssdt-playbook/` 8,265 · `notes/` 31,364 · 77,959 in all |
...
| `.editorconfig` (sidecar) | `end_of_line = lf` · `charset = utf-8` · no BOM in the proving ground's `.sql` |
