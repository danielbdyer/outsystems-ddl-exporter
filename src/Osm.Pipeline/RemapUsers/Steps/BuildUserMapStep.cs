...
WHERE m.SourceUserId IS NULL;",
        ...
        if (!context.DryRun && !context.PolicyWasExplicit && unresolvedCount > 0)
        {
            throw new InvalidOperationException(context.FormatStepMessage(
                "{0} source users remain unresolved; specify --policy prune or --policy reassign explicitly before committing.",
                unresolvedCount));
        }
