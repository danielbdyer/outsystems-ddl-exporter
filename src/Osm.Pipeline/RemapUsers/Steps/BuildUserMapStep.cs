...
WHERE m.SourceUserId IS NULL;",
        ...
        if (unresolvedCount > 0 && !context.PolicyExplicit)
        {
            throw new InvalidOperationException("Unresolved users remain; rerun with --policy specified to choose reassign or prune.");
        }
