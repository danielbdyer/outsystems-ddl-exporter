...
public sealed class ScriptDomDmmLens : IDmmLens<TextReader>
{
    ...
    {
        var projection = Project(reader);
        if (projection.IsFailure)
        {
            return Task.FromResult(Result<IAsyncEnumerable<DmmTable>>.Failure(projection.Errors));
        }

        return Task.FromResult(Result<IAsyncEnumerable<DmmTable>>.Success(
            EnumerateTables(projection.Value, cancellationToken)));
    }
