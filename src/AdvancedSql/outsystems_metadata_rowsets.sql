...
FROM #Ent AS en
ORDER BY en.EspaceId, en.EntityName;
...
SELECT
  ...
  a.RefEntityId,
  a.OriginalName,
  a.ExternalColumnType,
  a.DeleteRule,
  a.PhysicalColumnName,
...
SELECT
  ...
  cc.Definition,
  cc.IsNotTrusted
FROM #ColumnCheckReality AS cc
ORDER BY cc.AttrId, cc.ConstraintName;

SELECT
  ac.AttrId,
  ac.CheckJson
FROM #AttrCheckJson AS ac
ORDER BY ac.AttrId;

SELECT
  pc.AttrId
FROM #PhysColsPresent AS pc
ORDER BY pc.AttrId;
...
SELECT
  ...
  ai.DataSpaceName,
  ai.DataSpaceType,
  ai.PartitionColumnsJson,
  ai.DataCompressionJson
FROM #AllIdx AS ai
ORDER BY ai.EntityId, ai.IndexName;
...
SELECT
  ...
  ic.Direction,
  ic.HumanAttr
FROM #IdxColsMapped AS ic
ORDER BY ic.EntityId, ic.IndexName, ic.Ordinal;
...
SELECT
  ...
  fk.ReferencedEntityId,
  fk.ReferencedSchema,
  fk.ReferencedTable,
  fk.IsNoCheck
FROM #FkReality AS fk
ORDER BY fk.EntityId, fk.FkName;
...
SELECT
  ...
  fkc.Ordinal,
  fkc.ParentColumn,
  fkc.ReferencedColumn,
  fkc.ParentAttrId,
  fkc.ParentAttrName,
  fkc.ReferencedAttrId,
  fkc.ReferencedAttrName
FROM #FkColumns AS fkc
ORDER BY fkc.EntityId, fkc.FkObjectId, fkc.Ordinal;

SELECT
  fam.AttrId,
  fam.FkObjectId
FROM #FkAttrMap AS fam
ORDER BY fam.AttrId, fam.FkObjectId;

SELECT
  ah.AttrId,
  ah.HasFK
FROM #AttrHasFK AS ah
ORDER BY ah.AttrId;

SELECT
  fcj.FkObjectId,
  fcj.ColumnsJson
FROM #FkColumnsJson AS fcj
ORDER BY fcj.FkObjectId;

SELECT
  faj.AttrId,
  faj.ConstraintJson
FROM #FkAttrJson AS faj
ORDER BY faj.AttrId;
...
SELECT
  ...
  t.TriggerDefinition
FROM #Triggers AS t
ORDER BY t.EntityId, t.TriggerName;

SELECT
  aj.EntityId,
  aj.AttributesJson
FROM #AttrJson AS aj
ORDER BY aj.EntityId;

SELECT
  rj.EntityId,
  rj.RelationshipsJson
FROM #RelJson AS rj
ORDER BY rj.EntityId;

SELECT
  ij.EntityId,
  ij.IndexesJson
FROM #IdxJson AS ij
ORDER BY ij.EntityId;

SELECT
  tj.EntityId,
  tj.TriggersJson
FROM #TriggerJson AS tj
ORDER BY tj.EntityId;
