...
**Dependencies**: M2.1 (UAT-Users Verification Framework), M2.2 (Transformation Verification - Part A/B/C)
**Priority**: 🟡 Enhanced Quality (ship after M2.1/M2.2 adoption)
...
The UAT-users pipeline has **unit tests for individual components** (ValidateUserMapStepTests, SqlScriptEmitterTests, etc.) but lacks **comprehensive end-to-end integration tests** covering:
1. Edge cases (empty maps, all orphans, no orphans, malformed input)
2. Idempotence (rerun safety, deterministic output)
3. Error recovery (partial failures, retry scenarios)
4. Full-export integration (UAT-users within full-export pipeline)
5. Cross-mode consistency (INSERT vs UPDATE equivalence)

**Current coverage gap**: Unit tests verify individual steps work, but **don't prove the entire pipeline produces correct results** under edge conditions.
...
- Uses M2.1/M2.2 verifiers as test oracles
...
**Ship This First**:
- ✅ End-to-end pipeline tests (discovery → validation → emission)
- ✅ Edge case tests (empty maps, 100% orphans, no orphans, GUIDs, numeric IDs)
- ✅ Idempotence tests (rerun produces identical output)
- ✅ Error handling tests (malformed CSV, missing files, invalid targets)
- ✅ Full-export integration tests (`--enable-uat-users`)
- ✅ Cross-mode equivalence tests (INSERT vs UPDATE)

**Defer to Later**:
- 🔵 Performance/stress tests (10k+ orphans, 1M+ rows) - M4.1
- 🔵 Historical regression tests - M4.x
...
- Oracle-driven tests (using M2.1/M2.2 verifiers)
...
- M2.3 should ship **after** M2.1/M2.2 (uses verifiers as oracles)
...
- M2.2 (transformation verification - used as test oracle)
...
5. **Cross-Mode Equivalence**:
   ...
   - M2.2 verifier confirms equivalence
...
tests/Osm.Pipeline.Tests/UatUsers/Integration/
  │
  ├─► EdgeCaseTests/
  │   ├─ EmptyMapTests.cs (no orphans)
  │   ├─ AllOrphansTests.cs (100% orphans)
  │   ├─ MixedIdTypeTests.cs (GUID, numeric, text)
  │   └─ SpecialCharactersTests.cs (unicode, quotes)
  │
  ├─► IdempotenceTests/
  │   ├─ RerunIdenticalOutputTests.cs (artifacts unchanged)
  │   ├─ TimestampOmissionTests.cs (idempotent mode)
  │   └─ FileModTimeTests.cs (files not rewritten)
  │
  ├─► ErrorHandlingTests/
  │   ├─ MalformedCsvTests.cs (missing columns, bad data)
  │   ├─ MissingFilesTests.cs (file not found errors)
  │   ├─ InvalidTargetsTests.cs (targets not in UAT)
  │   └─ DbConnectionTests.cs (connection failures)
  │
  ├─► FullExportIntegrationTests/
  │   ├─ EnableUatUsersTests.cs (--enable-uat-users)
  │   ├─ ManifestIntegrationTests.cs (artifacts in manifest)
  │   └─ InsertTransformationTests.cs (transformations in INSERTs)
  │
  └─► CrossModeEquivalenceTests/
      ├─ InsertUpdateEquivalenceTests.cs (both modes produce same results)
      └─ VerifierOracleTests.cs (M2.2 verifier confirms equivalence)
```

### Test Oracle Pattern

Use M2.1/M2.2 verifiers as test oracles:
...
**Pattern**: Every integration test should use M2.1/M2.2 verifiers to prove correctness.
...
#### ✅ **Existing Scenario Test** - Pattern to Follow
...
#### ✅ **M2.2 Transformation Verifier** - Use as Oracle
- **File**: `src/Osm.Pipeline/UatUsers/Verification/InsertTransformationVerifier.cs` (from M2.2)
...
public sealed class EdgeCaseTests : UatUsersIntegrationTestBase
{
    ...
    public async Task AllOrphans_100Percent_Succeeds()
    {
        ...
        // Provide complete mapping (100→300, 101→301, ...)
    ...
    public async Task SpecialCharacters_Unicode_Succeeds()
    {
        ...
        Assert.That(previewCsv, Does.Contain("José García"));
        Assert.That(previewCsv, Does.Contain("María López"));
    }
}
```

**Pattern**: Test edge cases with realistic data, verify using M2.1/M2.2 oracles
...
public sealed class FullExportIntegrationTests
{
    ...
    public async Task EnableUatUsers_InsertTransformationsApplied_Succeeds()
    {
        ...
        // Verify using M2.2 transformation verifier
...
**Pattern**: Test full-export integration, verify INSERT transformations using M2.2 oracle
...
public sealed class CrossModeEquivalenceTests
{
    ...
    public async Task InsertUpdateModes_SameTransformations_Succeeds()
    {
        ...
        // Assert: M2.2 cross-mode validator confirms equivalence
    ...
    public async Task BothModes_NullPreservation_Succeeds()
    {
        ...
        // Verify using M2.2 NULL preservation verifier
...
**Pattern**: Compare INSERT and UPDATE modes, verify equivalence using M2.2 validators

---

### Testing Infrastructure

**No new infrastructure needed** - uses existing test frameworks and M2.1/M2.2 verifiers.
...
tests/Osm.Pipeline.Tests/UatUsers/
  ├─ UatUsersPipelineScenarioTests.cs (pattern to follow)
  ├─ ValidateUserMapStepTests.cs (fixtures to reuse)
  └─ SqlScriptEmitterTests.cs (test patterns)

src/Osm.Pipeline/UatUsers/Verification/
  ├─ UatUsersVerifier.cs (M2.1 - use as oracle)
  ├─ InsertTransformationVerifier.cs (M2.2 - use as oracle)
  ├─ UpdateScriptSimulator.cs (M2.2 - use as oracle)
  └─ CrossModeValidator.cs (M2.2 - use as oracle)
```

**New Files to Create**:
```
tests/Osm.Pipeline.Tests/UatUsers/Integration/
  ├─ UatUsersIntegrationTestBase.cs (base class)
  ├─ EdgeCaseTests.cs (empty map, all orphans, GUID, unicode)
  ├─ IdempotenceTests.cs (rerun, file mod times)
  ├─ ErrorHandlingTests.cs (malformed CSV, invalid targets)
  ├─ FullExportIntegrationTests.cs (--enable-uat-users)
  └─ CrossModeEquivalenceTests.cs (INSERT vs UPDATE)
```
...
### Phase 3: Idempotence Tests (0.5 days)
...
### Phase 4: Error Handling Tests (0.5 days)
...
### Phase 5: Full-Export Integration Tests (0.5 days)

Implement `FullExportIntegrationTests.cs`:
- `--enable-uat-users` triggers pipeline
- Artifacts in manifest
- INSERT transformations applied (verify with M2.2)

---

### Phase 6: Cross-Mode Equivalence Tests (0.5 days)

Implement `CrossModeEquivalenceTests.cs`:
- INSERT vs UPDATE equivalence (M2.2 validator)
...
**Expected**: M2.2 validator confirms equivalence
...
- M2.3 adds tests; doesn't modify production code
...
M2.3 is successful if:

✅ 95%+ code coverage for UAT-users pipeline

✅ All edge cases tested (empty map, 100% orphans, GUID, unicode)

✅ Idempotence proven (artifacts deterministic)

✅ Error handling tested (all failure modes have tests)

✅ Full-export integration verified

✅ Cross-mode equivalence proven

✅ CI/CD gates merges on test pass
