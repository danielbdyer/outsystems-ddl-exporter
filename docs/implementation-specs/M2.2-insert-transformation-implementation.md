...
**Priority**: 🔴 Critical Path (required for milestone - full-export + uat-users integration)
...
1. **Reorder Pipeline**: Run UAT-users discovery before build (discovery → build → deploy)
2. **Transformation Context**: Pass orphan→target mapping to `DynamicEntityInsertGenerator`
...
- ✅ Refactor `FullExportCoordinator` to run UAT-users before build
- ✅ Create `TransformationContext` to pass orphan mappings to INSERT generator
- ✅ Modify `DynamicEntityInsertGenerator` to accept transformation context
- ✅ Apply transformations in-memory during value emission
- ✅ Update `BuildSsdtDynamicInsertStep` to wire transformation context
...
**Required Changes**:
- Reverse order: UAT-users discovery → Build (with transformation context)
...
```
┌─────────────────────────────────────────────────────┐
│ 1. Extract Model    (get schema + entity metadata)  │
│ 2. Profile          (capture runtime data)          │
│ 3. Build            (generate INSERTs with QA data) │ ← No transformation
│ 4. Apply            (deploy schema)                 │
│ 5. UAT-Users        (generate UPDATE scripts)       │ ← Transformation here
└─────────────────────────────────────────────────────┘
```
...
│ 1. Extract Model    (get schema + entity metadata)  │
│ 2. Profile          (capture runtime data)          │
│ 3. UAT-Users Disco  (discover orphans, load map)    │ ← NEW: Split UAT-users
│ 4. Build            (generate INSERTs with UAT data)│ ← Transformation here
│ 5. Apply            (deploy schema)                 │
│ 6. UAT-Users Emit   (emit verification artifacts)   │ ← NEW: Artifact emission only
└─────────────────────────────────────────────────────┘
```
...
- Discover FK catalog → Load inventories → Validate map → Emit UPDATE scripts

**After** (split into discovery + emission):
- **Discovery Phase**: Discover FK catalog → Load inventories → Validate map → Return `TransformationContext`
...
#### ✅ **UAT-Users Pipeline** - Already Implemented (Split This)
...
#### ✅ **INSERT Generator** - Exists, Needs Modification
...
#### ✅ **Transformation Map** - Already Loaded
...
public sealed class TransformationContext
{
    ...
    public object? TransformValue(string tableName, string columnName, object? value)
    {
        ...
        if (_transformationMap.TryGetValue(userId, out var target))
            return target.Value; // Transform orphan → target
...
2. **`tests/Osm.Pipeline.Tests/UatUsers/TransformationContextTests.cs`**
   - Test: Transform orphan ID → target ID
...
#### 1. **Table Name Availability in INSERT Generator**
   - ❓ Does `DynamicEntityInsertGenerator` have table name in scope during value emission?
   - ❓ If not, how to pass table context for FK column lookup?
   - **Action**: Investigate current code structure before implementation

#### 2. **Backward Compatibility**
   - ❓ Should UPDATE script generation still be supported (standalone `uat-users` verb)?
   - ❓ Or deprecate UPDATE approach entirely?
   - **Recommendation**: Keep UPDATE support for legacy UAT database migration

#### 3. **Artifact Emission Timing**
   - ❓ Should UAT-users artifacts (map template, preview CSV) be emitted before or after build?
   - ❓ Preview CSV shows row counts - need build to complete first?
   - **Recommendation**: Emit after build (verification artifacts reference generated INSERTs)

#### 4. **Transformation Verification**
   - ❓ How to verify INSERTs contain transformed values?
   - ❓ Parse generated SQL scripts? Or capture during generation?
   - **Recommendation**: M2.3 Part B will address automated INSERT verification

---

### Independence & Backward-Safety Requirements

- **Flag-gated behavior**: The base `full-export` path must remain byte-for-byte identical when `--enable-uat-users` is **not** supplied. Transformation context must be optional and default to `null`, and coordinator refactors must no-op without the flag.
- **No shared-state bleed**: `UatUsersDiscoveryPipeline` and emission must operate on their own artifact root; discovery must not mutate `FullExportBuildResult` unless the specialization is explicitly enabled.
- **Tests for independence**: Add regression tests in `FullExportCoordinatorTests` (or new coverage) that run `full-export` with and without `--enable-uat-users` on the same fixtures and assert identical manifests/artifacts when the flag is absent. Include an integration test that proves toggling the flag switches between QA IDs and transformed IDs without cross-contamination.
- **CLI stability**: Default CLI options must still parse/execute without UAT-users inventories present. Add a negative test case covering `full-export` invoked without UAT inputs to confirm no new required parameters leaked into the base command.

### Test-Driven Development Plan (enforced)

1. **Red**: Land the new unit + integration tests listed below first (TransformationContext, DynamicEntityInsertGenerator transformation path, FullExportCoordinator independence). The independence tests initially fail because the code still shares the old execution order.
2. **Green**: Implement the discovery/build reorder + transformation plumbing until all tests pass, keeping the `--enable-uat-users` gating explicit.
3. **Refine**: Refactor for readability and reuse (e.g., `TransformationContext` helpers) only after the green state, and backfill any missing negative tests (NULL preservation, non-user FKs ignored).
4. **Guard**: Add a regression snapshot (manifest hash) in tests to catch future coupling between UAT-users specialization and the base full-export path.
...
✅ `full-export --enable-uat-users` generates INSERTs with UAT user IDs (no orphans)

✅ Backward compatible (standalone `uat-users` still works)

✅ No breaking changes to existing workflows

✅ Performance improvement (INSERT faster than UPDATE)

✅ Deployment simplification (load INSERTs directly, no UPDATE step)

✅ All tests pass (unit, integration, end-to-end)

✅ Manual inspection confirms transformed values in generated SQL
