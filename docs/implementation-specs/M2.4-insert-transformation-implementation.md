...
**Priority**: 🔵 Preferred Approach (preferred over UPDATE-only, but larger scope)
...
**Current Architecture** (as researched in M2-RESEARCH-FINDINGS.md):
- UAT-users runs **AFTER** build completes: Extract → Profile → Build → Apply → UAT-users
- `DynamicEntityInsertGenerator.cs` has **ZERO transformation logic**
- INSERTs are generated with QA user IDs (no transformations)
- UAT-users generates UPDATE scripts for post-load transformation

**Desired Architecture** (from design doc):
- UAT-users injects transformation logic **DURING** INSERT generation
- INSERTs contain UAT-ready data (pre-transformed user FK values)
- No UPDATE scripts needed (load INSERTs directly to UAT)

**Gap**: The entire INSERT transformation injection infrastructure does not exist.

### Solution Approach

Implement **pre-transformed INSERT generation** by:
1. **Refactoring pipeline orchestration** to run UAT-users BEFORE build
2. **Injecting transformation context** into `DynamicEntityInsertGenerator`
3. **Applying transformations in-memory** during row emission
4. **Eliminating UPDATE scripts** (data is UAT-ready at generation time)
...
- Reduced lock contention (table locks vs row/page locks)
- Idempotent by design (drop/reload anytime)
...
- ✅ Verification: Parse generated INSERTs to prove transformations applied (M2.5)

**Out of Scope**:
- UPDATE script generation (still supported via standalone `uat-users` verb for legacy migration)
- Cross-mode validation (M2.6 - after both modes exist)
...
**Depends On**:
- M2.1 (verification infrastructure - reuse for INSERT verification)
- M2.2 (UPDATE verification - provides comparison baseline)

**Enables**:
- M2.5 (INSERT transformation verification - proves transformations applied)
...
#### 4. **Transformation Verification**
   ...
   - **Recommendation**: M2.5 spec will address INSERT verification
...
4. M2.5 will add automated INSERT verification
...
*Preferred over UPDATE-only approach (M2.2), but larger implementation scope*
