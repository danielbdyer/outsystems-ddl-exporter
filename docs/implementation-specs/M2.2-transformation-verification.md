...
**Dependencies**: M1.8 (Data Integrity Verification), M2.1 (UAT-Users Verification Framework)
**Priority**: 🟡 Enhanced Verification (ship after M2.1 adoption)
...
M2.1 verifies UAT-users **artifacts** (map validity, catalog completeness, SQL safety), but there's **no verification that transformations are actually applied correctly** when:
1. INSERT scripts are generated (full-export mode with `--enable-uat-users`)
2. UPDATE scripts are executed (standalone mode)

**Current gap**: Operators have no proof that:
- Pre-transformed INSERT scripts contain correct user FK mappings (orphans→targets)
- Non-user-FK columns remain unchanged (data fidelity)
- NULL values are preserved (never transformed)
- Both modes (INSERT vs UPDATE) produce identical transformation results

**Risk**: Silent data corruption where transformations are incomplete or incorrect.

### Solution Approach

Build a **transformation verification layer** that:
- **Parses INSERT scripts** to extract user FK values and verify they match transformation map
- **Simulates UPDATE execution** to predict transformed values and verify correctness
- **Integrates with M1.8 data integrity** to verify non-transformed columns unchanged
- **Cross-validates modes** to prove INSERT and UPDATE transformations are equivalent
- **Emits transformation proof report** (JSON) for audit trails

**This extends M2.1 artifact verification with runtime transformation correctness.**

### MVP Scope (M2.2)

**Ship This First**:
- ✅ INSERT script parser (extracts VALUES, verifies user FK transformations)
- ✅ UPDATE script simulator (predicts transformation results without execution)
- ✅ NULL preservation verifier (proves NULLs never transformed in either mode)
- ✅ Non-transformed column verifier (integrates with M1.8 checksums)
- ✅ Transformation proof report generator (JSON output)

**Defer to Later**:
- 🔵 Live database verification (execute transformations and verify results) - M3.2
- 🔵 Historical transformation auditing - M4.1
- 🔵 Transformation rollback proof - M4.x

### Key Findings

**Already Exists** ✅:
- `SqlScriptEmitter.cs` (UPDATE script generation - pattern to simulate)
- `DynamicEntityInsertGenerator.cs` (INSERT generation - pattern to parse)
- M1.8 data integrity infrastructure (checksums, NULL counting)
- Transformation map (source→target mappings)

**Missing** ❌:
- INSERT script parser (extracts VALUES and verifies transformations)
- UPDATE script simulator (predicts results without execution)
- Transformation verification context
- Cross-mode validation (INSERT vs UPDATE equivalence)
...
└─────────────────────────────────────────────────────────────┘
                              │
                              ├─► InsertScriptParser
                              │   - Parses INSERT ... VALUES blocks
                              │   - Extracts user FK column values
                              │   - Verifies transformations applied
                              │
                              ├─► UpdateScriptSimulator
                              │   - Parses UPDATE CASE blocks
                              │   - Simulates transformation results
                              │   - Verifies mapping correctness
                              │
                              ├─► NullPreservationVerifier
                              │   - Counts NULLs in source data
                              │   - Counts NULLs in transformed data
                              │   - Proves NULLs never transformed
                              │
                              ├─► CrossModeValidator
                              │   - Compares INSERT vs UPDATE results
                              │   - Proves equivalence
                              │
                              └─► TransformationProofGenerator
                                  - Emits JSON proof report
                                  - Follows M2.1 pattern

┌─────────────────────────────────────────────────────────────┐
│      Existing Infrastructure (Leverage & Integrate)          │
└─────────────────────────────────────────────────────────────┘
                              │
                              ├─► SqlScriptEmitter (UPDATE pattern)
                              ├─► DynamicEntityInsertGenerator (INSERT pattern)
                              ├─► M1.8 Data Integrity (checksums)
                              └─► M2.1 Verification Framework (reports)
```
...
#### ✅ **UPDATE Script Pattern** - Already Implemented (Parse This)
...
#### ✅ **INSERT Generation Pattern** - Need to Trace (Understanding Required)
...
#### ✅ **Data Integrity Verification** - Already Implemented (Integrate)
...
public sealed class UpdateScriptSimulator
{
    ...
    private ColumnTransformationResult VerifyCaseBlock(
        ...
        IReadOnlyCollection<UserIdentifier> orphans)
    {
        // Verify CASE block contains all orphan→target mappings
...
2. **`tests/Osm.Pipeline.Tests/UatUsers/Verification/InsertTransformationVerifierTests.cs`**
   - Test: All orphans eliminated → PASS
   - Test: Orphans remain in INSERTs → FAIL
   - Test: NULLs preserved → PASS
   - Test: Transformations match map → PASS

3. **`tests/Osm.Pipeline.Tests/UatUsers/Verification/UpdateScriptSimulatorTests.cs`**
   - Test: CASE blocks match transformation map → PASS
   - Test: Missing mappings in CASE → FAIL
   - Test: Wrong target in CASE → FAIL

4. **`tests/Osm.Pipeline.Tests/UatUsers/Verification/NullPreservationVerifierTests.cs`**
   - Test: INSERT mode with NULLs → PASS
   - Test: UPDATE mode with NULL guards → PASS
   - Test: UPDATE mode missing NULL guards → FAIL

5. **`tests/Osm.Pipeline.Tests/UatUsers/Verification/CrossModeValidatorTests.cs`**
   - Test: Equivalent transformations → PASS
   - Test: Different orphan counts → FAIL
   - Test: Different mappings → FAIL
...
#### 1. **INSERT Transformation Injection Point** (RESEARCH REQUIRED)
   ...
   - ❓ Where in the code is the transformation applied?
   - ❓ Is there existing verification we can leverage?
   - **Action**: Investigate codebase before implementing INSERT verifier

#### 2. **SQL Parsing Complexity**
   - ❓ Use regex or full SQL parser?
   - ❓ What if operators manually edit SQL scripts?
   - **Recommendation**: Regex for MVP, document limitations

#### 3. **Live Database Verification**
   - ❓ Should M2.2 include live DB verification (execute UPDATE, verify results)?
   - ❓ Or defer to M3.2 (load harness)?
   - **Recommendation**: Defer to M3.2 - keep M2.2 static analysis only

#### 4. **Non-Transformed Column Verification**
   - ❓ How to integrate with M1.8 checksums?
   - ❓ Should we verify non-user-FK columns are unchanged?
...
src/Osm.Pipeline/Orchestration/
  └─ FullExportPipeline.cs (how UAT-users integrates)

src/Osm.Emission/DynamicData/
  └─ DynamicEntityInsertGenerator.cs (WHERE ARE TRANSFORMATIONS APPLIED?)

src/Osm.Pipeline/UatUsers/
  ├─ SqlScriptEmitter.cs (UPDATE script pattern)
  └─ UserMapLoader.cs (load transformation map)
```

**New Files to Create**:
```
src/Osm.Pipeline/UatUsers/Verification/
  ├─ TransformationVerificationContext.cs (aggregates results)
  ├─ TransformationApplicationResult.cs (transformation results)
  ├─ NullPreservationResult.cs (NULL preservation proof)
  ├─ CrossModeValidationResult.cs (mode equivalence)
  ├─ InsertScriptParser.cs (parses INSERT statements)
  ├─ InsertTransformationVerifier.cs (verifies INSERT transformations)
  ├─ UpdateScriptSimulator.cs (simulates UPDATE execution)
  ├─ NullPreservationVerifier.cs (proves NULL preservation)
  ├─ CrossModeValidator.cs (cross-validates modes)
  └─ TransformationProofGenerator.cs (proof report generator)

src/Osm.Cli/
  └─ VerifyTransformationsCommand.cs (NEW CLI command)

tests/Osm.Pipeline.Tests/UatUsers/Verification/
  ├─ InsertScriptParserTests.cs
  ├─ InsertTransformationVerifierTests.cs
  ├─ UpdateScriptSimulatorTests.cs
  ├─ NullPreservationVerifierTests.cs
  ├─ CrossModeValidatorTests.cs
  └─ TransformationProofGeneratorTests.cs

tests/Osm.Cli.Tests/Commands/
  └─ VerifyTransformationsCommandTests.cs
```
...
✅ Can verify INSERT scripts eliminate all orphans (no orphan IDs in VALUES)

✅ Can verify UPDATE scripts contain correct CASE mappings

✅ Can prove NULL preservation in both modes

✅ Can cross-validate INSERT and UPDATE modes produce identical results
