...
**Dependencies**: M2.1 (UAT-Users Verification Framework), M1.3 (Data Integrity Checks), M2.4 (INSERT Transformation Implementation)
**Priority**: 🟡 Enhanced Verification (ship after M2.4 ships - deferred verifiability)
**Scope**: Unified transformation verification for both INSERT and UPDATE modes
**Implementation Strategy**: Part A → Part B or C (shared fundamentals enable both modes)
**Pattern**: Like M1.1/M1.2 (verification) - proves M2.4 (critical path) works correctly

---

## Executive Summary

### Problem Statement (from tasks.md)

**After M2.4 ships** (INSERT transformation implementation), operators can generate pre-transformed INSERTs and load them to UAT. However, there's **no automated verification that transformations were correctly applied**.

M2.1 verifies UAT-users **artifacts** (map validity, catalog completeness, SQL safety), and M1.3 verifies **base data integrity** (row counts, NULL preservation), but there's **no verification that transformations are correctly applied in generated scripts**.

**Deferred Verifiability Pattern**: Like M1.0 shipped before M1.1/M1.2 verification, M2.4 can ship with manual inspection. M2.2 provides automated verification that can be deferred after milestone deadline.

**Current Gap**:
- **Full-export mode**: Generates pre-transformed INSERT scripts, no proof transformations applied
- **Standalone mode**: Generates UPDATE scripts, no proof CASE blocks are correct
- **Both modes**: No verification of NULL preservation in user FK columns, no proof non-user-FK columns unchanged

**Risk**: Silent transformation bugs (orphan IDs in INSERTs, incorrect CASE blocks in UPDATEs, NULL handling errors) go undetected until UAT deployment.
...
- **Part A: Shared Fundamentals** (deliver FIRST - enables both Part B and Part C)
  - Core transformation map validation (source→target completeness)
...
- ✅ Transformation map validator (source in QA, target in UAT, no duplicates)
- ✅ FK catalog verifier (catalog correctness, column identification)
- ✅ Core verification infrastructure (TSql150Parser, report patterns)
- ✅ Unified verification context (shared by INSERT and UPDATE)

**Part B: INSERT Verification** (PRIMARY):
- ✅ INSERT script parser (extracts VALUES using SQL DOM)
- ✅ Orphan detector (verifies no orphan IDs in INSERTs)
- ✅ UAT inventory validator (all user FK values exist in UAT)
- ✅ Row count comparator (no data loss)

**Part C: UPDATE Verification** (SECONDARY):
- ✅ UPDATE script parser (extracts CASE blocks using SQL DOM)
- ✅ WHERE clause validator (references only orphans)
- ✅ CASE block validator (assigns only UAT targets)
- ✅ NULL preservation guard verifier (WHERE IS NOT NULL)

**Cross-Validation** (when both modes available):
- ✅ Compare transformation counts (INSERT vs UPDATE)
- ✅ Verify user ID coverage matches
- ✅ Prove NULL preservation in both representations

**Modular Report Extension** (from tasks.md):
- ✅ Add `uatUsersVerification` section to `data-integrity-verification.json` (from M1.3)
- ✅ Base verification section (from M1.3) remains unchanged
- ✅ Transformation-specific metrics: map validity, orphan completeness, UAT compliance
...
public sealed class InsertScriptParser
{
    ...
    public Result<ParsedInsertScript> ParseScript(string scriptPath)
    {
        ...
        // Return ParsedInsertScript with column→values mapping
    }
}
...
**Tasks**:
1. Create `TransformationMapValidator` (validates source→target correctness)
...
- ✅ Transformation map validated (source in QA, target in UAT, no duplicates)
- ✅ FK catalog verified (correct columns identified)
- ✅ Unified context created and reusable by both INSERT and UPDATE

**Part B (INSERT Verification)**:
- ✅ INSERT scripts parsed successfully using TSql150Parser
- ✅ No orphan IDs detected in pre-transformed INSERTs
- ✅ All user FK values exist in UAT inventory
- ✅ Row counts match (no data loss)
- ✅ Verification report emitted with pass/fail status

**Part C (UPDATE Verification)**:
- ✅ UPDATE scripts parsed successfully using TSql150Parser
- ✅ WHERE clauses reference only orphans
- ✅ CASE blocks match transformation map exactly
- ✅ WHERE IS NOT NULL guards present (NULL preservation)
- ✅ Verification report emitted with pass/fail status

**Cross-Validation** (optional):
- ✅ Transformation counts match between INSERT and UPDATE
- ✅ User ID coverage identical
- ✅ NULL preservation verified in both modes

**Modular Report**:
- ✅ `uatUsersVerification` section added to M1.3 base report
- ✅ Base verification section unchanged (non-user-FK columns)
- ✅ Specialized verification section present (user FK columns only)
...
*Structure: Part A (shared) → Part B (INSERT primary) + Part C (UPDATE secondary)*
