...
**Ship This**:
- ✅ UPDATE script parser (extracts CASE blocks, validates mappings)
- ✅ NULL preservation verifier (verifies WHERE guards)
- ✅ Transformation map validator (CASE blocks match user map)
- ✅ UPDATE verification report generator (JSON output)
...
**Defer to Later**:
- 🔵 Live database verification (execute UPDATEs and verify results) - M3.2
...
- M1.3 (basic data integrity checks, NULL counting)
...
└─────────────────────────────────────────────────────────────┘
                              ...
                              ├─► M1.3 Data Integrity (NULL checks)
...
- **File**: M1.3 specification (basic NULL counting)
- **Status**: Pattern established - reuse for non-transformed columns
- **Integration**: Verify non-user-FK columns via NULL count comparison (proves basic data fidelity)
...
#### 4. **Non-Transformed Column Verification**
   - ❓ How to integrate with M1.3 NULL checks?
   - ❓ Should we verify non-user-FK columns are unchanged?
   - **Recommendation**: Yes - reuse M1.3 patterns for basic data fidelity proof (NULL counts sufficient for M2.2)
