...
**Priority**: 🔴 CRITICAL PATH - Ship this week
...
1. ✅ Bootstrap snapshot generation (`AllEntitiesIncludingStatic.bootstrap.sql`)
2. ✅ PostDeployment template with guard logic (`PostDeployment-Bootstrap.sql`)
3. ✅ Directory rename (Seeds → BaselineSeeds)
4. ✅ Observability measures: GO statements, PRINT diagnostics, topological comments
5. ✅ Per-table baseline seed emission (`EmitPerTableSeeds` configuration option)
...
**MVP Definition**: M1.0 + M1.1 constitute the "core MVP" (🔴 critical path). M1.2/M1.3 are "basic validation" (🟡 ship after core) that add safety checks without changing core functionality.
...
- ✅ Per-table emission included from day one (no migration needed later)
- ✅ Observability included in M1.0 provides debuggability from day one
- ✅ Proof generation consumes existing data structures (no M1.0 changes needed)
- ✅ Data integrity is independent orchestration step (verifies M1.0's output)
...
1. All static entities are topologically sorted globally ✅
2. Entities partitioned by module (preserving order within each module) ✅
3. Module files emitted in **alphabetical order** ❌
4. SSDT applies files in ItemGroup order (alphabetical) ❌

**Example Failure**:
- ModuleA contains `User` (FK → `Role`)
- ModuleB contains `Role`
- Alphabetical order: ModuleA before ModuleB
- Result: `User` MERGEs before `Role` exists → FK violation!
...
|-----------|------|-------|--------|
| **FK-aware topological sorting** | `EntityDependencySorter.cs` | 967 | ✅ Complete |
| **Kahn's algorithm** | `EntityDependencySorter.TopologicalSort()` | 264-302 | ✅ Implemented |
| **Cycle detection + fallback** | `EntityDependencySorter.SortByForeignKeys()` | 122-156 | ✅ Implemented |
| **Junction table deferral** | `JunctionTableClassifier` | 616-782 | ✅ Implemented |
| **Self-referencing FK ordering** | `DynamicEntityInsertGenerator.OrderRows()` | 152-405 | ✅ Implemented |
| **FK preflight validation** | `StaticSeedForeignKeyPreflight.cs` | 217 | ✅ Implemented |
| **SMO enrichment** | `RelationshipConstraintHydrator.cs` | 292 | ✅ **EXISTS!** |
| **LoadHarness trust manifest** | `LoadHarnessRunner.BuildScriptQueue()` | 169-206 | ✅ Verified |
| **Ordering telemetry** | `BuildSsdtStaticSeedStep.cs` | 158-172 | ✅ Implemented |

### 🔍 SMO Enrichment Discovery
...
### ❌ What M1.0 Must Address

| Issue | Current Impact | M1.0 Solution |
|-------|----------------|---------------|
| **Module partitioning breaks order** | When `GroupByModule=true`, static seed files ordered alphabetically by module → cross-module FK violations in SSDT | ✅ Bootstrap snapshot with global topological sort (no module partitioning) |
| **No bootstrap snapshot** | First SSDT deployment must apply static seeds in correct order, but module alphabetization breaks this | ✅ Generate `AllEntitiesIncludingStatic.bootstrap.sql` for first deployment |
| **No PostDeployment template** | Users must manually create PostDeployment scripts with guard logic | ✅ Generate `PostDeployment-Bootstrap.sql` template |
...
```
{OutputDirectory}/
├── Modules/              ← Tables emitted here (SsdtEmitter.cs:82)
│   └── [TableFiles]
├── Seeds/                ← Static seeds currently emitted here (BuildSsdtStaticSeedStep.cs:96)
│   ├── [Module1]/
│   │   └── StaticEntities.seed.sql
│   ├── [Module2]/
│   │   └── StaticEntities.seed.sql
│   └── StaticEntities.seed.sql (master file if EmitMasterFile=true)
├── DynamicData/          ← Dynamic data emitted here (BuildSsdtDynamicInsertStep.cs:134)
│   ├── [Module1]/        (PerEntity mode)
│   │   └── [TableName].dynamic.sql
│   ├── [Module2]/
│   │   └── [TableName].dynamic.sql
│   └── DynamicData.all.dynamic.sql (SingleFile mode)
└── manifest.json
```
...
```
{OutputDirectory}/
├── Modules/              ← UNCHANGED (tables go here, ForeignKeys are part of table definitions)
│   └── [TableFiles]
├── Bootstrap/            ← NEW
│   └── AllEntitiesIncludingStatic.bootstrap.sql
├── BaselineSeeds/        ← RENAMED from Seeds/
│   ├── StaticEntities.seed.sql            (single-file mode - default)
│   ├── [Module1]/                         (per-module mode)
│   │   └── StaticEntities.seed.sql
│   └── [Module1]/                         (per-table mode - NEW FEATURE)
│       ├── [Table1].seed.sql
│       └── [Table2].seed.sql
└── PostDeployment-Bootstrap.sql   ← NEW (copy to SSDT)
```

**Key Changes**:
- Add `Bootstrap/` directory (gitignored) for bootstrap snapshot
- Rename `Seeds/` → `BaselineSeeds/` (clearer terminology)
...
### Change 4: Per-Table Baseline Seed Emission (🟡 DEFERRED TO M1.5)
...
```
MyApp/
├── Modules/                                       (Tables - unchanged)
│   └── [TableFiles]
├── Bootstrap/
│   └── AllEntitiesIncludingStatic.bootstrap.sql  (300 entities, topologically sorted)
├── BaselineSeeds/
│   └── StaticEntities.seed.sql                   (100 static entities, idempotent MERGE)
└── PostDeployment-Bootstrap.sql                   (COPY THIS TO SSDT)
```
...
2. SSDT deploys:
   ...
   - Runs PostDeployment/01-Bootstrap.sql:
     a. Guard checks: SELECT 1 FROM [OSSYS_ROLE] → empty → first deployment!
...
3. SSDT deploys:
   ...
   - Runs PostDeployment/01-Bootstrap.sql:
     a. Guard checks: SELECT 1 FROM [OSSYS_ROLE] → has data → skip bootstrap!
...
{
    // Arrange: User (static) → Department (regular)
...
1. **Export** → Generate bootstrap snapshot + baseline seeds + PostDeployment template (M1.0)
2. **Verify Artifacts** → Validate files exist, checksums match (M1.1)
3. **Apply Bootstrap** → LoadHarness applies bootstrap to empty target DB
4. **Validate Ordering** → Runtime checks confirm no child-before-parent violations (M1.2)
5. **Verify Data** → Row counts and NULL counts match source (M1.3)
6. **Apply Baseline Seeds** → LoadHarness applies baseline seeds (should be no-op due to MERGE idempotency)
7. **Verify Proof** → Topological proof artifacts generated correctly (M1.7, deferred)
...
2. **Rename directory**: `Seeds/` → `BaselineSeeds/` (migration happens during first export after M1.0)
3. **Add new files**:
...
- ✅ Static seed generation unchanged (still uses MERGE, still topologically sorted)
- ✅ Dynamic data generation unchanged (no modifications)
- ✅ Existing exports continue to work
- ✅ Users can opt-in to bootstrap snapshot workflow when ready

### Migration for Existing Users
1. Run `osm full-export` with M1.0 → new `Bootstrap/` and `PostDeployment-Bootstrap.sql` files appear
...
1. ✅ Bootstrap snapshot generated with ALL entities in global topological order
2. ✅ Bootstrap snapshot includes observability measures (GO statements, PRINT diagnostics, topological comments)
3. ✅ PostDeployment-Bootstrap.sql template dynamically adapts to emission mode (single-file, per-module, or per-table)
4. ✅ Per-table emission mode works correctly (one .seed.sql file per static entity) - DEFERRED TO M1.5
5. ✅ Guard logic correctly detects first vs. subsequent deployments
6. ✅ Existing static seed emission unchanged (backward compatible)
7. ✅ Directory rename (Seeds → BaselineSeeds) happens automatically
8. ✅ All existing tests pass
9. ✅ New integration tests verify bootstrap snapshot correctness
10. ✅ Operators can verify execution order from SSDT deployment output

## Observability and Defensive Measures (🔴 IN M1.0 MVP)
...
1. ✅ Static entities ARE included in dynamic data (`SqlDynamicEntityDataProvider` does not filter by `IsStatic`)
2. ✅ Topological sorting works correctly (`EntityDependencySorter` handles FK dependencies properly)
3. ✅ The REAL issue: Static seed files partitioned by module + alphabetical module ordering in SSDT = cross-module FK violations
...
foreach (var script in scripts)
{
    ...
    builder.AppendLine();
    builder.AppendLine("GO");  // ← DEFENSIVE: Ensure transaction boundary
...
private string BuildMergeScript(...)
{
    ...
    builder.AppendLine($"PRINT '[M1.0] ✓ Completed MERGE for {definition.Schema}.{definition.PhysicalName} (static entity, {rows.Length} rows)';");
    builder.AppendLine("GO");
...
private string BuildInsertScript(...)
{
    ...
    builder.AppendLine($"PRINT '[M1.0] ✓ Completed INSERT for {definition.Schema}.{definition.PhysicalName} (regular entity, {rows.Length} rows)';");
    builder.AppendLine("GO");
...
-- Dependencies: Role (14), Department (12)  -- ← Shows parent positions
-- Dependents: AuditLog (23), Session (31)  -- ← Shows child positions
--------------------------------------------------------------------------------
```
...
✅ **Junction Table Classification** - Already Implemented
...
✅ **FK Preflight Validation** - Already Implemented
...
✅ **MERGE Script Generation** - Assumed Implemented
...
✅ **Manifest Tracking** - Already Supports Artifacts
...
foreach (var table in orderedTables)
{
    ...
    builder.AppendLine($"PRINT '[M1.0] ✓ Completed MERGE for {table.Definition.Schema}.{table.Definition.PhysicalName} ({table.Rows.Length} rows)';");
    builder.AppendLine("GO");
    builder.AppendLine();
}
```
...
.BindAsync((dynamicInserts, token) => _bootstrapSnapshotStep.ExecuteAsync(dynamicInserts, token), cancellationToken)    // ← NEW
.BindAsync((bootstrapSnapshot, token) => _postDeploymentTemplateStep.ExecuteAsync(bootstrapSnapshot, token), cancellationToken)    // ← NEW
...
2. **TelemetryPackagingStep State**:
   - ❓ Does it accept generic final state or specific `DynamicInsertsGenerated`?
   - ❓ If specific: Do we need to update it or insert steps differently?

3. **DI Container Location**:
   - ❓ Where are `BuildSsdt*Step` services registered?
   - ❓ Pattern: `AddTransient`, `AddScoped`, or `AddSingleton`?

4. **Build Environment**:
   - ❓ Can `dotnet build` run successfully now (NuGet restore fixed)?
   - ❓ What branch should implementation go on?
...
src/Osm.Emission/Seeds/
  └─ BootstrapScriptGenerator.cs (NEW - if needed)

tests/Osm.Pipeline.Tests/Orchestration/
  ├─ BuildSsdtBootstrapSnapshotStepTests.cs (NEW)
  └─ BuildSsdtPostDeploymentTemplateStepTests.cs (NEW)
```
...
**Acceptance Criteria**:
- ✅ `osm full-export` generates Bootstrap/ and PostDeployment-Bootstrap.sql
- ✅ User can copy PostDeployment-Bootstrap.sql into SSDT project
- ✅ First deployment applies bootstrap snapshot successfully
- ✅ Subsequent deployments skip bootstrap, apply baseline seeds
- ✅ No FK violations during SSDT deployment
- ✅ Bootstrap snapshot includes GO statements, PRINT diagnostics, topological comments
- ✅ Operators can verify execution order from SSDT output
...
|------|---------|----------------|-----------|
| `AllEntitiesIncludingStatic.bootstrap.sql` | Complete dataset (300 entities) topologically sorted | ❌ Gitignored | First deployment only |
| `PostDeployment-Bootstrap.sql` | SSDT PostDeployment template with guard + includes | ✅ Copy to SSDT once | Every deployment (guard skips bootstrap after first) |
| `StaticEntities.seed.sql` or per-table files | Baseline seeds (100 static entities) | ✅ Committed | Every deployment (idempotent) |

### Why This Solves the Problem

**Original Issue**: Static seed files ordered alphabetically by module → cross-module FK violations

**M1.0 Solution**:
1. **First deployment uses bootstrap snapshot** (global topological order, no module partitioning) → FK dependencies satisfied
2. **Subsequent deployments use baseline seeds** (data already exists from bootstrap, MERGE is idempotent) → order doesn't matter
3. **Module ordering becomes irrelevant** → baseline seeds can be organized however is convenient for developers
