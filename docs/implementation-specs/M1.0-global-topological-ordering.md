...
- ✅ PostDeployment template already designed to handle per-table mode (M1.5 ready)
...
│   └── [TableFiles]
├── Bootstrap/
│   └── AllEntitiesIncludingStatic.bootstrap.sql  (87 entities, topologically sorted)
├── BaselineSeeds/
│   └── StaticEntities.seed.sql                   (42 static entities, idempotent MERGE)
...
|------|---------|----------------|-----------|
| `AllEntitiesIncludingStatic.bootstrap.sql` | Complete dataset (87 entities) topologically sorted | ❌ Gitignored | First deployment only |
...
| `StaticEntities.seed.sql` | Baseline seeds (42 static entities) | ✅ Committed | Every deployment (idempotent) |
