...
### Change 4: Per-Table Baseline Seed Emission (NEW FEATURE)

**Status**: NEW feature requested by user for production scenarios
...
2. ✅ PostDeployment-Bootstrap.sql template dynamically adapts to emission mode (single-file, per-module, or per-table)
3. ✅ Per-table emission mode works correctly (one .seed.sql file per static entity)
4. ✅ Guard logic correctly detects first vs. subsequent deployments
5. ✅ Existing static seed emission unchanged (backward compatible)
6. ✅ Directory rename (Seeds → BaselineSeeds) happens automatically
7. ✅ All existing tests pass
8. ✅ New integration tests verify bootstrap snapshot correctness

## Defensive Measures for Bootstrap Snapshot Generation
...
1. Generate bootstrap snapshot with global topological sort
2. Generate PostDeployment-Bootstrap.sql template with guard logic
3. Add per-table emission mode for baseline seeds
4. Update directory structure (Seeds → BaselineSeeds, add Bootstrap/)

### Phase 2: Defensive Measures (1 day)
5. Add explicit GO statements between entities in bootstrap snapshot
6. Add diagnostic PRINT statements for execution tracing
7. Add topological position comments in generated scripts
8. Verify junction table deferral works on combined entity set

### Phase 3: Testing & Documentation (1 day)
9. Integration tests for bootstrap snapshot generation
10. Unit tests for per-table emission mode
11. Test SSDT deployment with PostDeployment-Bootstrap.sql
12. Update user documentation and examples
...
| Per-table emission mode | 0.5 days | UPDATE: `BuildSsdtStaticSeedStep.cs` |
| Update directory structure | 0.5 days | Rename Seeds → BaselineSeeds, add Bootstrap/ |
