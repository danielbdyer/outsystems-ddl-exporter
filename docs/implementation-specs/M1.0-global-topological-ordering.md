...
|-----|----------------|------------|
| **Dynamic data excludes static entities** | Static/dynamic sorted separately, cross-boundary FKs fail | ✅ Must fix |
| **No MERGE for static entities** | Cannot safely apply both static seeds + dynamic data | ✅ Must fix |
| **Module partitioning breaks order** | When `GroupByModule=true`, files ordered alphabetically | ⚠️ Detect + warn |
...
- Add unit tests for MERGE generation (simple PK, composite PK, no PK → error)
...
- When `true`:
  - `DynamicEntityInsertGenerator.BuildScript()` checks `IsStatic` → emit MERGE
...
3. ✅ Global topological sort applied to complete entity set
4. ✅ No FK violations during LoadHarness execution
5. ✅ Existing static seed emission unchanged
6. ✅ Can safely apply both static seeds + dynamic data without errors
7. ✅ All existing tests pass
8. ✅ New integration tests cover cross-boundary FK scenarios
...
**Additional Concerns**:
1. **Junction table prioritization**: Ensure parent → child → junction ordering, not just alphabetical
...
**Risk**: If junction classification only runs on static OR regular entities separately, cross-boundary junctions (e.g., static User + regular Department → junction UserDepartment) may not be detected.
...
2. **What is your LoadHarness execution sequence?**
   - Safe scripts → Remediation → Static seeds → Dynamic data?
   - Or different order?
...
5. **Are you seeing this with specific entity pairs?**
   - Can you provide example: `Table A (static/regular) → FK → Table B (static/regular)`?

## Implementation Priority

### Phase 1: Core Fix (2-3 days)
1. ✅ Verify static entities are included (CONFIRMED - they are)
2. ✅ Implement global topological sort (use existing `EntityDependencySorter` on combined set)
3. ✅ Add MERGE emission for static entities
4. ✅ Verify junction table deferral works cross-boundary

### Phase 2: Defensive Hardening (1 day)
5. ✅ Add explicit GO between entities in single-file mode
6. ✅ Add PRINT statements for execution tracing
7. ✅ Add topological position comments in scripts
8. ✅ Add FK constraint state validation
