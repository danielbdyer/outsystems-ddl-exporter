...
The UAT-users pipeline (in `src/Osm.Pipeline/UatUsers/`) already generates transformation artifacts (user map, UPDATE scripts, catalog), but there's **no verification system** to prove that:
1. Generated artifacts are internally consistent (all orphans mapped, no duplicate sources)
2. FK catalog discovery is complete (all user-referencing columns found)
3. Transformation maps comply with UAT inventory (no invalid targets)
4. Emitted UPDATE scripts will execute safely (all guards in place)

Currently, operators must manually inspect `01_preview.csv` and hope for the best. There's no machine-readable verification report that integrates with CI/CD or provides confidence that the transformation will succeed.

### Solution Approach

Build a **verification framework** for UAT-users artifacts that:
- Validates transformation map completeness (every orphan has a mapping, no duplicates)
- Verifies FK catalog discovery against model metadata (no missing columns)
- Proves UAT inventory compliance (all target IDs exist in UAT roster)
- Validates generated SQL safety (NULL guards, idempotence checks, target sanity checks)
- Emits machine-readable verification report (JSON) for automation

**This extends M1.1's export verification framework to cover UAT-users-specific concerns.**
