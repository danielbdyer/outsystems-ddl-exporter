# M1.8: Data Integrity Verification (DMM Replacement - Full Feature)

**Date**: 2025-11-18
**Status**: DEFERRED - DMM replacement (comprehensive validation)
**Dependencies**: M1.0 (Global Topological Ordering), M1.1 (Export Verification Framework), M1.3 (Basic integrity checks)
**Priority**: 🔵 Ship when ready for production validation and DMM replacement
...
After M1.3 provides basic row count and NULL count checks, **there is no byte-level verification that target database contains exactly the same data as source database**. This limits confidence in data migrations:

- **No Byte-Level Proof**: Can't prove data is byte-identical between source and target
- **Type Conversion Risks**: Silent data corruption from VARCHAR→NVARCHAR or similar conversions goes undetected
- **DMM Dependency**: Still requires vendor tool (DMM) for comprehensive data validation, which is expensive and slow
- **Production Anxiety**: Operators lack confidence that export→load process preserves all data correctly

**Example**: A subtle bug in INSERT generation converts `VARCHAR(50)` to `NVARCHAR(50)`, causing encoding differences. Basic checks pass (row counts match), but data is subtly corrupted. Production users report issues weeks later.

### Recommended Solution

**Comprehensive Data Integrity Verification (DMM Replacement)**

Implement cryptographic hash-based verification that runs after LoadHarness completes:

1. **Row-Level Hash Comparison**: Compute SHA2-256 hashes of entire rows in source and target, compare byte-for-byte
2. **Column-Level NULL Verification**: Verify NULL counts match for every nullable column
3. **Data Type Conversion Detection**: Compare source vs. target schema metadata, flag type changes
4. **Name Mapping Resolution**: Handle physical→logical name aliasing via `model.json` and naming overrides
5. **Comprehensive Reporting**: Generate detailed verification report with pass/fail per table

This provides **byte-level certainty** that target database is identical to source, replacing DMM.

### How It Fixes the Problem

1. **Byte-Level Proof**: Cryptographic hashes prove data is identical, not just row counts
2. **Type Conversion Detection**: Warns about schema differences that could cause corruption
3. **DMM Replacement**: Eliminates dependency on expensive vendor tool
4. **Production Confidence**: Provides mathematical certainty that export→load preserves data correctly
5. **Automated**: Runs automatically after load, no manual validation needed

### Alternative Approaches Considered

**Alternative 1: Keep Using DMM**
- Continue relying on vendor tool for data validation
- **Rejected**: Expensive license, slow execution, vendor lock-in, doesn't integrate with OSM pipeline

**Alternative 2: Basic Checks Only (M1.3)**
- Just row counts and NULL counts, skip byte-level comparison
- **Rejected**: Can't detect subtle corruption (type conversions, encoding issues), insufficient for production confidence

**Alternative 3: Sample-Based Validation**
- Compare hash of random sample (e.g., 10% of rows)
- **Rejected**: May miss corruption in unsampled rows, performance not significantly better for small datasets

**Alternative 4: Application-Level Validation**
- Let application verify data after deployment
- **Rejected**: Fails too late (in production), wastes deployment time, hard to diagnose

**Alternative 5: File-Level Checksums**
- Validate generated SQL files instead of loaded data
- **Rejected**: Doesn't prove data actually loaded correctly, M1.1 already does this

**Why Comprehensive Verification Wins**: Byte-level certainty, DMM replacement, automated, integrates with pipeline, catches all corruption types.
...
**Integration Points**:
- **M1.0 (Bootstrap Snapshot)**: Verifies data loaded via bootstrap snapshot or baseline seeds
- **M1.2 (Ordering Proof)**: Can reference topological ordering proof to diagnose FK-related data issues
...
- Verifies data loaded via M1.0's bootstrap snapshot (`AllEntitiesIncludingStatic.bootstrap.sql`)
- Also verifies baseline seeds (`BaselineSeeds/StaticEntities.seed.sql`) if loaded separately
- Comparison order matches topological sort from M1.0 (can reference M1.2 proof for debugging)
...
### A.1 Checksum Algorithm Selection

**Critical Finding**: Neither `CHECKSUM()` nor `BINARY_CHECKSUM()` are suitable for production data integrity verification.

**Key Limitations**:
1. **High Collision Probability**: Both use only 4 bytes, resulting in unacceptably high collision rates
2. **Intentional Collisions**: Different inputs can produce identical checksums (e.g., sign changes on decimals)
3. **Limited Processing**: `BINARY_CHECKSUM()` only processes first 255 characters of `varchar(max)`
4. **Microsoft Warning**: Documentation explicitly warns against using for critical integrity checks

**NULL Handling**:
- `CHECKSUM_AGG()` **ignores NULL values entirely** - critical flaw for integrity verification
- Example: `CHECKSUM_AGG` of `(1, 2, NULL)` equals `CHECKSUM_AGG` of `(1, 2)` - identical checksums despite different data!
- **Implication**: Tables with different NULL distributions can have identical checksums if non-NULL values match

**Collation Sensitivity**:
- `CHECKSUM()` is collation-dependent - same data with different collations produces different checksums
- Risk of false mismatches when comparing across databases with different collations
- `BINARY_CHECKSUM()` is NOT collation-sensitive but IS case-sensitive

**Recommendation**: **HASHBYTES() with SHA2_256**
- True cryptographic hash with ~2^-128 collision probability
- Processes full column content (no truncation)
- Microsoft's recommendation for change detection in critical systems
- Tradeoff: 10-50x slower than CHECKSUM, but acceptable for ~100k row tables

### A.2 NULL Preservation Verification

**Problem**: Checksum functions ignore NULLs, so NULL count mismatches go undetected.

**Solution**: Column-level NULL count comparison using `COUNT(*)` vs `COUNT(column)`:

```sql
-- Source query
SELECT
    COUNT(*) AS TotalRows,
    COUNT([EMAIL]) AS EmailNotNullCount,
    COUNT([PHONE]) AS PhoneNotNullCount
FROM [dbo].[OSUSR_ABC_CUSTOMER];
...
**Verification Logic**:
1. `TotalRows` must match (basic sanity check)
2. For each column: `NotNullCount` must match
3. Derived: `NullCount = TotalRows - NotNullCount` automatically matches

**Performance**: Single table scan per database, efficient even for ~100k rows.

### A.3 Pipeline Name Mapping Architecture

**Primary Artifact**: `model.json` (from `extract-model` stage)

Contains complete physical→logical mapping:
```json
{
  "entities": [{
    "name": "Customer",                    // Logical name
    "physicalName": "OSUSR_ABC_CUSTOMER",  // Physical SQL Server table
    "attributes": [{
      "name": "Email",                     // Logical attribute
      "physicalName": "EMAIL",             // Physical column
      "onDisk": {
        "isNullable": false,
        "sqlType": "nvarchar",
        "maxLength": 255
...
**Single Intervention Point**: `NamingOverrideOptions.GetEffectiveTableName()`

Priority order:
1. Table override: `dbo.OSUSR_ABC_CUSTOMER` → custom name
2. Entity override: `Customer` → custom name (e.g., `Customers`)
3. Default: Use `LogicalName`
4. Fallback: Use `PhysicalName`

**Implication**: Verification must use same name resolution logic as emission to match target table names.

### A.4 Column Ordering Requirement

**User Confirmation**: Column order MUST be preserved for checksum matching.

**Implementation**:
- Parse `model.json` attribute array (already in correct order per OutSystems schema)
- Build SELECT lists preserving attribute order
- Apply name mapping: `attribute.ColumnName` (source) → `attribute.LogicalName` (target)

**Example**:
```sql
-- Source: SELECT [ID], [EMAIL], [PHONE] FROM [dbo].[OSUSR_ABC_CUSTOMER]
-- Target: SELECT [Id], [Email], [Phone] FROM [dbo].[Customer]
-- Order matches model.json attributes: [Id, Email, Phone]
```

### A.5 Data Type Conversion Handling

**User Guidance**: Data type conversions are in-scope but flag for operator scrutiny (not hard failures).

**Strategy**:
1. Detect conversions by comparing `attribute.OnDisk.SqlType` with target schema
2. Report as warnings (not errors)
3. Continue checksum comparison
4. Let operator decide if mismatch is due to conversion vs actual data loss

**Common Conversions to Flag**:
- `VARCHAR(n)` → `NVARCHAR(n)` (ASCII to Unicode)
- `DECIMAL(p1,s1)` → `DECIMAL(p2,s2)` (precision change)
- `INT` → `BIGINT` (widening)
- `DATETIME` → `DATETIME2` (precision change)

**Checksum Behavior**: Semantically identical data may have different checksums due to binary representation differences. Flag as "suspected conversion mismatch" rather than "data loss".

### A.6 LoadHarness Current Capabilities

**What it does**:
- Executes SQL scripts (DDL, remediation, static seed, dynamic inserts)
- Measures performance (timing, wait stats, lock summary, index fragmentation)
- Reports warnings for missing scripts

**What it does NOT do**:
- ❌ Verify data integrity (no row counts, no checksums)
- ❌ Compare source vs target
- ❌ Validate transformations

**Conclusion**: LoadHarness is a **script executor**, not a **data verifier**. M1.3 adds verification layer on top.

### A.7 Key Files Referenced

1. **src/Osm.Domain/Model/EntityModel.cs** - `PhysicalName` + `LogicalName` fields
2. **src/Osm.Domain/Model/AttributeModel.cs** - `ColumnName` (physical) + `LogicalName`
3. **src/Osm.Domain/Configuration/NamingOverrideOptions.cs** - Name resolution logic (line 194)
4. **src/Osm.Pipeline/Runtime/FullExportRunManifest.cs** - Pipeline metadata
5. **src/Osm.LoadHarness/LoadHarnessRunner.cs** - Script execution orchestration
6. **src/Osm.LoadHarness/LoadHarnessReport.cs** - Performance report structure
