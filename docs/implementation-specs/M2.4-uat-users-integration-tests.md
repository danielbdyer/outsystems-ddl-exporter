...
**Priority**: 🟡 Enhanced Quality (ship after M2.2/M2.3 adoption)
...
**Example**: UAT-users works with simple numeric user IDs (unit tested) but fails with GUID user IDs (not tested). Production uses GUIDs → pipeline failure on first run.
...
tests/Osm.Pipeline.Tests/UatUsers/Integration/
  ...
  └─► CrossModeEquivalenceTests/
      ...
      └─ VerifierOracleTests.cs (M2.3 verifier confirms equivalence)
```
...
### Full-Export Independence Cases (mandatory)

- **No-flag parity**: Add a regression test that runs `full-export` without `--enable-uat-users` before and after the M2.x wiring and asserts identical manifests + dynamic INSERT payload hashes. This guards against implicit coupling from the split coordinator.
- **Toggle isolation**: Add a paired test that runs `full-export --enable-uat-users` on the same fixture and asserts that only user FK values differ (QA IDs vs transformed UAT IDs) while non-user columns and manifest metadata remain identical.
- **CLI defaults**: Cover a negative test where the uat-users inventories are omitted; the base `full-export` should still execute successfully without requesting UAT-specific inputs.
- **Test-first**: Land the three independence tests before refactoring the coordinator/build path to enforce TDD and prevent regressions while wiring the specialization.
...
#### ✅ **M2.3 Transformation Verifier** - Use as Oracle
...
src/Osm.Pipeline/UatUsers/Verification/
  ...
  ├─ InsertTransformationVerifier.cs (M2.3 - use as oracle)
  ├─ UpdateScriptSimulator.cs (M2.3 - use as oracle)
  └─ CrossModeValidator.cs (M2.3 - use as oracle)
```
