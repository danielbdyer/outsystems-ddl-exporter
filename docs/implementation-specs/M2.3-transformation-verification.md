...
**Priority**: 🟡 Enhanced Verification (ship after M2.2 ships - deferred verifiability)
...
**After M2.2 ships** (INSERT transformation implementation), operators can generate pre-transformed INSERTs and load them to UAT. However, there's **no automated verification that transformations were correctly applied**.
...
**Deferred Verifiability Pattern**: Like M1.0 shipped before M1.1/M1.2 verification, M2.2 can ship with manual inspection. M2.3 provides automated verification that can be deferred after milestone deadline.
