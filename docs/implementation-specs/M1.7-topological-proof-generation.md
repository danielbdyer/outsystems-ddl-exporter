...
**Priority**: 🔵 Ship when operators request proof artifacts for documentation/auditing

## Executive Summary

### The Specific Problem

After M1.0 generates topologically-sorted bootstrap snapshots and M1.2 validates ordering at runtime, **there is no persistent artifact that documents the dependency graph and proves ordering correctness**. This creates challenges:

- **No Audit Trail**: Can't prove to stakeholders that ordering is correct
- **Debugging Difficulty**: When FK violations occur, can't easily visualize which dependencies caused the issue
- **Cycle Investigation**: When cycles detected, can't see which entities form the cycle
- **Compliance Requirements**: Some organizations require documented proof of data migration correctness

**Example**: QA asks "Why does table X load before table Y?" Operator must manually inspect code and FK definitions to explain, no documentation artifact exists.

### Recommended Solution

**Topological Ordering Proof Artifacts**

Generate comprehensive proof artifacts after topological sorting completes:

1. **Machine-Readable Proof** (`ordering-proof.json`): Complete dependency graph with all FK edges, entity positions, topological depths
2. **Human-Readable Report** (`ordering-report.md`): Formatted documentation showing dependency trees, ordering rationale
3. **Violation Report** (`ordering-violations.json`): Any detected issues (cycles, missing edges, child-before-parent)
4. **Visualization Data**: JSON suitable for rendering dependency graphs in external tools

These artifacts are **generated once per export** and provide permanent documentation of ordering decisions.

### How It Fixes the Problem

1. **Audit Trail**: Persistent proof artifacts demonstrate ordering correctness for compliance/documentation
2. **Debuggability**: When issues occur, inspect proof to understand which FK caused specific ordering
3. **Visualization**: Human-readable reports make complex dependency graphs understandable
4. **Cycle Diagnosis**: Violations report shows exactly which entities form cycles
5. **Knowledge Transfer**: New team members can understand ordering logic by reading reports

### Alternative Approaches Considered

**Alternative 1: No Proof Artifacts (M1.2 Only)**
- Just runtime validation, no persistent documentation
- **Rejected**: Doesn't meet compliance needs, hard to debug historical issues, no knowledge transfer

**Alternative 2: Inline Comments in SQL**
- Add dependency comments directly in bootstrap snapshot
- **Rejected**: Clutters SQL files, hard to query/parse, no structured format for tooling

**Alternative 3: Real-Time Visualization Tool**
- Interactive web UI for exploring dependencies
- **Rejected**: Over-engineered for MVP, requires web server, artifacts more portable

**Alternative 4: Integration with M1.2 Validation**
- Combine proof generation with runtime validation in single step
- **Rejected**: Slows down validation, couples two concerns, proof is optional M1.2 is critical

**Alternative 5: Database-Stored Proof**
- Store proof metadata in target database tables
- **Rejected**: Pollutes data model, hard to version control, less portable than files

**Why Proof Artifacts Win**: Permanent documentation, supports compliance, enables debugging, portable (JSON/Markdown files), doesn't slow down critical path.
...
public enum EdgeClassification
{
    ...
    CrossBoundary,    // Static → Regular or vice versa
...
2. **Dependency Graph**: ASCII tree or table showing parent→child relationships
...
**Status**: ✅ PASS - All FK dependencies satisfied
...
│   ├── SESSION (2)
│   ├── AUDIT_LOG (2)
│   └── USER_DEPARTMENT (3) [junction]
├── PERMISSION (1)
│   └── ROLE_PERMISSION (2) [junction]
└── ROLE_HIERARCHY (1) [self-ref]
...
public sealed record EntityDependencySortResult(
    ...
    ImmutableDictionary<TableKey, int> TopologicalPositions,  // Table → 0-based position
    ImmutableDictionary<TableKey, int> TopologicalDepths,     // Table → max dependency chain length
...
├── BaselineSeeds/
├── Proofs/                           ← NEW
│   ├── ordering-proof.json          ← Machine-readable proof
│   ├── ordering-report.md           ← Human-readable report
│   └── ordering-violations.json     ← Only if violations detected
└── PostDeployment-Bootstrap.sql
```
...
return state with {
    ...
    BootstrapOrdering = ordering  // ← NEW: Store ordering result
};
```
...
BuildSsdtDynamicInsertStep
  ↓
BuildSsdtBootstrapSnapshotStep (M1.0) ← Generates bootstrap with topological order
  ↓
BuildSsdtOrderingProofStep (M1.2) ← NEW: Proves ordering correctness
  ↓
...
3. Check "Status" line: ✅ PASS or ❌ FAIL
...
**Status**: ❌ FAIL - 3 ordering violations detected
...
→ OSSYS_SESSION at position 15 depends on USER at position 5 ✅
→ No violation - USER appears before SESSION
→ FK violation must be caused by something else (data issue, constraint issue, etc.)
```
...
→ ROLE has no dependencies (depth 0, layer 0)
→ USER depends on ROLE (depth 1, layer 1)
→ SESSION depends on USER (depth 2, layer 2)
```
...
{
    // Arrange: User → Role, but Role not in dataset
...
1. ✅ `ordering-proof.json` emitted for bootstrap snapshot
2. ✅ `ordering-report.md` is human-readable and contains all sections
3. ✅ Violations correctly detected (child before parent, missing parent, cycles)
4. ✅ Topological depths calculated correctly
5. ✅ Dependency visualization shows parent→child relationships
6. ✅ Proof artifacts can be used to debug FK violations
7. ✅ All tests pass
