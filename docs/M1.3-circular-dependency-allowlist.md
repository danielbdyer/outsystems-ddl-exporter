...
**Note**: Organization has CreatedBy (nullable FK to User). Load Organization first with NULL CreatedBy, then User, then UPDATE Organization.CreatedBy.
...
--     Manual ordering configured with nullable FK support for phased loading
...
### Current State (M1.3 - COMPLETE)
- ✅ Configuration model defined
- ✅ JSON configuration loader implemented
- ✅ CLI flag `--circular-deps-config` wired up
- ✅ Allowlist matching implemented
- ✅ Manual ordering applied to EntityDependencySorter
- ✅ Nullable FK validation for phased loading
- ✅ Bootstrap script shows allowed vs. disallowed cycles

### Usage

```bash
# Run with circular dependency configuration
dotnet run -- build-ssdt \
  --model-path model.json \
  --output-dir output \
  --circular-deps-config circular-deps.json
```

The configuration file (circular-deps.json) should follow the JSON format shown in examples above.
