...
public class SmoColumnBuilderTests
{
    ...
    public void BuildColumns_aligns_reference_types_and_defaults()
    {
        ...
        var idColumn = columns.Single(c => c.Name.Equals("Id", StringComparison.Ordinal));
        Assert.False(idColumn.Nullable);
        Assert.True(idColumn.IsIdentity);

        var cityColumn = columns.Single(c => c.Name.Equals("CityId", StringComparison.Ordinal));
        Assert.Equal(SqlDataType.BigInt, cityColumn.DataType.SqlDataType);
        Assert.False(cityColumn.Nullable);

        var firstNameColumn = columns.Single(c => c.Name.Equals("FirstName", StringComparison.Ordinal));
        ...
        var isActive = cityColumns.Single(c => c.Name.Equals("IsActive", StringComparison.Ordinal));
