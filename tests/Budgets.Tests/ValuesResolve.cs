...
/// names things that exist, or says <c>pending M&lt;n&gt;</c>, accepted until M&lt;n&gt;'s exit raises
/// <see cref="Contract.Milestone"/> past n, or <c>pending W</c>, accepted until M8, or says <c>prose only</c>. A name
/// is a backticked test (<c>Budgets.Tests: Manifest</c>, <c>Kernel.Tests: "an English name"</c>, or a test class), a
/// file or directory, or <c>estate &lt;verb&gt;</c> once the verb is built. A pending clause whose test already exists
/// drops its <c>pending</c>. The rows that say <c>prose only</c> are printed, and their number rises only with a
/// DECISIONS.md line.
/// </summary>
public sealed class ValuesResolve(ITestOutputHelper output)
{
    ...
    public void Every_value_names_where_it_is_held()
    {
        ...
        Assert.Empty(rows.SelectMany(r => Clause.Split(r[^1]).SelectMany(c => Unresolved(c)).Select(p => "VALUES.md " + r[0] + ": " + p)));
    ...
    public void A_clause_resolves_when_it_names_what_exists_or_a_milestone_not_yet_exited(string clause, bool resolves) =>
        Assert.Equal(resolves, !Clause.Split(clause).SelectMany(c => Unresolved(c)).Any());

    /// <summary>What a clause fails to resolve, as the reason; nothing when it resolves.</summary>
    private static IEnumerable<string> Unresolved(string clause)
    {
        var pending = Pending.Match(clause);
        var names = Name.Matches(clause).Select(m => m.Groups[1].Value).ToList();
        if (pending.Success)
        {
            var until = pending.Groups[1].Success ? int.Parse(pending.Groups[1].Value, CultureInfo.InvariantCulture) : 8;
            if (until < Documents.Milestone)
            {
                yield return "'" + clause + "' is past its milestone's exit; name the test that holds it";
            }

            foreach (var built in names.Where(n => Test.IsMatch(n) && Exists(n)))
            {
        ...
        else if (!clause.Contains("prose only", StringComparison.Ordinal))
        {
            ...
            foreach (var missing in names.Where(n => !Exists(n)))
            {
    ...
    private static bool Exists(string name)
    {
        ...
        {
            return Contract.Verbs.Any(v => v.Name == name[7..] && v.Arrives <= Documents.Milestone);
        }
