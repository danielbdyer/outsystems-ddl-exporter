...
public sealed class BannedSymbolsTests
{
    ...
    public void Each_banned_symbol_planted_in_the_kernel_is_a_build_error()
    {
        ...
        var (exit, output, errors) = PlantedProject.Build(plant.Under("kernel.csproj"),
            ["namespace Planted;", "", "public static class Uses", "{", "    public static object[] All() =>", "    ["], [.. Planted.Select(p => $"        {p.Use},")], ["    ];", "}", ""]);

        Assert.NotEqual(0, exit);
        var missing = Planted.Where((_, i) => errors[i] != "RS0030").Select(p => p.Symbol).ToList();
