...
/// one imperative sentence on one line, starting with a capital letter and ending with a period, led by a verb, with any estate verb it
...
public sealed class Refusals
{
    ...
    private static readonly string[] Form = ["starts in lower case", "has no period", "names no verb of estate"];
    ...
    [InlineData("Run estate doctor.", null)]
    [InlineData("Run estate --help to see the verbs.", null)]
    [InlineData("Set the variable, or write the file outside git, that env:ESTATE_DEV names.", null)]
    [InlineData("estate doctor", "starts in lower case")]
    [InlineData("global.json names the SDK to install.", "starts in lower case")]
    [InlineData("ci/publish.sh, or ci/publish.ps1 on Windows, publishes dist/estate/; run estate from there", "starts in lower case")]
    [InlineData("Run estate doctor", "has no period")]
    [InlineData("Run estate frobnicate.", "names no verb of estate")]
    [InlineData("Then run estate again.", "names no verb of estate")]
    ...
    public void A_remedy_is_one_imperative_sentence_that_runs_a_verb_of_estate_if_any(string remedy, string? flaw) =>
    ...
    private static string? Flaw(string remedy)
    {
        ...
        var runs = RunsEstate.Matches(remedy).Select(m => m.Groups[1].Value.TrimEnd(',', ';', ':', '.')).ToList();
        return remedy.Contains('\n', StringComparison.Ordinal) ? "spans lines"
            ...
            : runs.Any(word => word != "--help" && !Contract.Verbs.Any(v => v.Name == word)) ? "names no verb of estate"
