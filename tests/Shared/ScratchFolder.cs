...
/// linked into the projects that reference io, under the repository's .estate/ for work that must find dist/estate/ or the
...
internal sealed partial class ScratchFolder : IDisposable
{
    ...
    public static ScratchFolder Temporary(string purpose) => new(Directory.CreateTempSubdirectory("estate-" + purpose + "-").FullName);
