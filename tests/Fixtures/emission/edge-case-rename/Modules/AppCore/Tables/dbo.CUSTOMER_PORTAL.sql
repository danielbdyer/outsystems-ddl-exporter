IF OBJECT_ID(N'[dbo].[CUSTOMER_PORTAL]', N'U') IS NULL
BEGIN
...
IF NOT EXISTS (
    SELECT 1
    FROM sys.indexes
    WHERE name = N'IDX_CUSTOMER_PORTAL_Email'
      AND object_id = OBJECT_ID(N'[dbo].[CUSTOMER_PORTAL]', N'U')
)
BEGIN
...
IF NOT EXISTS (
    SELECT 1
    FROM sys.indexes
    WHERE name = N'IDX_CUSTOMER_PORTAL_Name'
      AND object_id = OBJECT_ID(N'[dbo].[CUSTOMER_PORTAL]', N'U')
)
BEGIN
