CREATE TABLE [dbo].[CUSTOMER_PORTAL] (
    ...
    [CityId]    BIGINT         NOT NULL
        CONSTRAINT [FK_CUSTOMER_PORTAL_City_CityId]
            FOREIGN KEY ([CityId]) REFERENCES [dbo].[City] ([Id])
)

GO

CREATE UNIQUE INDEX [UIX_CUSTOMER_PORTAL_Email]
    ON [dbo].[CUSTOMER_PORTAL]([Email]) WHERE ([Email] IS NOT NULL) WITH (FILLFACTOR = 85, IGNORE_DUP_KEY = ON)
    ON [FG_Customers]

GO

CREATE INDEX [IX_CUSTOMER_PORTAL_LastName_FirstName]
    ON [dbo].[CUSTOMER_PORTAL]([LastName], [FirstName]) WITH (STATISTICS_NORECOMPUTE = ON)
    ON [FG_Customers]

GO

ALTER INDEX [IX_CUSTOMER_PORTAL_LastName_FirstName]
