...
public sealed class NullabilityEvaluatorTests
{
    ...
    [Fact]
    public void Cautious_Should_Block_MandatoryRelaxation_When_FlagDisabled()
    {
        ...
        Assert.Contains(TighteningRationales.CautiousRelaxationDisabled, decision.Rationales);
        Assert.Contains(TighteningRationales.RemediateBeforeTighten, decision.Rationales);
        Assert.Contains(TighteningRationales.DataHasNulls, decision.Rationales);
    }

    [Fact]
    public void Cautious_Should_Allow_MandatoryRelaxation_When_FlagEnabled()
    {
        var options = TighteningPolicyTestHelper.CreateOptions(
            TighteningMode.Cautious,
            nullBudget: 0.05,
            allowCautiousNullabilityRelaxation: true);
        ...
        Assert.False(decision.MakeNotNull);
        Assert.False(decision.RequiresRemediation);
        Assert.DoesNotContain(TighteningRationales.CautiousRelaxationDisabled, decision.Rationales);
    }
