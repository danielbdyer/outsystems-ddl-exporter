...
public sealed class AnswerSizeTests : IDisposable
{
    ...
    {
        var diff = Contract.Verbs.Single(v => v.Name == "diff") with { Body = (_, _) => Verbs.Diff(Side("dacpac:before.dacpac"), Side("dacpac:after.dacpac"), Large(), false, Verbs.Stamped(null, null)) };
    ...
    private static Verbs.Source Side(string target) => new(Ok(SqlServer.Target.Parse(target)), new Ssdt.ModelElements([], []), null, false);
