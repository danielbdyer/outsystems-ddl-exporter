...
public sealed class ImageTests : IDisposable
{
    ...
    {
        Assert.Null(ScratchServer.Image(Copy("Server=127.0.0.1,11433;User ID=sa"), (_, _) => null));
        Assert.Null(ScratchServer.Image(Copy("Server=127.0.0.1,11433;User ID=sa"), (_, _) => (1, "Error: No such container: estate-sql\n")));
    }
    ...
    private static Doctor.Command Docker(IReadOnlyList<string> repoDigests) => (file, arguments) => (file, arguments) switch
    {
        ("docker", ["container", "inspect", ..]) => (0, Built + " {\"1433/tcp\":[{\"HostIp\":\"127.0.0.1\",\"HostPort\":\"11433\"}]}\n"),
        ("docker", ["image", "inspect", .., var image]) when image == Built => (0, System.Text.Json.JsonSerializer.Serialize(repoDigests) + "\n"),
        _ => (1, ""),
    };
}
