...
module Projection.Tests.CdcSilenceCrossEmitterTests
...
//       ⟹ ∀ t ∈ T : CDC.captures(t) = ∅
//
// The existing CdcSilenceTests.Slice γ proves this for `StaticSeedsEmitter`
// alone (single emitter, single kind, no Phase-2 UPDATE). This file
// extends to the production-shape full pipeline:
//
//   - **C1 single-emitter redeploy via composer** — proves the
//     composer's pipeline preserves the per-emitter CDC silence
//     property
//   - **C2 cross-emitter redeploy** — Country (Static) + LegacyOrder
//     (Migration) populate disjoint tables; both CDC-tracked. Proves
//     the full pipeline's Phase-1 MERGE-only output is CDC-silent
//     across emitters.
//   - **C3 Phase-2 UPDATE CDC behavior** — LegacyOrder has a
//     self-FK on nullable `ParentId`. Migration row populates with
//     `ParentId` self-referencing → cycle → Phase-2 UPDATE fires.
//     **This is the test that discovers whether V2's Phase-2
//     UPDATE redeploy is CDC-silent in SQL Server 2022.**
//   - **C4 sensitivity** — changed-content redeploy via composer
//     DOES fire CDC (proves the canary mechanism is real; rules out
//     trivial-pass scenarios).
...
// MigrationDependenciesEmitter populates via context row.
// ----------------------------------------------------------------

let private legacyOrderKindKey = mkKey ["LegacyOrder"]

let private mkLegacyOrderKind () : Kind =
...
let private policyAllRemaining : Policy =
    { Policy.empty with
        Emission =
...
let private migrationCtxFor (legacyKindKey: SsKey) (parentId: string) : MigrationDependencyContext =
    { Rows =
...
// ----------------------------------------------------------------
// Multi-table CDC scenario runner. Extends the single-kind shape
// from CdcSilenceTests.fs to (a) enable CDC on every supplied
// (schema, table) pair, and (b) sum CDC capture rows across them.
// Returns (baseline, post) totals so the property can be expressed
// as `post = baseline` on the aggregated count.
// ----------------------------------------------------------------
...
let private buildSchemaSqlFor (catalog: Catalog) : string =
...
type private CdcTrackedTable =
    {
        Schema : string
        Table  : string
    }

let private cdcAwareTracked (kinds: Kind list) : CdcTrackedTable list =
    kinds
    |> List.map (fun k ->
...
/// the database.
let private enableCdcOn (cnn: SqlConnection) (tracked: CdcTrackedTable list) : Task<unit> =
    task {
...
let private countTotalCaptures (cnn: SqlConnection) (tracked: CdcTrackedTable list) : Task<int> =
    task {
        let mutable total = 0
        for t in tracked do
            let captureTable =
...
// ----------------------------------------------------------------
// C0 — pure SQL-shape diagnostic (no Docker). Dumps the composed
// output for the cross-emitter fixture so the structural shape
// surfaces in the test trail regardless of CDC behavior.
// ----------------------------------------------------------------
...
[<Fact>]
let ``5.13.cdc-silence-cross-emitter C1: composer single-emitter redeploy fires zero CDC captures`` () =
...
//
// LegacyOrder's row carries Id=1 + ParentId=999 (no cycle — references
// a non-existent parent; since the FK is not enforced in this fixture's
// schema, the row inserts fine. Phase-2 UPDATE does not fire on this
// row because ParentId is not deferred).
//
// Actually: V2's deferred-FK logic kicks in on TopologicalOrder.Cycles
// membership. With LegacyOrder self-FK on ParentId, the kind is in a
// 1-node SCC (self-loop). Whether the cycle resolver treats this as
// requiring deferral depends on SelfLoopPolicy.
// ----------------------------------------------------------------

[<Fact>]
let ``5.13.cdc-silence-cross-emitter C2: composer cross-emitter (Static + Migration) redeploy fires zero CDC captures`` () =
...
// open question.
//
// If C3 fails, V2 has a real CDC leak via Phase-2 UPDATE, and the
// implementation needs a change-detection predicate at
// `buildUpdateStatement`.
//
// If C3 passes, SQL Server's optimizer covers the standalone UPDATE
// case too — V2 can defer the predicate addition until evidence
// surfaces (e.g., older-SQL-Server cutover target).
// ----------------------------------------------------------------

[<Fact>]
let ``5.13.cdc-silence-cross-emitter C3: Phase-2 UPDATE redeploy fires zero NEW CDC captures (discovery)`` () =
    ...
    // a change-detection predicate. The failure diagnostic will
    // surface (baseline, post) so the implementation gap is
    // visible at the test boundary.
    Assert.Equal (baseline, post)

// ----------------------------------------------------------------
// C4 — sensitivity: changed-content redeploy DOES fire CDC via
// composer. Rules out trivial-pass scenarios (e.g., CDC scan
// silently broken; the property "post = baseline" holds because
// both are zero).
// ----------------------------------------------------------------

[<Fact>]
let ``5.13.cdc-silence-cross-emitter C4 sensitivity: changed-content composer redeploy DOES fire CDC`` () =
