...
// ---------------------------------------------------------------------------
// Inactive-records carry-through (chapter A.0' slice β; supersedes
// session-21 silent-drop disposition per DECISIONS 2026-05-15 — A.0'
// slice β amendment). The session-21 cites are preserved in the test
// names per the false-start documentation discipline; the test bodies
// now assert carry-through with IsActive = false.
// ---------------------------------------------------------------------------

[<Fact>]
let ``SnapshotRowsets: inactive modules carry through (slice β supersedes session 21 silent drop)`` () =
    ...
    | Ok actual ->
        // Both modules survive; the inactive one carries IsActive = false.
        Assert.Equal (2, List.length actual.Modules)
        let active = actual.Modules |> List.find (fun m -> m.Name = mkName "AppCore")
        let inactiveModule = actual.Modules |> List.find (fun m -> m.Name = mkName "Inactive")
        Assert.True active.IsActive
        Assert.False inactiveModule.IsActive

[<Fact>]
let ``SnapshotRowsets: inactive kinds carry through (slice β supersedes session 21 silent drop)`` () =
    let inactiveKind =
    ...
    | Error errors -> Assert.Fail (sprintf "Expected Ok; got Error: %A" errors)
    | Ok actual ->
        // Both kinds survive within the module; the inactive one carries
        // IsActive = false.
        Assert.Equal (2, List.length actual.Modules.[0].Kinds)
        let active = actual.Modules.[0].Kinds |> List.find (fun k -> k.Name = mkName "User")
        let inactiveKindResult = actual.Modules.[0].Kinds |> List.find (fun k -> k.Name = mkName "Archived")
        Assert.True active.IsActive
        Assert.False inactiveKindResult.IsActive

[<Fact>]
let ``SnapshotRowsets: inactive attributes carry through (slice β supersedes session 21 silent drop)`` () =
    ...
    | Error errors -> Assert.Fail (sprintf "Expected Ok; got Error: %A" errors)
    | Ok actual ->
        // All three attributes survive; the inactive one carries
        // IsActive = false.
        Assert.Equal (3, List.length actual.Modules.[0].Kinds.[0].Attributes)
        let inactiveAttribute =
            actual.Modules.[0].Kinds.[0].Attributes
            |> List.find (fun a -> a.Name = mkName "Deleted")
        Assert.False inactiveAttribute.IsActive
    ...
    let json =
        """{ "exportedAtUtc": "2026-05-10T00:00:00Z",
             "modules": [ { "name": "AppCore", "isSystem": false, "isActive": true,
                            "entities": [] } ] }"""
    let bundle : CatalogReader.RowsetBundle =
        { Modules = [ moduleRow None ]; Kinds = []; Attributes = []; References = []; Triggers = []; Sequences = []  }
...
[<Fact>]
let ``slice 2: inactive source attribute carries through with its reference (slice β supersedes session 21 chained drop)`` () =
    let bundle =
        { referenceBundle with
            Attributes =
                [ accountIdRow None
    ...
    | Ok actual ->
        let user = actual.Modules.[0].Kinds |> List.find (fun k -> k.Name = mkName "User")
        // Per DECISIONS 2026-05-15 — A.0' slice β amendment: the chained
        // reference-drop behavior retires. The attribute carries through
        // with IsActive = false; its reference also carries through. A
        // future emitter overlay can elect to suppress FK-emission on
        // inactive-source-attribute references.
        Assert.Equal (1, List.length user.References)
        Assert.Equal (2, List.length user.Attributes)
        let inactiveAttr =
            user.Attributes |> List.find (fun a -> a.Name = mkName "AccountId")
        Assert.False inactiveAttr.IsActive
