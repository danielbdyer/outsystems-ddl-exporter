...
[<Fact(Skip = "Matrix row 46 — 🔵 V2-EXTENSION. V1's `EntityModel` carries kind/origin via two binary booleans (`IsSystemEntity: bool`, `IsExternalEntity: bool`) — a 2-bit encoding with implicit 4 states. V2's `Kind` decomposes the axis into (a) closed-DU `Origin = OsNative | ExternalViaIntegrationStudio | ExternalDirect` (3 explicit states) + (b) `ModalityMark list` with payload-free `SystemOwned` variant (sibling to `TenantScoped`, `SoftDeletable`, `Temporal`, `Static`). V2's encoding makes the orthogonal axes (origin vs. ownership) type-distinct rather than convention-distinct. Pillar 9 (DataIntent dichotomy) classifies these as DataIntent evidence (sourced from V1 rowsets; no operator opinion). No DECISIONS needed.")>]
