...
let private genVariant : Gen<ProjectionConfig * Flow> =
    gen {
        ...
        let flow = { Name = "v"; From = from; To = "sink"; Rekey = None; Tables = []; Reconcile = []; ReconcileIgnore = []; SupportingScope = []; Scope = scope; Shape = None; Shaping = None; Strategy = None; Resumable = false; Streaming = false; Journal = None }
        let cfg =
...
let ``A44 clause 1 — the flow execution profile round-trips (strategy/resumable/streaming/journal)`` () =
    ...
    for (strat, resumable, streaming, journal) in profiles do
        let flow =
            { Name = "f"; From = FlowSource.Env "src"; To = "sink"; Rekey = None; Tables = []; Reconcile = []; ReconcileIgnore = []; SupportingScope = []
        ...
        for shape in shapeOpts do
          for tables in tableSets do
            for rekey in [ None; Some "file:users.csv" ] do
              let flow = { Name = "f"; From = from; To = "sink"; Rekey = rekey; Tables = tables; Reconcile = []; ReconcileIgnore = []; SupportingScope = []; Scope = scope; Shape = shape; Shaping = None; Strategy = None; Resumable = false; Streaming = false; Journal = None }
              let cfg = { ProjectionConfig.empty with Environments = baseEnvs; Flows = Map.ofList [ "f", flow ] }
              match ProjectionConfig.parse (ProjectionConfig.render cfg) with
              | Ok back -> Assert.Equal<Flow>(flow, Map.find "f" back.Flows)
              | Error es -> Assert.Fail(sprintf "round-trip failed for %A: %A" flow es)
...
let ``A44 clause 3 — the reverse leg (B→A) routes to RunReverseLeg; a peer (A→A) does not`` () =
    ...
    let mk srcR sinkR =
        ...
        let flow = { Name = "leg"; From = FlowSource.Env "src"; To = "sink"; Rekey = None; Tables = []; Reconcile = []; ReconcileIgnore = []; SupportingScope = []; Scope = Some Scope.Data; Shape = None; Shaping = None; Strategy = None; Resumable = false; Streaming = false; Journal = None }
        cfg, flow
