module Projection.Tests.LiveProfilerIntegrationTests
...
let private itemsKindKey = mkKey ["Items"]
let private idAttrKey    = mkKey ["Items"; "Id"]
let private nameAttrKey  = mkKey ["Items"; "Name"]
let private codeAttrKey  = mkKey ["Items"; "Code"]

let private itemsKind : Kind =
    let idAttr =
...
let private itemsCatalog : Catalog =
    {
        Modules =
            [ { SsKey = mkKey ["Module"]
...
let private dropDatabaseBestEffort (masterConn: string) (dbName: string) : Task<unit> =
    task {
        try
            ...
            do! cnn.OpenAsync()
            let q = Projection.Targets.SSDT.Render.quote dbName
            let sql =
...
let private runCaptureScenario () : Task<AttributeReality list> =
    task {
        return!
                    ...
                    try
                        let! result =
                            task {
                                ...
                                let! captureResult =
                                    LiveProfiler.capture cnn itemsCatalog
...
let private findReality (realities: AttributeReality list) (key: SsKey) : AttributeReality =
...
[<Fact>]
let ``A.4.7'-prelude.live-profiler: NAME column reflects HasNulls = true (one row with NULL)`` () =
...
[<Fact>]
let ``A.4.7'-prelude.live-profiler: NAME column reflects HasDuplicates = true (two rows share 'alpha')`` () =
...
[<Fact>]
let ``A.4.7'-prelude.live-profiler: CODE column reflects HasNulls = false and HasDuplicates = false (all distinct, no nulls)`` () =
...
[<Fact>]
let ``A.4.7'-prelude.live-profiler: IsNullableInDatabase reflects sys.columns.is_nullable per column`` () =
...
[<Fact>]
let ``A.4.7'-prelude.live-profiler: captured reality list contains an entry per attribute (PK + non-PK)`` () =
...
[<Fact>]
let ``A.4.7'-prelude.live-profiler: attach composes captured realities into Profile.AttributeRealities`` () =
                        ...
                        try
                            let! result =
                                task {
                                    ...
                                    do! Deploy.executeBatch cnn seedSql
                                    let! attachResult =
                                        LiveProfiler.attach cnn itemsCatalog Profile.empty
    ...
    let p = profile.GetAwaiter().GetResult()
