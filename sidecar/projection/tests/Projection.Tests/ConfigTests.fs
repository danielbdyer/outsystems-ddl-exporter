...
let ``Config.parse: defaults applied when sections are absent`` () =
    ...
    Assert.Equal("PerEntity", cfg.DynamicData.InsertMode)
    Assert.Equal("Include",   cfg.DynamicData.StaticSeedParentMode)
    Assert.False(cfg.DynamicData.DeferJunctionTables)
