module Projection.Tests.CdcSilencePropertyTests
...
// CDC change events.
//
// **Docker-gated.** Each scenario spins up an ephemeral SQL Server
// container; per-scenario cost ~5-8s. Limited to 4 variants for the
// CI budget; FsCheck-driven scaling sweep is the realistic-canary
// territory.
...
// ---------------------------------------------------------------------------
// Shared infrastructure — mirrors `CdcSilenceTests.fs` but typed for
// the shape-sweep variants. Bootstrap + cleanup helpers are inline
// (the existing CdcSilenceTests' helpers are file-private; shape-
// sweep is its own concern and reusing them via duplication keeps
// the cross-cutting Docker setup transparent).
// ---------------------------------------------------------------------------
                ...
                task {
                    ...
                    do! task {
                        ...
                        do! Deploy.executeBatch cnn
                                (System.String.Concat("CREATE DATABASE ", Render.quote dbName, ";"))
                    }
...
let private buildSingleRowFixture () : Catalog * Kind =
...
let private buildMultiTypeFixture () : Catalog * Kind =
...
let private buildManyRowsFixture () : Catalog * Kind =
...
// ---------------------------------------------------------------------------
// The property: for every shape variant, CDC-silence holds —
// idempotent redeploy adds zero CDC capture rows. Each scenario
// also asserts the baseline > 0 (the initial INSERT phase fires
// captures) so the property has bite.
// ---------------------------------------------------------------------------

[<Fact>]
let ``A.4.7'-prelude CDC-silence sweep (single-row variant): idempotent redeploy adds zero CDC capture rows`` () =
...
[<Fact>]
let ``A.4.7'-prelude CDC-silence sweep (multi-type variant Int/Text/Boolean/Decimal): idempotent redeploy adds zero CDC capture rows`` () =
...
[<Fact>]
let ``A.4.7'-prelude CDC-silence sweep (10-row variant): idempotent redeploy adds zero CDC capture rows`` () =
