...
| **7** B.3.7.sampling-multi-env | ✓ `SqlProfilerOptions` record (MaxRowsPerKind + EnvironmentTag); `LiveProfiler.captureEvidenceCacheWith` overload wires sampling into `SELECT TOP (@N) ... ORDER BY <pk>` for deterministic sampling. `Profile.merge` ships with commutative + associative + identity property tests (FsCheck.Xunit; 7 tests). `UserPopulation.union` added in Core. Rows 90 + 92 close to 🟢 PARITY. | ✓ shipped |
| **8** B.3.8.fk-correlation | ✓ Three new IR types: `ForeignKeyCardinality` (per-Reference; `NumericDistribution` over child-counts-per-parent), `ForeignKeySelectivity` (per-Reference value-frequency), `JointDistribution` (per-Kind multi-FK tuple-frequency). Three new `Cache.derive*` primitives + Profile axes + Profile.merge extensions. **Faker emitter's deferred trigger structurally met** — all four `ADMIRE.md` gating-evidence chain nodes shipped. | ✓ shipped |
