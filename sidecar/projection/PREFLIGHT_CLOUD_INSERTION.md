...
| **M1** ✅ | **`rendition` env flag + routing.** `rendition: physical \| logical` per environment (operator decision 2026-06-09) — landed as the `Rendition` DU + `Environment.Rendition` field parsed by `ProjectionConfig.parse` (`MovementSurface.fs`); default absent⇒`None` (the same-rendition default); env-metadata, not a `FlowSource` variant; not a gate. Distinguishes `peer` (physical source) from `legacy` (logical source). | M0 | `THE_DATA_PRODUCERS` §4 decision 6; `MovementSurface.fs` | J1 |
...
| **M3** ✅ | **`legacy` / `preview` — the B→A reverse leg.** LE-1 the reverse-leg transfer (logical on-prem source → physical cloud sink; same model, no foreign ingest) — done via `runWithRenames`; **LE-2 the B→A round-trip canary** — GREEN (`TransferCanaryTests` `M3/LE-2`: logical `[dbo].[Customer].[Email]` → physical `[dbo].[OSUSR_XF_CUSTOMER].[CONTACT]`, data round-trips by SsKey). *(Authored SsKey-stable contracts, per the rename-canary precedent — ReadSide SsKeys are name-derived.)* **M3.b flow-recognition face LANDED (2026-06-09):** `Command.reverseLegOf` (pure, tested) recognizes a flow as the reverse leg from the M1 rendition flag (live logical source → live physical sink) + resolves its two conns; `TransferRun.runReverseLeg` delegates to `runWithRenames` given the two contracts. **Remaining:** the per-flow runner arm that produces the two contracts from one authored model rendered in both renditions (the canary supplies them by construction today). | M1.5 | `THE_DATA_PRODUCERS` §6 LE-1/2; LE-2 canary | J3 |
...
| **M3.b reverse-leg recognition** — a logical→physical flow is recognized as the reverse leg + routed to `runWithRenames` given two contracts | M3 | **GREEN** (pure) | `MovementSurfaceTests` `reverseLegOf` ×4; `Command.reverseLegOf` / `TransferRun.runReverseLeg` |
...
| **G0c advisory survey** — a blocked-capability finding warns on stderr in the run-flow path but does NOT change the run's exit (R6) | M4 | **GREEN** (pure) | `CapabilitySurveyTests` `advisoryLines`/`blocked`; `runTransfer` emits, run exit stands |
| survey P10 resolves user-directory readability for the re-key | M4 | **GREEN (G0b)** — `ReadSide.userDirectoryReadability` + `EnvironmentReport.UserDirectory` (a field, not a `Capability` variant; totality preserved); Docker witness `UserDirectoryProbeTests`. Residual: real-instance user-table identity (operator-configurable candidate set; pairs with OPEN-2) | `CapabilitySurvey.fs` `probeEnvironment`; `ReadSide.fs` |
