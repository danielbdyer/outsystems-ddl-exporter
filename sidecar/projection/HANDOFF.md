# Handoff addendum — 2026-06-19 (later still), DELTA-GRADE DIFF POLISHED (six slices) — the migrate-preview surface gets sharper, safer, and scopable: the FK retarget names tables (a GUID-wall DEFECT fixed), the structural channels gain before/after (`uniqueness not unique → unique`), a "review these first" data-risk callout + an honest statement (a zero-drop NOT-NULL add no longer leads calm), lane items sort by name, `diff --only <channel>` / `--module <name>` scope it, and a per-module "top movers" rollup orients a multi-module diff. Branch `claude/delta-grade-polish` off `main` `6027c0d4`, NOT pushed

To the next agent.

**The operator asked to DEEPEN the just-merged delta-grade diff (#27) — "more verbs, more detail, more at-scale polish" — using subagents to propose where next, me to select.** Three subagents (verbs / evidence / at-scale) each read the real code and ranked proposals; I synthesized + picked, then shipped SIX gated slices (the operator then said "let's do those things" to the named backlog, so #E/#F followed). Full detail: `SPECTRE_REFINEMENTS.md` §27 (the Polish subsection); `DECISIONS 2026-06-19` (three entries).

**What shipped (all pure-Cli, zero Core/SQL, pure pool 3591/0, +18 tests):**
...
**Your next moves (the named follow-ons — none started):**
- **The FULL at-scale NAVIGABILITY (the remaining marquee).** #E (`--module`) + #F (rollup) now cover *orient-then-drill*, which is most of the at-scale need. The remaining piece is the IN-PLACE navigable tree: today a `Lane`'s items are NOT navigable children (`Navigator.children` nests only `Doc`/`Disclosure`), so a 2000-item lane is a dead-end at item 12 on a TTY. Two halves: (a) make `Lane` items navigable (a `children` arm + a PAIRED path-aware `Lane` render that windows to the cursor — must stay byte-identical when `OpenPath = None`, the load-bearing invariant); (b) group a large lane by module→kind as a `Disclosure` subtree (the kind→module map is now proven cheap — `Catalog.allModulesKinds`, used by #F). L-effort, touches the Navigator invariant — adversarially verify.
- **The cross-surface `SsKey.displayName cat` projection.** The diff names by `Name` now; `explain` / the reconciliation reports / the bench dump still name by `rootOriginal` (GUIDs on a real estate) — ~15 sites in `RunFaces` (576–1625) + many in Pipeline. A shared display projection unifies them, BUT many sites lack a `Catalog` in scope (they hold remap/displacement records keyed by SsKey) — the real work is THREADING a name resolver to each, not the projection itself. Broad; scope it as its own chapter.
- **resume-from-refusal** — the prior letter (below) is its full code-verified architecture, operator-approved. A different chapter (run/apply, Docker, DECISIONS-first).
- Lower: a `--stat`-only summary MODE (the #F rollup is its always-on form); concrete storage width (`int → bigint`, a Core diff-modulus, DECISIONS-gated); #2 Markup newtype; #26 The Threshold.

**The reuse pattern (it worked all six slices):** a pure `View`-projection addition to `Comparison.fs` + law-citing `ComparisonTests` pinning BOTH the human-visible evidence AND the one-substrate carry on `toJson` (the item is a `Lane` string → assert the json contains the same text). Fixtures are cheap: `withKinds (List.map …)` / `twoModule …` over the shared `customer`/`order`/`country`; direction-swap (`Between a b` vs `b a`) gives add-from-drop and the dangerous tightening direction free. For a GUID-legibility test you MUST use `OssysOriginal` keys (`SsKey.ossysOriginal`), not the synthesized fixtures. Render-to-`StringWriter` (a NoColors `AnsiConsole`, `Profile.Width <- 100`, `View.writeToDepth`) is the throwaway dump for eyeballing a surface — delete it before committing.
...
**State.** Branch `claude/delta-grade-polish` off `main` `6027c0d4` (post-PR-#632). Eight commits: `b1144d18` (#A) · `ac8c09e3` (#B) · `39592360` (#C) · `b98fbc17` (#D) · `25c6ef1d` (docs A–D) · `74f7ef64` (#E scoping) · `7670b7a4` (#F rollup + ledger) · the DECISIONS/HANDOFF docs — **committed, NOT pushed** (the operator drives push / PR). Pure pool 3591/0; Docker untouched by design (every change is Cli diff-rendering + a thin `ExplainDiff` DU thread). Hold the spine, hold the live-instrument charter: ease, enjoyment, seamless flow.
