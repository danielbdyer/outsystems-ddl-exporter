...
**The remaining three items landed, same discipline as the eight** (identity gate before every commit: pure pool + docker pool + Release-FS3511; no golden moved anywhere in the whole program). Substance per item in the DECISIONS entries ("PL-7 executed", "PL-6 executed", "PL-9 executed") and the plan's STATUS blocks. Highlights and the load-bearing pins:

- **PL-7**: `ReadSide.readSchema` = `readCore liftRows:false` — provably `read >> Catalog.stripStaticPopulations` (buildKind mints `Modality = []`; the lift loop is the only Static source on the readback path), pinned content-bearing in `PayOnceSchemaReadDockerTests` alongside the capture-verb both-ways equality and the preflight-verdict identity. The fourth letter's mark-consumption worry was verified empty (grep-proof: zero Modality reads on the transfer path). `Source.ofLive.ReadCatalog` deliberately NOT switched — it returns the marked catalog downstream. S10's scoped probe (`LiveProfiler.nullCountsFor` + pure `Preflight.violationsOfNullCounts`) replaced the full-EvidenceCache preflight; both verb callers already skip on an empty overlay.
- **PL-6**: the byte plane. `JsonWriting.writtenBytes` (in-place buffer decode), `CatalogCodec.serializeUtf8`→`WriteRawValue(utf8)` (direct `wCatalog` nesting REJECTED — depth re-indent moves stored bytes), the dead sort+Map-rebuild in `wStaticRow`, serialized-once CDC keys, `Render.toSql pendingDdl` direct write, `Deploy.executeSegments` (pre-split segments; `alterSql` KEEPS `executeBatch` — `Render.toText` can carry GO), and `ConstraintFormatter.formatInto` per-statement folding with THE trailing-newline pin (the whole-text pass's final Split element emitted one extra `\n`; the golden-scenario equivalence fact in `GoldenEmissionTests` pins per-statement ≡ whole-text forever).
- **PL-9**: `SurrogateCapture.CaptureKindSql`/`stageKind`, the streaming `KindWriteLane`, `phase2UpdateSqlStaged`, `basisByKind`, and S11's projected phase-2 re-stream (a FILTERED ingest Kind through the one stream core — not a new reader; the plan's `readRowsProjectedStream` carrier was a named departure). The S11 edge was adjudicated, not assumed: a non-deferred FK's unresolvable value no longer drops a phase-2 row, but that row was already dropped and reported by phase 1, so sink state and report are identical.

**Two FS3511 scars this session, both recorded in the PL-9 DECISIONS entry:** (1) the optimizer INLINES a small non-`rec` task wrapper into its caller's resumable code and the reported error span points at an unrelated caller line — mark such wrappers `rec`; (2) a task-level `let` of a staged closure inside a loop defeats reduction — apply staged renderers inline as the chooser argument.

**The one named residue: PL-6c (S24).** `executeBatchParallel` still parser-splits the per-kind rendered data scripts. The boundary paragraph in the PL-6 DECISIONS entry has the full adjudication: the parser is correctness-load-bearing for text of unknown structure (a data literal can carry an embedded GO line — line-folds mis-split it); the right fix mints `Phase1Segments`/`Phase2Segments` at the emitters (every GO frame flows through `renderDataBatch`, so the segment lists are the piece lists the emitters already concatenate) with the joined text deriving byte-identically. Gate it with the PL-2 bundle-identity witness + the golden corpus + `executeLeveledSeed` docker witnesses. Tier-3 survivors (G1-intra/G2–G6) remain plannable follow-ons per the fourth letter.
...
**The operator named the SCALE TARGET — a ~310-table OSSYS estate, where one channel can carry hundreds of FK concerns at once — and asked that every change-render surface be INTENTIONAL about that volume (never a silent 12-item wall).** That reframe drove slices #G/#H/#I on top of the #A–#F polish. The governing principle now: **each list surface states its true total, leads with what matters, and offers a path to ALL of it.** Full detail: `SPECTRE_REFINEMENTS.md` §27 (Polish + the Scale wave); `DECISIONS 2026-06-19/20` (four entries).

**The SCALE wave (`6f66b376` #G · `dc42db67` #H · `9ca1c35b` #I · `a8b7e4a9` #J · `a9e487f7` #K; pure pool 3606/0 + Docker TransferCanary 29/29):**
- **#G — the danger callout scales by risk CATEGORY.** Past 12 concerns the flat callout would bury the risk SHAPE behind "and N more". Now it groups by category (dropped / type change / null → not null / primary key change / identity change / cascade delete / uniqueness gained) — each a diggable sub-group with its count, the loud total on top — so 347 concerns read as their PROFILE. `dataDrops`/`rewrites` carry `(category, text)`; small sets stay flat.
- **#H — the navigable module-grouped move-lane (THE AT-SCALE MARQUEE).** A LARGE lane spanning ≥ 2 modules renders as a navigable `Disclosure` TREE grouped by module (hottest first; `moduleOfItem` resolves via `Catalog.allModulesKinds`). **The key insight: build the tree from `Disclosure`s — `Navigator.children` already nests those — so the in-place dig works with ZERO Navigator change and the `OpenPath`=child-index invariant is never touched.** The prior letter flagged this as the hard marquee needing path-aware windowing; it doesn't. Small / single-module lanes stay flat `Lane`s.
- **#I — cross-surface legibility STARTED.** `verify-data` names tables/columns by `Name` (the contract threaded out of the read task as `(report, contract)`; payload build extracted as the pure, testable `integrityPayload`). `explain` was already fine (its `rootOriginal` is the substring MATCH key, not display).
- **#J — the displayName chapter FINISHED.** ONE shared **`Catalog.nameIndex`** (Core) now backs every face — `Comparison.nameIndex` + the `RunFaces` narration delegate to it (three copies consolidated). The **transfer report** names by `Name`: the transfer face holds NO catalog (the report is built deep in the engine), so `TransferReport` gained a `Names : Map<SsKey,string>` index, populated from the contract catalog at each of the 5 construction sites (`Catalog.nameIndex catalog` materialized / `sourceContract` streaming); `narrateTransferReport`/`narrateDropExit` resolve through it (empty ⇒ `rootOriginal` fallback). **Additive metadata only — no engine logic reads `Names`, so transfer behaviour is unchanged** (TransferCanary 29/29). `suggest-config` names via `report.ReadCatalog`. The displayName test pattern: the narration is stdout/Voice-based, so EXTRACT a pure builder (`integrityPayload`) OR capture `Console.Out` in the `Global-MutableState` collection — and prove with `OssysOriginal` (GUID) keys (synthesized fixtures can't catch a name-wall).
- **#K — the L1 `/`-filter (`a9e487f7`).** `/` filters the live dig to branches matching a substring. The law (DYNAMIC_DISPLAY §7.6): `Navigator.Model` holds only the filter STRING + an `Editing` flag; the filtered tree is DERIVED (`effectiveTree` over the pure, total `filterView`), and the cursor navigates it with the SAME `step` — so the in-bounds CLAMPING invariant extends to the pruned tree FOR FREE, with **no `OpenPath`-invariant change and no second copy of state**. The shell routes by mode (`/` open · printable append live · Backspace delete/exit · Enter commit · Esc a LAYERED exit — clears the filter first, quits only when there's none). `step` stays total over `ConsoleKey` (text-entry is char-based, in the shell). Pairs with #E/#F/#H — the interactive at-scale toolkit (orient · scope · drill · filter) is COMPLETE.

**What shipped earlier this session (#A–#F; pure pool, +18 tests — full detail below the fold):** #A FK name-wall fix · #B structural-channel before/after · #C data-risk surface + honest statement · #D deterministic ordering · #E `--only`/`--module` scoping · #F per-module rollup.

**Your next moves (no high-value UX item left unbuilt — these are smaller / gated):**
- **A HEADLESS filter** — `diff --grep <substring>`, the #K `/`-filter's one-shot sibling for pipes / `--json` (`filterView` is already pure + reusable; wire it like `--only`/`--module` through `ExplainDiff`).
- **resume-from-refusal** — DEFERRED by the operator this session; the prior letter (below) is its full code-verified architecture when it's picked up.
- Lower: concrete storage width (`int → bigint`, Core diff-modulus, DECISIONS-gated); a `--stat`-only summary mode (the #F rollup is its always-on form); #2 Markup newtype; #26 The Threshold.

**The #A–#F detail (the earlier polish wave — the operator asked to DEEPEN #27 "more verbs, more detail, more at-scale polish" via subagent proposals + my selection):**
- **#A `b1144d18` — the FK name-wall fix (a real defect in the merged #27).** `referenceEvidence` rendered the `target` / `source column` facets via `SsKey.rootOriginal` — a bare GUID for `OssysOriginal` keys, so an FK retarget read `target <hex> → <hex>`, illegible exactly where the operator must read "this FK now points at a DIFFERENT table." Threaded the per-side name resolvers (`nm srcNames` / `nm tgtNames`) in, as the qualifier already was. *Scar:* the synthesized-key fixtures CANNOT catch this (their `rootOriginal` IS a name) — I proved it with OssysOriginal (GUID) fixtures (`ossysKind` + `SsKey.ossysOriginal`).
- **#B `ac8c09e3` — before/after for the structural channels.** Index `uniqueness` (`not unique → unique` FAILS on apply if dupes exist), sequence scalars (`start 1 → 1000`), kind `active` (`yes → no`) now carry the value. List/grouped facets stay facet-name (a before→after there is a wall). Retired the dead `channelReshapes`.
- **#C `39592360` — the data-risk surface + an honest statement.** `dataDrops` + `rewrites` (per-channel predicates over the retained source/target: a type conversion, `null → not null`, a cascade added, a uniqueness gained, a dropped column/table) drive (1) a "review these first" `Bad`-badged callout lane promoted to the TOP of the substantiation — a SEPARATE surface, so the move-lanes stay HOMOGENEOUS (the operator's "don't widen `Lane` to per-item status" holds), and (2) an honest `catalogStatement` that leads amber on a data-touching reshape, not just a removal (`N changes · D drops · R may rewrite data · review`). I eyeballed the rendered output — it reads clean: honest lead, callout first, then the full lanes + δ panel.
- **#D `b98fbc17` — deterministic item ordering.** Lane items `List.sort` (noun-prefixed ⇒ channel-grouped then name-ordered) so a capped lane's first 12 are scannable, both lenses one order (T1).
- **#E `74f7ef64` — the scoping verbs.** `diff --only <channel>` scopes the DISPLAY (keeps one channel's lane items + danger callout via `keepChannel`/`channelNoun`; the statement + rollup + ‖δ‖ panel stay whole). `diff --module <name>` scopes the COMPUTATION (keeps the named module's kinds before `Between` — a raw `{ cat with Modules = … }` scope, safe because the diff only OBSERVES catalogs). `Comparison` gained `*Scoped` variants (the historical functions are `None` wrappers — every test held); `ExplainDiff` gained `channel`/`onlyModule`, threaded parse→dispatch→`runDiff`; `--help` updated. *Note:* the DU-arity change broke one `MovementSurfaceTests` match — update it in the SAME commit.
- **#F `7670b7a4` — the per-module "top movers" rollup.** `moduleRollup` adds a churn-sorted `module · changes` `View.Table` (kind→module via `Catalog.allModulesKinds`) before the ‖δ‖ panel, shown only for ≥ 2-module diffs. Pairs with `--module`: see the hot module → dig it. Reuses `Table` (no new case).

**The reuse pattern (it worked all nine slices):** a pure `View`-projection addition to `Comparison.fs` + law-citing `ComparisonTests` pinning BOTH the human-visible evidence AND the one-substrate carry on `toJson` (the item is a `Lane` string → assert the json contains the same text). Fixtures are cheap: `withKinds (List.map …)` / `twoModule …` over the shared `customer`/`order`/`country`; direction-swap (`Between a b` vs `b a`) gives add-from-drop and the dangerous tightening direction free. For a GUID-legibility test you MUST use `OssysOriginal` keys (`SsKey.ossysOriginal`), not the synthesized fixtures. Render-to-`StringWriter` (a NoColors `AnsiConsole`, `Profile.Width <- 100`, `View.writeToDepth`) is the throwaway dump for eyeballing a surface — delete it before committing.

**Scars (this session, newest first):**
- **⚠️ FETCH `origin/main` BEFORE BUILDING — local `main` can be stale.** This session was started from a STALE local `main` (`1218f505`, PR #630); `origin/main` was already 3 commits ahead at `6027c0d4` (PR #632 = the merged delta-grade). I built a full duplicate of #27 on a branch off the stale main before catching it via memory recall. ALWAYS `git fetch origin && git log --oneline origin/main` at session start, and check the `spectre-chapter-progress` memory. (My duplicate branch `claude/delta-grade-diff` is abandoned — do not push it; this polish branch is off the REAL main.)
- **`scripts/test.sh fast | tail` (or any pipe) LIES about the exit** — the pipe's code is `tail`'s, not the pool's (the §4 `lint | tail` scar generalizes). Run BARE with `> log 2>&1; echo "EXIT=$?"`, or grep the TRX for a brand-new test name + the pass/fail counts. `dotnet` IS on the bash PATH (9.0.314); `TEST_FORCE_BUILD=1` after edits; `scripts/test.sh focus '<name>'` for a slice.
- **The F# compiler OOMs / CLR-fatals when the host is memory-starved** (`0x80131506`, `System.OutOfMemoryException` in FSC, exit `-1073741571`). Mid-session the host dropped to ~2 GB free and the Cli build crashed repeatedly. `dotnet build-server shutdown` frees the persistent compiler servers; `dotnet build … -m:1 -p:BuildInParallel=false` builds sequentially (one FSC at a time). It is the HOST, not the code — confirm with `Get-CimInstance Win32_OperatingSystem` free memory before suspecting a regression.

**State.** Branch `claude/delta-grade-polish` off `main` `6027c0d4` (post-PR-#632). Eleven slice-commits + docs: `b1144d18` (#A) · `ac8c09e3` (#B) · `39592360` (#C) · `b98fbc17` (#D) · `74f7ef64` (#E) · `7670b7a4` (#F) · `6f66b376` (#G) · `dc42db67` (#H) · `9ca1c35b` (#I) · `a8b7e4a9` (#J — crosses into Core+Pipeline) · `a9e487f7` (#K L1 filter) + the docs/HANDOFF commits — **committed, NOT pushed** (the operator drives push / PR). Pure pool **3606/0**; Docker `TransferCanary` **29/29** (the #J `TransferReport.Names` field is additive metadata — transfer behaviour unchanged). #A–#H + verify-data + #K are Cli; #J adds `Catalog.nameIndex` (Core) + `TransferReport.Names` (Pipeline). Hold the spine, hold the live-instrument charter: ease, enjoyment, seamless flow — intentional at scale, legible everywhere, and queryable in the hand.

---

# Handoff addendum — 2026-06-19 (later), DELTA-GRADE DIFF SHIPPED + COMMITTED, and RESUME-FROM-REFUSAL (true resume) DESIGNED + OPERATOR-APPROVED — the walkable changeset now carries every diff channel + before/after evidence, named by Name (`#27`, committed `403a6de1`); and the next chapter — resume a gate-halted run from the tripped gate, skipping the preamble — has a code-verified architecture ready to build (the operator chose "the plan is the deliverable")

To the next agent.

**You are still in the live-instrument chapter** (`SPECTRE_REFINEMENTS.md` §1 map; charter = EASE / ENJOYMENT / seamless flow). This session shipped the DIFF cluster, then DESIGNED the first operator-flow delight and stopped at the approved design — so the resume-from-refusal architecture below is YOUR build, pre-verified against the code.

**What shipped (committed `403a6de1`; Cli-only, zero Core/SQL touch, pure pool 3575/0): delta-grade diff (`#27`).** `CatalogDiff` already computed a rich delta (per-kind attribute/reference/index channels + the catalog sequence channel + kind-OWN facets, each add/remove/rename/reshape, RETAINING source+target) but `renderCatalogLanes` surfaced only kind moves + attribute reshapes-as-facet-names. Now: (1) EVERY channel folds into the same four HOMOGENEOUS move-lanes (rename/reshape/add/remove), channel-qualified (`column Customer.Email`) — did NOT widen `View.Lane` to per-item status (the operator was right: redundant); (2) before/after EVIDENCE on the ALTER surfaces (`column X.qty: type text → integer`, `relationship X.fk: on delete no action → cascade`) via `attributeEvidence`/`referenceEvidence` reading the retained source/target; (3) named by `Name`, not `rootOriginal` (a bare GUID for `OssysOriginal` keys → a GUID-wall) via a per-side `nameIndex`. NO new `View` case (reuses `Lane` ⇒ one-substrate holds by construction). 14 `ComparisonTests`; verified by a 6-lens adversarial-refute Workflow → SHIP (no defects). Full "As shipped" detail: `SPECTRE_REFINEMENTS.md` §27.

**YOUR BUILD — resume-from-refusal (true resume); architecture approved + code-verified.**

THE FEATURE: a run pays the expensive preamble (`ReadSide.read` = extract state A from the live DB), HALTS at a gate; the operator fixes the cause and wants to RESUME from the gate, SKIPPING the preamble. (Distinct from the existing `--journal` apply-resume, which resumes a PARTIAL APPLY; gate-resume is nothing-applied-yet — the value is skipping the extract.)

THE KEY DECISION (de-risks everything): **checkpoint the INPUTS, not the plan.** The plan (`Statement list`/`CatalogDiff`, `MigrationArtifacts`) has NO codec and would be large/fragile to build; but `MigrationRun.preview A B` is PURE + deterministic and catalogs A/B have a round-trip-tested `CatalogCodec` (`Projection.Targets.Json`). So checkpoint A + B + declaration + digest + the halted gate; on resume deserialize A/B and RE-DERIVE the plan purely.

ARCHITECTURE (pure core + thin shell — the Navigator's pure-`step`/thin-`driveLoop` discipline):
- PURE (new; `Projection.Pipeline` + `Projection.Targets.Json`; property-pool): a `ResumeCheckpoint` record (Version, RunId, Command, HaltedGate: `Preflight.GateLabel`, InputDigest, SourceFingerprint, SourceCatalog, TargetCatalog, Declaration, Flags) · `ResumeCheckpointCodec` (sibling of `CatalogCodec`, delegates catalog fields to it; round-trip law; re-validating; total over `GateLabel`; carry `Version` + refuse a mismatch as `NotResumable` — never mis-decode) · `Resume.decide : checkpoint → freshFingerprint → Resume | Stale of drift | NotResumable of reason` (the closed-DU analog of `Navigator.step`; property-test it exhaustively).
- IMPURE SHELL (one face): `resume <runId>` — load checkpoint → open sink → CHEAP source fingerprint → `Resume.decide` → on `Resume`: re-`MigrationRun.preview A B` → re-assert consent → existing migrate preflights → `runMigrateExecuteLeg`.

FIRST SLICE — migrate's preview→commit story ("the preview was expensive; I reviewed it; now commit"): checkpoint-on-preview in the live migrate preview path (after `ReadSide.read` + `MigrationRun.preview`), and a `resume <runId>` that re-applies with `--go`. Schema-only ⇒ a NO-APPLY halt ⇒ ZERO partial-apply hazard by construction. DEFER: transfer resume (rows are UNBOUNDED to checkpoint — migrate is why this slice is tractable), the tightening gate G7 (needs a profile checkpoint too), the `validateUserMap` orphan-halt, reverse-leg/with-data, and `--restat`.

THE THREE LOAD-BEARING SAFETY RULES (write the DECISIONS entry FIRST — this is run/apply load-bearing surface):
1. **Consent is RE-ASSERTED, never checkpointed.** `resume` re-presents the `--go`/`PROJECTION_ALLOW_EXECUTE` gate and re-checks the CURRENT approval store (`ApprovalRegistry`), not a checkpointed copy — a stale checkpoint must not smuggle a since-revoked sign-off.
2. **Staleness via a CHEAP fingerprint, refused by name.** The full `Run.inputDigest` is over the whole catalog ⇒ recomputing it = re-extracting (the thing we skip). Mirror the existing `CaptureJournal.fingerprintOf` precedent (`(FirstPk, LastPk, RawCount)`, deterministic via ReadSide's PK ordering) and its `transfer.resume.sourceDrift` named refusal: store a cheap re-derivable shape (per-table rowcounts + max-PK + a schema-shape hash), recompute ONLY that on resume, refuse via a NEW `Preflight.GateLabel` arm `ResumeSourceDrift` (wire into `Preflight.classify` + `Voice.gateStatement`). Name the residual honestly (a value-edit preserving aggregates can slip a cheap fingerprint); `--restat` (full re-extract + re-hash) is a deferred opt-in.
3. **Gate-resume (nothing applied) stays DISTINCT from journal-resume (partially applied).** Different keys (run-dir checkpoint vs `transfer-<digest>.ndjson` journal), different verbs (`resume <runId>` vs re-run with `--journal`). `runResume` should refuse by name if it detects the sink already partially mutated for the checkpoint's plan.

CRITICAL FILES (Plan-agent-mapped — VERIFY each against the code first, the stale-doc discipline): `RunFaces.fs` (`runMigrateExecute`/`runProjectLivePreview`/`runMigrateExecuteLeg`/`migratePreflights` — checkpoint-write + the new `runResume`) · `MigrationRun.fs` (`preview` = the pure re-derivable planner; `MigrationArtifacts`) · `CatalogCodec.fs` (reuse + template) · `CaptureJournal.fs` (the staleness/drift precedent) · `Preflight.fs` (`GateLabel`/`classify` + the new arm) · dispatch: `MovementSpec.fs` (`PlanAction.Resume of runId`), `MovementSurface.fs` (parse), `Program.fs` (dispatch under `withRun "projection resume"`), `Run.fs`/`RunEnvelope.fs` (store-dir + `inputDigest`), `Voice.fs` (`gateStatement`).

TESTS: pure-pool bulk (`ResumeCheckpointCodecTests` round-trip + re-validation; `ResumeDecisionTests` property over `decide`; Voice `code⇔copy` for `ResumeSourceDrift`). Docker 1–2 (end-to-end preview→resume-apply asserting `ReadSide.read` was NOT called on resume; a staleness test mutating the source → the named refusal, no apply). This slice BREAKS the chapter's Cli-only/pure-pool grain — a resume that applies MUST be proven against live SQL.

**The other open frontier:** **#6 echo-the-fix** — the gate-refusal copy is ALREADY strong (`Voice.gateStatement`, a closed `Preflight.GateLabel` DU naming the lever + next move), so echo-the-fix's real value is surfacing a DECLINED TRANSFORM's `suggestedConfig` LIVE during a run (via `LogSink.addSubscriber`, the shipped live-push primitive) instead of only post-run via `suggest-config <runId>`; push-model so it never blocks the emitter (the #20 `BlockingCollection` lesson). **#22 undo handle** — HIGH value but DESTRUCTIVE (the M21–D revert machinery deletes by captured key); its OWN gated chapter with Docker canaries, not bundled into the Cli-rendering grain. **L1 focus/filter** — a `/`-axis on the Navigator `Model` (DYNAMIC_DISPLAY §7.6). **#2** Markup newtype (type-hardening, no live bug).

**A finding worth carrying (delta-grade surfaced it):** every OTHER operator surface (`explain`, the reconciliation reports, the bench dump — `RunFaces`) still names entities by `rootOriginal` — a bare GUID for `OssysOriginal` keys. The diff now names by `Name`; a shared `SsKey.displayName cat`-style projection would unify every surface toward legibility (touches every operator face — a clean, scoped follow-on).

**Scars (newest first):**
- **THE STALE-STATUS TRAP (near-miss this session).** `scripts/test.sh fast`/`focus` spends ~1–2 min in `build_once` BEFORE it writes `RUNNING` to `<pool>.status` — so a wait-loop that greps `<pool>.status` for PASSED/FAILED matches the PRIOR run's stale verdict and you read a STALE `<pool>.trx`. Wait for the background-task completion NOTIFICATION, or confirm via the NEW test COUNT in the TRX (I expected 14 delta-grade tests; a stale TRX showed 8 — that's how I caught it), never the bare status line.
- **THE WORKTREE-PATH TRAP (build-side, unchanged).** A Bash `cd` to the SHORT absolute path (without `.claude/worktrees/<name>`) lands in MAIN; `test.sh` there tests MAIN — green while running zero of your tests. `cd` the FULL worktree path; grep the TRX for a brand-new test name before trusting a pass. (Read with the full path is always correct; `dotnet` IS on the bash PATH, 9.0.314.)
- `TEST_FORCE_BUILD=1` after edits (the DLL mtime-skip can run a stale build). `gh pr edit`/`merge` need `read:project` → use `gh api` instead.

**State.** Branch `claude/nice-kalam-7de2e9`. **delta-grade diff committed (`403a6de1`)** — `Comparison.fs` + `ComparisonTests.fs` + `SPECTRE_REFINEMENTS.md` §27; pure pool **3575/0** (3561 → +14); Docker UNTOUCHED (every change is Cli-rendering). Resume-from-refusal is DESIGN-ONLY (this letter IS its deliverable; nothing built yet). Hold the spine — and for the resume build: write the DECISIONS entry first, keep the checkpoint pure + property-tested, and never let a stale checkpoint smuggle consent.

---

# Handoff addendum — 2026-06-19 (latest), THE LIVE INSTRUMENT + THE BREATHING BOARD SHIPPED — the Spectre rendering chapter's NAVIGATE cluster is real (ViewPath #18 → the dig-as-motion Navigator #23/#11, with inspect/diff/explain all navigable → run-history walk #10 → cursor caret → L2 control surfaces), AND #20's board breathing (the dwell moved OFF the emitting thread → a breathing spinner → a stall-aware ETA). The whole Spectre chapter (24 commits) merged to `main` via this PR

To the next agent.

**You are in the Spectre rendering-refinement chapter** — the live ledger is `SPECTRE_REFINEMENTS.md` (§1 map = status; each ● carries its own "As shipped" detail; the §0 cluster names the headlines). It opened as 26 numbered rendering refinements, then the operator REFRAMED it (2026-06-18) away from reassurance/ceremony toward **EASE / ENJOYMENT / seamless flow** — *"projection as a tool you OPERATE in real time,"* *"true USER sparkle comes from delightful experiences."* That reframe is the charter: the prime directive still holds (pretty/plain/json are projections of ONE `View`; `toJson` always carries the full tree; a new `View` case ships its `toJson` arm + a law-citing test), but the GOAL is now the **live instrument**.

**What just landed (this PR = the whole chapter, 24 commits; the headline is the last 9):**
- *Substrate (the earlier 15):* #4 `RenderOptions` · #11 width cap · #12 `Table` · #13 bench-fold · #14 `Spark` · #15 `Trail` cap+depth · #16/#3/#17/#22 · #24/#19 stale-premise corrections · #26 The Threshold.
- **THE LIVE INSTRUMENT (NAVIGATE):** **#18 ViewPath** (`OpenPath: int list option` on `RenderOptions` — open exactly one branch deeply, the rest calm; byte-identical when `None`; `--open <dotted-path>` is the headless consumer, non-negative indices only) → **#23/#11 the Navigator** (a PURE `Navigator.step : ConsoleKey→Model→Model` + `project : Model→RenderOptions` — the cursor path IS the `OpenPath`, no remap; property-tested TOTAL over `ConsoleKey` + CLAMPING so the cursor can't leave the tree; the impure shell is ONE `driveLoop` — clear-and-redraw on `Console.Out`, NOT a Spectre `Live` region; `↑↓` cursor, `→/Enter` dig, `←` back, `PgUp/PgDn` walk runs, `Ctrl-C` quits clean via `TreatControlCAsInput`) → **inspect joined the substrate** (`buildInspectView` replaced its `printfn` dump → gained the json/`--query` lens) → **the cursor caret** (a left-gutter `❯` on the `OpenPath=Some []` tip — visible even on a leaf) → **L2** (`Navigator.present` makes inspect/diff/explain ALL dig live on a TTY, one-shot when piped/`--json`/`--query`).
- **#20 THE BOARD BREATHES:** the dwell moved OFF the emitting thread (the FIRST concurrency primitive in `src` — an unbounded `BlockingCollection`; subscriber enqueue-and-return inside emit's lock, body on `Task.Run`, drain loop on the ctx-affine `Live.Start` thread; reap-in-`finally` so a render `IOException` can't orphan the body; **DECISIONS 2026-06-19**) → the breathing spinner (the active line pulses `Theme.spinner phase` on the drain loop's 100ms idle wake) → the stall-aware ETA (quiet >`stallThresholdMs` ⇒ the estimate degrades to `stalled` + the spinner freezes/mutes — never a countdown that keeps lying).

**Your next moves (the operator-approved delight cluster — all unbuilt, all ride the shipped substrate):**
- **delta-grade diff** — the operator's *"syntax-aware diff coloring + walkable changeset."* DO NOT reflexively widen `View.Lane` to per-item `Status`: the move-lanes are HOMOGENEOUS (the "add" lane is all adds → per-item status is redundant). The real value is (a) per-item before/after EVIDENCE for ALTER/reshape lanes (`qty: int → bigint`) and (b) surfacing the MISSING diff channels (refs / indexes / sequences / attr add-remove-rename / kind-facets). **Explore `CatalogDiff` FIRST** — what does it already compute vs. what `Comparison.renderCatalogLanes` surfaces? L2 already navigates whatever the diff produces, so richer lanes land live for free.
- **The operator-flow delights** — **#22 undo handle**, **#6 echo-the-fix**, **resume-from-refusal** (skip straight to the tripped gate). These touch the run/apply/gate flow (NOT the read surfaces) — explore `RunLedger` / `OperatorConsole.withRun` / the R6 gate ladder before building.
- **L1 focus/filter** — a `/`-to-filter axis added to the Navigator `Model` (a cursor over the `View`, never a second copy of state — DYNAMIC_DISPLAY §7.6).
- Lower: **#2** Markup newtype (type-hardening, NO live bug); the leaf-caret is shipped but a `Panel`/`Table` cursor wears none (rare cursor target).

**The verification harness that worked — REUSE it.** Each keystone (#18, #23) AND the #20 concurrency rewrite went through a multi-lens **adversarial-refute Workflow**: spawn N skeptics, each trying to REFUTE one correctness claim by reading the actual code (force a structured verdict via a schema), then a synthesis judge → SHIP / SHIP_WITH_FIXES / BLOCK. It caught exactly one real defect each — ViewPath's negative-index clamp, the Navigator's `Ctrl-C`-skips-`finally` gap, #20's render-throw-orphans-the-body — all fixed before commit. For concurrency, give each lens a hazard (lock-held / frame-loss / determinism / thread-affinity / teardown) and demand a concrete INTERLEAVING, not a vibe.

**Scars (the ones that BIT this chapter, newest first):**
- **THE WORKTREE-PATH TRAP — the BUILD-side variant (cost real time).** A Bash `cd /c/.../sidecar/projection` (the SHORT absolute path, WITHOUT the `.claude/worktrees/<name>` segment) lands in the **MAIN checkout**, and `scripts/test.sh` there builds+tests MAIN — it read GREEN (3519 passed) while running ZERO of my new tests. ALWAYS `cd` to the full worktree path `…\.claude\worktrees\<name>\sidecar\projection`, and **grep the resulting TRX for a brand-new test name before trusting a pass.** (PowerShell with a RELATIVE path from its worktree-root cwd hits the worktree correctly — the two tools disagree by default.)
- **The Tests-DLL mtime skip.** `test.sh` skips the build when the DLL looks current (`build_is_current`); a concurrent edit during a focused run can leave you running a stale DLL. Use `TEST_FORCE_BUILD=1` after edits, then grep the TRX.
- **The house-NEW `BlockingCollection`** (#20) is the first concurrency primitive in `src` — a forward note in `renderWatchOn` marks the swallowed-`Add` window to re-check when the concurrent realization stream it enables actually lands.
- **A render `IOException` is a real surface** (broken pipe / closed terminal mid-run) — any future `Live`/`ctx.Refresh` work must reap its background producer in a `finally` or it orphans (the #20 fix).
- `dotnet` IS on the bash PATH here (9.0.314), contrary to older letters; never the pure+Docker pools as one `dotnet test` (OOM — `scripts/test.sh fast`, then `focus`/`docker`).

**State.** Branch `claude/vigilant-merkle-862fa8`, **merged to `main` via this PR** (24 commits = the whole Spectre chapter). Pure pool green throughout (last **3561/0**, +30-odd new law-citing tests). The Docker pool was untouched by design — every change is Cli-rendering (`View`/`Navigator`/`Watch`/`Theme`/`RunFaces`/`Program`), zero Core/SQL touch. The NAVIGATE + #20-breathing clusters are DONE; the DIFF cluster (delta-grade) + the operator-flow delights (#22/#6/resume) are the frontier. Hold the spine — name every refusal, count every crossing — and hold the live-instrument charter: ease, enjoyment, seamless flow.

---

# Handoff addendum — 2026-06-17 (prior — the parallel synthetic-data branch `claude/quirky-leakey-4da6cf`), THE OPERATOR SURFACE IS NOW CONFIG-PRIMARY — the synthetic policy moved into a `synthetic` config block (§11) and the flow gained its execution profile (strategy/resumable/streaming/journal), both with the CLI flags kept as per-run overrides. This came out of a CLI-vs-config audit the operator asked for. Carry the DOCTRINE forward: config is the primary surface; the CLI stays thin

To the next agent.

**The operator stated a doctrine — hold it.** *"Config is my primary surface. CLI should be kept very basic — for integration-testing needs and/or operator-facing memorable versions of commands that alias to precise config variants. No run should need >~100 chars."* And: *don't walk back / deprecate CLI we've already built — but every flag should justify itself as a per-run OVERRIDE or a FREQUENTLY-reached specialization.* So: **a flow should be a complete recipe in `projection.json`; the CLI is the thin override.** When you add an operator knob, give it a config home FIRST, then (only if it's a frequent per-run override) a memorable flag.

**Two slices landed this session under that doctrine (both gated + committed):**
- **§11 config-driven synthetic policy (commit `3870a620`).** The base synthetic policy (τ / preserve / synthesize / scale / seed) now has a declarative home: the top-level `"synthetic": {…}` block → `Config.SyntheticSection` → `ProjectionConfig.Synthetic` (a movement-vocabulary citizen — rendered by `renderConfig`, so `parse∘render=id` covers it). `SyntheticLoadRun.resolveConfig`/`resolveSeed` build the base `SyntheticConfig` + seed from the block, with `--scale`/`--seed` as the per-run override (config > CLI default; CLI flag > config). The richer per-column blessed intent stays the per-flow `correction` artifact, layered on top.
- **AUDIT follow-up — flow execution profile (commit `2ad07834`).** The audit's one real gap: a flow declared its structure but not its EXECUTION (strategy/resumable/streaming/journal were CLI-only). `Flow` gained those four (parse+render round-trip); `resolveFlowSpec` derives the spec from the flow config with the CLI flag as the per-run override (`--fresh` forces Fresh+genesis; `--resumable`/`--streaming` force ON; `--journal` overrides the dir). Daily run collapses to `projection <flow> --go`.

**The audit's map (so you don't re-derive it).** ALREADY config-primary (CLI = override): `--no-atomic`←env `atomicDeploy`, `--auto-revert`←env `revert`, `--seed`/`--scale`←`synthetic` block, `--correction`←flow `correction`, strategy/resumable/streaming/journal←flow fields. INTENTIONALLY CLI-only (per-run safety/intent — do NOT move to config): `--go` (commit gate), `--allow-drops` (loss ack — §4 "never from config"), `--allow-cdc` (CDC-gate override), and the display flags (`--json`/`--pretty`/`--watch`/`--verbose`/`--format`/`--depth`). Minor remaining: `--revert-dir` could ride env `revert` (a `revertDir`) — low priority.

**The synthetic program's own frontier is unchanged:** F3 (coverage corrections + L2-cov canary) → F6 (distribution fitting) → F4 (rotation; needs a threat model). See `THE_SYNTHETIC_DATA_FUZZING.md` §7.

**Scars (the build ones below in the prior letter still bite — `dotnet` not on PATH, the worktree-path trap, Bogus global seed, stray NUL).** Slice-specific this round: the A44 flow/config round-trip means EVERY new `Flow`/`ProjectionConfig` field needs its `renderConfig`/`renderFlow` arm (omit the default so existing configs stay byte-identical) AND ripples through ~6 `Flow` literal fixtures (the compiler enumerates them). `renderConfig` renders ONLY the movement vocabulary (env/flows/model/defaults/**synthetic**) — the `Shaping` sections are deliberately out of that round-trip, so a config-primary knob that must round-trip belongs on `ProjectionConfig` (movement vocab), not `Config.Config` (shaping).

**State.** Branch `claude/quirky-leakey-4da6cf`. Commits atop the 7-slice base `d6f0d4f0`: `99be701f` (F0c-I/O) · `a853f030`+`d95f32e8` (F-Faker + NUL fix) · `96ed507d` (handoff) · `3870a620` (§11 config) · `2ad07834` (flow execution profile) — **committed, NOT pushed**. Release 0/0; pure pool PASS (incl. the new §11 + AUDIT witnesses); Docker focus `Synthetic` 70/70 (π∘σ≈id 2s + the F-Faker load witness) at §11; full Docker pool last green 769s at F-Faker (C+D are pure config routing — the engine faces take their opts directly, not via `resolveFlowSpec`, so no Docker-tested behavior changed; default configs byte-identical). Hold the spine — and hold the config-primary doctrine.

---

# Handoff addendum — 2026-06-16 (earlier this session), F0c-I/O + F-Faker LANDED — the blessed-correction loop is operator-usable END-TO-END (durable write + `synth-correct` propose verb + `correction: file:<path>` flow wiring threading `Profile ⊕ Correction` into σ AND the Faker realization), and the Faker boundary is now COORDINATE-ADDRESSED and tunable (mask a column / permanent overrides / a wide generator catalog, bound by module-entity-attribute)
