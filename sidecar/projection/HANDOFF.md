...
**(B) IRBuilders shim full retirement — partial-close residual.** This arc migrated all qualified call sites (`IRBuilders.mkAttribute / mkKind / mkReference`) to production smart constructors via sed across 36+ test files. The pure delegation shims STAY in `IRBuilders.fs` for ~10 files using `open Projection.Tests.IRBuilders` + bare `mkX`. Full retirement triggers when the next IR aggregate field-extension would benefit from the shim-free surface (a hygiene-only slice deferred-with-trigger). The shims now name their soft-retirement candidacy in the module docstring.
