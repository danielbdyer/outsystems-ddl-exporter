# Handoff addendum — 2026-06-19 (later still), DELTA-GRADE DIFF POLISHED — the migrate-preview surface gets sharper and safer: the FK retarget names tables (a GUID-wall DEFECT fixed), the structural channels gain before/after (`uniqueness not unique → unique`), a "review these first" data-risk callout + an honest statement (a zero-drop NOT-NULL add no longer leads calm), and lane items sort by name. Four pure `Comparison.fs` slices, branch `claude/delta-grade-polish` off `main` `6027c0d4`, NOT pushed

To the next agent.

**The operator asked to DEEPEN the just-merged delta-grade diff (#27) — "more verbs, more detail, more at-scale polish" — using subagents to propose where next, me to select.** Three subagents (verbs / evidence / at-scale) each read the real code and ranked proposals; I synthesized + picked a cohesive, pure-`Comparison.fs`, one-substrate, no-new-`View`-case cluster (highest value-per-effort first) and shipped four gated slices. Full detail: `SPECTRE_REFINEMENTS.md` §27 (the Polish subsection); `DECISIONS 2026-06-19 (later)`.

**What shipped (all pure-Cli, zero Core/SQL, pure pool 3584/0, +10 `ComparisonTests`):**
...
**Your next moves (the named follow-ons — the bigger waves the subagents surfaced, none started):**
- **At-scale NAVIGABILITY (the marquee).** Today a `Lane`'s items are NOT navigable children (`Navigator.children` nests only `Doc`/`Disclosure`), so a 2000-item lane is a hard dead-end at item 12 on a TTY — JSON is the only path to item 500. Two halves: (a) make `Lane` items navigable (a `children` arm + a PAIRED path-aware `Lane` render that windows to the cursor — must stay byte-identical when `OpenPath = None`, the load-bearing invariant), and (b) group large lanes by module→kind as a `Disclosure` subtree (reuses the one shape the Navigator already digs; needs a Kind→module derivation — check whether `Kind` carries a module/owner field before parsing the `OSUSR_<module>_` name prefix). L-effort, touches the Navigator invariant — adversarially verify.
- **The scoping verbs.** `diff --only <channel>` (a `List.filter` over which channel-collectors run in `renderCatalogLanes`) and `diff --module <name>` (route the catalogs through the existing `ModuleFilter.apply` before `Between` in `RunFaces.runDiff` — A44 expressible⇔reachable). Both thread a mode through the `ExplainDiff` DU (`MovementSpec`/`MovementSurface`/`Program`). `diff --stat` = a per-table change histogram (a `View.Table` "top movers"). Config-thin doctrine: a memorable per-run override, not config.
- **The cross-surface `SsKey.displayName cat` projection.** The diff names by `Name` now; `explain` / the reconciliation reports / the bench dump (`RunFaces`) still name by `rootOriginal` (GUIDs on a real estate). A shared display projection unifies every operator face — clean, scoped, touches them all.
- **resume-from-refusal** — the prior letter (below) is its full code-verified architecture, operator-approved. Independent of the diff work; a different chapter (run/apply, Docker, DECISIONS-first).
- Lower: concrete storage width (`int → bigint`, a Core diff-modulus, DECISIONS-gated); #2 Markup newtype; #26 The Threshold.

**The reuse pattern (it worked all four slices):** a pure `View`-projection addition to `Comparison.fs` + law-citing `ComparisonTests` pinning BOTH the human-visible evidence AND the one-substrate carry on `toJson` (the item is a `Lane` string → assert the json contains the same text). Fixtures are cheap: `withKinds (List.map …)` over the shared `customer`/`order`/`country`; direction-swap (`Between a b` vs `b a`) gives add-from-drop and the dangerous tightening direction free. For a GUID-legibility test you MUST use `OssysOriginal` keys (`SsKey.ossysOriginal`), not the synthesized fixtures.
...
**State.** Branch `claude/delta-grade-polish` off `main` `6027c0d4` (post-PR-#632). Five commits: `b1144d18` (#A) · `ac8c09e3` (#B) · `39592360` (#C) · `b98fbc17` (#D) · `25c6ef1d` (docs) — **committed, NOT pushed** (the operator drives push / PR). Pure pool 3584/0; Docker untouched by design (every change is `Comparison.fs` rendering). Hold the spine, hold the live-instrument charter: ease, enjoyment, seamless flow.
