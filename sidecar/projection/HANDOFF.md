# Handoff letter — 2026-05-18 (emit-features arc + blind-spot closure; SCHEMA-axis V2-driver gate ready modulo named residuals)

To the next agent. The chapter-5.0 letter sits below this one; read both. The matrix discipline (`V1_PARITY_MATRIX.md`) and the operating-disciplines table (`CLAUDE.md`) are unchanged. This entry tells you what shipped on 2026-05-18, what's load-bearing for the next move, and where the (now-small) deferrals are.

## 2026-05-18 (emit-features arc + blind-spot closure — column / FK / index parity rows close AND the four operator-flagged gaps close before handoff)

### What shipped (five slices in one session arc)

Branch: `claude/transient-retry-exception-class-9MpoR`. Test baseline at end of arc: **1571 / 1571 non-canary passing** (1454 at chapter 5.0 close + 117 across the chapter 5.13 arc; the column/FK/index trio + smart-constructor lift + blind-spot closure all landed). Zero build warnings under `TreatWarningsAsErrors=true`. Lint count unchanged.

The five slices, in commit order:
...
2. **`5.13.smart-constructor-lift`** — Production-side smart constructors for the four IR aggregate records that lacked them: `Attribute.create / Reference.create / Index.create / Kind.create`. The pattern previously existed only on the test side via `IRBuilders.mkX` helpers; this slice lifts it to production. Five production-side literal sites migrated; ~20 test-side literal sites stayed stable (they already used IRBuilders). This slice was operator-directed mid-arc: "create a smart constructor for this and any and all use cases similar to this before doing the work." The discipline holds: closed-DU expansion errors light up at literal-construction sites only; field-extension propagation is bounded. The Reference IR gained `OnUpdate : ReferenceAction option` + `IsConstraintTrusted : bool` as carriage-only fields, consumed by the next slice.

3. **`5.13.fk-features-emit`** — ON UPDATE referential action + WITH NOCHECK trust-state preservation. `ForeignKeyDef` extended with `OnUpdate : ReferenceActionSql option` + `IsConstraintTrusted : bool`. New `Statement.AlterTableNoCheckConstraint of TableId * constraintName : string` variant. New `ScriptDomBuild.buildAlterTableNoCheckConstraint` builder uses `AlterTableConstraintModificationStatement` with `ExistingRowsCheckEnforcement = NoCheck` + `ConstraintEnforcement = Check` (renders empirically as `ALTER TABLE [Schema].[Table] WITH NOCHECK CHECK CONSTRAINT [FK_…]`). `SsdtDdlEmitter.untrustedFkAlters` yields one ALTER per untrusted FK after the kind's CREATE TABLE. Matrix rows 58 + 59 close to 🟢 PARITY.

4. **`5.13.index-features-emit`** — IGNORE_DUP_KEY + DATA_COMPRESSION + post-CREATE ALTER INDEX DISABLE. New `[<RequireQualifiedAccess>] type DataCompressionLevel = None | Row | Page` Core DU; `Index` extended with `IgnoreDuplicateKey : bool` + `IsDisabled : bool` + `DataCompression : DataCompressionLevel option`. New `Statement.AlterIndexDisable of TableId * indexName : string` variant. `ScriptDomBuild.buildCreateIndex` emits `IGNORE_DUP_KEY = ON` + `DATA_COMPRESSION = <level>` in the WITH clause; new `buildAlterIndexDisable` uses `AlterIndexStatement` with `AlterIndexType.Disable`. Matrix rows 55 + 56 close to 🟢 PARITY (emit-side; row 56's partition-scheme axis deferred until partitioned-index fixtures surface).

5. **`5.13.blind-spot-closure`** — four operator-flagged gaps from the prior handoff close before this rewrite. This was operator-directed: "Let's take care of these blind spot gaps before we do the handoff." The closure work:
   - **TransformRegistry Emitter-stage coverage**: new `SsdtDdlEmitter.registeredMetadata` with 11 classified Sites (one per V1-CreateTable + V1-CreateIndex feature V2 emits). `ManifestEmitter.build` prepends to `RegisteredTransforms.all`. 6 new tests in `EmitterRegistrationsTests.fs`.
   - **Rowset-adapter JOIN follow-up**: `MetadataSnapshotRunner.toBundle` performs the 3-step FK JOIN (`OssysReferenceRow.AttrId → OssysFkColumnRow.ParentAttrId → FkObjectId → OssysFkRealityRow`) so per-attribute References carry `(UpdateAction, not IsNoCheck)`. Index reality wires `OssysAllIdxRow.IsDisabled / IgnoreDupKey / DataCompressionJson` directly. Cluster-A1 cash-out complete.
   - **IRBuilders shim soft-retirement** (partial): all qualified call sites (`IRBuilders.mkAttribute / mkKind / mkReference`) migrated to production smart constructors via sed across 36+ test files. Pure delegation shims STAY for ~10 files using unqualified `mkX` via `open`.
   - **Render.fs StringBuilder retirement**: `toSql`'s per-variant arms collapsed to one `_ ->` delegation arm. Four dead-weight helpers retired (`columnSqlType`, `formatSqlLiteral`, `actionSql`, `renderColumn`). Render.fs reduces to 4 public functions.
...
| FK ON UPDATE referential action | 🟢 PARITY (this arc; emit + adapter both) |
| FK WITH NOCHECK preservation | 🟢 PARITY (this arc; emit + adapter both) |
...
| IGNORE_DUP_KEY | 🟢 PARITY (this arc; emit + adapter both) |
| DATA_COMPRESSION (single-value) | 🟢 PARITY (this arc; emit + adapter both) |
| IsDisabled (post-CREATE ALTER) | 🟢 PARITY (this arc; emit + adapter both) |
| DataSpace (Filegroup / PartitionScheme) + per-partition compression | 🟠 NOT-MAPPED (row 56 partition residual) |

Three SCHEMA-axis residuals remain: single-column-PK inline optimization, computed columns, partition-scheme axis. The first two are no-consumer-pressure deferrals; partition-scheme is the only one that requires non-trivial IR design (closed-DU `DataSpace = Filegroup of name | PartitionScheme of name × columns` + per-partition compression list). All other axes the canary surfaces — column features, FK features, index features — ship through V2 emission AND through V2 adapter wiring AND through V2's TransformRegistry as classified Sites.

### What's load-bearing for the next move

The SCHEMA-axis V2-driver gate now reaches "ready modulo named residuals." The remaining structural work to complete a V2-driver flip on SCHEMA is per-axis property tests (the V2_DRIVER spec's bar for V2-driver requires "structural-type-level enforcement plus per-axis property tests"). The 16+ canary tests across `SsdtDdlEmitterTests.fs` cover most of the territory; an explicit per-axis property test sweep is the highest-leverage SCHEMA-axis follow-up.

The remaining gate-flipping work is on OTHER axes: DATA (chapter 4.1.B closure on CDC silence + static-seeds emission posture); IDENTITY (chapter 4.2 close on UAT dry-run + the cutover-blocker user-matching property tests); DIAGNOSTICS (soft gate; operator-tolerant). Per `V2_DRIVER.md` per-axis stakes, SCHEMA is the most-advanced axis but no longer the gating one.

### Deferrals after this arc (small, well-scoped)

The prior handoff listed eight blind-spots; slice 5.13.blind-spot-closure closed four. The remaining four are either two-consumer-threshold deferrals or pattern-notes that don't require closure:

**(A) `PostCreateAlter` consolidation — two-consumer threshold approaching.** `Statement.AlterTableNoCheckConstraint` (FK trust preservation) and `Statement.AlterIndexDisable` (index disable preservation) both follow the pattern "post-CREATE ALTER scoped to a named entity, preserving deployed-state evidence." If a third such variant lands (the next candidate is the table-level `CHECK` constraint's `IsNotTrusted` axis when its WITH NOCHECK form is added; `ColumnCheckDef.IsNotTrusted` is carried but not emitted today), the two-consumer-threshold codification suggests folding them into a single `Statement.PostCreateAlter` variant with a closed-DU payload (`AlterKind = FkNoCheck | IndexDisable | CheckNoCheck | …`). Don't pre-extract — the third consumer hasn't arrived. Per the anticipation-vs-speculation discipline: structural alignment is visible, but Position A (full extraction) requires both shape visibility AND the third concrete consumer; only Position B (structural alignment) is met today.

**(B) IRBuilders shim full retirement — CLOSED 2026-05-18 (slice 5.13.shim-retirement).** The full retirement landed: six helpers retired (`mkAttribute / mkKind / mkReference / mkIndex / mkIndexColumn / mkIndexColumns`); three lifted to Core as canonical helpers (`IndexColumn.create / IndexColumn.ascendingList / Index.ofKeyColumns`); the test surface swept (qualified + unqualified) to the production smart constructors. The module shrinks to two skip-Result test-fixture conveniences (`mkModule` + `mkCatalog`) — production `Module.create` / `Catalog.create` return `Result<_>` for invariant-checking; the conveniences inline the unwrap for fixture-known-good values. Pillar-8 ubiquitous-language consistency now holds across emit + read + test surfaces.

**(C) `OssysFkColumnRow` composite-FK consumption — speculative.** Cluster A1 lifted the typed rowset; V2's `Reference` IR remains single-column-per-edge. The composite-FK refactor (multi-column FK with per-column ParentAttrId list) lives outside this arc — lift when composite-FK fixture surfaces in production OSSYS extraction.

**(D) `DataCompressionLevel` name collision — principled invariant.** The Core DU `Projection.Core.DataCompressionLevel` and ScriptDom's `Microsoft.SqlServer.TransactSql.ScriptDom.DataCompressionLevel` intentionally share the type-name (pillar 8 ubiquitous language). The join site in `ScriptDomBuild.fs` requires the namespace prefix to disambiguate. Not a deferral — a documented invariant. Watch for the same disambiguation pattern when extending either DU with columnstore variants (deferred-with-trigger when columnstore fixtures surface).
...
- **`V1_PARITY_MATRIX.md`** — Status-history amendments at rows 12 + 53 + 182 (column-features), 58 + 59 (FK-features), 55 + 56 (index-features), 17 + 18 (FK rowset-adapter JOIN), plus the TransformRegistry Emitter-stage + Render.fs StringBuilder retirement amendments. The amendment shape mirrors the slice — read those if you're about to land a parallel slice; the prose tells you what was decided.
- **`DECISIONS.md` (last five entries, all dated 2026-05-18)** — column-features-emit; smart-constructor-lift; fk-features-emit; index-features-emit; blind-spot-closure. Read in reverse chronological order; the blind-spot-closure entry names what each gap was and how it closed.
- **`BACKLOG.md` Phase 5.13 section** — retired entries: `5.13.column-checks-ir`, `5.13.column-features-emit`, `5.13.update-action`, `5.13.nocheck-state`, `5.13.index-disabled-igdupkey`, `5.13.fk-reality-join`, `5.13.emit-features-registry`, `5.13.render-stringbuilder-retirement`. Residual: `5.13.index-partition` (row 56 partition-scheme axis). LR3 + LR4 + LR6 + LR7 retired (partial); the partition residual remains.
- **`tests/Projection.Tests/SsdtDdlEmitterTests.fs`** — 16 canary tests across the three feature slices. They're the executable record of "what V1's emission shape looks like for these axes." Read the test names; they cite V1 default vs new-axis emission.
- **`tests/Projection.Tests/EmitterRegistrationsTests.fs`** — 6 tests asserting the SSDT emitter's `RegisteredTransform` surface (the Emitter-stage parallel to `AdapterRegistrationsTests.fs`). The Sites enumeration is the structural witness that every emit feature is classified.
- **`src/Projection.Core/RegisteredTransforms.fs`** — Core-resident registry list. `ManifestEmitter.build` prepends `SsdtDdlEmitter.registeredMetadata`; the assembled registry feeds the totality-coverage scan + manifest emission. When a sibling emitter (Json / Distributions / StaticPopulation / StaticSeeds / MigrationDependencies) lifts its `registeredMetadata`, mirror the SSDT precedent + the prepend chain.

### Highest-leverage next moves

The SCHEMA-axis is close to V2-driver ready; the gating axes are DATA + IDENTITY. Possible next slices, ordered by leverage:

1. **Per-axis property test sweep on SCHEMA emission** — the V2_DRIVER spec's bar for V2-driver is "structural-type-level enforcement plus per-axis property tests." Today the canary tests cover example-based assertions; FsCheck property tests for permutation invariance + T1 byte-determinism + V1↔V2 equivalence on the new emit axes (DEFAULT / CHECK / OnUpdate / NOCHECK / IGNORE_DUP_KEY / DATA_COMPRESSION / IsDisabled) would complete the SCHEMA-axis structural fortification. Estimated 200 LOC; closes the "verification depth Highest" bar for SCHEMA.

2. **Sibling emitter `registeredMetadata` lifts** — `SsdtDdlEmitter.registeredMetadata` shipped this arc. The five other sibling emitters (JsonEmitter, DistributionsEmitter, StaticPopulationEmitter, StaticSeedsEmitter, MigrationDependenciesEmitter) follow the same pattern. Each is ~30 LOC (Sites enumeration) + 6 mirror tests (~80 LOC). Extends pillar-9 totality coverage to the full sibling chorus. Two-consumer threshold for an emitter-metadata helper is met by SSDT + (any one of the others); extract the shared shape during the second.

3. **DATA-axis cutover-blocker closure** — `V2_DRIVER.md` per-axis stakes table flags CDC-silence-on-idempotent-redeploy as "the highest-leverage single deliverable in the entire chapter sequence." Chapter 4.1.B is in flight; the cdc-silence-cross-emitter slice (2026-05-18) shipped a structural fix. The next move is a property test that asserts the CDC-silence invariant across every static-population fixture.

4. **IDENTITY-axis cutover-blocker** — chapter 4.2 slice ε's user-matching property tests + the UAT-CLI verb (`5.13.uat-cli` in BACKLOG, ~1500 LOC). User CSV matching strategies + a real UAT dry-run.

### Mechanics worth carrying forward

- **Background test runs via `Bash run_in_background:true` + `Monitor` with until-loop on the output file.** Full suite is ~2-5 minutes; running it in the background while doing doc updates / fixture writes / commit prep saves linear time.
- **Empirical ScriptDom probing via `dotnet fsi /tmp/inspect.fsx`.** This arc probed three unfamiliar APIs (`AlterTableConstraintModificationStatement` for WITH NOCHECK, `AlterIndexStatement` for ALTER INDEX DISABLE, `DataCompressionOption` for DATA_COMPRESSION) before any production code landed. The 20-line fsi scripts replace speculation with evidence; the empirical-validation-before-commit pattern is high-leverage when adopting a new ScriptDom API.
- **Sed for bulk literal-site updates with a stable end-token.** When a closed-DU field-extension lights up ~20 literal sites, a single sed substitution closes the propagation if the literal end-token is positionally stable (e.g., `NoRecomputeStatistics = false` was the bulk hook for the index-features tests; `IRBuilders.mkAttribute` → `Attribute.create` was the bulk hook for the shim-retirement sweep).
- **Smart-constructor lift FIRST when extending an IR aggregate.** The smart-constructor lift slice was operator-directed mid-arc; it paid off twice. When you're about to extend an IR aggregate that lacks a smart constructor, lift the pattern first (A39 codification). The bench scoring "field-extension propagation bounded to N literal-construction sites" works at full strength only when the aggregate has a constructor.
- **Cluster-shaped slices for parallel-feature-axis closures.** The four feature slices this arc (column / FK / index emit + smart-constructor lift) followed an identical pattern. The pattern is documented in the prior handoff section's emit-features-arc playbook. When the next sibling axis surfaces (e.g., trigger emission via the chapter-4.10 cash-out), apply the playbook.
- **Per-slice commit discipline.** Five slices shipped this arc; five commits. The matrix's value is its append-only narrative of independent classification events; bundling destroys that.

### Closing posture

The codebase is in good shape. SCHEMA-axis V2-driver gate is close to ready (modulo named residuals + per-axis property test sweep). The TransformRegistry now reaches the Emitter stage on the SSDT side; pillar-9 totality coverage propagates one sibling emitter at a time. The Render.fs StringBuilder relic retired; every SQL-bearing statement flows through ONE pipeline (ScriptDomBuild → ScriptDomGenerate). The cluster-A1 cash-out is complete — rowset-adapter JOIN closes the structural evidence chain from V1 source (`#FkReality`, `#AllIdx`) to V2 SSDT emission.
