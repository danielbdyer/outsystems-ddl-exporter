# Handoff addendum — 2026-06-20, DELTA-GRADE DIFF POLISHED + MADE SCALE-INTENTIONAL (nine slices) — the migrate-preview surface is sharper, safer, scopable, AND intentional at ~310 tables: FK retarget names tables (a GUID-wall DEFECT fixed), structural channels carry before/after, a data-risk callout that GROUPS BY RISK CATEGORY at scale + an honest statement, `--only`/`--module` scope, a per-module rollup, a navigable module-grouped move-lane (the at-scale marquee — built from Disclosures, so the Navigator digs it with ZERO Navigator change), and cross-surface legibility started (verify-data names by Name). Branch `claude/delta-grade-polish` off `main` `6027c0d4`, NOT pushed
...
**The SCALE wave (`6f66b376` #G · `dc42db67` #H · `9ca1c35b` #I; pure pool 3597/0):**
...
- **#I — cross-surface legibility (the displayName chapter, STARTED).** `RunFaces` gained a shared `catalogNameIndex`/`nameOf`; `verify-data` names tables/columns by `Name` (the contract is threaded out of the read task as `(report, contract)`; payload build extracted as the pure, testable `integrityPayload`). `explain` was already fine (its `rootOriginal` is the substring MATCH key, not display).
...
**Your next moves:**
- **Finish the displayName chapter (#I's remainder).** `narrateTransferReport` (the load plan / cycle FKs / unmatched identities — 3 call sites at `RunFaces` ~751/932/2357) and `suggest-config` (it HOLDS `report.ReadCatalog`) still show GUIDs. Each needs its upstream catalog threaded to the narration, then reuse `catalogNameIndex`/`nameOf`. The transfer face's catalog is murkier than verify-data's (the report is produced deep in the engine) — find it at each of the 3 sites. The narration is stdout/Voice-based, so EXTRACT a pure payload builder (as `integrityPayload`) to keep it testable. Consider hoisting `catalogNameIndex` + `Comparison.nameIndex` to ONE shared `Catalog.nameIndex` in Core.
- **The L1 `/`-filter** on the Navigator `Model` (a cursor over the `View`, DYNAMIC_DISPLAY §7.6). With #H's grouped tree, filtering becomes very powerful at scale.
...
**State.** Branch `claude/delta-grade-polish` off `main` `6027c0d4` (post-PR-#632). Nine slice-commits + docs: `b1144d18` (#A) · `ac8c09e3` (#B) · `39592360` (#C) · `b98fbc17` (#D) · `74f7ef64` (#E) · `7670b7a4` (#F) · `6f66b376` (#G danger-by-category) · `dc42db67` (#H navigable module-grouped lane) · `9ca1c35b` (#I verify-data names + docs) + the earlier docs/HANDOFF commits — **committed, NOT pushed** (the operator drives push / PR). Pure pool **3597/0**; Docker untouched by design (#A–#H are Cli diff-rendering + a thin `ExplainDiff` DU thread; #I is `RunFaces` narration, pure-tested via `integrityPayload`). Hold the spine, hold the live-instrument charter: ease, enjoyment, seamless flow — and now, intentional at scale.
