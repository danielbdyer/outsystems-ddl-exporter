...
| 193 | V1 pushes the configured module/entity scope into the extraction SQL at query time (`@ModuleNamesCsv` / `@EntityFilterJson` / include flags bound in `MetadataSnapshotRunner.cs:217-253`; the `#E`/`#Ent` cascade scopes all ~24 rowsets server-side), AND re-applies the same filter in memory at every model load (`ModuleFilter.Apply`) — double enforcement | V2 restored the pushdown (reconciliation slice 4, `DECISIONS 2026-06-13`; operator adjudication C4): `SnapshotScopeBinding.fromModel` binds `model.modules` (+ entity narrowing + include flags) into the adapter's `SnapshotParameters`; `LiveModelRead.fromConnSpecWith` is the scope-bearing face on the full-export path; `ModuleFilter.apply` remains the semantic seam (double enforcement, V1's precedent). The pushdown ≡ filter equivalence is a Docker-witnessed LAW (`scopedRead(scope) ≡ ModuleFilter.apply(scope) ∘ fullRead`) — which on its first run exposed and fixed the dangling cross-scope-reference gap in `apply` itself (step-5 prune; scoped catalogs are `Catalog.create`-constructible again). `OnlyActiveAttributes` deliberately not pushed (would break the law; the dormant key keeps its deferral) | 🟢 PARITY | `SnapshotScopeBindingTests` (6 facts) + `OssysExtractionCanaryTests.``slice 4 equivalence: scoped pushdown read equals full read narrowed by ModuleFilter.apply`` ` (Docker) | The A7 opt-in polarity is shared verbatim with `ModuleFilterBinding` (empty `model.modules` ⇒ show-everything; flags inert without a named selection). |
