...
#     TWIN_BIN        path to an installed `twin` executable (the dotnet-tool
#                     distribution, plan B6). Unset: the monorepo's own
#                     `dotnet run --project src/Twin.Cli` form.
#     TWIN_TOOL_VERSION   manifest toolVersion override — the installed tool's
#                     package version, for runs where the monorepo source is
#                     not present. Unset: read from src/Twin.Runtime/Runs.fs
#                     when that file exists, else "unknown".
#     TWIN_TOOLCHAIN_MD   path to the toolchain ledger the pins bake from.
#                     Unset: ssdt-agent/estate/toolchain.md when present;
#                     an absent ledger rides through as UNPINNED — visible
...
# ---------------------------------------------------------------------------
# The monorepo's daemon helper when it is present; a bare probe where the
# script runs from the estate kit and no monorepo surrounds it.
if [ -f scripts/warm-sql.sh ]; then
...
# The tool seam (B6): an installed `twin` when TWIN_BIN names one (the
# estate's post-peel lane, no monorepo checkout), the monorepo's own
# project run otherwise. Both are the same tool; the manifest's
# toolVersion says which build.
if [ -n "${TWIN_BIN:-}" ]; then
    TWIN=("$TWIN_BIN")
else
...
# ---------------------------------------------------------------------------
TOOLCHAIN_MD="${TWIN_TOOLCHAIN_MD:-ssdt-agent/estate/toolchain.md}"
SQLPACKAGE_PIN=""
DACFX_CORPUS_PIN=""
if [ -f "$TOOLCHAIN_MD" ]; then
    SQLPACKAGE_PIN="$(awk -F'|' '$2 ~ /^ sqlpackage $/ { gsub(/ /, "", $3); print $3; exit }' "$TOOLCHAIN_MD")"
    DACFX_CORPUS_PIN="$(awk -F'|' '$2 ~ /DacFx \(Twin corpus\)/ { gsub(/ /, "", $3); print $3; exit }' "$TOOLCHAIN_MD")"
fi
TOOL_VERSION="${TWIN_TOOL_VERSION:-}"
if [ -z "$TOOL_VERSION" ] && [ -f src/Twin.Runtime/Runs.fs ]; then
