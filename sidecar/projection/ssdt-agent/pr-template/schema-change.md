<!--
  ...
  Source of truth: skills/author-pr/SKILL.md (this file mirrors it; if they disagree, the skill
  wins — fix this file in the same commit). This is the tree's canonical template mirror: the
  Copilot bundle emits it to .github/PULL_REQUEST_TEMPLATE/ (GitHub reads it with
  ?template=schema-change.md) and to .azuredevops/pull_request_template/ (Azure DevOps reads it
  as a default or branch-scoped template).

  Use this template for any change to a .sqlproj: table definitions, pre/post-deployment scripts,
  the refactorlog, publish profiles.
