...
## The compound exemplars (release-grain)
...
| [delete-seed-value](./delete-seed-value.md) | remove a Static Entity record | removing it from the seed does **not** delete the row (the seed is additive); a hard DELETE of a referenced value is **not** blocked here (no FK declared) — it **silently orphans** the referencing rows; prefer IsActive=0 |
