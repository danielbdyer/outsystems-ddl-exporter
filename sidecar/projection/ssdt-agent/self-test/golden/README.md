...
run** of this tree (make-mandatory two-release re-proved 2026-08-24, SQL Server 2022 in the warm
...
**two-release**. The content digest `1818783869` reproduced unchanged from the earlier 2026-08-22
run, on the same tool version — the same rows hash to the same value.

- `make-mandatory-pr.md` — the pull request body a reviewer reads, in the ten-section record form
  (`../../skills/author-pr/SKILL.md`). Every count, error text, and digest in it was observed on the
  disposable copy: the row-presence guard verbatim, the naive single-release block (`Msg 50000`,
  `pg_mm_naive`), the two-release landing (Release 1 pre-deploy + Release 2 no-op, content digest
  `1818783869` identical across both, `pg_mm`), the seed-must-ride-with-the-change proof (`Msg 515`),
  and the enforcement + idempotency checks.
