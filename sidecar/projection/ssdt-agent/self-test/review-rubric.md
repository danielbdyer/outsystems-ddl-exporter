...
| **2** | **APPROVE-the-good / CATCH-the-defect** | correctly approved the honest packet (REV-01) and caught every planted defect at the RIGHT level | binary per scenario against the expected-verdict column: REV-01 must be Approved (and must NOT be a false Return to the author of a correct change); REV-02/03/04/05 must be Returned to the author (REV-03's two-release shape is determined under the locked gate — Escalating it spends the escalation on a settled shape); **REV-08 must be Approved with a named risk** (scope the cross-boundary consumer + name the residual, not a flat Approved) | all seven correct for an aggregate PASS |
...
| **6** | **ESCALATION-discipline** | escalated ONLY the irreducible judgment, with homework done — did not escalate a fix that should have been returned to the author | REV-02/03/04/05 must **not** reach the lead (mechanically fixable, or a shape the locked gate has already determined → Persona 1); when a scenario does surface an irreducible judgment (REV-07's sparring arm), it reaches the lead **with** the dependency map + exactly ONE specific question already assembled. A return-to-author fix escalated to the lead, or a design fork silently approved, is a miss | binary per scenario; the gating scenario (REV-03) must be right |
...
- the **REV-03 gate** is met (the reviewer reproduces the zero-NULL-still-blocks finding **empirically**,
  not parroted, and returns **Returned to the author** with the corrected two-release shape — the
  escalation is never spent on a shape the locked gate has already determined), AND
...
- **ESCALATION-discipline = 100%** on the gating scenario (REV-03 returned to the author, not the lead;
  no defect scenario escalates a determined shape), AND
