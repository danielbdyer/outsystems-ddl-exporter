...
> shared reasoning lives in `../_index/`. Nothing here restates a guard or how a change flips.

**Family framing.** OutSystems "Static Entities" — the small, enumerated tables (Status, Country,
OrderType) whose rows are part of the model, not user input. In SSDT their rows are seeded by a
**post-deployment idempotent MERGE**: the schema project describes structure; the seed *data* ships
in the post-deployment script. The family's shared discipline is **silence-as-proof** — a no-op
redeploy must touch zero rows, keep an identical hash, and (if CDC-tracked) capture zero changes.

## Ops in this family

| Op | Per-op skill | What it is / how it flips |
|---|---|---|
| create-static-seed | `../op/create-static-seed/SKILL.md` | new lookup entity + idempotent MERGE seed; additive, any team member can review; ships as a schema change plus a post-deployment seed; added scrutiny if CDC-tracked; FK-target ⇒ parents-first |
...
- **`../_index/cdc/SKILL.md`** — CDC-silence and over-capture, when a seeded table (or its
