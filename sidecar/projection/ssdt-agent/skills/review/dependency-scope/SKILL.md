...
> nothing about the proc that joins the table or the ETL that pulls it nightly. A change that was
> never mapped cannot be approved. Scope is not a courtesy paragraph — it is the denominator of every
> disposition, and an unscoped dependency is an unbounded risk under a clean result.
...
>   named edge).
> - A cascade deep enough to be irreversible-adjacent forces **Escalated — one question
...
| **External / ETL / cross-DB** | nightly extracts, reports, other databases | **cannot be proven here** — the disposable copy is one catalog; name it, do not assert it away |
...
Strict publish is silent on them, and silence is not safety. "The nightly ETL and a report in another
database both read this column; neither is in the dacpac" is a scope finding, and it caps the
disposition at Approved with a named risk unless the lead accepts the out-of-band consumers.
...
- `talk-to-local-sql` — the dependency-closure + row-count probe SQL (sys.foreign_keys, sys.sql_expression_dependencies, sys.indexes, `COUNT(*)`).
...
- **No re-explanation of the constraint claim or cascade coexistence** — each
