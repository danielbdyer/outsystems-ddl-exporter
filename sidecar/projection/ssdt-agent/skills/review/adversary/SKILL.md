...
> too kind to write — the orphan they didn't plant, the row that makes the table non-empty — is
> where the risk hides, and SSDT's own engine passes the verdict on it. No new attack is invented
> here; the two moves the disposable-copy loop already owns are pointed
...
- **The injection leg** — for a *constraint / tightening* op with a data-loss block. If the author's
  ...
  never provoked. Turns "clean on this seed" into "Msg 547, FK Order_Customer_CustomerId, offending value 999."
...
data block — `edit-seed` (a MERGE), `add-optional` (a nullable add) — do **not** manufacture a
block that structurally cannot fire.
...
## The challenges (7 concrete, on real ops from the 10-table catalog)
...
6. **Refactorlog-cleanup left stale** — *op `rename-attribute` / a prior rename.* Claim: "rename done,
...
7. **Idempotent-seed over-write** — *op `edit-seed` / `create-static-seed`, target e.g. `Status`.*
   ...
   MATCHED`** touches rows on the *second* (no-op) run and the content-hash shifts — the seed is not
   idempotent. A guarded `WHEN MATCHED` touches **0** rows on redeploy and the hash holds. WHY ->
...
- **Identity ops** (`rename-attribute`, `rename-entity`) -> challenges 1, 6 (read the delta for
...
- **Coexistence / cascade ops** (`change-delete-rule`, `merge-tables`, `extract-to-lookup`,
  `temporal-convert`) -> challenge 5 + the cardinality/mapping proof.
- **No-block ops** (`edit-seed`, `add-optional`) -> challenge 7 for seeds;
...
- `talk-to-local-sql` — every injection/probe SQL (the orphan INSERT, the over-length value, the `MAX(LEN)` probe).
- `_index/tightening-class` (challenges 2, 4) · `_index/constraint-is-a-claim` (challenge 3) ·
  `_index/identity-and-refactorlog` (challenges 1, 6) · `_index/multi-phase` (challenge 5) ·
  `_index/idempotent-seed` (challenge 7) — every WHY, by pointer.
