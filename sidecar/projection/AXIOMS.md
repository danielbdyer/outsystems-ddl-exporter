...
- **A41 candidate (Transform registry totality + canonical strongly-typed shape)** — A.4.7 close (`V2_PRODUCTION_CUTOVER.md` §6.4.7; underwrites `PRODUCT_AXIOMS.md` L3-CC-Transform-Totality). Statement (placeholder, to be finalized at chapter close):
   ...
   *Bridge clause (added 2026-05-16 — Bridge wave; chapter 0.5).* Every public method in `Projection.Bridge.Core` and `Projection.Bridge.Runtime` is structurally classified `DataIntent` by its type signature. Bridge methods cannot accept `Policy` (or any `OverlayAxis`-shaped parameter); the `Projection000BridgeWallDiscipline` analyzer rejects the signature. The chain is *Evidence → Bridge (DataIntent) → E-passes (OperatorIntent applied) → Π (consumes Catalog × Profile per A18 amended)*. The skeleton-purity property extends to the Bridge wall: `Bridge.<Capability>Async(input)` produces identical output regardless of Policy state of the downstream pipeline. The audit-attribute system (per A42 candidate) is the manuscript history of every transformation site Bridge surfaces. Bridge methods are NOT registered in `Projection.Core.TransformRegistry` as `OperatorIntent` overlays; they are pre-Enrichment data sources. This clause cashes at chapter 0.5 close — the first chapter that lands `BridgeMethodAttribute` + the wall analyzer + the manifest test as the structural witnesses.

- **A42 candidate (Inheritance citation discipline)** — chapter 0.5 close. Every public method in `Projection.Bridge.Core` / `Projection.Bridge.Runtime` carries `[BridgeMethod(Chapter, AddedDate, V1Source, Current, Target, Determinism, Frequency)]` with all seven fields populated. The attribute IS the manuscript history of V2's inheritance from V1: `V1Source` cites the V1 file/type/method at the commit it was adopted from, or `"OriginAuthoredInV2"` for V2-for-V1 capabilities. The reflection-scanned `BridgeManifest` aggregates these citations; `Projection.Bridge.Tests.BridgeManifestTests` asserts well-formedness at every test run. The `Projection000BridgeWallDiscipline` analyzer enforces presence at compile time. Statement (placeholder, to be finalized at chapter 0.5 close):

   *Every public method in the Bridge namespace is a node on the V1→V2 inheritance phylogeny. The four-position gradient — `Delegated` → `Vendored` → `RefinedInPlace` → `TranslatedToFSharp` — declares how the method has progressed from V1 antecedent toward V2 publication. Each method's `Current` ≤ `Target` invariant is enforced at validation time; the cutover+30 sunset gate (`BridgeManifestSunsetGateTest`) asserts the conjunction of `Current = Target` over every manifest entry. The citation apparatus is structural, not advisory: a Bridge method lacking the attribute does not compile.*

   This amendment promotes the inheritance discipline from convention (Chapter 0.5 open framing + DECISIONS entry) to bidirectional structural enforcement (the audit attribute + analyzer + manifest test as a triple deliverable). Sibling to A41 (TransformRegistry totality): A41 enumerates every `OperatorIntent` site that consumes Policy; A42 enumerates every `DataIntent` site that crosses the V1↔V2 seam. Together they carry the dichotomy as a type-witnessed bidirectional contract — A18 amended forbids Policy in Π; A41 enumerates Pass-side OperatorIntent; A42 enumerates Boundary-side DataIntent.

   *Cross-references.* `DECISIONS 2026-05-16 — Bridge wave: V2 inherits from V1`; `CHAPTER_0_5_OPEN.md` (the seven slices); `Projection.Bridge.Core/Audit/BridgeMethodAttribute.cs` (the attribute definition); `Projection.Bridge.Core/Audit/BridgeManifest.cs` (the reflection-scanned validator); `Projection.Bridge.Tests/BridgeManifestTests.cs` (the structural witnesses).
