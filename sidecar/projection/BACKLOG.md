...
- **Wave 5 — opened under operator-as-consumer (2026-05-31):** the operator declared themselves the consumer for the synthetically-testable members of §5.2–5.9, firing their deferral triggers. **5.3 Lifecycle SHIPPED** (`DECISIONS 2026-05-31`; L-α→L-δ + §V E4 acceptance green). Still genuinely blocked: **5.1** needs a writable UAT SQL connection (OPEN-2). Still consumer-coupled to a second source (hold): **5.4** (SsKey/Identity rename needs a DACPAC reader), **5.9** (computed-column round-trip needs a source that populates `Attribute.Computed`). Remaining synthetically-testable queue: 5.2 / 5.5 / 5.6 / 5.7 / 5.8. IR-grows-under-evidence still holds — the consumer is now real, not speculative.
