...
| 5.13.fk-reality-join | 17 + 18 + 58 + 59 | Adapter JOIN at `MetadataSnapshotRunner.toBundle`: thread `OssysFkRealityRow.UpdateAction` + `IsNoCheck` per-FK-constraint metadata onto `ReferenceRow` per-attribute records via (EntityId, parent column) lookup; cashes the rowset wiring residuals from rows 58 + 59 | A deployed environment surfaces `#FkReality.IsNoCheck = 1` AND canary detects a V2/V1 emission delta (Tolerance-named) | ~150 |
