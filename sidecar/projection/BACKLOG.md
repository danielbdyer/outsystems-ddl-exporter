...
| F | **Seam split + full-export consumer (SHIPPED).** Per the architectural framing (2026-05-29 operator note: the remap is generic and most-often-affiliated with full-export / static artifacts; transfer is one of N consumers — don't overlay seams too liberally), the generic identity-domain primitives now live in their own home: `Projection.Core.SurrogateRemap.fs` carries `SourceKey`/`AssignedKey`, `IdentityDisposition`, `SurrogateRemapContext` + module (state ops), `UnresolvedReference`/`RemappedRows`, and `module SurrogateRemap` (consumption: `fkColumnsTargeting`, `remapRowFks`). `Transfer.fs` slimmed to apparatus only (`SubstrateRole`/`Environment`/`ConnectionRef`/`Substrate`/`TransferConnections`); `Reconciliation.fs` slimmed to acquisition only (`ReconciliationStrategy`/`ReconciledIdentity`/`reconcileKind`). **Full-export consumer:** `StaticSeedsEmitter.emitWithTopoAndRemap` is the full-explicit entry that threads an operator-supplied `SurrogateRemapContext` through MERGE construction — FK columns whose target is in the remap are re-pointed; rows with unresolvable FKs are dropped (skip-and-diagnose). New `OperatorIntent Insertion` site `staticRowSurrogateRemap` registered (sibling to MigrationDependenciesEmitter's `userRemapRewrite`); `StaticSeedsEmitter` now appears in `overlayView`, drops out of `skeletonView`. 5 new pure tests + classification tests updated; full pure pool (2411) + Docker pool (85) green. | ✅ shipped |
