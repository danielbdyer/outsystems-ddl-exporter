...
- **Wave 3 remaining (org-gated, NOT engineering-blocked):** 3.1 R6 `--execute` amendment needs operator sign-off (OPEN-4) — CDC pre-flight is buildable; 3.3 `osm uat-users` dry-run needs UAT access + real CSVs; 3.5 flip-ledger/runbook are cutover-ops-owned.
