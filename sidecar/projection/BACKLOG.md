...
**Update 2026-05-30 (later) — Wave 0 + Wave 1 EXECUTED (build + test verified vs real SQL Server).** dotnet 9 + Docker SQL Server 2022 confirmed available in-session; all slices below build clean and their tests pass.
- **Wave 0.2** — AsyncStream pruned to `{toList, probe}`; 8 zero-caller combinators retired (two-consumer-threshold retraction). 0.4 — `Catalog.create` two kind-walks → single fold (33/33 CatalogTests green; A39 order preserved). 0.1 — doc reconciliation (above). **0.3 DEFERRED-with-finding**: the TransformRegistry stage-order helpers are NOT dead — `TransformRegistryTests.fs` (chapter A.4.7 slice β) covers them; deleting would rip out a tested deliverable.
- **Wave 1.1** — `DacpacRoundTripTests.fs`: real DACPAC round-trip witness (emit → DacFx model → project tables+columns+FK-shape → assert modulo named A37 erasure). **Closes the phantom L3-S2** (audit claimed Bucket A citing a test that never existed). 4/4 green, pure pool. `AxiomTests.fs::L3-S2` now a live witness.
- **Wave 1.4** — writeBack drift-guard in `PassChainAdapterComposeTests.fs`: each ComposeState decision setter writes exactly its own field. 6/6 green.
- **Wave 1.2** — un-hollowed the canary's **DEFAULT** axis (one feature, full vertical, the proven template): `ReadSide.readDefaultConstraints` + `attachDefaults` (sys.default_constraints); `PhysicalColumn.Default` + `normalizeDefault` (A37 tolerance) on both producers; `CanaryRoundTripTests` "Slice 1.2" proves emit→deploy→ReadSide recovers the DEFAULT with empty PhysicalSchema diff **vs real SQL Server**. **L3-S6 D→A.** The other five hollow-canary features (triggers/sequences/checks/computed/ext-props — L3-S4/S5/S7/S8/S9) remain Bucket D, each a follow-on vertical on this template (1.3 scope).
- Verifiability gate PASS throughout (zero phantom); matrix live count 59→61 (L3-S2 + L3-S6 witnesses).
- **Wave 1.3** — un-hollowed the canary's **annotation axis** (triggers / sequences / checks / extended properties) end-to-end vs real SQL Server. New uniform `PhysicalSchema.Annotations` axis (Trigger/Check/Sequence/ExtendedProperty discriminator) + `PhysicalColumn.Computed`; four `ReadSide` `sys.*` probes + `attachAnnotations`/`buildSequences` populate the IR through Core smart constructors. `CanaryRoundTripTests` "Slice 1.3" proves recovery for all four. **L3-S4/S5/S8/S9 D→A**; **L3-S7 (computed) honestly Bucket C** (IR+emit+PhysicalSchema+H-050 AST reader done; real-SQL `sys.computed_columns` probe pending). Full pure pool 2418 passed / 0 failed; lint baseline 324 (0 new).
- **Fixture extraction (Wave-1 sweep start)** — `defaultBearingCatalog` + `annotationBearingCatalog` lifted from `CanaryRoundTripTests` into the shared `Projection.Tests.Fixtures` module as reusable named builders (per the review: don't maintain synthetic catalogs in business-logic-adjacent test files). Behavior-preserving; the broader inline-`Catalog`-literal migration folds into Wave 1 incrementally.
- **`dotnet fsi` + DacFx gotcha (recorded for future agents).** `#r "…/Microsoft.SqlServer.Dac.dll"` then `open Microsoft.SqlServer.Dac.Model` fails with "namespace 'Model' is not defined" — the `.Model` types (`TSqlModel`, `TSqlObject`, `Table.TypeClass`) live in the **sibling** `Microsoft.SqlServer.Dac.Extensions.dll` whose transitive deps aren't on a single-`#r` probe path (`GetTypes()` throws `ReflectionTypeLoadException`). **Fix:** use `#r "nuget: Microsoft.SqlServer.DacFx, 162.5.57"` (resolves the dependency closure). The project build never hit this because the `.fsproj` PackageReference pulls the whole closure — it was a scripting-only artifact of single-assembly `#r`.

**Cumulative Wave-0+1 axis status (schema round-trip):** DEFAULT (S6) ✅, triggers (S4) ✅, sequences (S5) ✅, checks (S8) ✅, ext-props (S9) ✅ — all verified vs real SQL Server; computed (S7) ◑ in-process-only; cross-DB FK (S10) and DACPAC predicate (S2) ✅. The "hollow for six features" canary defect is closed for five of six (computed's real-SQL leg is the lone remaining vertical).

**Update 2026-05-30 (later still) — L3-S7 closed + Wave 2 COMPLETE + Wave 3 (buildable-now) started.**
- **L3-S7 (computed) C→A** — `ReadSide.readComputedColumns` + `attachComputed` (sys.computed_columns) recover `Attribute.Computed`; `CanaryRoundTripTests` "Slice 1.3 / L3-S7" proves a PERSISTED computed column round-trips with state+definition restored vs real SQL Server. **All six hollow features now Bucket A** — the canary is no longer structurally blind to any schema feature.
- **Wave 2 — decision→emission loop CLOSED (2.1–2.5; A42 numbered).** V2 now EMITS the tightening it decides (was: decides correctly, emits untightened). 2.1 `DecisionOverlay` (A18-safe Set<SsKey> projection); 2.2 curried-prefix emitter seam (`statementsWith`/`emitSlicesWith`, byte-identical on empty — sibling-wrapper); 2.3 NOT NULL + UNIQUE additive (`∧¬enforce`/`∨enforce`); 2.4 FK drop + NOCHECK; 2.5a cashed **A42** + L3-S11; 2.5b FK silent-drop **witness** (`foreignKeyDropDiagnostics`, slice-μ retired — finding: `Catalog.create` already *rejects* dangling targets, so the reachable silent drop is the missing-PK case). Proofs at emission layer (pure) + canary layer (Docker, vs real SQL Server). All byte-identical-on-empty.
- **Wave 3.4 — Tolerance per-environment config, fail-closed.** `Tolerance.parse : string list -> Result<Tolerance, ToleranceError>` — unknown token ⇒ `Error` (never silently widens R6 gate semantics); empty ⇒ strict. NB: the Config-field + canary-tolerance-awareness wiring is deferred — `Tolerance` is currently UNCONSUMED in src/ (the diff is exact `isEqual`), so a Config field would be parsed-then-discarded (dead-overlay discipline). Primitive ships; consumer gated.
- **Wave 3.2 — ApprovalStore (durable R6 approval home).** JSON load/save (PRJ001 typed-AST; missing→empty, malformed→ParseFailure, byte-stable); `approve --store <path>` now persists (closes constructs-and-discards). SuggestConfig second-consumer gating deferred (suggestion→digest linkage needs investigation).
- **Wave 3.3 — uat-users collapsed (2026-05-30):** NO standalone `osm uat-users` verb. The Dev→UAT user-FK reflow is opt-in in `transfer --reconcile <UserTable>:<emailColumn>` (live source+sink re-key — the live sink obviates the planned CSV inventory) + `full-export` (`policy.transformGroups: [{name:"UserReflow", enabled:true}]` opt-in over `UserFkReflowPass`; off by default). One-line opt-in-default flip in `TransformGroupsBinding.fromConfig` + test updates; the ~1500-LOC cash-out vanished (machinery already in both verbs). `DECISIONS — uat-users is NOT a standalone verb`. The UAT *run* stays access-gated only.
- **Wave 4.1 — `V2.SsKey` persistence: CLOSED & VERIFIED (2026-05-30).** Commits `98fa616` (codec + 8 round-trip tests) / `ea25b2a` (emit `V2.SsKey` extended property) / `aa7aa9a` (ReadSide hydrate) / `8dbcdfd` (red-branch repair). `SsKey.serialize`/`deserialize` total over all four variants; `SsdtDdlEmitter` persists `V2.SsKey` at table level; `ReadSide.buildKind` recovers identity from it (synthesis fallback). Docker-gated round-trip passes: deploy `OssysOriginal`-keyed catalog → read back → recovered key IS the original GUID (A1 across the process boundary). **Incident:** `aa7aa9a` was pushed RED (6-wide annotation vs 7-tuple body; test referenced 3 non-existent helpers); repaired in `8dbcdfd`. Discipline reinforced: solution-build + `scripts/test.sh fast` before every commit.
- **Wave 4 remaining (buildable now, dependency-ordered):** 4.2 connection apparatus + CSV loader (`LiveOssysConnection`; `TransferConnections` already exists in `Transfer.fs`) → 4.3 cross-DB FK three-part name → 4.4 `osm verify-data` verb (orthogonal to `Reconciliation.fs`, confirmed) → 4.5 DacpacEmitter joins T11 sibling contract → 4.6 `Origin` variant rename. None externally blocked.
- **Wave 5 — opened under operator-as-consumer (2026-05-31):** the operator declared themselves the consumer for the synthetically-testable members of §5.2–5.9, firing their deferral triggers. **5.3 Lifecycle** + **5.5 applied-transforms** + **5.2 AssignedBySink (acyclic) SHIPPED** (`DECISIONS 2026-05-31`; L-α→L-δ + §V E4 green; per-artifact overlay enumeration in the manifest; sink-minted-key capture + FK re-point with the `data adjunction: AssignedBySink round-trips modulo SurrogateRemapContext` canary green on the live container). Still genuinely blocked: **5.1** needs a writable UAT SQL connection (OPEN-2). Still consumer-coupled to a second source (hold): **5.4** (SsKey/Identity rename needs a DACPAC reader), **5.9** (computed-column round-trip needs a source that populates `Attribute.Computed`). **5.6 leg 1 (`policy-diff` CLI verb) SHIPPED** (`DECISIONS 2026-05-31`; `Compose.buildPolicyFromConfig` exposed as second consumer; structural + per-kind diff over a shared catalog). 5.6 legs 2 (`LineageTree` fork — needs N≥3 candidates) and 3 (`VersionedPolicy.evolve` history — needs a SemVer-history reader) held on their distinct triggers. Remaining synthetically-testable queue: 5.7 (perf) / 5.8 (CLI verbs). **5.7 ASSESSED with bench evidence 2026-05-31** (`DECISIONS 2026-05-31 — §5.7`): all four items deferred — triggers unfired (topo-sort 2ms/300 kinds; rowset-parse path not exercised; schema-deploy dominated by container+data; OSSYS extraction out-of-scope). The hot paths are deploy/IO/data-emission, none of the candidates. Cyclic AssignedBySink (self-ref IDENTITY) is a named 5.2 follow-on. IR-grows-under-evidence still holds — the consumer is now real, not speculative.
- **Wave 6 — Substantiating the isomorphism (the A→B Total-Migration epic), OPENED 2026-05-31:** the NORTH_STAR matrix reached **L1 (witness-present) = 5/5**; a six-agent red-team (`AUDIT_2026_05_31_FIVE_AXIS_REDTEAM.md` — the full record; `DECISIONS 2026-05-31 — Five-axis red-team`) measured the gap below the floor and found the round-trips are **adjunctions, not equivalences**: every axis is a partial iso with ≥1 **silent** erasure (Schema retraction; Data exit-0-on-drop; Identity `Synthesized`-rename loss; Time `replayTo`-is-a-fetch; Decision 1/3 sub-axes), two axes **couple** (Decision→Data, Identity→Schema), the basis does **not span** the migration (no Permissions / Transactionality / Connection dimension), and the composed `migrate A B` does not exist. NORTH_STAR elevated to the **iso-ladder (L1→L2→L3)** + two new totalities (**T-V orthogonality, T-VI spanning**) + **Promise 8** (one-command A→B). Wave 6 is the buildable climb: **6.A** faithfulness (close the silent erasures per axis → L2), **6.B** orthogonality pre-flights, **6.C** spanning (permissions/transactionality/connection), **6.D** the `migrate` orchestrator (L3 bullseye), **6.E** matrix-reports-the-rung. **Critical path:** `6.A.10 (attribute-level CatalogDiff — the keystone) → 6.A.12 (diff→ALTER) → {6.B.*, 6.C.1/.2} → 6.D.1`. **Quick wins:** 6.A.1 (fail-loud transfer on dropped rows), 6.A.9 (DropFk audit). Master severity table + complete acceptance catalog: audit §6/§7. **Risk register update:** findings #1–6 (audit §7) are CRITICAL and on the A→B critical path; the one solid foundation is data-level CDC-silence (genuinely shipped + witnessed).
- **Wave 3 remaining (org-gated, NOT engineering-blocked):** 3.1 R6 `--execute` amendment needs operator sign-off (OPEN-4) — CDC pre-flight landed; 3.5 flip-ledger/runbook are cutover-ops-owned.
- Verifiability gate PASS throughout; matrix live count 61→64+ (L3-S2/S4/S5/S6/S7/S8/S9 + A42 + L3-X7 + L3-C1 witnesses); lint baseline 324; pure pool 2449 passed / 0 failed.
...
| ~~5.13.uat-cli~~ COLLAPSED 2026-05-30 | 113 | NO standalone verb — opt-in in `transfer --reconcile` (live re-key) + `full-export` (`transformGroups: UserReflow`); CSV obviated by live sink | `DECISIONS — uat-users is NOT a standalone verb` | 0 |
...
| ~~5.13.column-features-emit~~ | ~~12 + 53 + 182~~ | **SHIPPED 2026-05-18 (slice 5.13.column-features-emit).** Emit-side closure for chapter A.0' slice ε IR lifts: `Statement.ColumnDef` extended with `DefaultValue` + `DefaultName`; new `ColumnCheckDef` type; `Statement.CreateTable` constructor extended to carry CHECK list; `ScriptDomBuild.columnDefinition` emits `DefaultConstraintDefinition`; new `checkConstraint` builder parses via `TSql160Parser.ParseBooleanExpression`; `Render.fs` CREATE TABLE arm collapsed into ScriptDomGenerate delegation (single source of truth). 3 new canary tests assert DEFAULT + CHECK + T1 byte-determinism. | ~~Rows 12 + 53 + 182 close to 🟢 PARITY~~ — closed | ~~150~~ |
| ~~5.13.physical-columns-present~~ | ~~14~~ | **SHIPPED 2026-05-18 (slice 5.13.ossys-rowsets-cluster).** Typed `OssysPhysColsPresentRow` lifts at runner; orphan-attribute IR consumer deferred. | ~~V2 reports source-side orphan attributes~~ — typed rowset lifted; consumer gated | ~~50~~ |
...
| ~~5.13.ossys-foreign-keys~~ | ~~17 + 18~~ | **SHIPPED 2026-05-18 (slice 5.13.ossys-rowsets-cluster).** Typed `OssysFkRealityRow` + `OssysFkColumnRow` lift at runner; Reference IR enrichment (OnUpdate per row 58; IsNoCheck per row 59) gated on those slices. | ~~V2 reports source-vs-target FK drift OR IsNoCheck flag feeds tightening~~ — typed rowset lifted; IR enrichment gated | ~~200~~ |
| ~~5.13.ossys-triggers~~ | ~~23~~ | **SHIPPED 2026-05-18 (slice 5.13.ossys-rowsets-cluster).** Row 23 wired through `parseTriggerRowFor` into `Kind.Triggers`; rows with blank Definition filter at adapter boundary. **Retires V2's structural dependence on V1's #TriggerJson** (row 27 V1-SUNSET). | ~~V2 trigger emission lands~~ — closed | ~~50~~ |
| ~~5.13.update-action~~ | ~~58~~ | **EMIT-SIDE SHIPPED 2026-05-18 (slice 5.13.fk-features-emit, paired with smart-constructor-lift).** `Reference.OnUpdate : ReferenceAction option` lifted; `ScriptDomBuild.foreignKeyConstraint` emits `cons.UpdateAction` when `Some action`. Adapter rowset-path JOIN (`OssysReferenceRow` ⊕ `OssysFkRealityRow.UpdateAction`) deferred — defaults `None` until a JOIN slice wires the per-FK-constraint metadata onto per-attribute references. | ~~V2 SSDT emission needs ON UPDATE referential actions~~ — emit-side closed; rowset wiring follow-up gated on operator-reality evidence | ~~100~~ 30 (adapter JOIN residual) |
| ~~5.13.nocheck-state~~ | ~~59~~ | **EMIT-SIDE SHIPPED 2026-05-18 (slice 5.13.fk-features-emit).** `Reference.IsConstraintTrusted : bool` lifted; new `Statement.AlterTableNoCheckConstraint` variant + ScriptDom builder via `AlterTableConstraintModificationStatement`; SsdtDdlEmitter yields one ALTER per untrusted FK after CREATE TABLE. Adapter rowset-path JOIN (`#FkReality.IsNoCheck`) deferred — same JOIN dependency as row 58. | ~~Deployed target carries WITH NOCHECK FK constraints V2 must round-trip~~ — emit-side closed; rowset wiring follow-up | ~~80~~ 30 (adapter JOIN residual) |
| ~~5.13.fk-reality-join~~ | ~~17 + 18 + 58 + 59~~ | **SHIPPED 2026-05-18 (slice 5.13.blind-spot-closure).** Adapter JOIN at `MetadataSnapshotRunner.toBundle` threads `OssysFkRealityRow.UpdateAction` + `IsNoCheck` onto `ReferenceRow` via the 3-step lookup chain (`AttrId → OssysFkColumnRow.ParentAttrId → FkObjectId → OssysFkRealityRow`); also wires the parallel `OssysAllIdxRow.IsDisabled / IgnoreDupKey / DataCompressionJson` onto the new `IndexRow` fields. `CatalogReader.ReferenceRow` + `IndexRow` extended; `parseReferenceRowFor` + `parseIndexRowFor` thread through to `Reference.create … with` / `Index.create … with`. Cluster-A1 cash-out complete. | ~~Deployed environment surfaces evidence~~ — emit + adapter closed | ~~150~~ |
| ~~5.13.emit-features-registry~~ | (blind-spot #1) | **SHIPPED 2026-05-18 (slice 5.13.blind-spot-closure).** `SsdtDdlEmitter.registeredMetadata : RegisteredTransformMetadata` with 11 classified Sites (one per V1-CreateTable + V1-CreateIndex feature V2 emits). `ManifestEmitter.build` prepends to `RegisteredTransforms.all` so totality-coverage scan reaches Emitter stage. 6 new `EmitterRegistrationsTests.fs` tests. | ~~Pillar 9 totality coverage expansion to Emitter stage~~ — closed | ~~50~~ |
| ~~5.13.render-stringbuilder-retirement~~ | (blind-spot #5 + #6) | **SHIPPED 2026-05-18 (slice 5.13.blind-spot-closure).** `Render.toSql`'s per-variant arms collapsed into a single `_ ->` default arm routing every SQL-bearing Statement through `ScriptDomBuild.buildStatement` + `ScriptDomGenerate.generateOne`. Four dead-weight helpers retired (`columnSqlType`, `formatSqlLiteral`, `actionSql`, `renderColumn`). Render.fs reduces to 4 public functions. | ~~Single source of truth via ScriptDom typed AST~~ — closed | ~~30~~ |
...
| 5.13.index-partition | 56 (residual) | `Index.DataSpace : DataSpace option` (closed DU: `Filegroup of name | PartitionScheme of name × columns`) + per-partition `PartitionCompression list`; adapter + emitter | Production OSSYS uses partitioned indexes V2 must preserve | ~250 |
...
3. ~~Ship `5.13.uat-cli` (`osm uat-users`)~~ **COLLAPSED 2026-05-30** — no standalone verb; the reflow is opt-in in `transfer --reconcile` + `full-export`. UAT dry-run needs only a live UAT connection (access-gated), no CLI build.
...
| C′.2b | **Live profiling wiring (deferred).** Apparatus→open-connection resolution shipped with Slice D (`ConnectionResolver` + `TransferConnections.create`). Threading `LiveProfiler` through the apparatus's `ProfiledForIdentity` into `Transfer.runReconciling` (Source + Sink profile) lands when the first consumer demands it. `--user-map` CSV loader (the `ManualOverride` source) **explicitly deferred** (2026-05-29) — operator framing: the C′.2a skip-and-diagnose default suffices for now. | 🔵 scheduled (deferred) |
| D | **`transfer` CLI verb (SHIPPED).** `projection transfer --source-conn <env\|file:ref> --sink-conn <env\|file:ref> [--reconcile <table>:<match-column>]... [--execute]`. DryRun default; `--execute` gated by `PROJECTION_ALLOW_EXECUTE=1` (R6) and refused otherwise (exit 7). `ConnectionResolver.resolve` reads the OOB secret only here (D9). `TransferSpec` parses the operator strings into typed `ConnectionRef` / `ReconciliationStrategy.MatchByColumn` values; the apparatus binds the substrates with `TransferConnections.create`. Pure tests: 6 (ConnectionResolver) + 16 (TransferSpec) green; binary smokes the R6 refusal and the missing-ref error paths. **Deferred per operator:** preview row cap, CDC-safety check, ManualOverride/CSV loader. | ✅ shipped (R6 gate live) |
| F | **Seam split + full-export consumer (SHIPPED).** Per the architectural framing (2026-05-29 operator note: the remap is generic and most-often-affiliated with full-export / static artifacts; transfer is one of N consumers — don't overlay seams too liberally), the generic identity-domain primitives now live in their own home: `Projection.Core.SurrogateRemap.fs` carries `SourceKey`/`AssignedKey`, `IdentityDisposition`, `SurrogateRemapContext` + module (state ops), `UnresolvedReference`/`RemappedRows`, and `module SurrogateRemap` (consumption: `fkColumnsTargeting`, `remapRowFks`). `Transfer.fs` slimmed to apparatus only; `Reconciliation.fs` slimmed to acquisition only. **(Superseded by Slice G — the per-emitter `OperatorIntent Insertion` site `staticRowSurrogateRemap` retired at the data-load algebraic convergence.)** | ✅ shipped (then retired by G) |
| G | **Data-load algebraic convergence (SHIPPED).** Five parallel research agents (2026-05-29) confirmed: (a) `IsUserFk` is a redundant cache of `TargetKind = userKindKey`; (b) every emitter shares the per-row → typed-row → MERGE/UPDATE algebra; (c) the off-by-one altitude was bolting substitution onto each emitter. Resolved from first principles: a data load is `(Plan, Realization)` where the Plan carries POST-substitution rows and Realization is a direction-specific functor `Plan → Effect`. Identity substitution belongs to plan-construction. **`Projection.Core.DataLoadPlan.fs`** (replaces `TransferPlan.fs`) is the unified plan — `build catalog topo rawRowsByKind remap` is **the one** `OperatorIntent Insertion` site (`identitySubstitution`) for the entire family. `Transfer.runReconciling` retires `Transfer.prepare`, consumes plan loads directly. `StaticSeedsEmitter.emitFromPlan` + `MigrationDependenciesEmitter.emitFromPlan` + `BootstrapEmitter.emitFromPlan` are pure realizations; `UserRemap.toSurrogate (userKindKey) (ctx) : Result<SurrogateRemapContext>` bridges chapter 4.2's User-typed remap onto the generic shape (bit-exact parity — MigrationDeps's bespoke `rewriteUserFkColumns` + `userFkColumnNames` retired). Retired registry sites: `staticRowSurrogateRemap` (Slice F), `userRemapRewrite` (MigrationDeps); `StaticSeedsEmitter` returns to `skeletonView`. Full pure pool (2412) + Docker pool (85, including the Dev→UAT User re-key dual-DB canary) green; A18-amended preserved (emitters consume `Catalog × Profile × DataLoadPlan`, never Policy); `migrationRowEmission` stays `OperatorIntent Insertion` (it classifies the *inclusion of operator-supplied rows*, distinct from their value-rewriting). | ✅ shipped |
...
| standing follow-ons (inherited, named at open/close) | The 82-site lint disposition; the two transfer-leg Docker reds (`StagedMergeDeployE2E`, `T18CycleBreak`); the eleven `SamplePr*` FS3511s blocking solution-wide Release builds — all July-arc debt, none moved by this chapter. | two of three discharged since: FS3511s cleared at align-I.1 (2026-08-16); the lint debt retired on main (2026-08-30 merge — the hook chain runs green, commits no longer carry `--no-verify`). The two Docker reds remain owed |
...
**THE ALIGNMENT PROGRAM (the geometric-alignment audit cashed out; three chapters, 2026-08-16..30):**

| Chapter | Cash-out | Status |
|---|---|---|
| align-I — Teleology (Arcs 0+T; CLOSED 2026-08-16) | The fired `OverlayAxis` trigger resolved (`Identity` appended; `axisOfPolicyAxis` total — A50 live); identity-plane passes reclassified `OperatorIntent Identity`; `SynthesisConvention` closed registry + the extension-marker single lift; sequence-lane convergence (one mint); `ChainStep.Requires/Produces` + skeleton honesty (11→7, exclusions voiced); typed `AnnotationDetail` for identity decisions; `FiringSite` on registry metadata; the config-provenance rule table + property. Plus the eleven `SamplePr*` FS3511 hoists (Release builds solution-wide). | shipped |
| align-II — the Ruling & the Epistemics (Arcs R+E; CLOSED 2026-08-16) | `OperatorRuling<'anchor>` carrier + keyed fail-closed store; tightening provenance threaded (Relaxation no longer severed); per-subject `UniqueIndexOverride` + `Direction`; abstain honesty (`DeclaredShapeCarried` retires the `KeepNullable` lie; `ProbeReading` trichotomy; `Unreadable` typed); FindingKey reception on the DECIDE lane + the `projection rule` verb (record/render only); `RowsetContract` drives the 26-walk; typed `BundleErasure` list at `toBundle`; `AcquisitionScope` stamped + compared at the S13 gate; journal read-side decode (`parseLine ∘ renderLine = id`); typed fingerprint readings + `Miss.FingerprintMoved`; finding `Pedigree`. | shipped |
| align-III — State, Law, Grain & Lexicon (Arcs S+L+H+X; CLOSED 2026-08-30) | S: `SyncOrdinal`/typed instants; `ChainAdmission` on `LedgerSpec` (the tautological fingerprint unwritable; `sink.journal.brokenChain`); `CanaryVerdict` typed + fail-closed `RunIndex` reading; `EstateHistory.replay` (latest.json = cache-of-fold); the `Lifecycle` dead twin DELETED; `DataObservation` (measured-zero ≠ unmeasured); rename-isometry predicate. L: A44 resident; A45/T17/A47/A48 live (94 live + 34 deferred); generator honesty (multiline+comment-aware buckets, `@ladder`/`@axis` validation); `TriggerProbes` (machine-evaluable deferrals); `L3-Eject` registered. H: `PhysicalTableRef`/`SchemaBasis` (fabricated `dbo` retired); `Environment.parse` total+canonicalizing; sink env labels → digest-set current-member resolution; `TemporalConfig` typed pair. X: `ForecastEvidence`, the two-register rule, `WitnessedRow` + the Witness- freeze, `Place`, `RunIndex`; the a11 voice stage-2 sweep (zero lazy plurals in src; full-tree ratchet). | shipped |
| program deferrals (each with its named trigger; the Active-deferrals index owns them) | X6 statement-algebra rehome (second non-SSDT consumer); `CanaryVerdict.Aborted` (first started-but-unconcluded canary); append-only ruling HISTORY (first re-ruling consumer; `BasisAnchor.SinkEdition` widens); `SpineLedger` rename (if the ledger-noun overload still stings); the full Sink→Witness B-family rename (L-effort, operator surface); the consent-surface voice ruling (cashed 2026-08-31 — the operator ruled "Approved"; the ratchet covers the whole src tree); `Supplied<'T>` (first mis-wiring the rule table catches); ruling auto-application. | deferred-with-trigger |

**The estate/fidelity + operator-walk arc (the cutover-reconciliation loops; built 2026-07-14..18, walked 2026-08-31):**

| Chapter / program | Cash-out | Status |
|---|---|---|
| The estate/fidelity chapter (CLOSED 2026-07-17/18; this table added at the walk close — the chapter predated the arc-table discipline) | `check environments` (the convergence instrument): the deployed↔deployed comparison regime, the estate board (rich + plain), the three named divergences (`DataNotNull`/`DataOrphans`/`DataStaticIdentity`) + D10 static-content repair, per-env FindingKey-keyed remediation artifacts (commented repairs, RT-12 provenance headers), the interim posture (overlay + reopen probes), pay-once evidence (E2 fingerprints), the burndown + streak (E8), CI envelopes (E9); `check fidelity <flow>` (T17): the B5 container proof, the B6 proof cache, the offline reconcile, business-key naming; RT-10 (the board reads the proof). | shipped |
| THE OPERATOR-WALK PROGRAM (walk-1..5; CLOSED 2026-08-31) | The §12 loop walked as ONE witnessed fact over a live two-cell estate through the REAL CLI faces (`OperatorWalkDockerTests` — the first driver `runCheckEstate` ever had): find the estate's own finding → repair FROM the artifact → converge → diverge → remediate → re-converge → publish the bundle (with `apply-runbook.md`) → prove fidelity → the board reads the proof; the WP-12/13 trigger discharged with evidence (the WP-13 probe: red board and refused publish name the same cycle; `fidelity.proof.applyFailed` lands the §14 half; WP-12's premise pinned executable); the consent-surface voice ruling cashed. | shipped |
| walk residuals (the §12 ease items still open; each S/M) | The `remediate` executor verb (E7 — deliberately unbuilt; the walk mechanizes the operator's own step); overlay merge preview/apply (E6); board scoping flags `--env`/`--only`/`--module` (E3 residue); `check data --rows` config defaults (F1); the count-first "excepted/residual" proof headline (F2); the fidelity stage-board spine (F3); `--kind`/`--module` re-proof scoping (F6); `init` scaffolding `fidelityFlow` (E1 residue). | open (named in the walk-5 close entry; no triggers — commission-on-demand ease work) |
...
| LR3 | `ScriptDomBuild.buildCreateTable` defer-candidates (~~column defaults~~ + ~~CHECK constraints~~ + computed columns + single-column PK inline optimization) | 12 + 53 + 182 (slice ζ residuals) | **PARTIALLY SHIPPED 2026-05-18 (slice 5.13.column-features-emit).** DEFAULT + CHECK clauses emit via `DefaultConstraintDefinition` + `CheckConstraintDefinition` typed-AST builders (rows 12 + 53 close to 🟢 PARITY). Remaining: computed columns (no V2 consumer pressure yet — `Attribute.Computed` populated by no rowset path or JSON adapter); single-column PK inline optimization (V1 67-77; today V2 always emits PK as table-level constraint). | V1's CreateTableStatementBuilder has these axes; V2 IR fields exist; emit-layer remaining bundle deferred per IR-grows-under-evidence | ~~200~~ 80 (remaining axes) |
...
| ~~LR6~~ | ~~`EmissionPolicy.IncludeTrustedIndicators` — flag for emitting `WITH NOCHECK` on FK constraints~~ | ~~59~~ | **EMIT-SIDE SHIPPED 2026-05-18 (slice 5.13.fk-features-emit).** Not as an EmissionPolicy flag — V2 emits `WITH NOCHECK` structurally when `Reference.IsConstraintTrusted = false`. Pillar 9 framing: trust state is `DataIntent` (preserves source reality), not `OperatorIntent` (Selection/Emission/Insertion/Tightening); no Policy axis needed. | ~~Deployed-target round-trip needs WITH NOCHECK preservation~~ — closed structurally | ~~80~~ |
| ~~LR7~~ | ~~`EmissionPolicy.IncludeUpdateActions` — flag for ON UPDATE referential actions~~ | ~~58~~ | **EMIT-SIDE SHIPPED 2026-05-18 (slice 5.13.fk-features-emit).** Same framing as LR6: ON UPDATE is `DataIntent` (source carries it), not Policy. `Reference.OnUpdate : ReferenceAction option`; emit by default when `Some action`. | ~~Modern T-SQL convention emits explicit ON UPDATE NO ACTION~~ — closed structurally | ~~80~~ |
