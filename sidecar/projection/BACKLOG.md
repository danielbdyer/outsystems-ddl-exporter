...
| C′.2 | **Live integration (next).** Wire reconciliation into the realization (`ReconciledByRule` kinds skip insert; phase-2 re-points reconciled FKs through the remap), live dual-environment profiling (Sink read first via the apparatus), `--user-map` CSV loader, dual-DB integration canary. | 🔵 scheduled (operator headline case) |
