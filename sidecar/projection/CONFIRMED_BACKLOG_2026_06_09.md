...
| J3 | **`legacy` / `preview` — the B→A reverse leg** (NOT foreign schema) — the logical on-prem model (B) piped up into the physical cloud (A); same `SsKey` model. The reverse-leg transfer (LE-1) + the **B→A round-trip canary** (LE-2). | **DONE (engine + canary)** (2026-06-09) | `TransferCanaryTests` `M3/LE-2` green via `runWithRenames` (the 6.B.2 two-contract path resolves table+column rendition by SsKey against the sink contract — no OSUSR generator needed). **Remaining (deferred):** M3.b flow-wiring (route a `legacy` flow to `runWithRenames`) + the live two-DB attribute-SsKey recovery (`ReadSide.buildAttribute`); the authored-model flow needs neither. |
| J4 | **Capability survey as advisory G0** — wire the survey into the run verbs (S3, per R6) + wire `Preflight.all` (= A1) so the survey feeds one composed gate; add the P10 user-directory probe. | **PARTIAL** | **G0a DONE (= A1 closed):** `Preflight.all` now has its first production caller — `runCore`'s pre-plan CDC→spanning gates route through it (codes/precedence preserved; CDC canary + AC-I5/PE-1/classify green). **Deferred:** G0b P10 probe; G0c advisory-survey surfacing (R6: warn, never hard-stop). PREFLIGHT M4. |
