...
let private dispatchFullExport (argv: string[]) : int =
    argv |> VerbArgs.parse<FullExportArg> "projection full-export" (fun parsed ->
            ...
            if not (List.isEmpty muteErrors) then
                ...
                let run () = runFullExport configPath outputOverride verbosity mutedCategories storePath envLabel
                // --watch + a real TTY → the live stage board (§13); else the
                // unchanged path. The board is a rendering of the same LogSink
                // stage stream the run already emits.
