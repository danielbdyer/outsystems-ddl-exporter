...
let private usageLines : string list =
    [
        ...
        "  --against model; evidence rides the pay-once store when fingerprints hold —"
        "  [--refresh [env,…]] forces the re-capture, [--offline] reuses unprobed and"
        "  downgrades to advisory; [--format json] → estate.json; exits 0 unified / 5 / 6)."
        ""
