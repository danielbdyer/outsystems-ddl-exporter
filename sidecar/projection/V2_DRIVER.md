...
| **OSSYS metadata plane (the sink)** | Post-cutover retrieval impossible: a deleted entity's shape unrecoverable, physical-table claims unadjudicable, no offline model read, no temporal diff of the estate's registrations | K2 total `Catalog` parity (a sink read replays THE acquisition); T19 journal FTC + the erasure-witness inequality; A49 three-way selection law; contested-always adjudication property; the correspondence proposer's proposal-IFF-adopted-over-tombstones property | **Shipped** (the data-sink chapter, closed 2026-08-16): every live total read witnesses; `projection sync <env>` names environments; claims / residue / correspondence ride the estate board's DECIDE lane; offline operation via `sink.policy`; the eject carries the terminal states. |
| **Vocabulary/law alignment (the geometric-alignment program)** | Vocabulary drift forecloses outcomes silently: knowledge decays into prose/strings/convention, intent loses its carrier, laws go non-executable, and the outcome space stops being enumerable | The executable registries: the verifiability gate (94 live + 34 named-deferred witnesses, zero unclassified); A50 axis-totality; the bidirectional registry properties; `TriggerProbes` (machine-evaluable deferral triggers); the full-src voice ratchet | **Shipped** (THE ALIGNMENT PROGRAM, three chapters align-I/II/III, closed 2026-08-30): typed rulings/erasures/scope/state, laws resident and live, grain repaired, the lexicon renamed to its domain; every remainder is a named deferral with its trigger in the Active-deferrals index. |
| **Estate convergence & the operator walk (the §12 loop)** | The cutover-reconciliation loop exists only as separately-proven seams: an operator cannot actually walk check → remediate → burndown → publish → prove on a live estate, divergences drift unnoticed between environments, and a repaired estate cannot show its own convergence | The terminus witness (`OperatorWalkDockerTests`): the §12 loop end-to-end through the REAL CLI faces over a live two-cell estate — find the estate's own finding, repair it FROM the keyed artifact, converge with the burndown, diverge, remediate, re-converge, publish the bundle, prove row fidelity, and the board reads the proof (RT-10); plus the WP-13 probe (red board and refused publish name the same cycle) | **Shipped** (the estate/fidelity chapter closed 2026-07-17/18 built the loops; THE OPERATOR-WALK PROGRAM, closed 2026-08-31, walked them as ONE witnessed fact). The remaining §12 ease items are S/M-sized and enumerated in the program-close DECISIONS entry. |
...
| Phase 2 (closed) | **Chapter 4.1.A + Tolerance + multi-env** | SsdtDdlEmitter (production output); Tolerance taxonomy; multi-environment promotion property test | **shipped**. Chapter 4.1.A close arc + slice 8 reopen (2026-05-17). Cross-module FK ordering at emit layer; identity + defaults; ExtendedProperties + Descriptions emission; Tolerance taxonomy slices α landed; `CommentMetadataUnreflected` retired at slice 8. | ~600–900 | ✓ |
| **Phase 3 (closed; highest-stakes deliverable shipped)** | **Chapter 4.1.B (CDC-critical)** | StaticSeedsEmitter; MigrationDependenciesEmitter; BootstrapEmitter; **CDC silence on idempotent redeploy property test (highest-stakes deliverable)** | **shipped**. Chapter 4.1.B closed 2026-05-11; slices α through κ shipped end-to-end (CDC-silence canary GREEN at γ; two-phase insertion / cycle-breaking at δ; MigrationDependenciesEmitter at ε; BootstrapEmitter at ζ; DataEmissionComposer + EmissionPolicy.DataComposition DU at η; partition assertion at θ; multi-kind global Phase-1-then-Phase-2 reification + typed Values lift at ι/κ). Tier-3 hard-requirement deferral cashed at slice ε. See `CHAPTER_4_1_B_CLOSE.md`. | ~1,500–2,000 | ✓ CDC-silence property green at canary scale |
| **Phase 4 (closed)** | **Chapter 4.2** | UserFkReflowPass; UserMatchingStrategy DU; UserRemapContext; SourceTag value-object refactor of SsKey identity DU; per-strategy coverage (ByEmail / BySsKey / ManualOverride / FallbackToSystemUser) | **shipped**. Chapter 4.2 closed 2026-05-15; slices α through η shipped (UserMatchingStrategy DU + identity types; UserPopulation on Profile; UserRemap.fs + UserRemapContext smart-constructor disjointness invariant; UserFkReflowPass.discover all four strategies real; IsUserFk on Reference; MigrationDependenciesEmitter user-FK column rewrite; multi-environment commutativity property test as chapter signature deliverable). **A32 cashed out at this close.** Two deferrals: OSSYS adapter User-kind identification surface; CSV adapter for ManualOverride. See `CHAPTER_4_2_CLOSE.md`. | ~800–1,200 | ✓ Multi-env commutativity green |
| **Phase 5 (closed structural surface)** | **Chapter 4.3** | DecisionLogEmitter; OpportunitiesEmitter; ValidationsEmitter; three-channel split (operator / auditor / developer) activated under real consumer pressure | **shipped (structural slice arc α/β/γ)**. Chapter 4.3 closed; DecisionLogEmitter (slice α) + Routing primitive + OpportunitiesEmitter (slice β) + ValidationsEmitter + Routing partition property as chapter signature deliverable (slice γ). New `Projection.Targets.OperationalDiagnostics` project. Three-channel deferral retired. Slices δ (CLI wire-up) + ε (V1 differential) deferred-with-trigger. See `CHAPTER_4_3_CLOSE.md`. | ~600–800 | ✓ Per-channel routing partition property green |
| **Phase 6 (closed under dev-tooling reframe)** | **Chapter 3.x DacpacEmitter** | DacFx adoption; DacpacEmitter; A37 named erasure axes; T1 binary-normal-form amendment | **shipped under dev-tooling reframe**. Chapter 3.x closed; structural slice arc α/β/γ/δ_dock shipped. `Microsoft.SqlServer.DacFx` v162.x adopted (Tier-3 `text-builder-as-first-instinct` deferral cashed out). DacpacEmitter ships as dev-tooling consumption surface (sqlpackage / Visual Studio / DacServices.Deploy); production deploy stays SSDT-file. DockerImageEmitter ships the one-command dev stand-up artifact. Slices ε (modality marks → comments/extended properties) + ζ (byte-determinism cash-out) + per-Catalog parameterization deferred-with-trigger. See `CHAPTER_3_X_CLOSE.md`. | ~800–1,200 | ✓ DacFx round-trip green |
| **Phase 7 (closed)** | **Chapter 3.2 SnapshotRowsets** | SnapshotRowsets adapter variant; closes JSON-projection-lossiness; A1 holds unconditionally; lifts `ICatalogReader` port (Position B → A); Diagnostics-emission scaffolding for V1 silent drops | **shipped**. Chapter 3.2 closed 2026-05-10. Five slices: SnapshotRowsets variant + RowsetBundle; reference rowsets; EspaceKind activation; IsSystemEntity → ModalityMark.SystemOwned; cross-source parity. A1's JSON-projection-lossiness bound structurally resolved. See `CHAPTER_3_2_CLOSE.md`. | ~600–900 | ✓ A1 unconditional bound preserved |
...
**Current state as of 2026-05-17 (post-chapter-A.4.7' close).** Phases 1–7 are all closed end-to-end. Critical-path V2-driver structural commitments shipped: Π port keystone (Phase 1); SSDT DDL emitter + Tolerance taxonomy (Phase 2); CDC-aware data triumvirate + CDC-silence-on-idempotent-redeploy canary green (Phase 3, the highest-leverage single deliverable); UserFkReflowPass + multi-environment commutativity property (Phase 4); three-channel Diagnostics split + Routing partition property (Phase 5); DacpacEmitter + DockerImageEmitter under dev-tooling reframe (Phase 6); SnapshotRowsets adapter variant (Phase 7). Plus the post-V2_DRIVER architectural chapters: chapter A.0' (IR fidelity lifts; 10 L3 axioms D → A); chapter A.4.7 (TransformRegistry shipped; A41 cashed; L3-CC-Transform-Totality D → A); chapter A.4.7' (Compose.run registry-driven; A41 amended for execution totality). **The V2-driver structural surface is operationally complete.** What remains is Phase 8 (pragmatic close; consumer-pressure-driven) plus the deferred-with-trigger items per phase-close docs (chapter 4.3 slices δ/ε; chapter 4.2 OSSYS user-kind ID + CSV adapter; chapter 3.x slices ε/ζ; Module.ExtendedProperties emission; Sequence emission; chapter 4.4 Manifest diagnostic fields — Coverage / PredicateCoverage / PreRemediation / Unsupported still emit defaults; etc.).
