...
**Phase 0.5 — Bridge wave: V2 inherits from V1 (chapter 0.5; prerequisite for Phases 3/4/5 parallelization).** ~1 session for the bring-up. The V1↔V2 seam transitions from data boundary to inheritance phylogeny. Two new C# projects (`Projection.Bridge.Core`, `Projection.Bridge.Runtime`) make the inheritance machinery structural; F# adapters consume `Bridge.Core` to inherit V1 capabilities under a four-position gradient (`Delegated` → `Vendored` → `RefinedInPlace` → `TranslatedToFSharp`). The reflection-scanned `BridgeManifest` is the auditable witness; the `Projection000BridgeWallDiscipline` analyzer enforces the eight wall rules structurally. Seven slices (α audit primitives; β analyzer; γ first lift `ExtractMetadataAsync`; δ F# consumption; ε first adoption `Vendored`; ζ equivalence property test; η V2-for-V1 first method closing V1's static-seed FK-order bug). Per `DECISIONS 2026-05-16 — Bridge wave: V2 inherits from V1` + `CHAPTER_0_5_OPEN.md`. Once this closes, Phases 3/4/5 can ship in parallel — each chapter consumes a piece of the inheritance surface for its own deliverables (Phase 3 lifts `GenerateMergeInsert` / `ReadCdcRows`; Phase 4 lifts `MatchUsers`; Phase 5 stays pure V2 algebra). The Bridge wave reduces critical-path LOC by ~25% on Phases 3+4 and parallelizes them; sunset is structurally enforced at cutover+30 via `BridgeManifestSunsetGateTest`.
