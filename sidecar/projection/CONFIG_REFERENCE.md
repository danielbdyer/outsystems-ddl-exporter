...
| `bridgeRetargets` | array | `[]` | `[{ "id": "user-createdby", "entity": { "module": "M", "entity": "Child" }, "relationship": "CreatedBy", "bridge": { "module": "M", "entity": "Bridge", "attribute": "Upn" } }, …]` — retarget a foreign key to resolve THROUGH a bridge attribute (a named unique column on a second table) instead of its original parent's primary key; the child FK value is unchanged, only the constraint target moves. Opt-in (`policy.transformGroups: ["bridgeRetarget"]`) + signoff-gated (`emission.signoff` mode `"bridge-retarget"`). Each retarget stays BLOCKED (byte-identical, `RetargetFk` empty) until its data-half readiness is proven via `bridgeRetargetEvidence`. Format owner: `BridgeRetargetBinding.fs` |
