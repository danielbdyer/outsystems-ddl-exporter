...
| `bridgeRetargets` | array | `[]` | `[{ "id": "audit-bridge", "reference": <ref>, "bridge": { "module": "M", "entity": "Bridge", "attribute": "BizKey" } }, …]` where `<ref>` is **either** one explicit FK `{ "module": "M", "entity": "Child", "relationship": "CreatedBy" }` **or** DISCOVERY `{ "allReferencesTo": { "module": "M", "entity": "Parent" } }` — every reference in the resolved `model.modules` scope whose target is that entity, expanded to one plan per match with ids `<id>/<Module>.<Entity>.<Relationship>` (deterministic; matching nothing is a named refusal, never a silent no-op) — retarget a foreign key to resolve THROUGH a bridge attribute (a named unique column on a second table) instead of its original parent's primary key; the child FK value is unchanged, only the constraint target moves. Opt-in (`policy.transformGroups: ["bridgeRetarget"]`) + signoff-gated (`emission.signoff` mode `"bridge-retarget"`). Each retarget stays BLOCKED (byte-identical, `RetargetFk` empty) until its data-half readiness is proven via `bridgeRetargetEvidence`. Format owner: `BridgeRetargetBinding.fs` |
| `bridgeRetargetEvidence` | object | — | `{ "path": "overrides/bridge-retarget-evidence.json" }` — the operator-supplied DATA-half profiling for `bridgeRetargets` (the facts a catalog inspection cannot know): `{ "retargets": [ { "id", "unresolvedThroughBridge", "brokenOriginalParent", "orphanedBridgeRows", "payloadConflicts", "bridgeKeyDuplicates", "bridgeKeyNulls", "identityEvidence": "present\|missing\|ambiguous" } ] }`. Id-keyed to a declared retarget; OMITTED blocking counts default fail-closed (`1`) so a partial entry keeps the retarget blocked; absent ⇒ every retarget blocked (byte-identical). A retarget CLEARS only when its evidence proves the data half (all blocking counts `0`). Worked example: `examples/bridge-retarget-evidence.sample.json`; format owner: `BridgeRetargetBinding.fs` |
| `bridgeRowStaging[].cache` | string | `"off"` | The acquisition cache — the quick-iteration lever. `"off"` reads the live estate every run (byte-identical default). `"auto"` reuses the cached acquisition when BOTH the declaration digest AND the involved kinds' estate fingerprints (row count / max-pk / content hash) still match — any movement re-acquires. `"pinned"` reuses WITHOUT probing (fastest; visible staleness acceptance — `cache.json` says so), though a changed declaration still refreshes. Storage: `<store>/evidence/bridge-staging/<id>.json` under `PROJECTION_ESTATE_DIR` / `PROJECTION_LEDGER_DIR` (unset ⇒ live-only, named in `cache.json`). Delete the entry (or set `"off"`) to force re-acquisition |
| `bridgeRowStaging[].scope` | string | `"referenced"` | `"referenced"` derives bridge rows only for key values the in-scope retargeting FK columns actually reference (minimal blast radius; sound for the FK, since an unreferenced source row needs no bridge row). `"allSourceRows"` covers EVERY source row, so a later child insert naming any source row cannot violate the retargeted FK. Unrecognized values are refused |
| `bridgeRowStaging` | array | `[]` | `[{ "id": "person-directory", "source": { "module": "M", "entity": "Person", "key": "Id", "identity": "ExternalRef" }, "bridge": { "module": "M", "entity": "PersonDetail", "key": "PersonId", "identity": "ExternalRef" }, "insertMappings": [ { "bridge": "FullName", "from": "Name" } ], "insertConstants": [ { "bridge": "OriginId", "value": "1" } ] }, …]` — the DYNAMIC staging companion for `bridgeRetargets`: derives the bridge-row supply from the LIVE estate instead of a pre-authored supplement. `source`/`bridge` name the two tables in logical `{module,entity}` (espace-safe) or physical `{schema,table}` form, each with its `key` + `identity` attributes; scope auto-links the declared retargets (reference parent = source kind, bridge attribute = the staging bridge key). Per run: referenced keys (distinct non-null retargeting-FK values) → source/bridge snapshots → pure per-key delta (already complete / stage guarded INSERT / stage FILL-ONLY identity update / named block) → staged SQL + audit sextet under `BridgeStaging/<id>/` (incl. `staged.sql` + `unstage.sql`, the exact guarded inverse of the same recorded plan — run it BEFORE the retargeted DDL lands) in the bundle → planned-state evidence for the linked retargets (an id may not also appear in `bridgeRetargetEvidence` — competing sources are refused). Fail-closed: any block stages nothing and leaves the linked retargets unproven; no existing non-null bridge cell is reachable by a staged statement (never the full-row MERGE path). Needs the live source connection (`PROJECTION_MSSQL_CONN_STR`); signoff-gated (`emission.signoff` mode `"bridge-row-staging"`). Format owner: `BridgeRowStagingBinding.fs` / `BridgeRowStagingSeam.fs` |
