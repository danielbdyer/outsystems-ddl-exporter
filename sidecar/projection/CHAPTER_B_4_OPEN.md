...
| Slice | Scope | Status |
|---|---|---|
| **1** B.4.1.logging-format-contract | `sidecar/projection/docs/logging-format.md` — define V2's structured event categories (extract / profile / emit / canary / deploy / config), event properties (level, code, source, classification per pillar 9, lineage trail-ref, diagnostics route-prefix), encoding (UTF-8; line-delimited JSON; one event per line); cross-platform deterministic ordering. L3-Operational-LoggingContract promotes D → A. The doc lands BEFORE downstream subcommands consume the contract; subcommands at slices 6-8 emit events conforming to it. | not-started |
...
| **3** B.4.3.composite-pk-fk | Cash out the slice-1 `ForeignKeyReality` composite-PK deferral (`Outcome = AmbiguousMapping` for non-single-column-PK targets). Slice-6b's `projectTupleKeys` makes the multi-column Set.difference trivial. Add composite-PK FK fixture to `LiveProfilerIntegrationTests`; FK reality now resolves for every Reference regardless of target-PK arity. The cross-module FK reserved IR refinement deferral (separate, still open) is not affected. | not-started |
...
**Bonus close item**: L3-Operational-LoggingContract bucket-promotion confirmation. Slice 1 is the structural promotion; chapter close confirms it remains in Bucket A (the doc landed; CLI subcommands emit conforming events; tests verify).
...
6. **`PRODUCT_AXIOMS.md` L3-Operational-LoggingContract entry** — verify the axiom's current statement before slice 1 codifies; if any drift, fix at slice 1.
