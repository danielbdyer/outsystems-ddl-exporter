...
**L3-CC-Transform-Totality. Every artifact V2 emits decomposes into a registered baseline plus a registered overlay; both halves are recoverable, enumerable, and audit-traceable.** For any successful `Compose.run` invocation producing `Outputs` *O*, there exist (a) a **baseline** projection `Project(catalog, Policy.empty, profile) = O_skeleton` reachable from the same inputs without any operator-supplied policy intent, and (b) an **overlay** sequence — the ordered, named set of `Pass` invocations enumerated in `TransformRegistry` whose `LineageEvent` trail composes `O_skeleton` into *O*. Every applied transform is registered; every registered transform that fired emits its lineage; the manifest names the registry digest alongside the artifact list. Operators can ask for `--skeleton-only` and get the baseline; operators can ask for the full output and audit which overlays were applied.

  *Underwriting.* A18 amended (Π consumes Catalog × Profile, never Policy) + A25 (every transformation runs inside `Lineage<_>`) + A39 (aggregate-root smart constructors) + **A41 candidate** (Transform registry totality — see `AXIOMS.md` Amendments scheduled, A.4.7 close). The factual/objective half is structurally already enforced (A18); this axiom names the **operator-facing decomposition** as a product promise, and the registry-totality half as its missing structural commitment.
  *Failure mode.* A policy-driven mutation slips into the baseline (A18 leak at a new pass that bypasses the writer); operators can't tell which overlay touched what; CDC silence is asserted on the wrong baseline (rendering the highest-leverage canary deliverable meaningless); v1↔v2 differential disagreements become unattributable — was the divergence skeleton drift or overlay omission? The named precondition for laboratory-quality outcomes as the project scales: every transformation visible, registered, type-witnessed, recoverable.
  *Realization.* `Projection.Core.TransformRegistry` enumerates every `Pass` module by domain (`<Domain>Rules` discipline); `Pass.skeletonBaseline` exposes the `Policy.empty` invocation as a first-class value; `Compose.runWithSkeleton` emits the baseline only; `Compose.runWithOverlay` emits baseline + named-overlay set; `ManifestEmitter` includes a `registry.digest` + per-artifact `applied-transforms : SsKey list`. Coverage property: `TransformRegistryCompletenessTests.``L3-CC-Transform-Totality: every pass emitting a LineageEvent is registered`` ` asserts `discovered ⊆ registered` AND `registered ⊆ exercised in canary` — fails the build on either inclusion gap.
  *Tier.* **1 (cutover blocker).** Without it, the CDC-silence baseline is convention, not structure; canary-modulo-tolerance becomes canary-modulo-uncatalogued-mutation; the operator's promise — "ask for the vanilla projection and I get a deterministic factual baseline; every override I apply is named and recorded" — has no enforcement seam. **Bucket: D (candidate; promotes to A on A.4.7 close per `V2_PRODUCTION_CUTOVER.md` §6.4.7).**
