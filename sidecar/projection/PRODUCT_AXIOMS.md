...
**L3-CC-Transform-Totality. Every transformation site in V2 carries an explicit classification — `DataIntent` (preserves data intention; lands in the skeleton) or `OperatorIntent of OverlayAxis` (operator-supplied intent through Selection / Emission / Insertion / Tightening; lands as registered overlay) — and the classification is enforced bidirectionally at runtime.** For any successful pipeline invocation producing `Outputs` *O*:

  - There exists a **skeleton** projection `Project(catalog, Policy.empty, profile) = O_skeleton` reachable from the same inputs without operator opinion applied. The skeleton consumes Profile evidence (`DataIntent` observations like null counts, FK orphan rows, distribution percentiles); the skeleton emits zero `LineageEvent` classified `OperatorIntent` (the **skeleton-purity property**).
  - There exists an **overlay** sequence — the ordered, named set of `RegisteredTransform` invocations whose `Classification = OperatorIntent (Overlay axis)` compose `O_skeleton` into *O*. Every registered `OperatorIntent` transformation fires in at least one canary scenario (the **overlay-exercise property**); the manifest names the registry digest alongside each artifact's `applied-transforms`. Operators can ask for `--skeleton-only` and get the skeleton; operators reading the full output's manifest see every overlay that touched every artifact, classified by `OverlayAxis`.

  The classification is the *outcome of agent harvest analysis at consideration time*, not a property the code wears. The registry is the structural enforcement seam; pillar 9 (`DECISIONS 2026-05-15 (late) — Pillar 9`) is the meta-discipline that makes the classification happen before the transformation enters V2 thinking.

  *Cross-cutting concern.* TransformRegistry is the fourth structural-evidence-layer sibling — alongside **Lineage** (every decision), **Diagnostics** (every findable observation), **Bench** (every loop / hot path). Each plugs into every stage of the pipeline algebra that has its kind of activity; each is enforced structurally; each has its own writer/observer primitive. TransformRegistry plugs into the five stage seams (`Adapter | Pass | OrderingPolicy | Emitter | Pipeline`) and exposes `RegisteredTransform<'In, 'Out>` as the strongly-typed surface carrying both metadata AND the transformation-function definition itself. The single-definition-site discipline (per Q3 answer; no parallel enumeration) means each pass module's primary public surface IS its `<PassName>.registered : RegisteredTransform<'In, 'Out>` value.

  *Underwriting.* A18 amended (Π consumes Catalog × Profile, never Policy — the structural commitment for the Π side) + A25 (every transformation runs inside `Lineage<_>`) + A39 (aggregate-root smart constructors) + **A41 candidate** (Transform registry totality; the structural commitment for the Pass side — see `AXIOMS.md` Amendments scheduled, A.4.7 close). A18 + A41 are sibling structural commitments: A18 forbids `Policy` in emitters; A41 enumerates every transformation site that DOES consume `Policy` (or other operator-supplied intent) so the dichotomy holds bidirectionally.
  *Failure mode.* **Skeleton-overlay drift, three sub-modes:** (1) a transformation classified `DataIntent` when it's actually `OperatorIntent` (policy-driven mutation leaks into the skeleton) — caught by the skeleton-purity property test; (2) a transformation classified `OperatorIntent` that never fires (dead overlay; mis-registration) — caught by the overlay-exercise property test; (3) a v1 transformation harvested without classification (silent inclusion) — caught by the harvest-classification coverage test. The downstream cost of any of these: CDC silence asserted on an unenumerated baseline (the highest-leverage canary deliverable becomes hollow); v1↔v2 differential disagreements become unattributable; operators can't tell which overlay touched what; the laboratory-quality scaling precondition fails as the codebase grows.
  *Realization.* `Projection.Core/TransformRegistry.fs` defines `RegisteredTransform<'In, 'Out> = { Name; Domain; StageBinding; Sites : TransformSite list; Run : 'In -> Lineage<Diagnostics<'Out>>; Status : Active | NotImplementedInV2 of rationale }`. Each pass module exposes `<PassName>.registered`; `Compose.run` traverses `TransformRegistry.allInStageOrder` invoking each `registered.Run` at the appropriate 5-stage seam; `Compose.runWithSkeleton` filters the traversal to `Classification = DataIntent` (and `Sites` containing only `DataIntent` entries). `LineageEvent` gains a `Classification` field (registry-authoritative; per Q3 — the registry is canonical, LineageEvent looks up via `PassName` ↔ `registered`). `ManifestEmitter` includes `registry.digest` + per-artifact `applied-transforms : (SsKey × OverlayAxis option) list`. CLI surface: `osm emit --skeleton-only` invokes `Compose.runWithSkeleton`; binary toggle at A.4.7 with granular per-axis toggling deferred-with-trigger.
  *Property tests (bidirectional).*
   - `` ``L3-CC-Transform-Totality: Compose.runWithSkeleton emits zero OperatorIntent LineageEvents`` `` (skeleton-purity).
   - `` ``L3-CC-Transform-Totality: every registered OperatorIntent transformation fires in canary`` `` (overlay-exercise).
   - `` ``L3-CC-Transform-Totality: every transformation site is in TransformRegistry`` `` (totality — scan `Projection.Core/Passes/*.fs` + adapter rules + emitter strategies; assert each maps to a registered entry).
   - `` ``L3-CC-Transform-Totality: every Tolerance entry naming a v1 transformation references a NotImplementedInV2 registry entry`` `` (harvest-classification coverage; ties to the triple-deliverable harvest workflow).
  *Tier.* **1 (cutover blocker; co-equal with CDC silence as load-bearing structural enforcement).** Without it, the CDC-silence baseline is convention rather than structure; canary-modulo-tolerance becomes canary-modulo-uncatalogued-mutation; the operator's promise — "ask for the vanilla projection and I get a deterministic factual baseline; every override I apply is named, classified, and recorded" — has no enforcement seam. **Bucket: A (promoted at chapter A.4.7 close, 2026-05-16, per `DECISIONS 2026-05-16 (chapter A.4.7 close)`).** TransformRegistry + Classification + 18 site registrations + bidirectional property tests + intentional-fail probes shipped across slices α–θ. Manifest digest round-trip property + the runtime `Compose.runWithSkeleton` traversal land at slice η (consumer-pressure deferred).

  *Discipline relationship.* The structural surface enforces what pillar 9 ensures gets done correctly at harvest time. Pillar 9 (`DECISIONS 2026-05-15 (late) — Pillar 9: harvest-dichotomy classification`) is sibling to pillar 8 (domain-first naming catches *naming drift*), the LINT-ALLOW substantive-rationale discipline (catches *string-composition drift*), and the text-builder-as-first-instinct discipline (catches *typed-AST-bypass drift*) — each is a meta-discipline applied at consideration time with structural test enforcement. Four meta-disciplines; four cross-cutting concerns; four classes of failure that scale with codebase growth.
