...
| 3 | `Face` combinator + split `RunFaces` | 🟥 | ✅ **resolved** | All three concerns closed. **Full file split (2026-06-28): `RunFaces.fs` (2699 lines) is DELETED — every face now lives in its own `Faces/*.fs` (13 verb-family files + the `Faces/Common.fs` spine):** Slice / Diff / Inspect / Emit / Canary / Approve / Operational / Explain, then the data-movement core — Migrate / Export / Deploy / Transfer / Synthetic — on the `Common` spine (`nameOf` + the `Face` combinator: `run` / `watchInline` / `staged`). `narrateTransferReport` + `dispositionName` live in `Faces.Transfer`; `Synthetic` opens `Transfer` + `Migrate`. Verbatim relocation, zero behavior change; the lone `#nowarn "3511"` (the tightening `task{}` check) narrowed from file-wide to `Faces.Migrate` alone. `Program.runPlan` stays the one dispatcher. Concern 1 (CliExit) + Concern 2 (slice→typed Intent, `argv.[0]` dispatcher deleted) as before. Build clean (Debug+Release); pure 3748/0; docker 273/0 (reverse-leg streaming + migration canaries + transfer resume re-proven through the moved faces). |
...
> **Status (2026-06-28):** ✅ **Resolved — all three concerns closed; `RunFaces.fs` is DELETED.** **The full split (2026-06-28):** the 2699-line `RunFaces.fs` wall is gone — every face lives in its own `src/Projection.Cli/Faces/*.fs` over the `Faces.Common` spine (`nameOf` + the `Face` combinator `run`/`watchInline`/`staged`, lifted out of the wall as `[<RequireQualifiedAccess>] module Face`). Thirteen verb-family files + the spine: the self-contained verbs (Slice / Diff / Inspect / Emit / Canary / Approve / Operational / Explain) and the cohesive data-movement core split too — **Migrate** (preview/from-store/execute/with-data + the inexpressible/stopped/undeclared-drop renderers + `tighteningPreflight`), **Export** (full-export + load), **Deploy** (compose+docker, the `DeployStop` channel), **Transfer** (transfer + reverse-leg, owning `narrateTransferReport` + `dispositionName` + `narrateDropExit`), and **Synthetic** (project-live preview / synthetic-load / capture-profile / propose-correction, which `open`s `Transfer` + `Migrate`). Compile order: `Common` → `Migrate` → `Export`/`Deploy`/`Transfer`/`Synthetic` → the rest, then `Program`. Module names dodge the `Projection.Pipeline.{Deploy,Transfer}` collision because each face `open`s `Projection.Pipeline` last (the opened module shadows the namespace sibling). All relocations verbatim — zero behavior change. The file-wide `#nowarn "3511"` was narrowed to **`Faces.Migrate` alone** (the only surviving await-in-branch `task{}`, the tightening check); Export/Deploy/Transfer/Synthetic need no suppression. Tests updated: `Faces.Transfer.runReverseLegTransfer` (ReverseLegBoundaryTests), `Faces.Transfer.narrateTransferReport` (NarrationDisplayTests), `Faces.Migrate.tighteningPreflight` (TransferCanaryTests). Stale `RunFaces` doc-refs in `Comparison.fs`/`OperatorConsole.fs` retargeted. Build clean Debug **+ Release**; pure pool **3748/0**; docker pool **273/0** (reverse-leg 100k streaming + migration canaries + transfer resume all re-proven through the moved faces). **Concern 2 (2026-06-28):** the four slice data-portability verbs (`slice-extract`/`slice-apply`/`slice-reset`/`slice-run`) are lifted onto the typed surface — new `Intent.SliceExtract`/`SliceApply (reset, …)`/`SliceFlow` cases, `Command.parse` + `Command.plan` arms, `PlanAction.RunSlice*`, and `runPlan` dispatch — and the `argv.[0]` second dispatcher in `Program.main` is **deleted**: the CLI now runs ONE dispatch plane. Following the codebase's deferred-parse convention (`Check`/`Explain`/`Profile` carry raw `args`), the slice faces keep their bespoke flag/config parsing (which does run-time I/O the pure plan can't), so the action carries the args the way `PublishBundle` carries an unresolved config path. New `MovementSurfaceTests` pin all four routings; build clean, pure pool 3744/0. **Concern 1 closed (2026-06-27):** the 6 hand-rolled `anyCode` exit-ladders are collapsed into one total, test-pinned `CliExit` classifier (`src/Projection.Cli/CliExit.fs`) that delegates gate-axis codes to `Preflight.classify` and owns the CLI-specific `slice.*`/`synthetic.*`/`profile.*`/`correction.*`/`model.*` vocabulary. Layering note: the recon's "register in `classify`" sketch was realized CLI-side — exit-1 artifact-write and exit-2 input-parse codes are *not* pre-flight gates, so cramming them into the `GateLabel` DU would be a category error; the classifier instead *delegates* gate codes to the one `Preflight` seam, so the gate convention keeps a single definition. The three latent divergences the collapse surfaced were reconciled onto that convention (the complete fix, not preserved-as-found): `synthetic.cdcTrackedSink` 7→**9**, `slice.apply.grantProbeFailed` 6→**7**, and the two slice verbs now agree on the `slice.apply.*` sub-codes (`slice-run` no longer flattens the prefix to 6). Aggregation keys off the *primary (first)* error, matching `Preflight.refusalOf` (the six ad-hoc tier-scans were mutually inconsistent, so no single scan reproduced all six; first-error is the seam-aligned rule and is identical to the old `anyCode` on the common single-error refusal). Build clean; pure pool 3723/0 incl. 23 new `CliExitTests`. *(All "Open" items above are now closed — the per-verb file split landed in full, see the top of this Status.)*
...
> **Status (2026-06-28):** ◑ **Partial — the two headline items closed; the prose tail remains.** **(1) The two dispatchers are unified.** `errorFrame` was a 12-branch ordering-dependent `if/elif` carrying its operator copy INLINE, unreachable by any totality (the recon's "second dispatcher"). It is now `classifyError : code → ErrorFrame` (the prefix routing — ordering matters here and ONLY here, for the `transfer.connection.{spec,ref}` ⊂ `connection` subset, documented inline) composed with `frameCopy : ErrorFrame → View*View option` (TOTAL over the new closed `ErrorFrame` DU — the §10/§14 sibling of the §5 `gateStatement` over `Preflight.GateLabel` and `migrationStopDetail` over `MigrationError`). Behavior is byte-identical; new `VoiceTotalityTests` pin (a) `frameCopy` is register-clean over every closed-DU case and (b) the ordering-shadow (a `transfer.connection.*` code routes to the ARGUMENT frame, not the generic unreachable). **(2) The duplicated drop-warning + the transfer/migrate verdicts are voiced.** The drop refusal — duplicated VERBATIM across the transfer and synthetic faces (the recon's "one violation of say-prose-once") — collapses to one `transfer.rowsDropped`; the move verdicts `transfer.previewPlan` / `transfer.applied` and the migrate execute-leg `migrate.applied` / `migrate.verificationFailed` are voiced, routed through `TtyRenderer.renderVoicedTo`, and registered in `Voice.all` + the three `VoiceTotalityTests` code-sets (`inScopeCodes`, `knownEmittableCodes`, `renderVoicedCallSiteCodes`). The load-plan TABLE stays a face-side structured disclosure (per-kind — not a single-code payload; voicing it would trade cohesion for catalog-count). Build clean Debug + Release; pure pool 3750/0, docker pool 273/0. **Open (the prose tail, lower value):** the `approve` record/store-error lines, the `inspect` store-missing / run-not-found refusals, the migrate-WITH-DATA leg verdicts, and the tightening-relax acknowledgments — located refusals / narration whose voicing is mechanical but whose register-drift is smaller than the two items above.
