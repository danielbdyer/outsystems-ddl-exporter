...
| 3 | `Face` combinator + split `RunFaces` | 🟥 | ◑ **partial** | `Face` combinator + watched faces in (`1cfb99e5`,`2208e10c`); the 6 `anyCode` exit-ladders collapsed into one total `CliExit` classifier (`CliExit.fs`, 2026-06-27) — Concern 1 closed, the 3 latent exit-code divergences reconciled onto the gate convention. **Open:** per-verb file split; lift `slice-*` verbs into the typed `Intent` surface (kill the `argv.[0]` 2nd dispatcher). *Top-ranked; the bulk is still open.* |
