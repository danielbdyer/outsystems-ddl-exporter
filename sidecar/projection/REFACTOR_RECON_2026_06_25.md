...
| 3 | `Face` combinator + split `RunFaces` | 🟥 | ◑ **partial** | `Face` combinator + watched faces (`1cfb99e5`,`2208e10c`); Concern 1 — 6 `anyCode` exit-ladders → one `CliExit` classifier (2026-06-27); **Concern 2 — slice-`*` verbs lifted onto the typed `Intent`/`Command` plane, the `argv.[0]` 2nd dispatcher deleted (2026-06-28)**. **Open:** the per-verb `Faces/*.fs` file split (the mechanical bulk). |
