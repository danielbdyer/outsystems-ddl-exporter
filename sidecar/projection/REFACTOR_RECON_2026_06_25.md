...
| 3 | `Face` combinator + split `RunFaces` | 🟥 | ◑ **partial** | Concerns 1 (CliExit) + 2 (slice→typed Intent, `argv.[0]` dispatcher deleted) done. **File split underway (2026-06-28):** `Faces/Slice.fs` + `Faces/Diff.fs` + `Faces/Inspect.fs` extracted (RunFaces 2699→2217); coupling mapped — 8 more families are clean self-contained extractions, the transfer/migrate/synthetic/explain core needs a shared `FacesCommon` (`nameOf`/`narrate`/the `Face` combinator/migrate helpers) first. **Open:** the remaining extractions + `FacesCommon`. |
