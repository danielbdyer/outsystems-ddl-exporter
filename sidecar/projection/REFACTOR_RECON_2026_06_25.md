...
| 3 | `Face` combinator + split `RunFaces` | 🟥 | ◑ **partial** | `Face` combinator + watched faces in (`1cfb99e5`,`2208e10c`). **Open:** per-verb file split; route the 6 `anyCode` exit-ladders through `Preflight.classify`; lift `slice-*` verbs into the typed `Intent` surface (kill the `argv.[0]` 2nd dispatcher). *Top-ranked; the bulk is still open.* |
...
> **Status (2026-06-27):** ◑ **Partial.** The `Face` combinator + watched faces landed (`1cfb99e5`, `2208e10c`). **Open (the bulk — go XL):** the per-verb file split (`Faces/*.fs`); route the 6 `anyCode` exit-ladders through `Preflight.classify`; lift the `slice-*` verbs into the typed `Intent` surface and delete the `argv.[0]` second dispatcher.
