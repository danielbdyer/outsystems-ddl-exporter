...
public static class SchemaText
{
    ...
    private static readonly Regex Variable = new(@"\$\([A-Za-z_][A-Za-z0-9_-]{0,127}\)", RegexOptions.CultureInvariant);

    private static readonly Regex VariableLiteral = new(@"\A(?:N?['""])?\$\([A-Za-z_][A-Za-z0-9_-]{0,127}\)['""]?\z", RegexOptions.CultureInvariant);
