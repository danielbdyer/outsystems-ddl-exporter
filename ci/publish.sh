...
stub="$(sed -n 's/.*<PackageDownload Include="Microsoft.NETFramework.ReferenceAssemblies.net472" Version="\[\([^]]*\)\]".*/\1/p' "$root/cli/cli.csproj")"
if [ -z "$dacfx" ] || [ -z "$stub" ]; then
  echo "ci/publish.sh: the DacFx pin (Directory.Packages.props) or the reference stub's (cli/cli.csproj) was not found" >&2
  exit 2
fi
...
from="$packages/microsoft.netframework.referenceassemblies.net472/$stub/build/.NETFramework/v4.7.2"
