...
# on net10.0 with DacFx and every dependency beside it, DacFx's net10.0 SqlTasks targets, and the reference stub a classic
...
$stub = (Select-Xml -Path (Join-Path $root 'cli/cli.csproj') -XPath "//PackageDownload[@Include='Microsoft.NETFramework.ReferenceAssemblies.net472']").Node.Version.Trim('[', ']')
if (-not $dacfx -or -not $stub) {
    throw "ci/publish.ps1: the DacFx pin (Directory.Packages.props) or the reference stub's (cli/cli.csproj) was not found"
}
...
$from = Join-Path $packages "microsoft.netframework.referenceassemblies.net472/$stub/build/.NETFramework/v4.7.2"
