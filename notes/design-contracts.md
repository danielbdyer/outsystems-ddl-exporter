...
- `FullExportPipelineResult.ExecutionLog` stitches extraction (`fullExport.extract.*`), profiling (`fullExport.profile.*`), emission (`fullExport.build.*`), and schema apply (`fullExport.apply.*`) metadata into a single ordered stream so downstream automation can reason about emitted artifacts (model JSON, profile manifest, decision log, safe/remediation scripts, telemetry packages) without replaying console output. Persist the schema of these metadata keys whenever new artifacts or counts are introduced. 【F:src/Osm.Pipeline/Orchestration/FullExportPipeline.cs†L93-L211】
- `DynamicEntityExtractionTelemetry` captures per-table row counts, batch durations, and CRC32 checksums for every dynamic data table and is persisted to `DynamicData/dynamic-data.telemetry.json`. The CLI surfaces this artifact path when present and the full-export run manifest records it under the dynamic-insert stage for downstream parity validation tooling. 【F:src/Osm.Cli/Commands/CommandConsole.cs†L476-L517】【F:src/Osm.Pipeline/Runtime/FullExportRunManifest.cs†L338-L388】
