...
`dbchange` reads what an environment holds and what the repository says, predicts read-only whether
...
OutSystems applications' external entities; Octopus still deploys.

## The verbs

`dbchange --help` lists every verb, whether this build has it, and the exit codes with their
remedies; `dbchange --help --json` is the same table as data, with the schema of each JSON answer.
From M8, `cli/VERBS.md` is generated from it and never edited by hand.

## Try it in five minutes

1. `dbchange doctor` names the .NET SDK, the DacFx pin, the local server (Docker or LocalDB) and
   Git LFS, each with a remedy when it is missing.
2. `dbchange synthetic-copy up` builds the repository's schema, publishes it to the local server
   (SQL Server in Docker from the pinned image, or LocalDB where Docker is absent), generates rows
   from the committed evidence, and caches the result by fingerprint under `.dbchange/`. A second run
   starts from that cache.
3. `dbchange prove --project tests/Golden/project/SampleCatalog.sqlproj --target synthetic-copy`
...
At run time `dbchange` opens no network connection except to the SQL Server it is given. The one
...
- `cli/` — the `dbchange` executable: the verb table, the dispatcher, the renderers.
...
- `knowledge/` — from M7: the operations, the pull request description's format, the findings and
  the ledgers, vendored to the SSDT repository.
...
## For developers working in the SSDT repository

This repository is not needed in the SSDT repository, which carries the published tool folder under
`tools/dbchange/`, the `dbchange.cmd` and `dbchange.ps1` shims at its root, and the generated knowledge
