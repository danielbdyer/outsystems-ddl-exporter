...
module Hydration =
    ...
    let private renderKindAtDrain
        ...
        let load, _skipped =
            DataLoadPlan.loadFor deferralScopes SurrogateRemapContext.empty tgt rows
        let script =
    ...
    /// per-kind derivation. `deferralScopes` MUST be the RENDER catalog's
    /// (`TopologicalOrder.deferralScopes` of the post-prefix-chain topo);
                ...
                | Ok openConnection ->
                    return!
                        Ingestion.collectInOrderForConcurrentWith
                            (renderKindAtDrain targetKinds deferralScopes opts cdc nullability evidenceKinds retainRows)
                            (max 1 concurrency)
                            openConnection
                            eligible
                            sourceCatalog
