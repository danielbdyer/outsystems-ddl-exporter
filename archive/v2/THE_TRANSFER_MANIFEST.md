...
| **4 — Per-act consent** | consent + fidelity | act-set-equality gate rewrite | the every-act-blessed execution gate — **BUILT 2026-07-10** (4a narrate-only: `ActConsent.fs` in Core + `ActEvidence.fs`, the board’s `consent` axis, the workbench’s `d`/`a` bless arms, the dual-shape `signoff` array. 4b enforcement, ALWAYS-ON for the peer path: `transfer.writeSignoff.actUnblessed` appended LAST in the refusal chain, board and gate sharing one derivation; 21 pure witnesses + the two `ActConsentDockerTests` legs live) |
