# CLAUDE_ARCHIVE_2026_06_11.md — the superseded navigation file (provenance only)

> **SUPERSEDED 2026-06-11.** This is the navigation file as it stood at adoption of the
> from-scratch rebuild (operator-approved; `DECISIONS 2026-06-11 — CLAUDE.md rebuilt from
> scratch`). **Do not read this for current state** — the live index is `CLAUDE.md`; the
> disciplines this file restated live in `DECISIONS.md`; known drift in this text at archive
> time (the retired `AsyncStream` combinator row, the pre-decomposition `dumpBench` pointer,
> the stale T11 "aspirational" note, stale counts) is enumerated in the DECISIONS entry. It
> is preserved because its operating-disciplines table remains a richly annotated historical
> index into `DECISIONS.md`, and because the operating surface is the file, not git log.

# CLAUDE.md — V2 Sidecar Navigation (as archived)
