...
function gateCitations() {
  ...
  const known = new Set();
  for (const root of [TREE, join(REPO, "archive", "v1", "handbook"), join(REPO, "archive", "v1", "ssdt-playbook")]) {
...
function gatePackaging() {
  ...
  // The .claude/ pointers were removed at the archive move (M7 regenerates them from knowledge/), so only the Copilot bundle is checked.
  for (const [verb, root] of [["copilot-check", "archive/v2/ssdt-agent/copilot-package"]]) {
