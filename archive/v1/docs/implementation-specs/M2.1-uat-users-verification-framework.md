...
### Independence & Test-First Guardrails

- **Optional, not invasive**: The verification layer must run purely against already-emitted artifacts; it must not mutate `full-export` outputs or be required for the base command. Add a regression test that executes `full-export` without UAT-users artifacts present and asserts invoking the verifier is a safe no-op with a clear warning rather than a failure.
- **Flag gating**: `uat-users --verify` should accept only the inputs it needs; conversely, the base `uat-users` execution path must remain unchanged when `--verify` is omitted. Add CLI tests that prove defaults still work without supplying verification paths.
- **TDD discipline**: Land failing unit + CLI tests for the new verification models and exit codes first so implementation is driven by the expected JSON payloads and independence behaviors.
...
| `UserMapEntry` | `Osm.Pipeline.UatUsers` | `src/Osm.Pipeline/UatUsers/UserMapEntry.cs` | Source→Target mapping |
...
| `ImmutableArray<T>` | `System.Collections.Immutable` | BCL | **⚠️ Struct, not class** |
...
- ✅ Happy path (all orphans mapped, valid catalog)
- ✅ Empty user map (no orphans case)
- ✅ Incomplete user map (unmapped orphans)
- ✅ Duplicate source mappings
- ✅ Invalid target IDs (not in UAT inventory)
- ✅ Missing FK catalog entries
- ✅ Malformed CSV files
...
⚠️ **GOTCHA**: CSV files may have different line endings (CRLF vs LF)!

```csharp
// ❌ WRONG - Assumes specific line ending
var lines = File.ReadAllText(path).Split('\n');

// ✅ CORRECT - Handle both CRLF and LF
var lines = File.ReadAllLines(path); // Handles line endings automatically
```

### User ID Comparison

⚠️ **GOTCHA**: User IDs might be GUIDs, integers, or strings!

```csharp
// ❌ WRONG - Case-sensitive comparison
if (orphanId == targetId)

// ✅ CORRECT - Case-insensitive for string IDs
if (orphanId.Equals(targetId, StringComparison.OrdinalIgnoreCase))
```

### SQL Script Parsing

⚠️ **GOTCHA**: `TSql150Parser` requires proper error handling!

```csharp
// ❌ WRONG - Doesn't check for parse errors
var parser = new TSql150Parser(false);
var fragment = parser.Parse(new StringReader(sql), out var errors);

// ✅ CORRECT - Check errors collection
var parser = new TSql150Parser(false);
var fragment = parser.Parse(new StringReader(sql), out var errors);
if (errors.Count > 0)
{
...
⚠️ **GOTCHA**: Artifact paths are relative to output root!

```csharp
// ❌ WRONG - Assumes absolute paths
var mapPath = artifacts.UserMapPath;
var content = File.ReadAllText(mapPath);

// ✅ CORRECT - Combine with output root
...
⚠️ **GOTCHA**: `ImmutableArray<T>` is a **struct**, not a class!
