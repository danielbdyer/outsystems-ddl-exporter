...
src/Osm.Pipeline/Runtime/PipelineServiceCollectionExtensions.cs (line ~123-140)
  └─ Add verification service registrations after existing validator registrations
```
...
## 9. Type Reference Guide

### 9.1 Core Verification Types

| Type | Namespace | Location | Notes |
|------|-----------|----------|-------|
| `FullExportRunManifest` | `Osm.Pipeline.Runtime` | `src/Osm.Pipeline/Runtime/FullExportRunManifest.cs` | Read-only, already exists |
| `FullExportManifestArtifact` | `Osm.Pipeline.Runtime` | Same as above | Nested record |
| `PipelineExecutionLogBuilder` | `Osm.Pipeline.Orchestration` | `src/Osm.Pipeline/Orchestration/PipelineExecutionLogBuilder.cs` | For telemetry |
| `ImmutableArray<T>` | `System.Collections.Immutable` | BCL | **⚠️ Struct, not class** |
| `OsmModel` | `Osm.Domain.Model` | `src/Osm.Domain/Model/OsmModel.cs` | Use this (not EntityModel) |

### 9.2 New Types to Create

All new types go in `src/Osm.Pipeline/Verification/`:

| Type | Purpose | Approx Lines |
|------|---------|--------------|
| `ExportVerificationContext` | Context model passed to validators | ~150 |
| `IExportValidator` | Validator interface | ~40 |
| `ValidationResult` | Individual validator result | ~80 |
| `ExportVerificationReport` | Aggregated report model | ~90 |
| `ExportVerificationOrchestrator` | Orchestrates validator execution | ~250 |
| `ExportVerificationReportSerializer` | JSON serialization | ~50 |

### 9.3 Validation Result Pattern
...
public enum ValidationStatus
{
    ...
    PartialPass, // Non-critical warnings
    Skipped      // Validator didn't run (missing dependencies)
}
```
...
### 10.1 Available Test Helpers

**Location**: `tests/Osm.Pipeline.Tests/` (various files)

| Helper Method | Location | Purpose |
|---------------|----------|---------|
| `FixtureFile.GetPath("model.json")` | `tests/Fixtures/FixtureFile.cs` | Load test data files |
| `CreateTestManifest()` | Create in test file | Generate minimal manifest for testing |
| `TempDirectory` | `tests/Osm.Testing/TempDirectory.cs` | Create disposable temp directories |

### 10.2 Test Data Providers

**Available Fixtures** (`tests/Fixtures/`):
- `model.json` - Standard OutSystems model
- `model.edge-case.json` - Edge cases (empty modules, unusual FK patterns)
- `manifest-valid.json` - Valid manifest example
- `manifest-invalid.json` - Invalid manifest for error testing

### 10.3 Testing Patterns for Validators

**Pattern 1: Test validator in isolation**
```csharp
[Fact]
public async Task ValidateAsync_ManifestMissingFile_ReturnsFailure()
{
    // Arrange
    using var temp = new TempDirectory();
    var manifest = CreateManifestWithMissingFile(temp.Path);
    var context = new ExportVerificationContext(
        manifest,
        temp.Path,
        entityCatalog: ImmutableArray<string>.Empty);

    var validator = new FilesystemVerificationValidator(logger);
...
**Pattern 2: Test full orchestrator**
```csharp
[Fact]
public async Task VerifyAsync_ValidExport_ReturnsPassedReport()
{
    // Arrange
    using var temp = new TempDirectory();
    // ... create manifest and files ...

    var orchestrator = new ExportVerificationOrchestrator(validators, logger);

    // Act
    var report = await orchestrator.VerifyAsync(
        manifestPath: Path.Combine(temp.Path, "manifest.json"),
        outputRoot: temp.Path,
        cancellationToken: CancellationToken.None);

    // Assert
    Assert.True(report.OverallStatus == ValidationStatus.Passed);
}
```
...
- ✅ Empty manifest
- ✅ Partial failures (some files missing, not all)
- ✅ Validation skipped (dependency missing)

---

## 11. Post-Implementation Checklist

### 11.1 Verification Step

After implementing M1.1, verify it works end-to-end:

```bash
# 1. Run full-export to generate artifacts
dotnet run --project src/Osm.Cli -- full-export \
  --connection-string "Server=localhost;Database=QA;..." \
  --build-out ./test-export

# 2. Verify the export (should pass)
dotnet run --project src/Osm.Cli -- verify-export \
  --manifest ./test-export/manifest.json \
  --output ./test-export/export-validation.json

# 3. Check the validation report
cat ./test-export/export-validation.json | jq '.overallStatus'
# Expected: "Passed"

# 4. Test failure case: Delete a file
rm ./test-export/DynamicData/some-entity.sql

# 5. Re-run verification (should fail)
dotnet run --project src/Osm.Cli -- verify-export \
  --manifest ./test-export/manifest.json

# Expected: Validation fails with clear error about missing file
```
...
dotnet test tests/Osm.Pipeline.Tests/Osm.Pipeline.Tests.csproj \
  --filter "FullyQualifiedName~Verification"
```

**Expected new tests**:
- `ManifestIntegrityValidatorTests` (4-6 tests)
- `FilesystemVerificationValidatorTests` (6-8 tests)
- `ChecksumValidatorTests` (5-7 tests)
- `CompletenessValidatorTests` (4-6 tests)
- `ExportVerificationOrchestratorTests` (3-5 tests)

### 11.3 DI Registration Verification

**Verify services are registered**:
```csharp
// In tests or debugging, check DI container:
var serviceProvider = services.BuildServiceProvider();
var orchestrator = serviceProvider.GetService<ExportVerificationOrchestrator>();
Assert.NotNull(orchestrator); // Should not be null

var validators = serviceProvider.GetServices<IExportValidator>();
Assert.Equal(4, validators.Count()); // Should have 4 validators
```

---

## 12. C# Gotchas and Common Pitfalls

### 12.1 ImmutableArray Null Handling

⚠️ **GOTCHA**: `ImmutableArray<T>` is a **struct**, not a class!

```csharp
// ❌ WRONG - This doesn't work!
var artifacts = manifest.DynamicArtifacts ?? ImmutableArray<...>.Empty;

// ✅ CORRECT - Use IsDefaultOrEmpty
var artifacts = manifest.DynamicArtifacts.IsDefaultOrEmpty
    ? ImmutableArray<FullExportManifestArtifact>.Empty
    : manifest.DynamicArtifacts;
```

### 12.2 File Path Comparison

⚠️ **GOTCHA**: File paths are case-sensitive on Linux, case-insensitive on Windows!

```csharp
// ❌ WRONG - Won't work cross-platform
if (filePath == expectedPath)

// ✅ CORRECT - Use proper path comparison
if (Path.GetFullPath(filePath).Equals(
    Path.GetFullPath(expectedPath),
    StringComparison.OrdinalIgnoreCase))
```

### 12.3 Checksum Computation Performance

⚠️ **GOTCHA**: SHA256 is expensive! Don't compute in tight loops.

```csharp
// ❌ WRONG - Creates new SHA256 instance per file
...
⚠️ **GOTCHA**: Don't mix sync and async file operations!

```csharp
// ❌ WRONG - File.ReadAllText is synchronous
var json = File.ReadAllText(path);
await ProcessJsonAsync(json);

// ✅ CORRECT - Use async APIs consistently
var json = await File.ReadAllTextAsync(path, cancellationToken);
await ProcessJsonAsync(json);
```

### 12.5 JSON Deserialization Options

⚠️ **GOTCHA**: Default JSON options don't handle immutable types well!

```csharp
// ❌ WRONG - May fail to deserialize records with init-only properties
var report = JsonSerializer.Deserialize<ExportVerificationReport>(json);

// ✅ CORRECT - Use proper JSON options
var options = new JsonSerializerOptions
{
    PropertyNamingPolicy = JsonNamingPolicy.CamelCase,
    WriteIndented = true,
    PropertyNameCaseInsensitive = true // Important for records!
};
var report = JsonSerializer.Deserialize<ExportVerificationReport>(json, options);
```
...
### 13.2 Future Enhancements (M1.1.1+)
