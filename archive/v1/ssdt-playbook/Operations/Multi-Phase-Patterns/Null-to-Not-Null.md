...
- **The two-release shape (this estate's remedy).** Release 1: a pre-deployment script backfills every NULL and applies the `ALTER COLUMN ... NOT NULL` by hand, with the model still declaring `NULL` — so the deploy generates no tightening step and the guard never exists. Release 2: the model catches up and publishes as a no-op, promoted to an environment only after Release 1 has landed there. This estate's pipeline (Azure DevOps → Octopus) always publishes with `BlockOnPossibleDataLoss=True` and cannot relax it for one deployment — a guard relaxation is not an available path here.
