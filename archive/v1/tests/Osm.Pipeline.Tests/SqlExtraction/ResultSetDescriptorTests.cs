...
public class ResultSetDescriptorTests
{
    [Fact]
    ...
    [Fact]
    public async Task ModuleJsonProcessor_ShouldCaptureSerializedModule()
    {
        var processor = new ModuleJsonResultSetProcessor();
        var accumulator = new MetadataAccumulator();
        ...
        Assert.Equal(1, count);
        var module = Assert.Single(accumulator.ModuleJson);
        Assert.Equal("Sales", module.ModuleName);
        Assert.False(module.IsSystem);
        Assert.True(module.IsActive);
        Assert.Equal("{\"entities\":[]}", module.ModuleEntitiesJson);
    }
}
