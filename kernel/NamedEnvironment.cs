...
/// Each refusal leads with the subject its caller gives, where in the posture the value sits, and quotes no value.
/// </summary>
public sealed record NamedEnvironment : IComparable<NamedEnvironment>
{
        ...
        SecretReference connection, string profilePath, IEnumerable<SqlCmdVariable> sqlCmd, SecretReference? metamodel)
    {
        ...
        return !Named.IsMatch(name)
            ? new Refusal("posture.environment-name", subject + " names an environment in other than 1 to 32 lowercase letters, digits and hyphens.",
            ...
            : !InsideTheEstate(profilePath)
                ? new Refusal("posture.profile-path", subject + " gives its profile a path that leaves the estate or names no .publish.xml.",
            ...
            : values.Where((v, i) => i > 0 && string.Equals(values[i - 1].Name, v.Name, StringComparison.OrdinalIgnoreCase)).Any()
                ? new Refusal("posture.sqlcmd-repeated", subject + " gives one SQLCMD variable twice; sqlcmd reads names in any case as one.",
...
public sealed record SecretReference : IComparable<SecretReference>
{
    ...
    public static Result<SecretReference> Of(string subject, string? text) =>
        ...
        || (text?.StartsWith(FilePrefix, StringComparison.Ordinal) == true && text[FilePrefix.Length..] is { Length: > 0 } path
            ...
            : new Refusal("reference.malformed", subject + " is neither env:NAME nor file:path; a path holds no '=' or ';', the marks of a connection string.",
...
public sealed record Confirmation
{
    ...
    public static Result<Confirmation> Of(string subject, string? lead, string? on) =>
        string.IsNullOrWhiteSpace(lead) || lead.Length > 128 || lead.Any(char.IsControl) ? new Refusal("posture.confirmation",
        ...
        : !DateOnly.TryParseExact(on, "yyyy-MM-dd", CultureInfo.InvariantCulture, DateTimeStyles.None, out var date) ? new Refusal("posture.confirmation",
...
public abstract record Classification
{
    ...
    public static Result<Classification> Of(string subject, string? claimed, Confirmation? confirmation) => claimed switch
    {
        ...
        "synthetic" => new Refusal("posture.unconfirmed", subject + " is marked synthetic with no lead's dated confirmation.",
...
public abstract record SqlCmdVariable : IComparable<SqlCmdVariable>
{
    ...
    public static Result<SqlCmdVariable> Of(string subject, string name, string literal) =>
        ...
        : Credential.Any(word => name.Contains(word, StringComparison.OrdinalIgnoreCase))
            ? new Refusal("sqlcmd.literal-credential", subject + " gives $(" + name + ") a literal, and a name shaped like a credential takes a reference.",
    ...
    public static Result<SqlCmdVariable> Of(string subject, string name, SecretReference reference) =>
        Unnamed(subject, name) is { } refusal ? refusal : new Referenced(name, reference);
    ...
    /// writes no file. A variable with no value is refused by its name, and no value is quoted.
    /// </summary>
    public static Result<string> Substitute(string script, IReadOnlyDictionary<string, string> values)
    {
        ...
        return Used.Matches(script).Select(m => m.Groups["name"].Value).FirstOrDefault(name => !byName.ContainsKey(name)) is { } missing
            ? new Refusal("sqlcmd.undefined", "The script uses $(" + missing + "), and no SQLCMD value is given for it.",
    ...
    private static Refusal? Unnamed(string subject, string name) => Named.IsMatch(name) ? null
        : new Refusal("sqlcmd.name", subject + " names a SQLCMD variable in other than letters, digits, '_' and '-' from a letter or '_'.",
