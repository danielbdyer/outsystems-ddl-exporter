...
| 3′ the model is complete | A diff of two procedures differing in a password literal alone alters the Definition and prints both values left out | `Estate.Io.Tests.SchemaTextTests.A_diff_of_two_procedures_differing_in_a_password_literal_alone_alters_the_Definition_and_prints_both_values_left_out` |
...
| 3′ the model is complete | Models a serialization without lengths tags or counts would confuse fingerprint differently | `Estate.Kernel.Tests.ElementTests.Models_a_serialization_without_lengths_tags_or_counts_would_confuse_fingerprint_differently` |
...
|---|---|---|
| A2 | Doctor on a bare machine prints DEGRADED with a remedy per missing item | `Estate.Budgets.Tests.ContractTests.Doctor_on_a_bare_machine_prints_DEGRADED_with_a_remedy_per_missing_item` |
| A2 | A bare machine gets a remedy for each item missing | `Estate.Io.Tests.DoctorTests.A_bare_machine_gets_a_remedy_for_each_item_missing` |
| A2 | The published doctor finds its tool folder and names its DacFx release | `Estate.Io.Tests.ToolFolderTests.The_published_doctor_finds_its_tool_folder_and_names_its_DacFx_release` |
| A4 | Every hand written file holds its budget | `Estate.Budgets.Tests.Manifest.Every_hand_written_file_holds_its_budget` |
| A4 | Every markdown file outside the archive and every ci file is a row | `Estate.Budgets.Tests.Manifest.Every_markdown_file_outside_the_archive_and_every_ci_file_is_a_row` |
| A8 | Every line under the heading is one dated decision with its pull request | `Estate.Budgets.Tests.Decisions.Every_line_under_the_heading_is_one_dated_decision_with_its_pull_request` |
| A10 | Every markdown file outside the archive and every ci file is a row | `Estate.Budgets.Tests.Manifest.Every_markdown_file_outside_the_archive_and_every_ci_file_is_a_row` |
| A10 | Every row names a file that exists and says who reads it and when | `Estate.Budgets.Tests.Manifest.Every_row_names_a_file_that_exists_and_says_who_reads_it_and_when` |
| D2 | Each banned symbol planted in the kernel is a build error | `Estate.Budgets.Tests.BannedSymbolsTests.Each_banned_symbol_planted_in_the_kernel_is_a_build_error` |
| D2 | An answer is the same bytes under tr TR de DE and the invariant culture | `Estate.Budgets.Tests.ContractTests.An_answer_is_the_same_bytes_under_tr_TR_de_DE_and_the_invariant_culture` |
...
| D3 | A write the file system refuses is file unwritable and leaves the old content and no temporary file | `Estate.Io.Tests.WriteTests.A_write_the_file_system_refuses_is_file_unwritable_and_leaves_the_old_content_and_no_temporary_file` |
| D3 | An appended entry is on disk when Append returns and a reader sees every earlier entry | `Estate.Io.Tests.WriteTests.An_appended_entry_is_on_disk_when_Append_returns_and_a_reader_sees_every_earlier_entry` |
| D3 | Writes utf8 without a bom in the line ending on disk | `Estate.Io.Tests.WriteTests.Writes_utf8_without_a_bom_in_the_line_ending_on_disk` |
| D3 | Text is fingerprinted in its canonical form | `Estate.Kernel.Tests.FingerprintTests.Text_is_fingerprinted_in_its_canonical_form` |
| G6 | No verb writes to a named environment | `Estate.Io.Tests.CapabilityTests.No_verb_writes_to_a_named_environment` |
| L1 | A document of the repository uses no retired word and no banned form | `Estate.Budgets.Tests.Register.Prose.A_document_of_the_repository_uses_no_retired_word_and_no_banned_form` |
| L1 | An error carries a remedy that is one move in the register | `Estate.Budgets.Tests.Register.Refusals.An_error_carries_a_remedy_that_is_one_move_in_the_register` |
| L1 | Every test name read as words is in the register | `Estate.Budgets.Tests.Register.TestNames.Every_test_name_read_as_words_is_in_the_register` |
| L2 | A hand written document restates no count the build computes | `Estate.Budgets.Tests.NoRestatedCounts.A_hand_written_document_restates_no_count_the_build_computes` |
| L3 | Each budget holds its ceiling | `Estate.Budgets.Tests.Budgets.Each_budget_holds_its_ceiling` |
| L3 | The code together holds its ceiling | `Estate.Budgets.Tests.Budgets.The_code_together_holds_its_ceiling` |
| L3 | Every hand written file holds its budget | `Estate.Budgets.Tests.Manifest.Every_hand_written_file_holds_its_budget` |
| L4 | Every markdown file outside the archive and every ci file is a row | `Estate.Budgets.Tests.Manifest.Every_markdown_file_outside_the_archive_and_every_ci_file_is_a_row` |
| L4 | Every row names a file that exists and says who reads it and when | `Estate.Budgets.Tests.Manifest.Every_row_names_a_file_that_exists_and_says_who_reads_it_and_when` |
| L6 | LAWS md is what the generators write from the tests and regenerates byte identically | `Estate.Budgets.Tests.Laws.LAWS_md_is_what_the_generators_write_from_the_tests_and_regenerates_byte_identically` |
| L6 | LAWS md lists each Law Value and Exit trait the tests declare and names each row and exit none declares | `Estate.Budgets.Tests.Laws.LAWS_md_lists_each_Law_Value_and_Exit_trait_the_tests_declare_and_names_each_row_and_exit_none_declares` |
| L6 | No test skips itself | `Estate.Budgets.Tests.NoSkips.No_test_skips_itself` |
| L7 | Each banned symbol planted in the kernel is a build error | `Estate.Budgets.Tests.BannedSymbolsTests.Each_banned_symbol_planted_in_the_kernel_is_a_build_error` |
| L7 | Io does not depend on the cli and the kernel on neither | `Estate.Budgets.Tests.DependenciesPointOneWay.Io_does_not_depend_on_the_cli_and_the_kernel_on_neither` |
| L7 | No project reaches into the knowledge files or ci | `Estate.Budgets.Tests.DependenciesPointOneWay.No_project_reaches_into_the_knowledge_files_or_ci` |
| L7 | No public kernel member is asynchronous | `Estate.Budgets.Tests.KernelCannotDoIo.No_public_kernel_member_is_asynchronous` |
| L7 | The kernel references the BCL and nothing else | `Estate.Budgets.Tests.KernelCannotDoIo.The_kernel_references_the_BCL_and_nothing_else` |
| L9 | Every test carries exactly one category fast fixture or build | `Estate.Budgets.Tests.NoSkips.Every_test_carries_exactly_one_category_fast_fixture_or_build` |
...
| S2 | An unexpected exception answers exit 6 with a finding naming its type and its message | `Estate.Budgets.Tests.ContractTests.An_unexpected_exception_answers_exit_6_with_a_finding_naming_its_type_and_its_message` |
...
| X1 | A literal connection string as a target is connection literal and quoted nowhere | `Estate.Io.Tests.TargetTests.A_literal_connection_string_as_a_target_is_connection_literal_and_quoted_nowhere` |
| X1 | A SQLCMD name shaped like a credential never holds a literal and takes a reference | `Estate.Kernel.Tests.EnvironmentsTests.A_SQLCMD_name_shaped_like_a_credential_never_holds_a_literal_and_takes_a_reference` |
| X1 | Anything else given as a reference is refused without being quoted | `Estate.Kernel.Tests.EnvironmentsTests.Anything_else_given_as_a_reference_is_refused_without_being_quoted` |
| X2 | An unexpected exception withholds its message when the run read a named environment with no env argument | `Estate.Budgets.Tests.ContractTests.An_unexpected_exception_withholds_its_message_when_the_run_read_a_named_environment_with_no_env_argument` |
| X2 | A denied login names the environment and quotes nothing | `Estate.Io.Tests.AggregateQueryTests.A_denied_login_names_the_environment_and_quotes_nothing` |
| X2 | A failed query against a real environment reports only its number and its site | `Estate.Io.Tests.AggregateQueryTests.A_failed_query_against_a_real_environment_reports_only_its_number_and_its_site` |
| X2 | The model and the plan of a named environment read as the read only principal | `Estate.Io.Tests.AggregateQueryTests.The_model_and_the_plan_of_a_named_environment_read_as_the_read_only_principal` |
| X2 | A refusal names the form and its place and quotes no literal | `Estate.Io.Tests.AllowlistTests.A_refusal_names_the_form_and_its_place_and_quotes_no_literal` |
| X2 | Estate read of a database holding a SQL login prints no password | `Estate.Io.Tests.DiffTests.Estate_read_of_a_database_holding_a_SQL_login_prints_no_password` |
| X2 | A denied login prints one sentence naming the environment and a lead's prediction | `Estate.Io.Tests.DriftTests.A_denied_login_prints_one_sentence_naming_the_environment_and_a_lead_s_prediction` |
| X2 | A literal connection string as a target is exit 6 and printed nowhere | `Estate.Io.Tests.DriftTests.A_literal_connection_string_as_a_target_is_exit_6_and_printed_nowhere` |
...
| X2 | Nothing a named environment prints carries a literal or what a reference names | `Estate.Kernel.Tests.EnvironmentsTests.Nothing_a_named_environment_prints_carries_a_literal_or_what_a_reference_names` |
| X3 | Every package is allowed and every allowed package is used | `Estate.Budgets.Tests.PackagesAllowlist.Every_package_is_allowed_and_every_allowed_package_is_used` |
| X3 | Every project restores from its lock file and CI restores locked | `Estate.Budgets.Tests.PackagesAllowlist.Every_project_restores_from_its_lock_file_and_CI_restores_locked` |
| X5 | Each banned symbol planted in the kernel is a build error | `Estate.Budgets.Tests.BannedSymbolsTests.Each_banned_symbol_planted_in_the_kernel_is_a_build_error` |
| X5 | Main opts out of telemetry before anything else | `Estate.Budgets.Tests.ContractTests.Main_opts_out_of_telemetry_before_anything_else` |
| X5 | Git runs with LC ALL C and without the caller's LANGUAGE and LC MESSAGES | `Estate.Io.Tests.GitTests.Git_runs_with_LC_ALL_C_and_without_the_caller_s_LANGUAGE_and_LC_MESSAGES` |
| X5 | Loading io opts out of telemetry before DacFx loads | `Estate.Io.Tests.TelemetryTests.Loading_io_opts_out_of_telemetry_before_DacFx_loads` |
...
|---|---|---|
| M0.1 | Every v3 project builds with warnings and the culture rules as errors | `Estate.Budgets.Tests.WarningsAsErrors.Every_v3_project_builds_with_warnings_and_the_culture_rules_as_errors` |
| M0.3 | Doctor on a bare machine prints DEGRADED with a remedy per missing item | `Estate.Budgets.Tests.ContractTests.Doctor_on_a_bare_machine_prints_DEGRADED_with_a_remedy_per_missing_item` |
| M0.3 | A bare machine gets a remedy for each item missing | `Estate.Io.Tests.DoctorTests.A_bare_machine_gets_a_remedy_for_each_item_missing` |
| M0.3 | The published doctor finds its tool folder and names its DacFx release | `Estate.Io.Tests.ToolFolderTests.The_published_doctor_finds_its_tool_folder_and_names_its_DacFx_release` |
...
| M1.4 | Each sample change to the golden project changes the fingerprint | `Estate.Io.Tests.ModelElementsTests.Each_sample_change_to_the_golden_project_changes_the_fingerprint` |
| M1.4 | Two builds of the golden project read into equal models and one fingerprint | `Estate.Io.Tests.ModelElementsTests.Two_builds_of_the_golden_project_read_into_equal_models_and_one_fingerprint` |
| M1.4 | Any single edit to a key a property value a relationship target or a script changes the fingerprint | `Estate.Kernel.Tests.ElementTests.Any_single_edit_to_a_key_a_property_value_a_relationship_target_or_a_script_changes_the_fingerprint` |
| M1.4 | Every sample change changes the fingerprint | `Estate.Kernel.Tests.ElementTests.Every_sample_change_changes_the_fingerprint` |
| M1.4 | Models a serialization without lengths tags or counts would confuse fingerprint differently | `Estate.Kernel.Tests.ElementTests.Models_a_serialization_without_lengths_tags_or_counts_would_confuse_fingerprint_differently` |
