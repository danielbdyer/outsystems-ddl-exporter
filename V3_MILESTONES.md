...
the five inputs, prediction and proof, the data conditions · §4 what this changes in the
architecture, and what it keeps · §5 the milestones at a glance · §6–§14 M0 to M8 and the cutover
tools · §15 the estate side · §16 how agents build it · §17 decisions and prerequisites · §18 risks ·
...
five, the synthetic copy on day six (at the conditions tier; the shape tier on day eight), proof on
...
script carries its own data-loss checks as ordinary `SELECT`s. The OutSystems metamodel is tables.
...
is the deploy script's own data-loss check, run read-only. Whether a value survives a retype is SQL
...
target's schema, the target's data conditions, the DacFx version and the server, the publish
profile. Prediction reads *which data conditions* an environment presents; proof shows *what DacFx
does* under them. So v3 makes the **data conditions** a first-class value that travels with the
change. `estate predict` reads them, read-only, from each environment the author or the lead can
reach; the pull request carries them; the gate creates a disposable copy at exactly those conditions
and proves there, with the same DacFx version under the same publish profile. A proof applies to an
environment by construction, and a lead proves "as UAT" without a row of UAT leaving UAT. The
synthetic copy is refreshed only when the live conditions disagree with the committed evidence.
...
| 12 | SQL Server comes from Docker, change data capture included | `docker pull mcr.microsoft.com/mssql/server:2022-latest` (digest `sha256:4402d880…`); a container started with `MSSQL_AGENT_ENABLED=true`; a table enabled for CDC and two rows inserted | the pull reaches the registry; the container answers within seconds as SQL Server 2022 Developer (16.0.4295.3); the capture job records both rows within two seconds. Docker therefore provides the scratch server wherever it is installed, and the only one on which CDC can be proven; LocalDB is the fallback |
...
2. **A verb answers one question, and what parameterizes it is data wherever it can be.** The
   ...
   pipeline's own `.publish.xml`. The environments, their classification, their readers and their
   references are one committed file, `estate/posture.json`. The agent surfaces are rows in a targets
...
3. **Capabilities are types.** A named environment and a disposable copy are different C# types.
   A `Copy` has no public constructor: only `io/ScratchServer` makes one, for a database it created
...
| A live database's plan | DacFx's own deploy script and report | `io/SqlServer.Plan`: `DacServices.Script` under the pipeline's profile; the script's AST; the data-loss check sites | read | `sqlpackage /Action:Script` in `prove.mjs` | ~250 |
...
|---|---|---|---|---|---|
| Can this machine do the work? | `doctor` | the machine, the committed DacFx | `estate/ledgers/toolchain.md` | — | `estate.doctor/1` |
...
one entry in `estate/posture.json`. An agent surface costs one row in `knowledge/targets.json`. The
kernel stays pure because the element reader hands it plain `Element` records: the diff, the
preconditions, the data conditions, the classification, the generator's typed schema and the pull
request description are all pure functions over them, and all property-testable without a database.

---

## 3. The five inputs, prediction and proof, the data conditions
...
public sealed record Provenance(Fingerprint Change, Fingerprint Schema, Fingerprint? DataConditions,
    DacFxVersion DacFx, Server? Server, Fingerprint PublishProfile, string Target, DateTimeOffset At);

// A place where existing rows decide the outcome: a table under the data-loss check, or a rule the new schema asserts about existing rows.
...
public sealed record DataConditions(SortedArray<PreconditionState> States);                        // SortedArray equality is element-wise

public sealed record Prediction(EnvironmentName Environment, DataConditions Conditions, SortedArray<DataLossCheckSite> DataLossChecks,
                                SortedArray<PreconditionCount> Counts, Shipping Predicted, Provenance Provenance);
public sealed record Proof(CopyName Copy, DataConditions Conditions, PublishOutcome Strict, PublishOutcome? Permissive,
                           Readback Readback, Provenance Provenance);

// A proof speaks for an environment only at its data conditions, DacFx version and publish profile, on a server of the same major version and compatibility level (the image digest is recorded, never compared).
public static bool AppliesTo(Proof proof, Prediction prediction) =>
    proof.Conditions == prediction.Conditions
...
The `DataConditions` hold every precondition in that environment's own plan. So baseline drift (a
...
**The data conditions travel with the change.**

```
author's laptop                            the pull request                 the gate (a hosted agent, the clone only)
estate predict            ─► conditions(dev) ─┐
lead's laptop                                 ├─► a machine block in the body ─► per distinct set of conditions: generate a fresh copy at it,
estate predict --env qa,uat ─► conditions(qa) │                                   prove, compare with the author's provenance
                               conditions(uat)┘                                 ─► the evidence table: per environment,
                                                                                   predicted · proven at its conditions · applies
```

1. **A proof applies by construction.** The description never reports a proof for an environment at
   data conditions, a DacFx version or a publish profile other than the ones read for it (law T, M4).
...
4. **A retype's data conditions come only from a prediction.** Whether any value fails to convert
...
|---|---|---|---|---|---|
| 1 | Order | kernel and `emit` first (§14.2 steps 1–2); byte parity with v2's emitter before any lifecycle verb | the lifecycle first (M1–M6); `emit`, `decide`, `move` and the full OSSYS reader form the cutover tools (W) | the lifecycle has consumers every day; the cutover tools' only consumer is Prod's cutover, which v2 serves, frozen | a second estate, or a re-emission from OSSYS, needs the cutover tools before Prod is done |
...
| 4 | Building a package | `dotnet build` or MSBuild for proofs; v2's Twin's own `AddObjects` build | one build for everything: `dotnet build` with the committed DacFx targets | it carries the refactorlog and the pre- and post-deploy scripts (§1 facts 1, 8); the build's DacFx is the committed DacFx; no Visual Studio and no download | S1 finds the estate's project cannot build this way; then, and only then, Visual Studio's own MSBuild and SSDT targets, stamped with that DacFx version |
...
| 6 | Prediction | `classify`, pure, from committed evidence | `classify` stays provisional; a new verb, `predict`, reads each environment's data conditions read-only from DacFx's own data-loss checks; proofs run at each environment's data conditions | the lifecycle invariants: prediction and proof, and the condition under which a proof applies (Appendix B) | — |
| 7 | Laws | twelve | fourteen: adds **P** (on a copy, prediction equals proof) and **T** (a proof is reported for an environment only at that environment's data conditions, DacFx version and publish profile). Laws 1–5 take lifecycle forms here and keep their emit forms for the cutover tools; 11 and 12 are restated to match the conditions tier (Appendix C) | P holds every query v3 writes to SQL Server's behaviour; T is the condition under which a proof applies | — |
...
| 11 | Refreshing evidence | the bake lane, on a schema change and weekly | when the live data conditions disagree with the committed evidence's (`check evidence`, and every prediction) | §3 consequence 2 | — |
...
| 13 | Where the synthetic copy's fingerprints live | R5: written into the database; values O6: a lock on `[twin].[__state]` | in the synthetic copy's database name (`estate_synthetic_<schema>_<evidence>_<seed>_<tier>[_<conditions>]`), with a lock file under `.estate/` (`FileLock.Take`) | a state table would make the synthetic copy's schema differ from the repository's, and `check drift` could no longer check it exactly; the name carries the same fingerprints and `synthetic-copy status` verifies them | — |
| 14 | The estate's configuration | `estate/posture.json` (the publish posture, the writable targets, the scratch server) and two mirrored profiles, `estate/profiles/{strict,permissive}.publish.xml` | `estate/posture.json` keeps its name and holds each environment (classification, readers, references, the profile's path, SQLCMD values, the metamodel's reference) and the scratch server preference; `estate/profiles/` holds the pipeline's one profile | §1 fact 10: a hand-kept Strict copy had already drifted from the pipeline's | — |
| 15 | What exit 3 means | blocked: the data-loss check refused | blocked by the data: `blockedBy: data-loss-check` (the check fires on row presence) or `blockedBy: constraint-violation` (SQL Server refused the change on existing rows, such as Msg 547 or Msg 2628); anything else is a tooling failure | the operation catalog already says a foreign key over orphans "blocks"; one code with a `blockedBy` keeps both honest | — |
| 16 | The scratch server | LocalDB on every machine; Docker optional and never pulled (the lifecycle prompt) | Docker with the SQL Server image pinned by tag and digest (pulled when absent) wherever Docker is installed, including cloud sessions and the gate; LocalDB where it is not | §1 fact 12: the pull works and the container runs CDC, which LocalDB cannot; one SQL Server for laptops, cloud sessions and CI | a machine has no Docker: LocalDB, with CDC reported *not provable here* |
...
| M3 | Synthetic copy | build a current synthetic copy from the clone in one command, at the conditions, shape or scale tier; refresh evidence in one command and one pull request | R3, R4, R5, R6 (population), R7, R8, R12 (the synthetic copy's half) | 11′, 12′ | `bake.mjs`, the Twin CLI, `twin.json`, scenarios | ~4,840 | days 2–8, lane T; `synthetic-copy up` at the conditions tier by day 6 | M0 (the generator); M1 (`synthetic-copy up`) |
| M4 | Prove | prove a change on a fresh copy at each environment's data conditions, with its provenance, and see whether it applies to that environment | R6 (blocks at every tier), R9, R10, R13 (trust), R15, R25, R27 (tool side) | 6, 7, 8, 9, 10, P, T | `prove.mjs`; the `sqlpackage` install step | ~790 | days 7–9 | M2; WPs 3.1–3.5 |
| M5 | Describe and gate | open a pull request whose description is generated and whose gate reproduces it from the clone in under ten minutes | R1, R2, R16 (whole chain), R17, R18, R19, R23 (the gate's cells), R24 (estate side), R26 (in part) | 1′ (description), 5′ | `inflight-check.mjs`; the placeholder pipeline; the ledger overwrite | ~1,440 | days 10–11; the description from day 6 | M4; WP 7.2's `description.md` |
...
| Lands | — | drift (day 3) | predict (day 5) | the synthetic copy at the conditions tier | the shape tier (day 8), proof (day 9) | the gate (day 11) | the page, the pilot (day 13) |

**The stations, and the milestone that enables each.**

| Station | The one command | Enabled at |
|---|---|---|
| Can this machine do the work? | `estate doctor` | M1 (a first version at M0) |
...
| The synthetic copy | `estate synthetic-copy up` | M3 |
| Prove | `estate prove` | M4 |
| The pull request description | `estate describe` | M5 |
...
| 0.7 | B | **The tool folder, the first fixture, CI.** `ci/publish.{ps1,sh}`: `dotnet publish cli` (`net10.0`, framework-dependent) into `dist/estate/`, then the `net10.0` `Microsoft.Data.Tools.Schema.SqlTasks.targets` from the DacFx package and the reference assemblies (§1 fact 1) beside it, size printed. `Io.Tests` share one SQL Server per run, with a registered database per test (`estate_<host>_<pid>_<rand>`) so concurrent agents never collide: a container from the pinned image (pulled when absent, SQL Server Agent enabled) on Ubuntu and in cloud sessions, LocalDB on Windows. `tests/Golden/classic-minimal/`: Appendix E's classic project. GitHub Actions, on both CI operating systems (Ubuntu with SQL Server in Docker; Windows with LocalDB): a fast lane, and a SQL lane running `Io.Tests` (the CDC tests on Ubuntu), one of whose jobs denies all outbound traffic except the SQL port once the image is present. | `dist/estate/estate --version` runs; the minimal classic project builds against `dist/estate/` on both operating systems; the outbound-deny job passes |

**Day-one spikes, each run where its fact can be measured.**

| Spike | The question | Where it runs | The answer lands in | Blocks |
|---|---|---|---|---|
| S1 | Does §1's build run on `windows-latest` and on a team laptop, and does the estate's project reference `master.dacpac` (so the tool folder must carry the system dacpacs)? | this repository's CI; one laptop | `estate/ledgers/toolchain.md`; a decision line | WP 1.1 |
| S2 | Which data-loss check forms does the pipeline's DacFx write for each sample change, and do they match the committed DacFx's? | this repository's CI, both DacFx releases | the extracted checks only, under `tests/Golden/data-loss-checks/` | WP 2.2 |
| S3 | Do `Script`, `LoadFromDatabase` and an aggregate query run against Dev as the developers' group, and how long does `LoadFromDatabase` take there? | a developer's laptop | `STATE.md`; `estate/ledgers/toolchain.md` | M1 exit 1 |
| S4 | How long does a restore of a shape-tier synthetic copy take at the estate's size, in the container and in LocalDB? | this repository's CI; a laptop | a decision line | WP 3.4 |
...
| S7 | Which publish profile and which DacFx version does the Octopus step apply? | the Octopus project's settings, read by release engineering | `estate/profiles/pipeline.publish.xml` (options only); the toolchain row | M4 exit 4's meaning; §17 items 1 and 15 |
...
2026-09-24). What shows each exit: 1, the `fast` jobs of `.github/workflows/estate.yml` on both
...
LocalDB in CI), 7 (with `RefusalPaths` and the denied-login test) and 8 (on a matching environment
...
5. *A named environment cannot be written*: `Publish` does not compile against
   ...
   exit 9, and so is a scratch server on a host that any environment's reference resolves to (R15).
6. R13's stamp and window: every provenance record names its DacFx version; the committed DacFx
   equals the pin in `estate/ledgers/toolchain.md` or the release immediately before it; anything
...
1. On a branch that makes a populated column NOT NULL, a developer's `estate predict` prints,
   labelled *predicted · read-only · time · DacFx version*: Dev blocks; the data-loss check fires on
...
5. `estate classify` on each rename-by-typing sample change (column, table, schema move) refuses
   and names the remedy (`estate diff --refactorlog`, M4).
...
## 9. M3 — Synthetic copy (days 2–8, lane T; `synthetic-copy up` at the conditions tier by day 6)

**Outcome.** One command, from the estate's clone alone, builds a synthetic copy current to any
ref: the repository's schema built and published the way the pipeline does it, with rows generated
from the committed evidence and the head commit's week, at the conditions, shape or scale tier. The
second time takes seconds. A lead refreshes the evidence with one command and one pull request, and
only when the live data conditions say it is stale.

WPs 3.1 to 3.5 land by day 6, because M4 proves on copies of the conditions-tier synthetic copy.
...
`estate/evidence.shape.json`, and data conditions and tiers replace its scenarios); the hand-seeded
...
under twenty seconds) or a generated set at the conditions tier from scratch (seconds, always
...
**Outcome.** One command proves a change on a fresh copy at each environment's data conditions,
...
predicted, proven at its data conditions, applies or not, and why.

| WP | Lane | What | Done when |
|---|---|---|---|
| 4.1 | B | `io/Prove`. For each distinct set of data conditions (Dev's by default; those in a prediction file or a PR body's machine block with `--conditions-from`): a fresh copy, restored from the cached synthetic copy at those conditions or generated at the conditions tier, never the synthetic copy itself; build head; plan against the copy; publish **Strict** (the pipeline's profile as loaded) and record the outcome: *published*; *blocked*, with `blockedBy: data-loss-check` (Msg 50000) or `blockedBy: constraint-violation` (SQL Server refused the change on existing rows, such as Msg 547, 515, 1505, 2628 or 245), and the message verbatim, since the copy holds only generated rows; or *failed* for anything else. On a block, publish **Permissive** on the same copy and record the consequence (rows before and after per touched table, widths, trust, and the error number at each violating site). Read back trust (`is_not_trusted` on every foreign key and check, a catalog read). Compute conservation hashes for a multi-phase step. Publish again and require the plan empty and every seeded table's content hash unchanged (value D5). Assert CDC silence on the Docker scratch server, where SQL Server Agent runs the capture job; on LocalDB record *not provable here*. The provenance: the copy's name, the script's SHA-256, the DacFx version and the server, the time, the five inputs. Drop the copy unless `--keep`. | the make-mandatory sample change yields exit 3, `blockedBy: data-loss-check`, the verbatim message and its provenance |
| 4.2 | A | kernel `Outcome.Prove`, `Readback`, `AppliesTo` (data conditions, DacFx version, server and publish profile, §3); the shapes (`one release`, `two releases`, `refused`), each with its reason; "does not apply: <the input that differs>". | law T as a kernel property |
| 4.3 | B | the verb `prove`: `--target synthetic-copy` means fresh copies restored from the synthetic copy, and `--target copy:<registered name>` a copy the registry holds; `env:` is exit 9 with "prove never publishes to a named environment; use predict". A one-screen Markdown summary by default and the JSON with `--json` (R10). With no scratch server on the machine, exit 4 with the remedy, and nothing classified from the text (R27). The refactorlog writer: `estate diff --refactorlog <old> <new>` writes the entry with `XmlWriter`, with v2's `RefactorLogEmitter` as the specification. | law 6 green |
| 4.4 | C, A | the corpus completed: every operation's sample change and the compound case, each with its expected classification, its prediction per set of data conditions, and its proof, the `INSERT` fixtures replaced by generated sets at the conditions tier; one parameterized test runs them all (the proof lane: nightly, on both CI operating systems). | the proof lane green on the committed DacFx |
| 4.5 | B | laws 6, 7, 8, 9, 10, P and T as `Io.Tests` (Appendix C); law 8's CDC half on the Docker scratch server. | all green |

**Exit.**
1. The make-mandatory sample change: `estate prove` exits 3 with the data-loss check's message
   verbatim, the Permissive consequence and its provenance, and the description's line for Dev
   reads *predicted blocks · proven blocked at Dev's data conditions · applies*. A proof whose
...
2. Every sample change in the corpus is green on the committed DacFx (R9). Law P holds on every
   ...
   reported at data conditions, a DacFx version or a publish profile it did not run at.
3. R6's check: NOT NULL on a populated table, a reference over recorded orphans, and a narrowing
   below the recorded maximum each block at the conditions, shape and scale tiers.
...
7. R27: on a machine with no scratch server, `prove` exits 4 with the remedy, and `classify` still
...
| WP | Lane | What | Done when |
|---|---|---|---|
| 5.1 | A | kernel `PullRequestDescription` and `Locks`, reading `knowledge/description.md` (WP 7.2 lands it by day 5). The evidence table first: the build; the change, with its data-loss steps and renames with refactorlog presence; *first time on this estate*, from `estate/ledgers/operations.md` (value G4); the prediction per environment, or *pending a lead*; the proof per set of data conditions, with *applies* or the input that differs; baseline drift; in-flight collision; the DacFx pin; the profile's provenance; consumers to republish, or *not checked*. Then the ten sections, with "The data" carrying rows before, rows after and the approver for any data-modifying pre-deploy (value S5), and "Not checked" pre-filled with the four standing items of Appendix B's last row (application behaviour in Service Studio; the correctness of the business answer; Prod's population until it is measured; any change between the last read and the deploy) above a placeholder for the author's own. Placeholders for the intent and the business answer with its owner. The banned-word check as a pure function. `Locks`: open windows from `estate/ledgers/in-flight.md`; a change with data-loss steps on more than one table and no program row. | a description over the corpus passes `Register.Samples`' rules; every claim in it carries its provenance with its five inputs or names the one it lacks (law 5′); placeholders are detected by position |
| 5.2 | B | `io/Gate` and the verbs `gate`, `describe`, `check inflight`. Base and head from git. `classify`'s blocking findings (a rename without its entry, for a column, a table or a schema move; a CDC column-list change) and the lock checks run first and exit 9 with the row, before any proof. The data conditions come from the PR body's machine blocks; a block whose provenance names a different head package is stale and reported so; with no block, Dev's data conditions come from the committed evidence. The synthetic copy at the conditions tier; a proof per distinct set of data conditions (environments that present the same conditions share a proof; the base is published once and restored per set). The gate reads `estate/ledgers/row-tiers.md` for each environment it reports on, and an environment with no dated row reads *not measured* (value G5). The description is regenerated; placeholders and banned words fail the gate, by line (R18); a body section that disagrees with the regenerated one, and an author's provenance (script SHA-256, DacFx version, publish profile, data conditions) that disagrees with the gate's, are findings posted with the table and never failures. `changelog.json` (created, dropped, renamed with refactorlog keys, retyped; the open windows). The page's repository cells (open windows, changes in review) are regenerated (R23). `gate.json`; exit 0, 3 with prove's verdict attached, 9, or a tooling code that says it is one. | `estate gate` run locally and in CI on one branch give identical `gate.json` apart from the provenance's target and time |
...
The conditions tier generates in seconds, so the builds and the publishes take most of the time.
How the tool folder crosses into the corporate network is §17's item 14, and LFS on the estate
...
5. The planted-value scan across the whole chain: `predict`, `prove` at the pasted data conditions,
...
| WP | What | Done when |
|---|---|---|
| 7.1 | Citations rewritten: findings by identifier, samples by shape, handbook by title, ledgers by their new path (`knowledge/ledgers/` for the templates the tool seeds; `estate/ledgers/` for the estate's own). | the citation test is green at every commit |
...
emitter on the golden project and on the estate), oracle 4 (v1's edge-case fixtures), and laws 1 to
...
1. **Built here, embedded there.** The tool is built in this repository; its source, its tests and
   its CI never enter the corporate one. What the estate embeds is the published tool folder, two
...
| 4 | Git LFS enabled on the estate | the Azure DevOps administrator | M5 exit 4 on the estate, and nothing else | the rest of M5 runs on this repository and a scratch estate |
...
| 7 | The .NET 10 SDK and Docker on every laptop and hosted agent (S6) | IT; the corporate agent's `STATE.md` | building a `.sqlproj` there, and so every verb that builds | `estate doctor` names a missing SDK with its remedy; without Docker, LocalDB is the scratch server and CDC reads *not provable here* |
...
| 10 | Which tables CDC captures, per environment | the dev leads | M2's CDC finding; M4's CDC proofs | `check cdc` proposes the ledger on day five; CDC proofs run on the Docker scratch server (§1 fact 12) |
| 11 | Prod's counts before its first release | the operator | the page's Prod row; any prediction for Prod | Prod greyed; `estate measure --env prod --counts` before its first release |
...
| 16 | Declared bytes: the estate's `.gitattributes` and `.editorconfig` lines for its SSDT files | the dev leads | M5 exit 5 (R24) | WP 5.4 proposes lines matching what the files carry; `check drift`'s exactness on Windows waits for the merge |
...
| 18 | The first real Dev deploy of a declarative foreign key, read back | the team, at their next such change | turns law 10 from a copy's fact into the estate's | `check drift` and the trust query (`is_not_trusted`) after that deploy; the result recorded in `knowledge/findings.md` with its provenance |
...
- **The element read at scale.** A 300-table estate is on the order of a hundred thousand property
...
| R6 | tiered volumes; the conditions tier exact | M3, M4 | M3 exit 3 (population); M4 exit 3 (blocks at every tier) |
...
| R27 | without the tool or a scratch server, stop and say so | M4, M7 | M4 exit 7; WP 7.2 |
...
| A proof applies only under equal inputs | `DataConditions`; `AppliesTo` over data conditions, DacFx version, server and publish profile; law T; a proof at each environment's data conditions | M2, M4 |
...
| 9 | the data-loss check is data-blind | as stated, and as a property of `Outcome.Predict` | M2, M4 | — |
...
| T | — | a proof is reported for an environment only at that environment's data conditions, DacFx version and publish profile | M4 | — |
...
// fact 3: the data-loss check, read from DacFx's own script and run verbatim inside the allowlisted form
...
sealed class DataLossChecks(List<QueryExpression> found) : TSqlFragmentVisitor
{
...
// (io/Ssdt.Elements also descends composing relationships and skips a property DacFx cannot read on an object)
...
| `Preconditions.cs` (preconditions, `DataConditions`, baseline drift) | 320 | M2 |
...
to ≤ 3,000 lines as `V3_ARCHITECTURE.md` §6.5 sets; S2 contributes the extracted data-loss checks
