...
2. **A verb answers one question, and what parameterizes it is data wherever it can be.** The
   ...
   references are one committed file, `estate/environments.json`. The agent surfaces are rows in a targets
...
| What existing data does each environment present? | `predict` | the head package; each reachable environment's plan and data | `estate/environments.json` | `Preconditions.Of`, `ExistingData.Of`, `Outcome.Predict` | `estate.predict/1`, and a Markdown block carrying a machine block |
...
| One page | `check environments --page` | the checks above; `deployments.md` beside the page | `estate/environments.json` | `Page.Of` | a wiki page |
...
one entry in `estate/environments.json`. An agent surface costs one row in `knowledge/targets.json`. The
...
| 14 | The estate's configuration | `estate/environments.json` (the environments, the writable targets, the local server) and two mirrored profiles, `estate/profiles/{strict,permissive}.publish.xml` | `estate/environments.json` (renamed from `posture.json` by the operator, 2026-09-25) holds each environment (classification, reader groups, references, the profile's path, SQLCMD values, the metamodel's reference) and the local server preference; `estate/profiles/` holds the pipeline's one profile | §1 fact 10: a hand-kept Strict copy had already drifted from the pipeline's | — |
...
- the permissions ask before any verb that pushes (`measure --commit`, `check cdc --commit`,
  ...
  target;
- `cli/CONFIG.md` is generated from `estate/environments.json`'s schema;
...
7. R16: a literal connection string in `estate/environments.json`, a profile or an argument is exit 6; a
...
**Exit.**
1. The make-mandatory sample change: `estate prove` exits 3 with `BlockOnPossibleDataLoss`'s message
...
5. **The estate owns** `estate/environments.json`, `estate/ledgers/`, `estate/evidence.shape.json`,
...
| 7 | The .NET 10 SDK and Docker on every laptop and hosted agent (S6) | IT; the corporate agent's `STATE.md` | building a `.sqlproj` there, and so every verb that builds | `estate doctor` names a missing SDK with its remedy; without Docker, LocalDB is the local server and CDC reads *not provable here* |
