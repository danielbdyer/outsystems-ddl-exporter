...
`estate predict` reads it, read-only, from each environment the author or the lead can reach; the
...
| What existing data does each environment present? | `predict` | the head package; each reachable environment's plan and data | `estate/posture.json` | `Preconditions.Of`, `ExistingData.Of`, `Outcome.Predict` | `estate.predict/1`, and a Markdown block carrying a machine block |
...
| A current synthetic copy | `synthetic-copy` | a ref, the evidence, the seed, optionally the existing data | the tier | `SyntheticData.Generate` | `estate.synthetic-copy/1` |
| What does DacFx do? | `prove` | a fresh copy per `ExistingData` value, restored from the synthetic copy | the pipeline's profile (Strict and Permissive differ in `BlockOnPossibleDataLoss` alone) | `Outcome.Prove`, `AppliesTo` | `estate.prove/1` |
...
| Is the evidence still true? | `check evidence` | the committed evidence; the live existing data over its standing sites | — | `ExistingData` equality | `estate.check/1` |
...
public sealed record Prediction(EnvironmentName Environment, ExistingData ExistingData, SortedArray<DataLossCheckSite> DataLossChecks,
...
estate predict            ─► existing(dev)   ─┐
lead's laptop                                 ├─► a machine block in the body ─► per distinct existing data: generate a fresh copy of it,
estate predict --env qa,uat ─► existing(qa)   │                                   prove, compare with the author's provenance
...
2. **The synthetic copy rarely needs a person.** A schema change is absorbed because the synthetic
   ...
   committed evidence's `ExistingData` with the live one and asks for `estate profile` only when they differ. The seed is the ISO week of the head commit, so
...
| 6 | Prediction | `classify`, pure, from committed evidence | `classify` stays provisional; a new verb, `predict`, reads each environment's existing data read-only from DacFx's own data-loss checks; proofs run at each environment's existing data | the lifecycle invariants: prediction and proof, and the condition under which a proof applies (Appendix B) | — |
...
`estate/evidence.shape.json`, and existing data and tiers replace its scenarios); the hand-seeded
