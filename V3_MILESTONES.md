...
| 14 | The estate's configuration | `estate/posture.json` (the publish posture, the writable targets, the local server) and two mirrored profiles, `estate/profiles/{strict,permissive}.publish.xml` | `estate/posture.json` keeps its name and holds each environment (classification, readers, references, the profile's path, SQLCMD values, the metamodel's reference) and the local server preference; `estate/profiles/` holds the pipeline's one profile | §1 fact 10: a hand-kept Strict copy had already drifted from the pipeline's | — |
